...
pub fn tray_get_config(app: AppHandle) -> Result<serde_json::Value, String> {
    ...
    let enabled = store.get("trayEnabled").cloned().unwrap_or(serde_json::Value::Bool(true));
    let minimize_to_tray = store.get("minimizeToTray").cloned().unwrap_or(serde_json::Value::Bool(false));
    Ok(serde_json::json!({
        "enabled": enabled,
