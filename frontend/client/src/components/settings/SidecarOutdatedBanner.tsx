/**
 * SidecarOutdatedBanner — 当客户端调 sidecar API 拿到 404 时显示的提示横幅。
 *
 * 场景: 用户升级了桌面客户端 (electron app)，但 Windows NSIS 安装时
 *   sidecar.exe 没被替换（旧进程占用 / 杀软拦截 / 覆盖安装保留旧文件）。
...
interface Props {
  /** 错误消息（来自 SidecarOutdatedError.message） */
  message?: string
  /** sidecar 上报的 API 版本（如果有） */
  sidecarVersion?: number | null
  /** 重试回调（用户点"重试"按钮） */
  onRetry?: () => void
}

export function SidecarOutdatedBanner({ message, sidecarVersion, onRetry }: Props) {
  return (
    <div className="space-y-4 py-6">
      <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-5 text-sm">
        ...
        <div className="space-y-2 text-[12px] text-amber-700/90 dark:text-amber-300/80">
          <p>
            客户端向 sidecar 请求了一个新版才有的端点，但 sidecar 返回了 404。
            这通常意味着桌面客户端升级了，但 <code className="font-mono">sidecar.exe</code> 没有同步更新
          ...
          {typeof sidecarVersion === 'number' && (
            <p className="text-[11px]">
              当前 sidecar API 版本: <code className="font-mono">v{sidecarVersion}</code>
              {' '}（客户端期望 v2+）
            </p>
          )}
        </div>
