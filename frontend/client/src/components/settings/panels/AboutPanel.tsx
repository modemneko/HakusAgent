 ...
 *
 * Sections:
 *   1. App version + sidecar API version + electron/chrome/node versions
...
import type { DiagnosticsInfo, SidecarVersionInfo } from '@/api/types'
...
export function AboutPanel() {
  ...
  const [sidecarVer, setSidecarVer] = useState<SidecarVersionInfo | null>(null)
  ...
  // Read platform + electron/chrome/node versions + diagnostics + sidecar version.
  useEffect(() => {
    const electron = (window as any).electron
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2 rounded-xl border border-border bg-card/40 p-4 text-xs">
          ...
          <VersionRow
            label="Sidecar API"
            value={sidecarVer ? `v${sidecarVer.sidecar_api_version}` : '—'}
          />
          <VersionRow label="Sidecar Server" value={sidecarVer?.server_version || diag?.version || '—'} />
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-xl border border-border bg-card/40 p-4">
          ...
          <div className="mt-3 flex items-center gap-3 text-[11px] text-muted-foreground">
            ...
            >
              <Github className="h-3 w-3" /> GitHub Releases
            </a>
            <span>·</span>
            <a
              ...
              className="flex items-center gap-1 transition-colors hover:text-foreground"
            >
              <Github className="h-3 w-3" /> Nightly 构建
            </a>
          </div>
        </div>
      </div>
