...
HakusCLI has three related concepts:
...
`~/.hakuscli/settings.toml`, so switching to Operate survives a restart. The
...
live mode again is how you make it durable; HakusCLI confirms with a
...
HakusCLI writes `settings.toml` under a lock that spans processes, and replaces
the file atomically, so a second HakusCLI instance on the same home directory
...
- `suggest` (**Ask**, default): tool approvals may interrupt, and HakusCLI asks
...
HakusCLI deliberately adopts only the exact-action structured decision,
...
returns `approve` directly. HakusCLI borrows Kimi's no-question autonomous UX,
...
that path. HakusCLI's autonomous posture adds only the single stateless
...
Run `hakuscli --help` for the canonical list. Common flags:

- `-p, --prompt <TEXT>`: one-shot prompt mode (prints and exits)
- `hakuscli exec --auto --output-format stream-json <PROMPT>`: run the tool-backed non-interactive agent and emit one JSON object per line for harnesses and backend wrappers. Exit codes: `0` on success, `1` for genuine task/agent failures, `75` (`EX_TEMPFAIL`) when the turn ended on a retryable infrastructure failure (provider/transport `network`/`timeout` after all in-session retries) so harnesses can tell a retryable infra exit apart from a task failure; the terminal stream `metadata` event's `error_category` carries the same classification
- `hakuscli exec --resume <ID|PREFIX> <PROMPT>` / `--session-id <ID|PREFIX>`: continue a saved session non-interactively
- `hakuscli exec --continue <PROMPT>`: continue the most recent saved session for this workspace non-interactively
- `hakuscli fork <ID|PREFIX>` / `hakuscli fork --last`: copy a saved session into a new sibling session; forked sessions retain additive parent-session metadata and show that lineage in session listings
- `--model <MODEL>`: when using the `hakuscli` facade, forward a DeepSeek model override to the TUI
...
HakusCLI has three related but intentionally separate recovery paths:

- `hakuscli fork <ID>` creates a new saved session from an existing saved
