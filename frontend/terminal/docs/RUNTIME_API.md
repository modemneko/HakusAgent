...
`hakuscli app-server` is the canonical local runtime API and control plane.
...
mobile page. `hakuscli doctor --json` provides machine-readable health, and
`hakuscli serve --acp` speaks the Agent Client Protocol over stdio for editors
such as Zed.

`hakuscli serve --http` / `serve --mobile` remain as **compatibility aliases**
for `hakuscli app-server --http` / `--mobile`; both launch the identical
server. New integrations should target `app-server`.

`hakuscli exec` is the separate one-shot headless worker path (stream-json,
...
applications (and other local supervisors) that embed the HakusCLI engine.
...
local supervisor / SDK / automation harness
        │
        ├─ hakuscli app-server --http     → HTTP/SSE runtime API (/v1/*)        [canonical]
        ├─ hakuscli app-server --mobile   → runtime API + mobile control page
        ├─ hakuscli app-server --stdio    → JSON-RPC control transport over stdio
        ├─ hakuscli doctor --json         → machine-readable health & capability
        ├─ hakuscli serve --acp           → ACP stdio agent for editors such as Zed
        ├─ hakuscli serve --mcp           → MCP stdio server
        ├─ hakuscli serve --http/--mobile → legacy aliases for `app-server --http/--mobile`
        └─ hakuscli exec [args]           → one-shot headless worker (stream-json)
```
...
| Entry | Transport | Use |
|---|---|---|
| `hakuscli web [--port 7878]` | HTTP/SSE on `127.0.0.1:7878` + embedded client | First-class loopback-only browser client; opens the default browser |
| `hakuscli app-server --http` | HTTP/SSE on `127.0.0.1:7878` | Full `/v1/*` runtime API (canonical) |
| `hakuscli app-server --mobile` | HTTP/SSE on `0.0.0.0:7878` + `/mobile` | Runtime API + phone control page |
| `hakuscli app-server --stdio` | JSON-RPC 2.0 over stdio | Local SDK / control probe (no listener) |
| `hakuscli app-server` | HTTP on `127.0.0.1:8787` | Legacy in-process app-server (`/healthz`, `/thread`, `/app`, `/prompt`, `/tool`, `/jobs`) |
| `hakuscli serve --http` / `--mobile` | same server as `app-server --http`/`--mobile` | Compatibility aliases |
...
runtime API token is read from `--auth-token`, then `HAKUS_RUNTIME_TOKEN`,
...
The legacy in-process `hakuscli app-server` also requires an explicit
`--auth-token` or `HAKUS_APP_SERVER_TOKEN` before binding a non-loopback
host; its generated one-time `cwapp_*` token is loopback-only.

### Runtime and account identity

`GET /v1/runtime/info` reports `hakuscli_version` plus the full 40-character
`hakuscli_commit` embedded by the shared CLI/TUI build. A source archive that
...
```json
{
  "account": {
    "schema_version": 1,
    "state": "authenticated",
    "api_base": "https://api.hakuscli.net",
    "account_id": "acct_...",
    "session_id": "session_...",
    "scopes": [],
...
secure record written by `hakuscli account login`; it does not run a second
...
For one-shot/headless automation, prefer `hakuscli exec` with explicit
...
The matrix discovers configured providers from `hakuscli auth list`, skips
...
covered by `scripts/release/app-server-smoke.test.sh` against a fake `hakuscli`
binary.

## ACP stdio adapter: `hakuscli serve --acp`

`hakuscli serve --acp` speaks JSON-RPC 2.0 over newline-delimited stdio for
...
Prompt requests are routed through the configured HakusCLI client and current
...
`hakuscli serve --http` for the full local runtime API and `hakuscli serve --mcp`
when another client needs HakusCLI's tools as MCP tools.

## Capability endpoint: `hakuscli doctor --json`
...
| `legacy_state.primary_root` | string | Primary HakusCLI state root inspected for known state paths |
...
| `legacy_state.session_recovery.recovery_command` | string or null | `hakuscli sessions` when additive automatic recovery is available; null for isolated, complete, empty, or failed scans |
...
| `skills.global.path` / `.present` / `.count` | — | HakusCLI global skills dir (`~/.hakuscli/skills`, with legacy `~/.deepseek/skills` support) |
...
{
  "version": "0.8.9",
  "config_path": "/Users/you/.hakuscli/config.toml",
  "config_present": true,
  "workspace": "/Users/you/projects/hakuscli-tui",
  "api_key": {
  ...
  "memory": {
    "enabled": false,
    "path": "/Users/you/.hakuscli/memory.md",
    "file_present": true
  },
  "mcp": {
    "config_path": "/Users/you/.hakuscli/mcp.json",
    "present": true,
    "servers": [
...
## HTTP/SSE runtime API: `hakuscli app-server --http`

```bash
hakuscli app-server --http [--host 127.0.0.1] [--port 7878] [--workers 2] [--auth-token TOKEN] [--insecure-no-auth]
hakuscli app-server --mobile [--host 0.0.0.0] [--port 7878] [--auth-token TOKEN]
hakuscli app-server --mobile --host 127.0.0.1 [--port 7878] [--insecure-no-auth]
hakuscli web [--port 7878]

# Compatibility aliases — identical server, serve flag names:
hakuscli serve --http   [...] [--insecure]
hakuscli serve --mobile [...] [--insecure]
```
...
`/v1/*` routes require a bearer token unless `hakuscli app-server` is started
...
`hakuscli serve` compatibility aliases use `--insecure` for the same loopback
escape hatch.
Pass `--auth-token TOKEN` or set `HAKUS_RUNTIME_TOKEN=TOKEN` before starting
...
`X-HakusCLI-Runtime-Token: TOKEN`, the legacy
`X-DeepSeek-Runtime-Token: TOKEN`, or the `hakuscli_runtime_token` cookie.
Query-string authentication is not supported.

### Local browser client

`hakuscli web` starts the canonical Runtime API on `127.0.0.1`, serves
...
capability for a
`hakuscli_web_session=…; HttpOnly; SameSite=Strict; Path=/` cookie backed by a
...
`hakuscli serve --mobile` starts the same HTTP/SSE runtime API and serves a
...
While a session is open in an interactive HakusCLI process, that process holds
...
instead. A standalone `hakuscli web` holds nothing open and is never blocked.
...
```json
{
  ...
  "head": "abc1234",
  "dirty": false,
  "workspace": "/Users/you/projects/hakuscli",
  "archived": false,
...
write, and `GET /v1/skills` refreshes that shared state so another HakusCLI
...
- CLI flag (repeatable): `hakuscli serve --http --cors-origin http://localhost:5173`
- Env var (comma-separated): `DEEPSEEK_CORS_ORIGINS="http://localhost:5173,http://localhost:8080"`
- Config (`~/.hakuscli/config.toml`):
  ```toml
  [runtime_api]
...
`Content-Type`, `Accept`, `X-HakusCLI-Runtime-Token`, and the compatibility
...
the `@hakuscli/runtime-sdk` workspace package. It is deliberately thin: every
helper calls the local Rust Runtime API and therefore cannot bypass HakusCLI's
sandbox, approval prompts, provider configuration, or fleet ledger authority.

```js
import { createRuntimeClient } from "@hakuscli/runtime-sdk";

const client = createRuntimeClient({
  baseUrl: "http://127.0.0.1:7878",
  token: process.env.HAKUS_RUNTIME_TOKEN,
});
...
```bash
npm test --workspace @hakuscli/runtime-sdk
```

## Agent Run Receipts

Sub-agent lanes persist compact run receipts in
`.hakuscli/state/subagents.v1.json`. The Runtime API exposes those receipts as
...
```bash
cargo test -p hakuscli-protocol --test parity_protocol --locked
```
...
```bash
cargo test -p hakuscli-app-server capabilities
```
