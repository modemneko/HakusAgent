...
…or add to `~/.hakuscli/config.toml`:
...
`memory_path` anchor points — by default `memory_path = "~/.hakuscli/memory.md"`
re-roots to `~/.hakuscli/memory/`:
...
tree. The shipped example keeps `~/.hakuscli/memory.md` so the store
lands at `~/.hakuscli/memory/global/MEMORY.md`.
...
| Store root            | `~/.hakuscli/memory/`        | derived from `memory_path`            |
