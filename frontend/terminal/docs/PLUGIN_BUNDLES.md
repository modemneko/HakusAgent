...
HakusCLI supports a deliberately small plugin-bundle boundary. The boundary
...
and Hooks through HakusCLI's existing engines. Unsupported declarations stay
...
HakusCLI scans only its own roots, looking in each `<name>/` directory for a
...
v0.9.8), or `plugin.toml` (the legacy HakusCLI format, still fully readable):

- User: `~/.hakuscli/plugins/<name>/`
- Workspace: `<workspace>/.hakuscli/plugins/<name>/`
...
`hakuscli-plugin-capabilities-v3` activation-policy review below.
...
while unknown keys inside HakusCLI's own `extensions["net.hakuscli"]`
...
`url`). HakusCLI-only server options — timeouts, tool filters, env-backed
credentials, enablement — ride per-server under `extensions["net.hakuscli"]`.
...
### `plugin.toml` (legacy HakusCLI format)
...
values. Plugin children inherit only HakusCLI's base secret-scrubbed child
...
HakusCLI 0.9.10 activates declarative `[skills]`, `[mcp_servers.*]`,
...
- Compatibility is per-component: `full` when every declared surface has an
  ...
  only declares surfaces HakusCLI cannot activate yet. The same versioned
  activation policy (v3) drives those labels, the runtime adapters, and the
  capability hash. A future HakusCLI that starts executing LSP or native code
...
- An **all-unsupported** bundle can be reviewed and trusted, but `/plugin
  ...
  HakusCLI can honestly activate.
- An **unrecognized** section or field is a validation failure, not an
  ...
  `extensions["net.hakuscli"]` are all rejected outright. The single
...
copies the complete reviewed tree into a HakusCLI-owned, content-addressed
...
A local stdio server is a child process running with the HakusCLI user's host
...
Trust receipts live in `~/.hakuscli/plugins/state.json`. Atomic owner-only
...
parse local Kimi-, Claude-, Codex-, and HakusCLI-format catalog documents; see
...
schemas (Kimi, Claude, Codex, HakusCLI native; Codex via its policy markers)
...
- Sources HakusCLI cannot fetch (npm packages, `command:` sources, non-tarball
