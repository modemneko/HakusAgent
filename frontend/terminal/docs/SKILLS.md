...
Skills are reusable `SKILL.md` instruction packs. HakusCLI discovers them from
...
| Scope | Path |
| --- | --- |
| Project | `<workspace>/.hakuscli/skills/` |
| Global | `~/.hakuscli/skills/` |
...
HakusCLI presents its shipped skills in two compact tiers so agentic workflows
...
Workspace, user, and compatible-harness skills stay labeled **custom**; HakusCLI
...
the actual HakusCLI bundle. This is a decision matrix, not a request to copy
reference text or advertise unsupported tools:

| Reference skill | HakusCLI decision | Runtime grounding |
| --- | --- | --- |
...
| `imagine` | Intentionally out of scope | HakusCLI has no image-generation/edit tool, so the starter pack must not advertise one. |
...
2. Never hand-edit `.claude` / `.agents` / `.cursor` trees to “install” for
   HakusCLI — import into `.hakuscli/skills` instead.
