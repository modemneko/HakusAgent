# HakusCLI Architecture

This document provides an overview of the hakuscli architecture for developers and contributors.
...
- **`crates/lane`** - Lane runtime: durable, attachable running instances of
  Fleet/Workflow work (`hakuscli lane list/status/attach/logs/stop`).
...
- **`runtime_api.rs`** - HTTP/SSE runtime API (`hakuscli serve --http`)
...
1. Before sending user input, the TUI writes a checkpoint snapshot to `~/.hakuscli/sessions/checkpoints/latest.json`
2. Startup remains fresh by default; prior sessions are resumed explicitly via `--resume`/`--continue` (or `Ctrl+R` in TUI)
3. While degraded/offline, new prompts are queued in-memory and mirrored to `~/.hakuscli/sessions/checkpoints/offline_queue.json`
...
6. Action-capable turns also take pre/post-turn side-git workspace snapshots under `~/.hakuscli/snapshots/<project_hash>/<worktree_hash>/.git`; `/restore N` and `revert_turn` restore file state without changing conversation history or the user's `.git`
...
2. `task_manager.rs` persists task + queue entry under `~/.hakuscli/tasks`
...
1. Configure in `~/.hakuscli/mcp.json`
...
3. Place in a CodeWhale-owned root (`~/.hakuscli/skills/` or
   `<workspace>/.hakuscli/skills/`), or import from a compatible harness root
   through `/skills`
...
Configure in `~/.hakuscli/config.toml`:
...
- `~/.hakuscli/config.toml` - Main configuration (`~/.deepseek/config.toml` is still read as a legacy fallback)
...
- `~/.hakuscli/mcp.json` - MCP server configuration
- `~/.hakuscli/skills/` - User skills directory
- `~/.hakuscli/sessions/` - Session history
- `~/.hakuscli/sessions/checkpoints/` - Crash checkpoint + offline queue persistence
- `~/.hakuscli/snapshots/` - Side-git pre/post-turn workspace snapshots for `/restore` and `revert_turn`
- `~/.hakuscli/tasks/` - Background task records, queue, timelines, artifacts
- `~/.hakuscli/audit.log` - Append-only audit events for credential + approval/elevation actions
