...
runtime records that HakusCLI already owns: thread metadata, turn status, turn
...
```text
hakuscli receipt export --thread <thread_id> --turn <turn_id> --format json
...
`hakuscli review --write-receipt` writes a local JSON receipt for the reviewed
diff under the HakusCLI state directory (`review-receipts/`) unless
...
`hakuscli review --write-receipt` after changing the diff; reviewers should
compare the `diff_fingerprint` before reusing a receipt in a PR handoff.

`hakuscli review --check-receipt` is the local pre-push gate. It does not call
...
```json
{
  "schema_id": "hakuscli.conformance-receipt/v0",
  "thread": {
    "id": "thr_...",
