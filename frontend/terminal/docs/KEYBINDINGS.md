...
| Mouse drag           | Select transcript text in HakusCLI                |
| `Ctrl-C`             | Copy an active HakusCLI selection                 |
...
terminal and are intentionally separate from HakusCLI's `Ctrl-C` selection
binding. Over SSH, HakusCLI sends copy requests back through OSC 52, or via
...
- **Configurable keymap (#436) and `tui.toml` (#437) remain deferred.** The `TuiPrefs` struct and loader exist in `settings.rs` but are not wired at startup. The named-binding registry that would let `~/.hakuscli/tui.toml` override individual entries is still pending.
