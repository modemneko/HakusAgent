...
hakuscli can load additional tools via MCP (Model Context Protocol). MCP servers can be local stdio processes that the TUI starts, or remote URL-based servers that speak Streamable HTTP with legacy SSE fallback.
...
Server mode note:
- `hakuscli-tui serve --mcp` runs the MCP stdio server.
- `hakuscli-tui serve --http` runs the runtime HTTP/SSE API (separate mode).
- The `hakuscli` dispatcher exposes `hakuscli mcp-server` as an equivalent stdio
...
| Safe static health probe (missing command/url, broken absolute path, missing bearer env) | `hakuscli mcp validate`, live connect, OAuth login |
| Point at safe on-ramps (`/mcp`, `hakuscli mcp init`, `hakuscli doctor`) | Install community skills, trust skills, enable plugins |
...
`hakuscli doctor` reports MCP/skills/tools/plugins health with the same
...
```bash
hakuscli-tui mcp init
```

`hakuscli-tui setup --mcp` performs the same MCP bootstrap alongside skills setup.

Common management commands:

```bash
hakuscli-tui mcp list
hakuscli-tui mcp tools [server]
hakuscli-tui mcp add <name> --command "<cmd>" --arg "<arg>"
hakuscli-tui mcp add <name> --url "http://localhost:3000/mcp"
hakuscli-tui mcp add <name> --url "https://example.com/mcp" --bearer-token-env-var MCP_TOKEN
hakuscli-tui mcp login <name>
hakuscli-tui mcp logout <name>
hakuscli-tui mcp enable <name>
hakuscli-tui mcp disable <name>
hakuscli-tui mcp remove <name>
hakuscli-tui mcp validate
```
...
`/mcp recommendations` is HakusCLI's native, curated suggestions surface.
...
| Browser Use | Skill plus separately installed Python runtime | Skill/runtime release `0.13.8` | [Official browser-use project](https://github.com/browser-use/browser-use) | Optional companion: not an MCP server. HakusCLI does not auto-run the upstream Skill installer or install its browser/runtime dependencies. |
| Anthropic Sandbox Runtime | Sandbox adapter companion | `@anthropic-ai/sandbox-runtime@0.0.73` | [Official anthropic-experimental project](https://github.com/anthropic-experimental/sandbox-runtime); beta | Documentation-only adapter candidate in v0.9.10: not an MCP server and not an active HakusCLI plugin adapter. It does not replace HakusCLI's sandbox policy. |
...
only writes config and HakusCLI never downloads it.
...
HakusCLI keeps its stricter rule: provenance and foreign policy are display
...
```bash
hakuscli-tui mcp add remote --url "https://example.com/mcp"
hakuscli-tui mcp login remote
```

HakusCLI discovers the server OAuth metadata, opens the authorization URL in
your browser, listens on a local callback, exchanges the code, and stores the
token response through the HakusCLI secrets backend. Stored OAuth tokens are
...
Unauthorized during connect/discovery, `hakuscli mcp connect <name>` reports
that OAuth authentication is required and points to
`hakuscli mcp login <name>`. Resource helper listings also surface an
...
datasets, Spaces, and community tools. HakusCLI does not call Hugging Face's
...
2. Choose the MCP client closest to your HakusCLI config shape and copy the
...
HakusCLI reads both `servers` and `mcpServers`, so settings-generated snippets
...
- `~/.hakuscli/mcp.json` (`~/.deepseek/mcp.json` is still read when the HakusCLI file is absent)
...
`hakuscli-tui mcp init` (and `hakuscli-tui setup --mcp`) writes to this resolved path.
...
## Running HakusCLI as an MCP Server

You can register your local HakusCLI binary as an MCP server so other HakusCLI sessions (or any MCP client) can call its tools.

### Quick Setup

```bash
hakuscli-tui mcp add-self
```

This resolves the current binary path, generates a config entry that runs `hakuscli-tui serve --mcp`, and writes it to your MCP config file. The default server name is `hakuscli`.

Options:

- `--name <NAME>` — custom server name (default: `hakuscli`)
- `--workspace <PATH>` — workspace directory for the server

### Manual Config

Equivalent manual entry in `~/.hakuscli/mcp.json`:

```json
{
  "servers": {
    "hakuscli": {
      "command": "/path/to/hakuscli",
      "args": ["serve", "--mcp"],
      "env": {}
    }
  }
...
The `hakuscli-tui` binary supports `serve --mcp` directly. The `hakuscli`
dispatcher offers the equivalent `hakuscli mcp-server` stdio entrypoint. Use
whichever is on your `PATH` (run `which hakuscli` or `which hakuscli-tui` to
...
For example, the `shell` tool from the default server (named `hakuscli`)
becomes `mcp_hakuscli_shell`.

### MCP Server vs HTTP/SSE API vs ACP

| | `hakuscli-tui serve --mcp` | `hakuscli-tui serve --http` | `hakuscli-tui serve --acp` |
|---|---|---|---|
...
| **Config** | `~/.hakuscli/mcp.json` entry | Direct URL connection | Editor `agent_servers` custom command |
| **Lifecycle** | Spawned per client session | Long-running daemon | Spawned per editor agent session |

Use `mcp add-self` when you want HakusCLI tools available to other MCP clients.
Use `serve --http` when building applications that consume the API directly.
Use `serve --acp` when an editor wants to talk to HakusCLI as an ACP agent.
...
```bash
hakuscli-tui mcp validate
hakuscli-tui mcp tools hakuscli
```
...
- Run `hakuscli-tui doctor` to confirm the MCP config path it resolved and whether it exists.
...
- If the MCP config is missing, run `hakuscli-tui mcp init --force` to regenerate it.
