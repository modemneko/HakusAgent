...
retry budget is exhausted, HakusCLI preserves a checkpoint and returns a
...
Sub-agents help HakusCLI move faster, but the parent agent still owns the
...
by `todo_write`. The child's `<hakuscli:fork_state>` block carries the bounded
...
For parallel edit lanes, launch the child with `worktree: true`. HakusCLI
...
`.hakuscli-worktrees/`, so the parent checkout stays clean.
...
- `worktree_path`: exact checkout path. Relative paths stay under the default
  sibling `.hakuscli-worktrees/` root.
...
SCOPE: cargo test -p hakuscli-tui --bin hakuscli-tui --locked prompt; subagent filter if needed.
...
configurable via `[subagents].max_concurrent` in `~/.hakuscli/config.toml` up to
...
```toml
# ~/.hakuscli/config.toml or workspace config
provider = "deepseek"
...
```toml
# .hakuscli/agents/local-formatter.toml
id = "local-formatter"
role_hint = "formatter"
provider = "lm-studio"
model = "qwen-2.5-7b"
...
`--provider lm-studio` to `hakuscli exec`, which resolves the same
...
persisted state from `.hakuscli/state/subagents.v1.json`. The parent can open a
...
`.hakuscli/state/subagents.v1.json`. The record is the current run-ledger
...
`NativeMemoryStore` to `~/.hakuscli/memory/global/MEMORY.md` or
`~/.hakuscli/memory/workspace/<id>/MEMORY.md`. Writes do not go through the
...
- Persisted state: `<workspace>/.hakuscli/state/subagents.v1.json`. Schema
