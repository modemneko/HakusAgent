...
hakuscli reads configuration from a TOML file plus environment variables.
...
A workspace is not configuration authority. HakusCLI therefore ignores
...
HakusCLI has several instruction surfaces. They are deliberately separate so a
...
- **User-global constitution** — the normal guided setup output. Manage it with
  `/constitution` or `/setup`; HakusCLI stores structured data at
  `$HAKUS_HOME/constitution.json` (default `~/.hakuscli/constitution.json`)
  and renders it into a separate `<hakuscli_user_constitution>` prose block.
...
- **Repo-local constitution** — optional project policy in
  `.hakuscli/constitution.json`, described below.
...
- **`.hakuscli/constitution.json`** — HakusCLI-specific **repo authority /
  prioritization policy**: when local sources conflict, which should HakusCLI
  trust first, and what to verify before claiming a task is done. `.hakuscli/`
  ...
  This is the **repo-local law** layer in HakusCLI's hierarchy: *bundled global
  Constitution* → *user-global constitution* (`$HAKUS_HOME/constitution.json`,
  rendered as prose) → *repo constitution* (`.hakuscli/constitution.json`, this
...
> HakusCLI no longer reads `WHALE.md` as project or global context. If one is
> present, setup/context diagnostics report it as ignored so you can migrate it.
> Move ordinary instructions to `AGENTS.md` and HakusCLI-specific authority
> policy to `.hakuscli/constitution.json`. Personal standing guidance belongs
> in `/constitution` / `$HAKUS_HOME/constitution.json`. (The global
> HakusCLI Constitution shipped in the model prompt is a separate thing and is
> unaffected.)
...
- **Repo-local only.** Only the repo's `.hakuscli/constitution.json`
...
1. Drop the replacement at `~/.hakuscli/prompts/constitution.md` (under
   `$HAKUS_HOME` when set).
2. Set the explicit opt-in flag `HAKUS_ALLOW_BASE_PROMPT_OVERRIDE=1`
...
Management, and the Compaction Relay are still owned by HakusCLI's runtime
...
prefer `AGENTS.md` + `.hakuscli/constitution.json` above.

## Where It Looks

Default config path:

- `~/.hakuscli/config.toml`
- Legacy fallback: `~/.deepseek/config.toml`

Overrides:

- CLI: `hakuscli --config /path/to/config.toml`
- Env: `HAKUS_CONFIG_PATH=/path/to/config.toml`
...
`<workspace>/.hakuscli/config.toml` is applied after this user entry. A
...
`<workspace>/.hakuscli/config.toml`, the safe values declared in that file are
merged on top of the global config. Legacy
`<workspace>/.deepseek/config.toml` files are still read when the HakusCLI path
...
`~/.hakuscli/config.toml`. Pass `--no-project-config` to skip the overlay for
one launch.
...
HakusCLI ignores that key and keeps the user's global setting.

The `hakuscli` facade and `hakuscli-tui` binary share the same config file for
DeepSeek auth and model defaults. `hakuscli auth set --provider deepseek` (and
the legacy `hakuscli login --api-key ...` alias) saves the key to
`~/.hakuscli/config.toml` (migrating legacy `~/.deepseek/config.toml` on first
launch when needed), and `hakuscli --model deepseek-v4-flash` is forwarded to
the TUI as `DEEPSEEK_MODEL`.

That provider credential is distinct from the optional managed-product
account. `hakuscli account login` starts the HakusCLI browser device flow;
`hakuscli account status` and `hakuscli account logout` inspect or remove the
...
development fallback. `hakuscli account keys list|set|remove` manages the
signed-in account's BYOK vault without displaying secret values. The older
`hakuscli cloud ...` spelling remains a command alias.
...
user-global `~/.hakuscli/config.toml` (#5045, #5193).
...
1. **Route-specific auth contract.** Routes whose `auth_mode` disables API
   ...
   `[providers.xai] auth_mode = "oauth"` reads HakusCLI's own xAI
...
5. **Secret store.** The durable per-provider slot written by
   `hakuscli auth set` (file-backed under `~/.hakuscli/secrets/` by
...
`~/.hakuscli/config.toml` on first launch, `DEEPSEEK_*` environment
variables remain accepted aliases for the `HAKUS_*` forms, and
`DEEPSEEK_SECRET_BACKEND` is the legacy alias for `HAKUS_SECRET_BACKEND`.

Run `hakuscli auth status` to inspect the active provider's config
...
`hakuscli --provider <id>`. The canonical provider IDs are `deepseek`,
...
`hakuscli auth set --provider nvidia-nim --api-key "YOUR_NVIDIA_API_KEY"` or
`hakuscli auth set --provider openai --api-key "YOUR_OPENAI_COMPATIBLE_API_KEY"` or
`hakuscli auth set --provider atlascloud --api-key "YOUR_ATLASCLOUD_API_KEY"` or
`hakuscli auth set --provider wanjie-ark --api-key "YOUR_WANJIE_API_KEY"` or
`hakuscli auth set --provider xiaomi-mimo --api-key "YOUR_XIAOMI_KEY"` or
`hakuscli auth set --provider fireworks --api-key "YOUR_FIREWORKS_API_KEY"` or
`hakuscli auth set --provider siliconflow --api-key "YOUR_SILICONFLOW_API_KEY"` or
`hakuscli auth set --provider arcee --api-key "YOUR_ARCEE_API_KEY"` or the
...
`http://localhost:11434/v1` and sends model tags such as `hakuscli-coder:1.3b`
...
`hakuscli auth set --provider ollama-cloud`. Ambient auth reads
...
and uses `trinity-large-thinking` by default for HakusCLI agent work.
...
base URL HakusCLI does not recognize as one of those two routes, the question
...
region of your API key. HakusCLI keeps `qwen-plus` scoped to the `openai`
...
differs from HakusCLI's static model metadata. See
...
`insecure_skip_tls_verify = true` is still parsed so `hakuscli doctor` can
...
```bash
DEEPSEEK_ALLOW_INSECURE_HTTP=1 hakuscli
```
...
hakuscli sends those custom headers on model API requests. The equivalent
...
HakusCLI's chat provider and `image_analyze` tool are configured separately.
...
With `model = "auto"`, HakusCLI routes each turn between a strong and a cheap
...
To bootstrap MCP and skills directories at their resolved paths, run `hakuscli-tui setup`.
To only scaffold MCP, run `hakuscli-tui mcp init`.

Note: `setup`, `doctor`, `mcp`, `features`, `sessions`, `resume`/`fork`, `exec`,
`review`, and `eval` are subcommands of the `hakuscli-tui` binary, and the
`hakuscli` dispatcher accepts every one of them as a passthrough
...
By default, the TUI starts a background check for the latest stable HakusCLI
...
The answer is cached in `~/.hakuscli/update-check.json` and reused for
...
| Variable | Why |
| --- | --- |
| `HAKUS_NO_UPDATE_CHECK` | Explicit opt-out. |
...
HakusCLI never installs anything on its own — it only tells you an update
exists. The command it names depends on how the running binary was installed,
detected from its path:

| Install | Command offered |
| --- | --- |
| GitHub release binary (including Termux) | `hakuscli update` |
| npm (`node_modules` on the path) | `npm install -g hakuscli@latest` |
| Homebrew (`Cellar` / `linuxbrew` prefix) | `brew upgrade hakuscli` |
| `cargo install` (`~/.cargo/bin`) | `cargo install hakuscli-cli --locked --force` |

For package-managed installs the notice also warns against `hakuscli update`:
...
Override the detection with `HAKUS_INSTALL_METHOD=npm|homebrew|cargo|binary`
...
`tag_name` field is accepted; if `assets` are present, HakusCLI requires the
same uploaded asset set as the official release before showing the toast.

```toml
[update]
check_for_updates = true
update_uri = "https://internal.mirror.example/hakuscli/releases/latest"
```

When `update_uri` is not set, startup checks honor release mirror environment
variables such as `HAKUS_RELEASE_BASE_URL` before falling back to the
...
total token window HakusCLI budgets against, and it drives the header/footer
context percent, the auto-compaction trigger, context-pressure checks, and the
request output cap. If HakusCLI compacts at 128K on a model you know serves a
...
```bash
hakuscli config set providers.moonshot.context_window 1048576
hakuscli config unset providers.moonshot.context_window   # back to automatic
```
...
6. `fallback` — the static per-provider capability table: 200,000 for
   ...
   otherwise HakusCLI's static per-model metadata, and finally 128,000 when
...
serves a window HakusCLI's catalog does not model. HakusCLI will not invent a
...
- `HAKUS_MAX_OUTPUT_TOKENS` (environment variable; legacy alias
  `DEEPSEEK_MAX_OUTPUT_TOKENS`): overrides the requested output cap. Without an
  override, HakusCLI starts at the safe `65536` request cap and intersects it
...
default_text_model = "hakuscli-coder:1.3b"
...
- CLI: `hakuscli --profile work`
- Env: `DEEPSEEK_PROFILE=work`

If a profile is selected but missing, hakuscli exits with an error listing available profiles.
...
The three user-facing slots — provider, model, base URL — expose `HAKUS_*`
aliases. When both forms are set the `HAKUS_*` value wins; the
`DEEPSEEK_*` form is kept for older shells:

- `HAKUS_PROVIDER` (preferred) / `DEEPSEEK_PROVIDER` (legacy alias) —
...
- `HAKUS_MODEL` (preferred) / `DEEPSEEK_MODEL` (legacy alias) — default model for the active provider
- `HAKUS_BASE_URL` (preferred) / `DEEPSEEK_BASE_URL` (legacy alias) — base URL for the active provider

`HAKUS_BASE_URL` applies to the **active** route only. A request pinned to
...
For every product-level `HAKUS_*` variable below, the matching legacy
`DEEPSEEK_*` name is still read as a compatibility fallback; when both are set,
the `HAKUS_*` value wins.

- `HAKUS_LOG_LEVEL` or `RUST_LOG` (`info`/`debug`/`trace` enables lightweight verbose logs)
- `HAKUS_SKILLS_DIR`
- `HAKUS_MCP_CONFIG`
- `HAKUS_NOTES_PATH`
- `HAKUS_MEMORY` (`1|on|true|yes|y|enabled` turns user memory on)
- `HAKUS_MEMORY_PATH`
- `HAKUS_TELEMETRY` / `DEEPSEEK_TELEMETRY` (legacy alias) — anonymous usage
...
- `HAKUS_TELEMETRY_ENDPOINT` / `DEEPSEEK_TELEMETRY_ENDPOINT` (legacy alias)
  — `https://`, or plain `http://` only for loopback. Overrides the config file.
  Unset selects the shipped default,
  `https://telemetry.hakuscli.net/v1/telemetry`; setting it to the **empty
...
- `HAKUS_ALLOW_SHELL` (`1`/`true` enables)
- `HAKUS_APPROVAL_POLICY` (`on-request|untrusted|never`)
- `HAKUS_SANDBOX_MODE` (`read-only|workspace-write|danger-full-access|external-sandbox`)
- `HAKUS_NO_NEW_PRIVS` (`0`/`false`/`no`/`off`/`disabled` opts out) — Linux only. The
  TUI process sets the kernel's irreversible no-new-privileges flag at startup
  as defense-in-depth, which blocks `sudo`/`su`/setuid helpers for HakusCLI's
  whole process tree. Set this variable to a falsey value before launching if
  you administer through HakusCLI as a wheel-group user and need escalation
...
- `HAKUS_MANAGED_CONFIG_PATH`
- `HAKUS_REQUIREMENTS_PATH`
- `HAKUS_MAX_SUBAGENTS` (clamped to `1..=128`)
- `HAKUS_TASKS_DIR` (runtime task queue/artifact storage, default
  `~/.hakuscli/tasks`, with legacy `~/.deepseek/tasks` fallback when only the
  legacy directory exists)
- `HAKUS_ALLOW_INSECURE_HTTP` (`1`/`true` allows non-local `http://` base URLs; default is reject)
- `HAKUS_FORCE_HTTP1` (`1|true|yes|on` pins the HTTP client to HTTP/1.1, disabling HTTP/2; useful on Windows or behind proxies that mishandle long-lived H2 streams)
- `HAKUS_HOME` (override the base data directory; defaults to `~/.hakuscli`).
  If you previously exported `DEEPSEEK_HOME`, rename it to `HAKUS_HOME`;
  the old env var is not used for new HakusCLI state paths.
- `HAKUS_RELEASE_BASE_URL` (release asset mirror used by `hakuscli update`
...
- `HAKUS_AUTOMATIONS_DIR` (override the automations storage directory; uses
  `~/.hakuscli/automations` by default, with legacy `~/.deepseek/automations`
...
- Only user-owned config, profiles, and managed config may set this array.
  Project config (`<workspace>/.hakuscli/config.toml`, or legacy
...
the engine turn loop it drives; `hakuscli exec`, the CLI subcommands, the
...
and background semantics, and which three of them can steer HakusCLI. The
...
command = "~/.hakuscli/hooks/inject-context.sh"
timeout_secs = 2
...
The entire serialized document is capped at 32 KiB. HakusCLI retains the
...
[[hooks.hooks]]
event = "tool_call_before"
command = "~/.hakuscli/hooks/clamp-shell-timeout.sh"
condition = { type = "tool_name", name = "exec_shell" }
```
...
Repositories can ship policy in `<workspace>/.hakuscli/hooks.toml`,
using the same shape as the `[hooks]` table (top-level fields plus
`[[hooks]]` entries). Project hooks are executable shell
configuration, so HakusCLI only loads them after the workspace has
...
[[hooks.hooks]]
event = "turn_end"
command = "~/.hakuscli/hooks/turn-audit.sh"
timeout_secs = 2
continue_on_error = true
```
...
command = "~/.hakuscli/hooks/subagent-audit.sh"
timeout_secs = 2
continue_on_error = true
```
...
`~/.hakuscli/composer_stash.jsonl`. `/stash list` shows parked
...
hakuscli also stores user preferences in:

- `~/.hakuscli/settings.toml` on new installs
...
- `theme` (`system`, `terminal`, `dark`, `light`, `grayscale`,
  ...
  background detection, `dark`/`light` use the HakusCLI Whale pair,
  ...
  User-authored overlays live only at `~/.hakuscli/themes/<name>.json` (or
  `$HAKUS_HOME/themes/<name>.json`) and are selected with
...
- `launch_screen` (`on`/`off`; default `off`): show the pre-session Work/Chat/
  ...
  screen off, HakusCLI enters a new session directly; resume remains
...
- `session_auto_resume` (`on`/`off`; default `off`): reattach to this
  workspace's most recent session when HakusCLI starts. Off by default so
  plain `hakuscli` keeps starting fresh. `--resume`, `--continue`, and
  ...
  — `hakuscli "<prompt>"` and `hakuscli exec` are never silently prefixed
...
- `auth_mode` (string, optional provider-table key): selects a provider-specific authentication contract. Kimi Code membership uses `auth_mode = "api_key"` (or omit the field), a key created in the [Kimi Code console](https://www.kimi.com/code/console), `base_url = "https://api.kimi.com/coding/v1"`, and bare `model = "k3"` for K3. HakusCLI gives that route a safe 262,144-token baseline; set `context_window = 1048576` only when the Kimi Code plan includes 1M access (Allegretto and above). `k3[1m]` is a Claude Code-only convention, not an API model ID, and HakusCLI rejects it instead of silently changing the wire model or assuming an entitlement. `model = "kimi-for-coding"` remains the valid K2.7 compatibility route available to all Kimi Code members. Legacy `auth_mode = "kimi_oauth"` fails closed with API-key guidance and never probes, reads, refreshes, or rewrites `kimi_cli`/`kimi_code_cli` credential files. First-class OAuth requires HakusCLI's own vendor-registered client identity and remains tracked in #4417.
...
- `verbosity` (string, optional): `normal` or `concise`. `normal` keeps the
  ...
  Override per process with `HAKUS_VERBOSITY` or the legacy
...
- `telemetry` (bool, optional): anonymous usage counting, **`true` by default**,
  ...
  *opt-out* — it deletes the random install id,
  ...
  `HAKUS_TELEMETRY=1` both lose to it, and turning telemetry back on means
  writing `true` here. Override per process with `HAKUS_TELEMETRY` (legacy
  ...
  dry-run records. A repo-local `.hakuscli/config.toml` cannot set it. The
  resolved consent is visible with its source — `hakuscli doctor` prints a
  `telemetry=on (default)` row in the runtime-posture section, and
  `hakuscli config get telemetry` reports `on (default)`, `on (config)`, or
...
- `telemetry_endpoint` (string, optional): where batches are POSTed. Leaving it
  unset selects the shipped default,
  **`https://telemetry.hakuscli.net/v1/telemetry`** — the first-party ingest
  ...
  is then written to `$HAKUS_HOME/telemetry/dryrun.jsonl` and no HTTP client
  ...
  per process with `HAKUS_TELEMETRY_ENDPOINT` (legacy alias
  `DEEPSEEK_TELEMETRY_ENDPOINT`), where an empty value means the same "contact
  nobody". A repo-local `.hakuscli/config.toml` cannot set it.
...
- `permissions.toml` (sibling file, optional): typed permission rule records
  loaded next to `config.toml`, for example `~/.hakuscli/permissions.toml`.
...
- `[[hotbar]]` (array of tables, optional): user-owned 1-8 slot bindings for
  ...
  hotbar bindings write this same schema to the resolved `~/.hakuscli/config.toml`
...
- `skills_dir` (string, optional): defaults to `~/.hakuscli/skills` (each skill is
  ...
  CodeWhale-owned roots (`<workspace>/.hakuscli/skills` and
  `~/.hakuscli/skills`) are writable install/import targets; compatible harness
...
- `[skills].scan_hakuscli_only` (bool, default `false`): when `true`, session
  skill discovery ignores cross-tool roots such as `.claude/skills`,
  `.opencode/skills`, `.cursor/skills`, and `~/.agents/skills`. HakusCLI still
  scans `<workspace>/.hakuscli/skills`, `~/.hakuscli/skills`, and any explicit
...
- `mcp_config_path` (string, optional): defaults to `~/.hakuscli/mcp.json`, with
  legacy `~/.deepseek/mcp.json` fallback when the HakusCLI path is absent.
...
- `notes_path` (string, optional): defaults to `~/.hakuscli/notes.txt`, with
  legacy `~/.deepseek/notes.txt` fallback when the HakusCLI path is absent, and
...
- `memory_path` (string, optional): anchors the native memory store. The
  ...
  `~/.hakuscli/memory.md` yields `~/.hakuscli/memory/global/MEMORY.md`
...
- `snapshots.*` (optional): side-git workspace snapshots for file rollback:
  ...
  - snapshots live under
    `~/.hakuscli/snapshots/<project_hash>/<worktree_hash>/.git`, with legacy
...
`.hakuscli/notes.md` (legacy `.deepseek/notes.md` is the fallback path when
no `.hakuscli/notes.md` exists yet).
...
# ~/.hakuscli/memory/global/MEMORY.md (see MEMORY.md).
memory_path = "~/.hakuscli/memory.md"
...
HakusCLI therefore builds notifications from a typed payload with a fixed
...
needs HakusCLI to ship a real `.app` bundle. Tracked in
...
HakusCLI loads a small core native tool catalog by default and leaves less
...
- `hakuscli-tui --enable web_search`
- `hakuscli-tui --disable subagents`

Use `hakuscli-tui features list` to inspect known flags and their effective state.
...
HakusCLI does not guess geography from locale or model provider.
...
keep `provider = "duckduckgo"` and set `base_url`; HakusCLI appends the `q`
query parameter to that endpoint and applies network policy to its host.
Custom endpoints do not fall back to public Bing. `HAKUS_SEARCH_BASE_URL`
...
`base_url = "https://your-searxng.example"`; HakusCLI calls
`/search?q=...&format=json`. HakusCLI does not use a public SearXNG instance
...
`METASO_API_KEY` or `[search] api_key`; HakusCLI does not ship a shared key.
...
`FIRECRAWL_API_KEY` or `[search] api_key` for authenticated limits. HakusCLI
...
`HAKUS_SSH_CLIPBOARD=graphical`; set it to `terminal` to force terminal
...
hakuscli supports a policy layering model:
...
## Notes On `hakuscli-tui doctor`

`hakuscli-tui doctor` follows the same config resolution rules as the rest of the
TUI. That means `--config`, `HAKUS_CONFIG_PATH`, and the legacy
...
To bootstrap missing MCP/skills paths, run `hakuscli-tui setup --all`. You can
also run `hakuscli-tui setup --skills --local` to create a workspace-local
`./skills` dir.

Both plain `hakuscli-tui doctor` and `doctor --json` are structural and
...
output. Run `hakuscli mcp validate` to explicitly start enabled servers and
...
`hakuscli-tui setup` accepts a few flags beyond the existing `--mcp`,
...
- `--tools` — scaffold `~/.hakuscli/tools/` with a `README.md` describing the
  ...
  MCP, hooks, or skills.
- `--plugins` — scaffold `~/.hakuscli/plugins/` with a `README.md` and an
...
- `--clean` — list `~/.hakuscli/sessions/checkpoints/latest.json` and
  `offline_queue.json` if they exist. Legacy
  `~/.deepseek/sessions/checkpoints/` files are not scanned automatically; set
  `HAKUS_HOME=~/.deepseek` for a one-off legacy cleanup. Pass `--force` to
...
hakuscli sends and receives tool calls only over the API tool channel
...
markers — `[TOOL_CALL]`, `<hakuscli:tool_call`, `<tool_call`, `<invoke `,
