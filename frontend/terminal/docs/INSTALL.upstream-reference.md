...
```bash
curl -fsSL https://hakuscli.net/install.sh | sh
```

It downloads the matching `hakuscli` and `codew` release binaries,
verifies them against `hakuscli-artifacts-sha256.txt`, installs to
...
Published HakusCLI releases ship matched `hakuscli` and `codew` prebuilt binaries for their supported platform/architecture
...
| Platform     | Architecture | `cargo install` | GitHub release asset / `scripts/install.*`            |
| ------------ | ------------ | :---------: | :-------------: | ----------------------------------------------------- |
| Linux        | x64 (x86_64) |     ✅      |       ✅        | `hakuscli-linux-x64`        |
| Linux        | arm64        |     ✅      |       ✅        | `hakuscli-linux-arm64`    |
| Android / Termux | arm64 (aarch64) | ⚠️⁴ preview | ⚠️⁴ preview | `hakuscli-android-arm64.tar.gz` preview archive when published |
| Linux        | riscv64      |     ❌¹     |       ❌³       | temporarily unsupported until upstream bindings land |
| macOS        | x64          |     ✅      |       ✅        | `hakuscli-macos-x64`        |
| macOS        | arm64 (M-series) | ✅      |       ✅        | `hakuscli-macos-arm64`    |
| Windows      | x64          | ✅ | `hakuscli-windows-x64.exe`（或一键脚本） |
| Windows      | arm64        | ✅ | `hakuscli-windows-arm64.exe`（或一键脚本） |
...
| FreeBSD 14+ / OpenBSD          | x64, arm64 |   ❌      |       ✅²       | `cargo install hakuscli-cli --locked` (no prebuilt; see § FreeBSD) |
...
⁴ The v0.9.10 source-candidate npm wrapper recognizes Android arm64 and resolves
  the matching `hakuscli` and `codew` Android assets. npm
...
Linux `hakuscli-linux-arm64` archive in Termux; use the Termux-specific
...
The npm wrapper, `hakuscli update`, and the Unix archive installer retain their
...
```bash
cargo install hakuscli-cli --locked   # installs `hakuscli`
```
...
> from `npm i -g hakuscli`. v0.8.8 publishes `hakuscli-linux-arm64`, so a plain `npm i -g hakuscli` works
...
When the release includes `hakuscli-android-arm64.tar.gz`, install it with the
...
```bash
cd "$HOME"
curl -L -O https://github.com/Hmbown/CodeWhale/releases/latest/download/hakuscli-android-arm64.tar.gz
curl -L -O https://github.com/Hmbown/CodeWhale/releases/latest/download/hakuscli-bundles-sha256.txt
sha256sum -c hakuscli-bundles-sha256.txt --ignore-missing

tar xzf hakuscli-android-arm64.tar.gz
cd hakuscli-android-arm64
...
temporary credentials. `hakuscli auth set` is available, but the Termux build
has no supported OS keyring integration and falls back to file-backed secrets
by writing `~/.hakuscli/config.toml` and mirroring keys to
`~/.hakuscli/secrets/secrets.json`. Both are plaintext files protected by
`0600` permissions and are not encrypted at rest.

```bash
hakuscli auth set --provider deepseek
hakuscli auth status
hakuscli doctor
```
...
```bash
command -v hakuscli codew
test -x "$PREFIX/bin/hakuscli"
test -x "$PREFIX/bin/codew"

hakuscli --version
hakuscli doctor
hakuscli exec --auto "run pwd"
```

Known limitations:

- Commands inherit Android's per-app UID, SELinux, and seccomp protections and
  any permissions granted to Termux. HakusCLI's opt-in bubblewrap
  child-process sandbox is Linux-only and is not built on Android, so approved
  commands receive no HakusCLI-specific filesystem narrowing.
- The Termux build has no supported Android Keystore or desktop Secret Service
  integration. Use `hakuscli auth status` to confirm the active source and
...
- Terminal rendering varies by Android terminal app. The TUI always owns the
  ...
  use `hakuscli exec` for headless runs instead.
...
`hakuscli`. Do not install release assets from look-alike repositories,
...
`hakuscli-artifacts-sha256.txt` for bare binaries and
`hakuscli-bundles-sha256.txt` for `.tar.gz` / `.zip` platform archives. If you
download binaries manually, verify them before running:

```bash
# Run from the directory containing the downloaded binaries.
curl -L -O https://github.com/Hmbown/CodeWhale/releases/latest/download/hakuscli-artifacts-sha256.txt
sha256sum -c hakuscli-artifacts-sha256.txt --ignore-missing
```

On macOS, use
`shasum -a 256 -c hakuscli-artifacts-sha256.txt --ignore-missing` instead of
`sha256sum`.
...
- the filename, for example `hakuscli-linux-x64`
...
```bash
npm install -g hakuscli
hakuscli --version   # prints the published version that was installed
```

`postinstall` downloads the matching `hakuscli` and `codew` binaries, verifies
them against that source's SHA-256 manifest, and exposes `hakuscli` and `codew`
on your `PATH`.
...
explicit release base URL or `HAKUS_USE_CNB_MIRROR=1`, it concurrently
fetches the small `hakuscli-artifacts-sha256.txt` manifests from GitHub
...
`hakuscli.bat` next to the bare x64 exe; that launcher prefers `wt.exe` and
...
| Variable                            | Purpose                                                                                |
| ----------------------------------- | -------------------------------------------------------------------------------------- |
| `HAKUS_RELEASE_BASE_URL`        | Override the download root. Skips the Linux x64 GitHub/CNB race.                        |
| `HAKUS_USE_CNB_MIRROR=1`        | Force the CNB first-party mirror on Linux x64 / OpenHarmony x64. Other targets fail.   |
| `HAKUS_VERSION`                 | Pin which release the wrapper downloads (defaults to `hakuscliBinaryVersion`).        |
| `HAKUS_GITHUB_REPO`             | Point the downloader at a fork (`owner/repo`).                                          |
| `HAKUS_FORCE_DOWNLOAD=1`        | Re-download even if a cached binary marker matches.                                    |
| `HAKUS_DISABLE_INSTALL=1`       | Skip the `postinstall` download entirely (CI smoke, vendored binaries).                 |
| `HAKUS_OPTIONAL_INSTALL=1`      | Don't fail `npm install` on retryable download errors — useful in CI matrices.          |
| `HAKUS_QUIET_INSTALL=1`         | Suppress installer progress messages.                                                   |
| `HAKUS_DOWNLOAD_TIMEOUT_MS`     | Override the total download budget in milliseconds.                                     |
| `HAKUS_DOWNLOAD_STALL_MS`       | Override the no-progress stall budget in milliseconds.                                  |

The corresponding `DEEPSEEK_TUI_*` and `DEEPSEEK_*` variables remain accepted
as legacy aliases, after the canonical `HAKUS_*` names. New automation and
support instructions should use only the HakusCLI names.
...
`hakuscli-cli` installs the `hakuscli` command. npm and prebuilt releases also
...
> from source, and on Linux the `hakuscli-cli` crate links against
...
Nix builds `hakuscli` (single binary) and then starts the dispatcher. Pass
...
```nix
{
  inputs = {
    ...
    hakuscli.url = "github:Hmbown/CodeWhale";
    hakuscli.inputs.nixpkgs.follows = "nixpkgs";
  };
}
```
  ...
  in
  {
    ...
    nixosConfigurations.yourhostname = nixpkgs.lib.nixosSystem {
      ...
      modules = [
        # ...
        {
          environment.systemPackages = [ hakuscli.packages.${system}.default ];
        }
      ];
    };
  };
...
The formula is `hakuscli`. The tap GitHub repo is still
`Hmbown/homebrew-deepseek-tui` until it is renamed; `brew tap Hmbown/deepseek-tui`
keeps working either way.

```bash
brew tap Hmbown/deepseek-tui
brew install hakuscli
```

Update with `brew upgrade hakuscli`. Existing Cellar installs under the
legacy `deepseek-tui` formula name can still run `brew upgrade deepseek-tui`
for one overlap release; new installs should use `hakuscli`.
...
the **bare binaries** (`hakuscli-<platform>` and `codew-<platform>`, no extension) and a **`.tar.gz` / `.zip` archive**
(`hakuscli-<platform>.tar.gz`) that bundles the same commands plus an
`install.sh`. The npm wrapper and the in-app `hakuscli update` download the
...
# Linux ARM64 example
mkdir -p ~/.local/bin
curl -L -o ~/.local/bin/hakuscli      \
    https://github.com/Hmbown/CodeWhale/releases/latest/download/hakuscli-linux-arm64
curl -L -o ~/.local/bin/codew          \
    https://github.com/Hmbown/CodeWhale/releases/latest/download/codew-linux-arm64
chmod +x ~/.local/bin/hakuscli ~/.local/bin/codew
hakuscli --version
```
...
> ```bash
> xattr -d com.apple.quarantine ~/.local/bin/hakuscli ~/.local/bin/codew 2>/dev/null || true
> ```

Verify integrity against the per-release SHA-256 manifest:

```bash
curl -L -o /tmp/hakuscli-artifacts-sha256.txt \
    https://github.com/Hmbown/CodeWhale/releases/latest/download/hakuscli-artifacts-sha256.txt
( cd ~/.local/bin && sha256sum -c /tmp/hakuscli-artifacts-sha256.txt --ignore-missing )
```

(Use `shasum -a 256 -c /tmp/hakuscli-artifacts-sha256.txt --ignore-missing`
...
npm install -g hakuscli@X.Y.Z

# Cargo path: one package installs hakuscli
cargo install hakuscli-cli --version X.Y.Z --locked --force
```
...
```bash
# individual binaries
curl -L -o hakuscli-artifacts-sha256.txt \
  https://github.com/Hmbown/CodeWhale/releases/download/vX.Y.Z/hakuscli-artifacts-sha256.txt

# platform archives
curl -L -o hakuscli-bundles-sha256.txt \
  https://github.com/Hmbown/CodeWhale/releases/download/vX.Y.Z/hakuscli-bundles-sha256.txt
```

Inside a HakusCLI workspace, `/restore list [N]` lists side-git file snapshots
...
The `hakuscli` package is listed in Scoop's main bucket:

```powershell
scoop update
scoop install hakuscli
hakuscli --version
```
...
single-binary release ships only `hakuscli` + `codew` — no `hakuscli-tui` asset.
...
and the portable ZIP fallback (`hakuscli-windows-x64.zip` / `hakuscli-windows-arm64.zip`). winget
selects the matching architecture automatically; both install the single binary (`hakuscli.exe` + `codew.exe`).
The zips also include `hakuscli.bat`. Double-click that launcher (not the raw `.exe`) so the first
window is Windows Terminal when it is installed.

Update via `winget upgrade Hmbown.CodeWhale` or `hakuscli update`. The winget package is
...
If `winget install` reports a hash mismatch, verify `hakuscli-artifacts-sha256.txt` for the same
...
> `npm install -g hakuscli` under native ARM64 Node.js, or download
> `hakuscli-windows-arm64.zip` directly — all paths install native ARM64 binaries.
...
`hakuscli-windows-arm64.zip` from the same release; both paths then use native
ARM64 binaries.
...
- Installs `hakuscli.exe` and `codew.exe` side-by-side (single binary, no `hakuscli-tui.exe`) into
  `%LOCALAPPDATA%\Programs\CodeWhale\bin`
- Installs `hakuscli.bat`, which prefers Windows Terminal (`wt.exe`) when it is on `PATH` and
...
Uninstall removes the binaries, `hakuscli.bat`, the Start Menu shortcut, and the user `PATH` entry.
...
for each user profile that needs HakusCLI.

The release-built installer is currently unsigned and may trigger Windows
SmartScreen. Verify the SHA-256 checksum from `hakuscli-artifacts-sha256.txt`
...
# Place hakuscli.exe and codew.exe here (single binary, no hakuscli-tui.exe), then:
makensis /DVERSION=<version> hakuscli.nsi
```
...
cargo install --path crates/cli --locked   # installs `hakuscli`
...
FreeBSD has no prebuilt GitHub Release asset — `npm install -g hakuscli` intentionally
...
`1582ba965`/`5eb0385e8`). No separate `pkg install hakuscli` port exists yet —
a native port is tracked as the follow-up to #1097 under `packaging/freebsd/`
(contributions welcome). Validate with `cargo check --target x86_64-unknown-freebsd -p hakuscli-cli --locked`
...
# Per build
cross build --release --target aarch64-unknown-linux-gnu -p hakuscli-cli   # single binary
```

The resulting binary lands in
`target/aarch64-unknown-linux-gnu/release/hakuscli`. Copy it to the ARM64 host
...
cargo build --release --target aarch64-unknown-linux-gnu -p hakuscli-cli   # single binary
```
...
The Cargo-built binary appears at `target\release\hakuscli.exe`. Release
...
binaries. Either upgrade (`npm i -g hakuscli@latest`) or use
...
npm install -g hakuscli
# or
cargo install hakuscli-cli --locked --force
```

### `hakuscli update` reports `no asset found for platform hakuscli-linux-aarch64`
...
the first source that validates. You do not need `HAKUS_USE_CNB_MIRROR=1`
for that automatic path.

If both first-party sources fail, set `HAKUS_RELEASE_BASE_URL` to a
...
`DEEPSEEK_TUI_RELEASE_BASE_URL` name is still accepted. `HAKUS_USE_CNB_MIRROR=1`
still forces CNB only on Linux x64 / OpenHarmony x64.

### `hakuscli update` is blocked by GitHub from mainland China

`hakuscli update` normally contacts GitHub Releases for metadata and binary
assets. On networks where GitHub is blocked or unreliable, use the CNB source
mirror instead and install the `hakuscli-cli` package from the release tag.
Cargo installs the `hakuscli` command:

To check the latest release without downloading or replacing binaries, run
`hakuscli update --check`.

```bash
cargo install --git https://cnb.cool/hakuscli.net/hakuscli --tag vX.Y.Z hakuscli-cli --locked --force   # single binary
```

If you operate a binary asset mirror, `hakuscli update` can use it directly:

```bash
HAKUS_RELEASE_BASE_URL=https://your-mirror.example.com/CodeWhale/vX.Y.Z/ \
HAKUS_VERSION=X.Y.Z \
hakuscli update
```

The mirror directory must contain `hakuscli-artifacts-sha256.txt` and the
...
hakuscli-cli --locked` fails while compiling the keyring or D-Bus secret
storage crates, install the Linux build dependencies inside the WSL distro,
then rerun the one Cargo package install command. It installs `hakuscli`:
...
only apply when WSL2 is compiling HakusCLI from source.

### Wrapper installs but `hakuscli` isn't found
...
4. **Use `cargo install hakuscli-cli --locked`** from crates.io — this
...
If `hakuscli` waits several seconds and prints `connect ETIMEDOUT` or
...
2. Mirror the release assets internally and set `HAKUS_RELEASE_BASE_URL`:

   ```bash
   export HAKUS_RELEASE_BASE_URL=https://your-mirror.example.com/CodeWhale/
   hakuscli
   ```

   The directory must contain `hakuscli-artifacts-sha256.txt` and the platform
...
4. Download both matching `hakuscli` and `codew`
...
```bash
hakuscli --version
hakuscli doctor       # checks API key, provider, runtime, and PATH integrity
hakuscli doctor --json
```
