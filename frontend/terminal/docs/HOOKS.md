...
Hooks run a shell command when the HakusCLI **TUI** reaches a lifecycle
...
| Surface | Fires hooks |
| --- | --- |
| `hakuscli` / `codew` interactive TUI | yes |
| `hakuscli exec` (headless one-shot) | no |
| the `hakuscli` CLI dispatcher and its subcommands | no |
| app-server / ACP | no |
...
name = "announce"
event = "session_start"
command = "echo 'HakusCLI session started'"
```
...
command = "~/.hakuscli/hooks/gate.sh"  # required; `sh -c` on Unix, `cmd /C` on Windows
...
**Termination is best-effort, and the bound that is guaranteed is HakusCLI's,
...
HakusCLI does guarantee is that it stops waiting: the containment handle is
...
A hook command inherits the environment of the HakusCLI process, plus the
`DEEPSEEK_*` variables for its event. HakusCLI does not filter that
inheritance, so treat a hook exactly as you would treat any command you type
in the same shell that launched HakusCLI: whatever is exported there is
visible to it.
...
- **Unsupported conditions are rejected at load.** A condition that references
  ...
  believes is armed. HakusCLI drops those hooks at load, logs the reason
...
A repository may ship `<workspace>/.hakuscli/hooks.toml` using the same shape,
...
trusted project file logs a warning and HakusCLI falls back to global hooks
...
Observer means HakusCLI ignores the hook's **result**: stdout is discarded, a
...
is change what HakusCLI itself does next.
...
- `background = true` hooks are submitted and never awaited, so they have no
  verdict and cannot steer; HakusCLI logs a warning when one is configured
  for this event
...
HakusCLI's ambient environment. Its environment is built as:
...
HakusCLI are **not** forwarded to a local `exec_shell` on their own.
...
backend, HakusCLI does not construct the process environment at all: it hands
...
HakusCLI's own audit log still records key names only, but that says nothing
...
Resolved **key names — never values** — are written to `~/.hakuscli/audit.log`
...
- Hooks are arbitrary shell commands from your own config; treat
  `~/.hakuscli/config.toml` as executable.
- Project-supplied hooks require an explicit workspace trust decision in
  user-owned config.
- Hook commands inherit HakusCLI's own environment. A local `exec_shell` does
  not — see [`shell_env`](#shell_env).
- `shell_env` audit records contain key names only. That covers HakusCLI's own
...
- Nothing HakusCLI persists in a denial echoes the stdin payload, hook
