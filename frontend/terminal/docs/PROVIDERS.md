...
HakusCLI codebase. It is intentionally conservative: shipped entries are
...
- `crates/agent/src/lib.rs` - static `ModelRegistry` used by
  `hakuscli model list` and `hakuscli model resolve`.
...
- CLI: `hakuscli --provider <id>`
- TUI: `/provider <id>` or the provider picker
- Env: `HAKUS_PROVIDER=<id>`; `DEEPSEEK_PROVIDER=<id>` is the legacy alias
- Config: `provider = "<id>"`
...
`hakuscli auth external-consent --provider deepseek --mode read-only`.
HakusCLI never writes that file and only reads `DEEPSEEK_API_KEY`.
...
Fresh shared config writes to `~/.hakuscli/config.toml`. Existing
...
Set the route with `provider = "<id>"`, `HAKUS_PROVIDER=<id>`, or
`hakuscli --provider <id>`. Set the request model with `HAKUS_MODEL`, a
provider-specific model env var, top-level `default_text_model`, or
`[providers.<table>].model`. Set the endpoint with `HAKUS_BASE_URL`, a
provider-specific base URL env var, or `[providers.<table>].base_url`. Set auth
with `hakuscli auth set --provider <id>`, `[providers.<table>].api_key`, or
...
For hosted providers, `hakuscli auth set --provider <id>` saves an API key for
...
- `HAKUS_BASE_URL` / `HAKUS_MODEL` for the active provider.
...
model-specific**, and HakusCLI fails closed: they are sent only when the
...
entry, so HakusCLI does not offer one on this route.
...
`insecure_skip_tls_verify = true` key is still parsed so `hakuscli doctor` can
...
runner, then point HakusCLI at it with `--provider` / `/provider` or a config
table.
...
through an OpenAI-compatible API. Start DS4, then open HakusCLI's prefilled,
keyless setup form:

```bash
./ds4-server --ctx 100000 --kv-disk-dir /tmp/ds4-kv --kv-disk-space-mb 8192
hakuscli
# In HakusCLI: /setup provider ds4
```
...
HakusCLI reuses its existing OpenAI-compatible transport and DeepSeek
...
`max_tokens`, and no strict-tool mode; HakusCLI follows those exact route
...
hakuscli --provider ollama --model <model>
```
...
then run `hakuscli auth set --provider ollama-cloud`. For ambient auth,
...
`ollama` secret slot. HakusCLI does not rewrite the config, copy or delete a
...
hakuscli --provider vllm --model <model>
```

vLLM's OpenAI-compatible server listens on port 8000 by default, matching
HakusCLI's `VLLM_BASE_URL`.

### SGLang

```bash
python -m sglang.launch_server --model-path <model> --port 30000
hakuscli --provider sglang --model <model>
```

SGLang's default port 30000 matches HakusCLI's `SGLANG_BASE_URL`.
...
| `openai-codex` | Run `codex login`, then explicitly grant HakusCLI read-only access to that exact credential file; no HakusCLI API key is stored. |
...
| `ollama-cloud` | Create an [Ollama API key](https://ollama.com/settings/keys), save it with `hakuscli auth set --provider ollama-cloud`, or set `OLLAMA_CLOUD_API_KEY` / `OLLAMA_API_KEY` in that precedence order. |
...
| `xai` | [xAI Console](https://console.x.ai/) for an API key, HakusCLI-owned device login, or explicitly consented read-only Grok CLI credentials. |
| `mistral` | [Mistral Console (la Plateforme)](https://console.mistral.ai/api-keys) |
| `google` | [Google AI Studio](https://aistudio.google.com/apikey) — HakusCLI uses the official Gemini OpenAI-compatible endpoint and never reads Google OAuth files. |
| `antigravity` | Sign in with the official `agy` CLI (1.1.13). HakusCLI can read that login's OAuth token read-only from the exact pinned `state.vscdb` after `hakuscli auth external-consent`; it never writes or refreshes the file. An `ANTIGRAVITY_API_KEY` or the process's `AGY_ADC_AUTH` wins over the file. The cloud-code wire protocol is not implemented: requests fail closed with an actionable message — use `google` for Gemini models. |
...
secret. HakusCLI links straight to that console and accepts the copied key.
It never probes or impersonates `kimi_cli`/`kimi_code_cli`; first-class Kimi
OAuth remains blocked on a vendor-registered HakusCLI identity.
...
HakusCLI currently supports exact-path, provider-scoped **read-only** grants
for the Codex CLI and Grok CLI:

```bash
codex login
hakuscli auth external-consent --provider openai-codex --mode read-only

grok login
hakuscli auth external-consent --provider xai --mode read-only

hakuscli auth status --provider openai-codex
hakuscli auth external-revoke --provider openai-codex
```
...
yet preserve every unknown external schema field safely. HakusCLI-started xAI
device login instead atomically activates a HakusCLI-owned generation named
`$HAKUS_HOME/credentials/xai-auth-<generation>.json`, stores only that
...
`hakuscli auth external-consent --provider deepseek --mode read-only` grants
exact-path read access to `DEEPSEEK_API_KEY` in `$DSH_HOME/.credentials.yaml`
(or `~/.dsh/.credentials.yaml`), which HakusCLI never writes, refreshes, or
loads into the process environment. This is separate from the DSH *harness*
integration (`hakuscli integrations dsh …`, see
[INTEGRATIONS_DSH.md](INTEGRATIONS_DSH.md)), which never touches credentials
in either direction: it pins HakusCLI's route identity into a `--patch`
...
| --- | --- | --- | --- | --- | --- |
| `deepseek` | `[providers.deepseek]` | `DEEPSEEK_API_KEY` | `HAKUS_BASE_URL` / `DEEPSEEK_BASE_URL`; default `https://api.deepseek.com/beta` | `deepseek-v4-pro`, `deepseek-v4-flash`; compatibility aliases `deepseek-chat`, `deepseek-reasoner` | First-class default. The live Pro backend is labeled `DeepSeek-V4-Pro-0813`; the callable API ID remains `deepseek-v4-pro`. Beta URL enables strict tool mode, chat prefix completion, and FIM completion. Set `https://api.deepseek.com` or `/v1` explicitly to opt out of beta-only features. Reasoning effort maps to the documented wire ladder `low`/`high`/`max` plus the `thinking` toggle: `off` sends `thinking: {"type":"disabled"}`, `low` sends `reasoning_effort: "low"`, `medium` rounds up to `"high"` (the wire has no medium), and `high`/`max` pass through. |
...
| `xiaomi-mimo` | `[providers.xiaomi_mimo]` | `XIAOMI_MIMO_TOKEN_PLAN_API_KEY`, `MIMO_TOKEN_PLAN_API_KEY`, `XIAOMI_MIMO_API_KEY`, `XIAOMI_API_KEY`, `MIMO_API_KEY` | `XIAOMI_MIMO_BASE_URL`, `MIMO_BASE_URL`, `XIAOMI_MIMO_MODE`, `MIMO_MODE`; default `https://token-plan-sgp.xiaomimimo.com/v1` | Chat: `mimo-v2.5-pro`, `mimo-v2.5-pro-ultraspeed`, `mimo-v2.5`; speech/TTS: `mimo-v2.5-tts`, `mimo-v2.5-tts-voicedesign`, `mimo-v2.5-tts-voiceclone`, `mimo-v2-tts` | Xiaomi MiMo OpenAI-compatible chat completions route. Token Plan keys (`tp-...`) use `api-key` auth and the token-plan endpoint by default; pay-as-you-go mode uses standard API keys (`sk-...`) and `https://api.xiaomimimo.com/v1`. It sends `max_completion_tokens` and uses MiMo's `thinking` field for reasoning control. Token Plan cost/usage is credit/quota based; HakusCLI shows it as unknown until Xiaomi exposes a reliable balance API. `hakuscli speech` / `tts` uses the TTS models. |
| `novita` | `[providers.novita]` | `NOVITA_API_KEY` | `NOVITA_BASE_URL`; default `https://api.novita.ai/openai/v1` | `deepseek/deepseek-v4-pro`, `deepseek/deepseek-v4-flash` | OpenAI-compatible hosted route for DeepSeek model IDs. Use config or `HAKUS_MODEL` / `DEEPSEEK_MODEL` for model overrides. |
| `fireworks` | `[providers.fireworks]` | `FIREWORKS_API_KEY` | `FIREWORKS_BASE_URL`; default `https://api.fireworks.ai/inference/v1` | `accounts/fireworks/models/deepseek-v4-pro` | OpenAI-compatible hosted route. Use config or `HAKUS_MODEL` / `DEEPSEEK_MODEL` for model overrides. |
...
| `xai` | `[providers.xai]` | `XAI_API_KEY`, HakusCLI-owned device OAuth, or explicit read-only Grok CLI consent | `XAI_BASE_URL`; default `https://api.x.ai/v1` | `grok-4.6` (default), `grok-4.5`, `grok-4.3`, `grok-build`, `grok-composer-2.5-fast`, `grok-4.20-0309-reasoning`, `grok-4.20-0309-non-reasoning` | xAI/Grok OpenAI-compatible Chat Completions route. Grok 4.6 has a 500K context window, text/image input, function calls, structured output, server-side web search, and `low`/`medium`/`high`/`xhigh` reasoning (default `high`). Its standard rates double when the prompt reaches 200K tokens; the same 2x long-context rule applies to `grok-4.5` (500K context, $2.00 / $0.30 cached / $6.00) and `grok-4.3` (1M context, $1.25 / $0.20 cached / $2.50) per their [model pages](https://docs.x.ai/docs/models/grok-4.5). There is no documented `latest`/`fast` alias and no published numeric output limit. **API-key** (default): Bearer token from console.x.ai via `XAI_API_KEY` / keyring / `api_key`. **OAuth**: `hakuscli auth xai-device` uses SSH-friendly device login and HakusCLI-owned storage, which may refresh itself. Existing Grok CLI credentials require `hakuscli auth external-consent --provider xai --mode read-only`; the granted external file is never refreshed or rewritten. OAuth may return HTTP 403 on some SuperGrok tiers — keep API-key as the reliable fallback. `XAI_MODEL` is accepted. Provider aliases: `x-ai`, `x_ai`, `grok`. |
...
HakusCLI's `huggingface` provider ID is only the OpenAI-compatible chat
inference route through Hugging Face Inference Providers. It is selected with
`/provider huggingface`, `HAKUS_PROVIDER=huggingface`, or
...
`hakuscli --provider xiaomi-mimo speech "text" --model tts` (or the `tts`
...
Plan docs and console expose credit/quota semantics, but HakusCLI does not
...
includes `mimo-v2.5` for image input. HakusCLI exposes image analysis through the
...
HakusCLI preserves the `deepseek/` wire-model prefix under the OpenRouter
...
fields, HakusCLI treats them as absent for that response rather than as a
...
`hakuscli model list` and `hakuscli model resolve` use the static registry in
`crates/agent/src/lib.rs`. This is not the same as live `/models` discovery.
Use `/models` or `hakuscli models` to fetch model IDs from the active API
...
| `xiaomi-mimo` | `mimo-v2.5-pro`, `mimo-v2.5-pro-ultraspeed`, `mimo-v2.5`; speech/TTS IDs are selected through `hakuscli speech` / `tts` | yes | yes for chat models; no for speech/TTS models |
...
`hakuscli-tui doctor --json` exposes the `capability` object. It is static
...
MiniMax M3 uses input-length and service tiers. HakusCLI omits
...
disabled. HakusCLI sends explicit controls when the user selects a reasoning
mode.
...
HakusCLI only executes tools when the provider returns Chat Completions
...
- If in doubt, test a small `File` `read` or `search_content` action against a
  known tool-calling model before debugging HakusCLI's tool registry.

Changing `provider`, `base_url`, or `model` can select a route that supports the
OpenAI-compatible payload shape, but HakusCLI cannot convert arbitrary JSON
...
15:59 UTC. HakusCLI migrates either name to `deepseek-v4-flash` before a
...
- **Direct Moonshot K3** (exact `https://api.moonshot.ai/v1` with
  ...
  The direct platform publishes no `off` state for K3, and HakusCLI will not
...
Normal dispatched
`auto` uses HakusCLI's auto-reasoning selector and sends a concrete tier;
...
- A unified `Provider` trait in `hakuscli-agent` that owns env precedence,
