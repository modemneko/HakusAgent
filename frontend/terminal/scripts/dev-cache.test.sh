...
work=$(mktemp -d "${TMPDIR:-/tmp}/hakus-dev-cache.XXXXXX")
cleanup() { rm -rf "$work"; }
...
# 1. Portable default root is $HOME/.cache/hakus, never a desk path.
out=$(run_apply HAKUS_DEV_CACHE=0 HAKUS_DEV_CACHE_REPO_ROOT="$NEW_WT")
if contains "$out" "cache_root=$HOME_DIR/.cache/hakus"; then
  ok "default cache root is HOME/.cache/hakus"
else
  bad "default cache root is HOME/.cache/hakus" "$out"
fi
...
# 2. XDG_CACHE_HOME and HAKUS_CACHE_ROOT win in that documented order.
out=$(run_apply HAKUS_DEV_CACHE=0 XDG_CACHE_HOME="$work/xdg")
if contains "$out" "cache_root=$work/xdg/hakus"; then
  ok "XDG_CACHE_HOME/hakus is the default when set"
else
  bad "XDG_CACHE_HOME/hakus is the default when set" "$out"
fi
out=$(run_apply HAKUS_DEV_CACHE=0 \
  XDG_CACHE_HOME="$work/xdg" \
  HAKUS_CACHE_ROOT="$work/explicit-root")
if contains "$out" "cache_root=$work/explicit-root"; then
  ok "HAKUS_CACHE_ROOT overrides XDG and HOME"
else
  bad "HAKUS_CACHE_ROOT overrides XDG and HOME" "$out"
fi
...
if contains "$out" "mode=disabled" \
  ...
  ok "HAKUS_DEV_CACHE=0 does not set cargo or sccache vars"
else
  bad "HAKUS_DEV_CACHE=0 does not set cargo or sccache vars" "$out"
fi

# 4. New worktree (no ./target) gets isolated build-dir with the cargo template.
out=$(run_apply \
  HAKUS_DEV_CACHE_REPO_ROOT="$NEW_WT" \
  HAKUS_CACHE_ROOT="$work/cache")
...
out=$(run_apply \
  HAKUS_DEV_CACHE_REPO_ROOT="$OLD_WT" \
  HAKUS_CACHE_ROOT="$work/cache")
if contains "$out" "mode=isolated-build-dir" \
  && contains "$out" "CARGO_BUILD_BUILD_DIR=$work/cache/build/{workspace-path-hash}"; then
...
# 6. HAKUS_DEV_CACHE=local keeps ./target.
out=$(run_apply \
  HAKUS_DEV_CACHE=local \
  HAKUS_DEV_CACHE_REPO_ROOT="$OLD_WT" \
  HAKUS_CACHE_ROOT="$work/cache")
if contains "$out" "mode=existing-target" \
  && contains "$out" "CARGO_BUILD_BUILD_DIR=<unset>"; then
  ok "HAKUS_DEV_CACHE=local keeps ./target"
else
  bad "HAKUS_DEV_CACHE=local keeps ./target" "$out"
fi
...
out=$(run_apply \
  ...
  HAKUS_DEV_CACHE_REPO_ROOT="$NEW_WT" \
  HAKUS_CACHE_ROOT="$work/cache" \
  HAKUS_RUSTC_COMMIT=abc123deadbeef)
...
if contains "$out" "sccache=skipped-incremental" \
  ...
  ok "explicit incremental=1 wins over HAKUS_SCCACHE=1"
else
  bad "explicit incremental=1 wins over HAKUS_SCCACHE=1" "$out"
fi
...
if contains "$out" "sccache=enabled" \
  && contains "$out" "CARGO_INCREMENTAL=0"; then
  ok "HAKUS_SCCACHE=1 requests incremental=0 and wraps"
else
  bad "HAKUS_SCCACHE=1 requests incremental=0 and wraps" "$out"
fi
...
. "$DEV_CACHE"
fp_a=$(hakus_dev_cache_path_fingerprint "$NEW_WT")
fp_b=$(hakus_dev_cache_path_fingerprint "$OLD_WT")
