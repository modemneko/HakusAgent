...
use hakus_config::{ProviderKind, opencode_go_chat_model_id};
...
impl Default for ModelRegistry {
    fn default() -> Self {
        let models = vec![
            ...
            // aligned with `DEFAULT_ZAI_MODEL` in hakus-config.
            ModelInfo {
...
impl ModelRegistry {
    ...
    pub fn resolve(
        ...
        if let Some(name) = requested {
            ...
            // Messages, while Hakus's provider slice intentionally speaks
...
mod tests {
    ...
    fn zai_direct_models_resolve_when_provider_hinted() {
        ...
        // Keep the agent registry fallback aligned with hakus-config's
        // DEFAULT_ZAI_MODEL.
