...
//! can append to that file, `$HAKUS_HOME` is a predictable path, and this
...
//!    `hakus_state::Thread` carries `git_sha`, `git_branch`,
...
///
/// Deliberately **not** derived from the executable: `hakus-tui` serves at
/// least five surfaces, and app-server runs in-process inside `hakus`, so
...
pub enum Surface {
    ...
    Tui,
    /// `hakus exec` — one non-interactive run.
    Exec,
    /// Terminal `config` / `auth` / `update` subcommands.
    Cli,
    /// The app-server protocol surface (in-process inside `hakus`).
    AppServer,
...
///
/// Mirrors `hakus_state::SessionSource` by value, deliberately re-declared
...
pub fn is_known_provider_id(value: &str) -> bool {
    hakus_config::provider::all_providers()
        .iter()
