...
use hakus_config::{ResolvedRuntimeOptions, SetupState};

use crate::buffer;
use crate::event::Surface;

/// Directory name under `$HAKUS_HOME` that holds every telemetry file.
...
/// buffer every time they ran one `hakus exec` with a
/// transient `HAKUS_TELEMETRY=0` — the recipe the runtime docs themselves
/// prescribe.
#[derive(Debug)]
...
impl TelemetryConsent {
    ...
    /// `hakus config set telemetry false`, an external write by another
...
/// `HAKUS_ALLOW_INSECURE_HTTP` is not consulted: it authorizes an insecure
...
/// Resolve the emit predicate, reading the Hakus home from the environment.
///
...
/// Resolve the emit predicate against an explicit Hakus home.
///
...
pub(crate) fn permission_still_enabled_in_home(
    ...
    let Ok(store) = hakus_config::ConfigStore::load(config_path.map(Path::to_path_buf)) else {
        return false;
    };
    let resolved = store
        .config
...
/// setup state, so a `hakus config set telemetry false` written by another
