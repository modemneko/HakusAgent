...
/// `$HAKUS_HOME/telemetry/install_id.json`. Minting a fresh random id is
...
pub fn read_or_create_install_id(root: &Path) -> Result<InstallId> {
    buffer::try_with_lock(root, || {
        ...
        hakus_config::persistence::atomic_write_json(&path, &record)
...
pub fn write_state(root: &Path, state: &TelemetryState) -> Result<()> {
    buffer::try_with_lock(root, || {
        ...
        hakus_config::persistence::atomic_write_json(&path, state)
            .with_context(|| format!("failed to write {}", path.display()))
    })?
    .ok_or_else(|| anyhow::anyhow!("telemetry privacy lock is held"))
}
...
///
/// Sourced from `HAKUS_RELEASE_BUILD_SHA`, a rustc-env this crate's build
...
///
/// Never `HAKUS_BUILD_COMMIT` — that falls back to the builder's own `HEAD`
...
pub fn release_build_sha() -> Option<String> {
    option_env!("HAKUS_RELEASE_BUILD_SHA").and_then(short_hex_sha)
}
