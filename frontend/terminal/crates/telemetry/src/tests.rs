...
fn redaction_kinds_over_leaves(json: &Value) -> Vec<String> {
    ...
    for (_, value) in string_leaves(json) {
        let redaction = hakus_workflow::redaction::redact_for_disclosure(&value);
...
fn panic_site_is_the_only_field_that_may_carry_a_path() {
    ...
    let planted = hakus_workflow::redaction::redact_for_disclosure("crates/tui/src/main.rs");
    assert!(
...
/// tool call the session made on the model's behalf, since `$HAKUS_HOME`
...
fn git_sha_is_null_without_release_env() {
    // The build script emits `HAKUS_RELEASE_BUILD_SHA` only when
    ...
    // The rule that produces it lives in `hakus-build-support` and is
...
fn an_unparseable_env_value_forces_off_and_does_not_wipe() {
    // The floor in `hakus-config` turns an unreadable `HAKUS_TELEMETRY`
...
fn a_run_scoped_kill_switch_costs_a_consenting_user_nothing() {
    // The documented one-command recipe — `HAKUS_TELEMETRY=0 hakus` —
...
fn no_environment_variable_can_authorize_plaintext() {
    // `HAKUS_ALLOW_INSECURE_HTTP` is a *provider* trust decision — it
    ...
    unsafe { std::env::set_var("HAKUS_ALLOW_INSECURE_HTTP", "1") };
    let with_env = validate_endpoint("http://example.com/t");
    unsafe { std::env::remove_var("HAKUS_ALLOW_INSECURE_HTTP") };
...
fn install_id_is_random_and_rotates() {
    ...
    hakus_config::persistence::atomic_write_json(&buffer::install_id_path(&first_root), &stale)
...
fn wipe_and_delivery_share_one_ordering_boundary() {
    ...
    let send = std::thread::spawn(move || {
        crate::client::send_with_transport(
            &send_root,
            Some("https://telemetry.hakus.ai/v1/batch"),
            &every_field_batch(),
            move |_, _, _| {
                send_entered.wait();
                send_release.wait();
...
fn custom_provider_emits_literal_custom() {
    ...
    counters.record_provider(hakus_config::ProviderKind::Custom);
    counters.record_provider(hakus_config::ProviderKind::Deepseek);
    counters.record_provider(hakus_config::ProviderKind::Custom);
...
fn golden_payload_v1() {
    ...
    // v1 was.
    //
    // Re-bless with: `HAKUS_BLESS_TELEMETRY_GOLDEN=1 cargo test -p hakus-telemetry`
    let batch = every_field_batch();
    assert_eq!(
    ...
    if std::env::var("HAKUS_BLESS_TELEMETRY_GOLDEN").is_ok() {
...
fn the_notice_summarizes_what_the_schema_collects_and_states_every_red_line() {
    ...
    assert!(body.contains("hakus config set telemetry false"));
    assert!(!body.contains("HAKUS_TELEMETRY=0"));
