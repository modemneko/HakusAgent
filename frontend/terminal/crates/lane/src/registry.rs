//! Durable lane registry under `$HAKUS_HOME/lanes/`.
...
/// Registry root: `$HAKUS_HOME/lanes`.
pub fn lanes_dir() -> Result<PathBuf> {
    hakus_config::ensure_state_dir(LANES_SUBDIR)
}
...
pub fn lane_registry_root() -> Result<PathBuf> {
    Ok(hakus_config::hakus_home()?.join(LANES_SUBDIR))
}
...
impl LaneRegistry {
    /// Open the default registry under `$HAKUS_HOME/lanes`.
