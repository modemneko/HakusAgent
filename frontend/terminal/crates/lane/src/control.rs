...
//!
//! Why this lives in `hakus-lane`: it is the lowest crate that both the
//! thin `hakus` CLI facade and the TUI (slash commands, hotbar, and the
//! `hakus fleet …` entrypoints that the facade delegates to) already
...
pub enum ControlSurface {
    /// `hakus …` (and the `hakus-tui …` entrypoints it delegates to).
    Cli,
...
pub enum ControlDomain {
    /// One running Workflow, recorded in `$HAKUS_HOME/lanes/`.
    Lane,
    /// Fleet workers and runs, recorded in `<workspace>/.hakus/fleet.jsonl`.
    Fleet,
}
...
pub enum PersistenceScope {
    ...
    Session,
    /// `$HAKUS_HOME/lanes/` records and logs.
    LaneRegistry,
    /// `<workspace>/.hakus/fleet.jsonl`.
    FleetLedger,
}
...
impl ControlOperation {
    ...
    /// dispatch of the same command, and `hakus lane interrupt` cannot
...
pub enum UnavailableReason {
    ...
    /// `$HAKUS_HOME/lanes/` has no records yet.
    NoLaneRegistry,
    /// This workspace has no `.hakus/fleet.jsonl`.
    NoFleetLedger,
}
        ...
        match self.persistence {
            PersistenceScope::LaneRegistry if !ctx.lane_registry_present => {
                Availability::unavailable(
                    ...
                    "no Lane registry yet; start one with `hakus lane start`",
                )
            }
            PersistenceScope::FleetLedger if !ctx.fleet_ledger_present => {
                Availability::unavailable(
                    UnavailableReason::NoFleetLedger,
                    "this workspace has no .hakus/fleet.jsonl; create it with \
                     `hakus fleet init`",
                )
            }
...
const LANE_RESTART_HINT: &str = "Lane restart has no backend: a Lane is one running Workflow and is re-created by \
     `hakus lane start` / `hakus workflow run`, not restarted in place.";
...
const FLEET_RESTART_HINT: &str = "Fleet restart re-leases a task and then drives the manager loop to completion, which \
     only the CLI runs. Use `hakus fleet restart <worker-id>`.";
...
/// `queued` receipt with a ticket. See `hakus-tui::lane_control`.
...
pub static OPERATIONS: &[OperationDescriptor] = &[
    OperationDescriptor {
        ...
        slash_command: "lane",
        cli_invocation: "hakus lane list",
    ...
    OperationDescriptor {
        ...
        slash_command: "lane",
        cli_invocation: "hakus lane status <lane-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "lane",
        cli_invocation: "hakus lane interrupt <lane-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "lane",
        cli_invocation: "hakus lane restart <lane-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "lane",
        cli_invocation: "hakus lane resume <lane-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "hakus fleet list",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "hakus fleet status",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "hakus fleet interrupt <worker-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "hakus fleet restart <worker-id>",
    ...
    OperationDescriptor {
        ...
        hotbar_bare_dispatch: false,
        slash_command: "fleet",
        cli_invocation: "hakus fleet resume <run-id>",
...
pub struct ControlReceipt {
    ...
    ///
    /// Deliberately **not** serialized: it exists so `hakus lane
...
///
/// `hakus lane …`, `/lane …`, and the hotbar dispatch of `/lane` all call
...
/// caller that already resolved `$HAKUS_HOME/lanes`).
#[must_use]
    ...
    if execution.reconciles() {
        ...
        warnings.push(
            "runtime reconciliation skipped on this surface; statuses are as last recorded. \
             Run `hakus lane list` for a reconciled view."
                .to_string(),
        );
    }
    ...
    if execution.reconciles() {
        ...
        detail.push(
            "runtime reconciliation skipped on this surface; status is as last recorded. \
             Run `hakus lane status <lane-id>` for a reconciled view."
                .to_string(),
        );
    }
...
mod tests {
    ...
    fn both_surfaces_map_to_the_same_operation_ids() {
        ...
        for descriptor in OPERATIONS {
            ...
            assert!(
                descriptor.cli_invocation.starts_with("hakus "),
    ...
    fn surface_limited_backends_are_available_only_where_they_exist() {
        ...
        );
        {
            ...
            assert!(
                availability
                    .hint()
                    .is_some_and(|hint| hint.contains("hakus fleet restart")),
    ...
    /// answer is the off-loop executor in `hakus-tui::lane_control` — not a
