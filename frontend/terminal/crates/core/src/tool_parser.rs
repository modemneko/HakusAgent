...
fn get_xml_tool_call_regex() -> &'static Regex {
    XML_TOOL_CALL_REGEX.get_or_init(|| {
        // Match <hakus:tool_call>...</hakus:tool_call> or similar XML patterns
        Regex::new(r"(?s)<(?:hakus:)?tool_call[^>]*>\s*(.*?)\s*</(?:hakus:)?tool_call>")
...
pub fn parse_tool_calls(text: &str) -> ParseResult {
    ...
    // Parse XML-style <hakus:tool_call> or <tool_call> format
