...
use hakus_config::{CliRuntimeOverrides, ConfigToml, ProviderKind};
use hakus_execpolicy::{
    AskForApproval, ExecApprovalRequirement, ExecPolicyContext, ExecPolicyDecision,
    ExecPolicyEngine,
};
use hakus_hooks::{HookDispatcher, HookEvent};
use hakus_mcp::{
...
use hakus_tools::{ToolCall, ToolRegistry};
...
pub fn job_record_to_agent_run(
    record: &JobRecord,
) -> hakus_protocol::agent_run::AgentRunSnapshot {
    use hakus_protocol::agent_run::{
...
impl ThreadManager {
    ...
    pub fn spawn_thread_with_history(
        ...
        let thread = Thread {
            ...
            source: match source {
                SessionSource::Interactive => hakus_protocol::SessionSource::Interactive,
                SessionSource::Resume => hakus_protocol::SessionSource::Resume,
                SessionSource::Fork => hakus_protocol::SessionSource::Fork,
                SessionSource::Api => hakus_protocol::SessionSource::Api,
                SessionSource::Unknown => hakus_protocol::SessionSource::Unknown,
            },
            name: None,
        };
...
impl Runtime {
    ...
    pub async fn mcp_startup(&self) -> McpStartupCompleteEvent {
        ...
        for update in updates {
            let status = match update.status {
                McpManagerStartupStatus::Starting => hakus_protocol::McpStartupStatus::Starting,
                McpManagerStartupStatus::Ready => hakus_protocol::McpStartupStatus::Ready,
                McpManagerStartupStatus::Failed { error } => {
                    hakus_protocol::McpStartupStatus::Failed { error }
                }
                McpManagerStartupStatus::Cancelled => {
                    hakus_protocol::McpStartupStatus::Cancelled
                }
            };
            self.hooks
                .emit(HookEvent::GenericEventFrame {
                    frame: Box::new(EventFrame::McpStartupUpdate {
                        update: hakus_protocol::McpStartupUpdateEvent {
        ...
        self.hooks
            .emit(HookEvent::GenericEventFrame {
                frame: Box::new(EventFrame::McpStartupComplete {
                    summary: hakus_protocol::McpStartupCompleteEvent {
                        ready: summary.ready.clone(),
                        failed: summary
                            .failed
                            .iter()
                            .map(|f| hakus_protocol::McpStartupFailure {
    ...
    pub fn app_status(&self) -> AppResponse {
        ...
        let events = jobs
            .iter()
            .flat_map(|job| {
                job.history.iter().map(|entry| EventFrame::TextDelta {
...
fn to_persisted_source(source: &hakus_protocol::SessionSource) -> SessionSource {
    match source {
        hakus_protocol::SessionSource::Interactive => SessionSource::Interactive,
        hakus_protocol::SessionSource::Resume => SessionSource::Resume,
        hakus_protocol::SessionSource::Fork => SessionSource::Fork,
        hakus_protocol::SessionSource::Api => SessionSource::Api,
        hakus_protocol::SessionSource::Unknown => SessionSource::Unknown,
    }
}
...
fn tool_output_value(output: &hakus_protocol::ToolOutput) -> Value {
...
mod tests {
    use super::*;
    use hakus_protocol::ThreadResumeParams;
    use hakus_tools::ToolCallSource;

    fn temp_core_state(name: &str) -> StateStore {
        let dir =
            std::env::temp_dir().join(format!("hakus-core-{name}-{}", Uuid::new_v4().simple()));
    ...
    fn permission_path_for_call_ignores_shell_payload() {
        let call = ToolCall {
            ...
            payload: ToolPayload::LocalShell {
                params: hakus_protocol::LocalShellParams {
    ...
    fn job_record_to_agent_run_maps_non_terminal_states() {
        use hakus_protocol::agent_run::RunState;
        ...
        for (status, expected) in [
            ...
            assert_eq!(
                snapshot.source,
                hakus_protocol::agent_run::RunSource::CoreJob
            );
            ...
            assert_eq!(
                snapshot.budget,
                hakus_protocol::agent_run::BudgetSummary::default()
            );
        }
    }
    ...
    fn job_record_to_agent_run_maps_terminal_states_without_fabricating_fields() {
        use hakus_protocol::agent_run::{RunState, TerminalOutcome};
    ...
    async fn invoke_tool_returns_timeout_status_for_slow_tools() {
        ...
        use hakus_agent::ModelRegistry;
        use hakus_config::ConfigToml;
        use hakus_execpolicy::{AskForApproval, ExecPolicyEngine};
        use hakus_hooks::HookDispatcher;
        use hakus_mcp::McpManager;
        use hakus_protocol::{ToolKind, ToolOutput, ToolPayload};
        use hakus_tools::{FunctionCallError, ToolDescriptor, ToolHandler, ToolInvocation};
