...
use hakus_agent::ModelRegistry;
use hakus_app_server::{
...
use hakus_execpolicy::{AskForApproval, ExecPolicyContext, ExecPolicyEngine};
use hakus_mcp::{McpServerDefinition, run_stdio_server};
use hakus_secrets::Secrets;
use hakus_state::{StateStore, ThreadListFilters};
use hakus_telemetry::{
...
#[command(
    ...
    bin_name = "hakuscli",
    override_usage = "hakuscli [OPTIONS] [PROMPT]\n       hakuscli [OPTIONS] <COMMAND> [ARGS]"
)]
struct Cli {
    #[arg(long)]
    ...
    /// Workspace directory for Hakus file tools.
...
")]
    ...
    #[command(after_help = "\
Examples:
  hakus workflow run stopship --fleet stopship --runtime tmux --goal verify-release-candidate
  hakus workflow run stopship --fleet stopship --runtime inline --verify
...
Examples:
  hakus lane list
  hakus lane status <lane-id>
  hakus lane attach <lane-id>
  hakus lane logs <lane-id>
  hakus lane interrupt <lane-id>
  hakus lane interrupt <lane-id>@<lifecycle-seq>
  hakus lane start --workflow stopship --fleet stopship --runtime tmux --goal verify-release-candidate -- echo hello

Lane records persist under $HAKUS_HOME/lanes/. tmux durability belongs to
Runtime, not Fleet.
...
")]
    Lane(LaneArgs),
    /// Run a Hakus-powered code review over a git diff.
    ...
    /// Connect third-party harnesses through Hakus (e.g. `integrations dsh status`).
    Integrations(TuiPassthroughArgs),
    /// Run a local Hakus server.
    #[command(after_help = "\
...
`hakus serve --http` and `hakus serve --mobile` remain compatibility
aliases for `hakus app-server --http` and `hakus app-server --mobile`.
New integrations should prefer `hakus app-server`.")]
    ...
    #[command(after_help = "\
Transports:
  hakus app-server --http              Full HTTP/SSE runtime API (/v1/*) on 127.0.0.1:7878
  hakus app-server --mobile            Runtime API + phone control page (binds 0.0.0.0)
  hakus app-server --stdio             JSON-RPC control transport over stdio (no listener)
  hakus app-server                     Legacy in-process app-server HTTP on 127.0.0.1:8787

`--http` and `--mobile` serve the same mature runtime API as `hakus serve
--http`/`--mobile`, which remain as compatibility aliases. The runtime API token
is read from --auth-token, HAKUS_RUNTIME_TOKEN, or DEEPSEEK_RUNTIME_TOKEN.
...
See docs/RUNTIME_API.md.")]
  ...
  Bash (current shell only):
    source <(hakus completion bash)

  Bash (persistent, Linux/bash-completion):
    mkdir -p ~/.local/share/bash-completion/completions
    hakus completion bash > ~/.local/share/bash-completion/completions/hakus
    # Requires bash-completion to be installed and loaded by your shell.

  Zsh:
    mkdir -p ~/.zfunc
    hakus completion zsh > ~/.zfunc/_hakus
  ...
  Fish:
    ...
    hakus completion fish > ~/.config/fish/completions/hakus.fish

  PowerShell (current shell only):
    hakus completion powershell | Out-String | Invoke-Expression
...
The command prints the completion script to stdout; redirect it to a path your shell loads automatically."#)]
    ...
    Metrics(MetricsArgs),
    /// Check for and apply updates to the `hakus` binary.
    Update(UpdateArgs),
}
...
/// `hakus lane …` — running workflow instances (#4176).
#[derive(Debug, Args)]
struct LaneArgs {
    #[command(subcommand)]
    command: LaneCommand,
...
enum LaneCommand {
    ...
    Start {
        ...
        /// Worktree path (defaults to `<repo>/.hakus/lanes/<lane-id>`).
        #[arg(long, value_name = "DIR")]
...
/// `hakus workflow …` — Workflow entrypoints backed by Lanes (#4177/#4178).
#[derive(Debug, Args)]
struct WorkflowArgs {
    #[command(subcommand)]
...
fn start_lane(request: LaneStartRequest) -> Result<()> {
    ...
    let worktree = match (worktree_repo, branch) {
        (Some(repo_root), Some(branch_name)) => {
            let path = worktree_path
                .unwrap_or_else(|| repo_root.join(".hakus").join("lanes").join(&record.id));
    ...
    let spec = LaneStartSpec {
        ...
        log_proxy: (kind == RuntimeBackendKind::Tmux)
            ...
            .transpose()
            .context("resolve current Hakus executable for tmux log proxy")?,
        worktree,
    };
...
/// [`hakus_lane::ControlReceipt`] the slash command and hotbar render, so
/// the three surfaces cannot drift in what they report (#1888).
fn emit_control_receipt(receipt: &hakus_lane::ControlReceipt, json: bool) -> Result<()> {
    if json {
        ...
        match receipt.operation {
            hakus_lane::ControlOperation::LaneList => {
                println!("{}", serde_json::to_string_pretty(&receipt.lane_records)?);
            }
            hakus_lane::ControlOperation::LaneStatus => match receipt.lane_records.first() {
...
fn run_lane_control(
    operation: hakus_lane::ControlOperation,
    lane_id: Option<&str>,
    json: bool,
) -> Result<()> {
    let receipt = hakus_lane::control::execute_lane_control(
        hakus_lane::ControlSurface::Cli,
        operation,
        lane_id,
    );
...
fn run_lane_command(args: LaneArgs) -> Result<()> {
    use hakus_lane::{ControlOperation, LaneRegistry, backend_for};
...
fn run_lane_log_proxy_command(args: LaneLogProxyArgs) -> Result<()> {
    let exit_code = hakus_lane::run_lane_log_proxy(hakus_lane::LaneLogProxySpec {
    ...
    match args.command {
        WorkflowCommand::Run {
            ...
            if let Some(name) = fleet.as_deref() {
                ...
                let loaded =
                    hakus_workflow::load_named_fleet(name, &roots).with_context(|| {
...
fn validate_workflow_source_file(path: &Path) -> Result<()> {
        ...
        || source.contains("\nworkflow(")
    {
        ...
        if path.extension().and_then(|ext| ext.to_str()) == Some("ts") {
            hakus_workflow::compile_typescript_workflow(&identifier, &source)
                .with_context(|| format!("parse declarative Workflow {}", path.display()))?;
        } else {
            hakus_workflow::compile_javascript_workflow(&identifier, &source)
                .with_context(|| format!("parse declarative Workflow {}", path.display()))?;
        }
    }
    Ok(())
...
fn workflow_exec_command(spec: WorkflowExecSpec<'_>) -> Result<WorkflowProcessSpec> {
    ...
    let argv = {
        ...
        let executable = std::env::current_exe()
            .context("resolve current Hakus executable for workflow lane")?;
        let executable = executable.into_os_string().into_string().map_err(|path| {
            anyhow!(
                "current Hakus executable path is not valid UTF-8: {}",
...
struct AppServerArgs {
    ...
    /// API surface; it delegates to the same server as `hakus serve --http`.
    #[arg(long, conflicts_with_all = ["stdio", "mobile"])]
    http: bool,
    /// Serve the runtime API plus the phone-friendly mobile control page.
    /// Equivalent to the legacy `hakus serve --mobile`.
    ...
    match command {
        ...
        Some(Commands::Auth(args)) => match args.command {
            ...
            command @ AuthCommand::Status {
                ...
                // `$HAKUS_HOME/telemetry` before the report could truthfully
        ...
        Some(Commands::Completion { shell }) => {
            ...
            generate(shell, &mut cmd, "hakuscli", &mut io::stdout());
            Ok(())
        }
...
fn root_tui_passthrough(cli: &Cli) -> Result<Vec<String>> {
    ...
    if !prompt.is_empty() {
        if cli.continue_session {
            bail!(
                "`hakus --continue` resumes the interactive TUI. Use `hakus exec --continue <PROMPT>` to continue a session non-interactively."
            );
        }
...
/// what makes `hakus config set telemetry false` take effect on the very run
...
fn run_login_command_with_secrets(
    ...
    let destination = if secret_store_saved {
        ...
        hakus_config::quote_os_path(store.path())
    };
...
fn run_logout_command_with_secrets(store: &mut ConfigStore, secrets: &Secrets) -> Result<()> {
    hakus_config::with_xai_oauth_revocation_transaction(|| {
...
/// workspace-scoped document (`<repo>/.hakus/config.toml`), login and
...
fn credential_metadata_store(store: &ConfigStore) -> Result<Option<ConfigStore>> {
    if !hakus_config::config_path_is_workspace_scoped(store.path()) {
        return Ok(None);
    }
    let global = hakus_config::default_config_path()?;
    eprintln!(
        "ambient config {} is workspace-scoped; writing credential metadata to the user-global {} instead",
        hakus_config::quote_os_path(store.path()),
        hakus_config::quote_os_path(&global),
    );
...
fn no_keyring_secrets() -> Secrets {
    Secrets::new(std::sync::Arc::new(
        hakus_secrets::InMemoryKeyringStore::new(),
    ))
}
...
fn persist_provider_api_key_unlocked(
    ...
    let secret_store_saved = match prior_secret.as_ref().map_err(|error| error.to_string()) {
        Ok(_) => match secrets.set(slot, api_key) {
            ...
            Err(err) => {
                ...
                return Err(anyhow::anyhow!(
                    "Secret storage write failed for {slot}: {err}. Refusing to write the API key in plaintext to {}. Fix the configured secret backend and retry; Hakus did not change that file.",
                    hakus_config::quote_os_path(store.path())
                ));
            }
        },
        Err(error) => {
            store.config = original_config;
            return Err(anyhow::anyhow!(
                "Secret storage snapshot failed for {slot}: {error}. Refusing to write the API key in plaintext to {}. Fix the configured secret backend and retry; Hakus did not change that file.",
                hakus_config::quote_os_path(store.path())
            ));
        }
    };
    ...
    hakus_config::scrub_plaintext_api_keys_from_config_backup(store.path())?;
...
fn openai_codex_auth_file_path() -> PathBuf {
    if let Ok(path) = std::env::var("OPENAI_CODEX_AUTH_FILE") {
        ...
        if !path.as_os_str().is_empty() {
            return hakus_config::resolve_external_credential_path(&path).unwrap_or(path);
        }
    }
    ...
    hakus_config::resolve_external_credential_path(&path).unwrap_or(path)
}
...
fn external_credential_target(
...
) -> Result<(hakus_config::ExternalCredentialSource, PathBuf)> {
    let (source, default_path) = match provider {
        ...
        ProviderKind::Moonshot => bail!(
            "Kimi is API-key-only in Hakus. Create a key at https://platform.kimi.ai/console/api-keys; Kimi CLI OAuth import is unsupported."
        ),
        _ => bail!(
    ...
    let path =
        hakus_config::resolve_external_credential_path(path_override.unwrap_or(default_path))?;
    Ok((source, path))
}
...
fn external_consent(
    store: &ConfigStore,
    provider: ProviderKind,
) -> Option<&hakus_config::ExternalCredentialConsentToml> {
    store
        .config
        .providers
        .for_provider(provider)
...
enum XaiAuthDiagnosticRoute {
    ...
    ApiKey,
    /// A syntactically valid Hakus-owned generation pointer selects the
...
fn xai_oauth_generation_pointer(store: &ConfigStore) -> XaiOAuthGenerationPointer {
        ...
        .as_deref()
    {
        ...
        Some(generation) if hakus_config::is_valid_xai_oauth_generation(generation) => {
...
fn xai_status_summary_source(
    ...
    match diagnostics.route {
        XaiAuthDiagnosticRoute::OwnedOAuth => {
            "Hakus-owned OAuth configured/unprobed (valid generation pointer)".to_string()
        }
...
fn xai_credential_route_label(
    ...
    match diagnostics.route {
        XaiAuthDiagnosticRoute::OwnedOAuth => {
            "Hakus-owned OAuth configured/unprobed (valid generation pointer; storage unprobed)"
                .to_string()
        }
        XaiAuthDiagnosticRoute::NeedsRepair => {
            let api_key = api_key
            ...
            format!(
                "xAI OAuth needs repair (invalid Hakus-owned generation pointer; Grok CLI consent blocked; API-key fallback: {api_key})"
            )
        }
        XaiAuthDiagnosticRoute::ExternalConsent => {
...
fn xai_lookup_order(diagnostics: &XaiAuthDiagnostics) -> String {
    match diagnostics.route {
        XaiAuthDiagnosticRoute::OwnedOAuth => {
            "lookup order: configured Hakus-owned OAuth generation (storage unprobed); Grok CLI consent blocked".to_string()
        }
        XaiAuthDiagnosticRoute::NeedsRepair => {
            "lookup order: invalid Hakus-owned OAuth generation blocks Grok CLI consent; runtime-effective API-key fallback: CLI -> config -> secret store -> env".to_string()
        }
        XaiAuthDiagnosticRoute::ExternalConsent => {
...
fn xai_get_line(diagnostics: &XaiAuthDiagnostics, api_key: Option<&XaiRuntimeApiKey>) -> String {
    match diagnostics.route {
        XaiAuthDiagnosticRoute::OwnedOAuth => {
            "xai: configured (source: Hakus-owned OAuth generation; valid pointer; storage unprobed)".to_string()
        }
        XaiAuthDiagnosticRoute::NeedsRepair => {
            ...
            format!(
                "xai: needs repair (invalid Hakus-owned OAuth generation pointer; Grok CLI consent blocked; API-key fallback: {api_key})"
            )
        }
        XaiAuthDiagnosticRoute::ExternalConsent => {
...
fn auth_status_all_providers_with_runtime(
    ...
    lines.push(format!(
        "active provider: {} (set via config or HAKUS_PROVIDER)",
    ...
    lines.push(String::new());
    lines.push("* = active provider (from config or HAKUS_PROVIDER)".to_string());
    lines.push("Run `hakus auth status --provider <id>` for detailed info.".to_string());
    lines
}
...
const fn secret_backend_kind_label(
    kind: hakus_secrets::SecretBackendDiagnosticKind,
) -> &'static str {
    match kind {
        hakus_secrets::SecretBackendDiagnosticKind::File => "file",
        hakus_secrets::SecretBackendDiagnosticKind::System => "system",
        hakus_secrets::SecretBackendDiagnosticKind::Unknown => "unknown",
    }
}

const fn secret_backend_inspection_label(
    inspection: hakus_secrets::SecretBackendInspection,
) -> &'static str {
    match inspection {
        hakus_secrets::SecretBackendInspection::MetadataOnly => "metadata_only",
        hakus_secrets::SecretBackendInspection::NotProbed => "not_probed",
    }
}

const fn secret_backend_presence_label(
    presence: hakus_secrets::SecretBackendPresence,
) -> &'static str {
    match presence {
        hakus_secrets::SecretBackendPresence::Present => "present",
        hakus_secrets::SecretBackendPresence::Absent => "missing",
        hakus_secrets::SecretBackendPresence::Unknown => "unknown",
    }
}
...
fn auth_diagnostic_lines(store: &ConfigStore, provider: Option<ProviderKind>) -> Vec<String> {
    let explicit_home = hakus_paths::hakus_home_is_explicit();
    let resolved_home = hakus_paths::hakus_home();
    let mut lines = vec![
    ...
    let home = match resolved_home {
        ...
        Ok(None) => {
            lines.push("hakus home: unavailable (no user home resolved)".to_string());
            None
        }
        Err(error) => {
            lines.push(format!("hakus home: unavailable ({error})"));
            None
        }
    };
    ...
    if let Some(home) = home.as_ref() {
        ...
        lines.push(format!(
            "settings: {} ({})",
            hakus_config::quote_os_path(&settings),
            diagnostic_path_state(&settings, false),
        ));
    } else {
        lines.push("settings: unavailable (Hakus home unresolved)".to_string());
    }

    let backend = hakus_secrets::diagnose_secret_backend();
    lines.push(format!(
...
fn auth_status_lines_for_provider_with_runtime(
    ...
    if let Ok((source, expected_path)) = external_credential_target(provider, None) {
        let status = hakus_config::external_credential_consent_status(
            external,
            provider,
            source,
            &expected_path,
...
fn xai_auth_status_lines_for_provider(
    ...
    lines.push(match diagnostics.generation {
        ...
        {
            "xAI OAuth generation: configured Hakus-owned pointer (storage unprobed)"
                .to_string()
        }
        XaiOAuthGenerationPointer::Valid => {
        ...
        XaiOAuthGenerationPointer::Invalid => {
            "xAI OAuth generation: invalid Hakus-owned pointer".to_string()
        }
    });
    ...
    match diagnostics.route {
        XaiAuthDiagnosticRoute::OwnedOAuth => {
            lines.push(
                "external credentials: blocked by the configured Hakus-owned xAI OAuth generation (file not probed)"
                    .to_string(),
            );
            return lines;
        }
        XaiAuthDiagnosticRoute::NeedsRepair => {
            lines.push(
                "external credentials: blocked by the invalid Hakus-owned xAI OAuth generation pointer (file not probed)"
                    .to_string(),
            );
            lines.push(
                "repair: run `hakus auth xai-device` to replace the owned generation, or switch [providers.xai] auth_mode to \"api_key\" and remove oauth_credential_generation. Grok CLI consent remains blocked until the pointer is absent."
                    .to_string(),
            );
            return lines;
        }
    ...
    match command {
        ...
        AuthCommand::ExternalConsent {
            ...
            if mode == ExternalCredentialModeArg::Managed {
                bail!(
                    "managed external credential access is unsupported in v0.9.1: no provider has a reviewed schema-safe preservation adapter. Use --mode read-only, or use Hakus-owned login/API-key storage."
                );
            }
            ...
            hakus_config::mutate_config_document(store.path(), |document| {
                if matches!(provider, ProviderKind::OpenaiCodex | ProviderKind::Xai) {
                    hakus_config::set_config_document_value(
                        document,
            ...
            println!(
                ...
                source.as_str(),
                hakus_config::quote_os_path(&path),
                hakus_config::EXTERNAL_CREDENTIAL_CONSENT_VERSION,
                hakus_config::EXTERNAL_CREDENTIAL_READ_ONLY_SEMANTICS,
            );
            println!(
                "revoke with: hakus auth external-revoke --provider {}",
                provider.as_str()
            );
            Ok(())
        }
        AuthCommand::ExternalRevoke { provider } => {
            ...
            let provider_key = provider.provider().provider_config_key();
            hakus_config::mutate_config_document(store.path(), |document| {
                hakus_config::unset_config_document_value(
                    document,
...
fn external_consent_preview_lines(
    ...
    vec![
        ...
        format!(
            "  revoke: hakus auth external-revoke --provider {}",
            provider.as_str()
        ),
    ]
}
...
fn run_config_command(store: &mut ConfigStore, command: ConfigCommand) -> Result<()> {
    match command {
        ...
        ConfigCommand::List => {
            ...
            println!(
                "# a running session keeps the route it resolved at launch; `hakus model resolve` reports the route a new session would take"
            );
...
fn clear_recorded_telemetry_opt_out_if_reenabled(key: &str, value: &str) -> Result<()> {
    ...
    {
        state.record_telemetry_notice(hakus_config::TELEMETRY_NOTICE_VERSION, true);
        state.save()?;
    }
    Ok(())
}
...
fn run_model_command(
    ...
    match command {
        ...
        ModelCommand::Resolve { model, provider } => {
            ...
            // a hypothetical made `hakus --provider moonshot --model
...
fn run_thread_command(
    ...
    // launched this way re-resolved from `$HAKUS_HOME/config.toml` with no
...
/// `hakus app-server --http`/`--mobile`. Maps app-server flags onto the
...
/// the runtime API reads HAKUS_RUNTIME_TOKEN/DEEPSEEK_RUNTIME_TOKEN itself.
...
fn app_server_token_from_env() -> Option<String> {
    std::env::var("HAKUS_APP_SERVER_TOKEN")
        .ok()
...
fn run_dispatcher_resume_picker(
    ...
    println!("You can also run `hakus resume --last` to skip this prompt.");
...
fn apply_tui_env(cli: &Cli, resolved_runtime: &ResolvedRuntimeOptions, passthrough: &[String]) {
    ...
    if let Some(provider) = cli.provider.as_deref() {
        ...
        unsafe {
            std::env::set_var("HAKUS_PROVIDER", &provider);
    ...
    if let Some(base_url) = cli.base_url.as_ref() {
        unsafe {
            std::env::set_var("HAKUS_BASE_URL", base_url);
...
mod tests {
    ...
    use hakus_config::{ModelSource, ProviderSource};
    ...
    impl hakus_secrets::KeyringStore for RecordingKeyringStore {
        fn get(
            &self,
            key: &str,
        ) -> std::result::Result<Option<String>, hakus_secrets::SecretsError> {
            self.gets
                .lock()
        ...
        fn set(
            ...
            key: &str,
            value: &str,
        ) -> std::result::Result<(), hakus_secrets::SecretsError> {
            self.set_value(key, value);
            Ok(())
        }

        fn delete(&self, key: &str) -> std::result::Result<(), hakus_secrets::SecretsError> {
            self.values
                .lock()
                .expect("recording values lock")
                .remove(key);
    ...
    fn resolved_runtime_for_test(
        ...
        ResolvedRuntimeOptions {
            ...
            log_level: None,
            telemetry: false,
            telemetry_source: hakus_config::TelemetrySource::Default,
    ...
    fn parses_update_beta_flag() {
        ...
        let cli = parse_ok(&["hakus", "update", "--beta"]);
        assert!(matches!(
            cli.command,
            Some(Commands::Update(UpdateArgs {
                beta: true,
        ...
        let cli = parse_ok(&["hakus", "update", "--check"]);
        assert!(matches!(
            cli.command,
            Some(Commands::Update(UpdateArgs {
                beta: false,
        ...
        let cli = parse_ok(&["hakus", "update", "--proxy", "socks5://127.0.0.1:1080"]);
    ...
    fn model_command_provider_hint_uses_subcommand_then_top_level_provider() {
        ...
        let cli = parse_ok(&["hakus", "--provider", "zai", "model", "list"]);
    ...
    fn web_command_is_typed_and_delegates_without_auth_material() {
        let cli = parse_ok(&["hakus", "web", "--port", "9091"]);
    ...
    fn web_command_defaults_to_runtime_port_and_documents_bootstrap_boundary() {
        let cli = parse_ok(&["hakus", "web"]);
        assert!(matches!(
            cli.command,
            Some(Commands::Web(WebArgs { port: 7878 }))
        ));
        let help = help_for(&["hakus", "web", "--help"]);
    ...
    fn serve_help_documents_forwarded_runtime_modes() {
        let help = help_for(&["hakus", "serve", "--help"]);
    ...
    fn parses_direct_tui_command_aliases() {
        ...
        let cli = parse_ok(&["hakus", "fleet", "init"]);
    ...
    fn exec_and_fleet_accept_builtin_and_raw_provider_identifiers() {
        let builtin = parse_ok(&["hakus", "--provider", "openrouter", "exec", "Reply OK"]);
    ...
    fn raw_provider_ids_remain_restricted_to_exec_and_fleet() {
        let cli = parse_ok(&["hakus", "--provider", "lm-studio", "model", "list"]);
        ...
        let err = Cli::try_parse_from(["hakus", "auth", "set", "--provider", "lm-studio"])
    ...
    fn hidden_lane_log_proxy_parses_child_argv_and_preserves_other_commands() {
        ...
        let cli = parse_ok(&["hakus", "lane", "list", "--json"]);
        let (proxy, command) = split_lane_log_proxy_command(cli.command);
    ...
    fn cli_lane_subcommands_cover_the_shared_control_contract() {
        use hakus_lane::{ControlDomain, ControlOperation, ControlSurface};

        for descriptor in hakus_lane::control::operations_for_domain(ControlDomain::Lane) {
            let argv = [
                "hakus".to_string(),
                "lane".to_string(),
    ...
    fn lane_stop_and_interrupt_resolve_to_one_verb() {
        use hakus_lane::{ControlDomain, ControlOperation};
        ...
        let stop = parse_ok(&["hakus", "lane", "stop", "lane-a1b2c3d4"]);
        assert!(matches!(
            stop.command,
            Some(Commands::Lane(LaneArgs {
    ...
    fn deepseek_login_uses_isolated_file_store_and_preserves_tui_defaults() {
        let _lock = env_lock();
        let dir = tempfile::TempDir::new().expect("tempdir");
        let hakus_home = dir.path().join("hakus-home");
        let hakus_home_value = hakus_home.to_string_lossy().into_owned();
        let _home = ScopedEnvVar::set("HAKUS_HOME", &hakus_home_value);
        let _backend = ScopedEnvVar::set("HAKUS_SECRET_BACKEND", "file");
        let path = hakus_home.join("config.toml");
    ...
    /// #5198: with HAKUS_CONFIG_PATH pointing at a workspace-scoped
    /// `<repo>/.hakus/config.toml`, login must write the provider binding
    ...
    fn login_with_repo_scoped_ambient_config_writes_user_global_metadata() {
        ...
        let repo_config_dir = repo.join(".hakus");
        ...
        let hakus_home = dir.path().join("hakus-home");
        let _home = ScopedEnvVar::set("HAKUS_HOME", &hakus_home.to_string_lossy());
        let _config = ScopedEnvVar::set("HAKUS_CONFIG_PATH", &repo_config.to_string_lossy());
        ...
        let global_config = hakus_home.join("config.toml");
    ...
    fn auth_set_with_repo_scoped_ambient_config_writes_user_global_metadata() {
        ...
        let global = std::fs::read_to_string(hakus_home.join("config.toml"))
    ...
    fn auth_set_refuses_plaintext_config_when_secret_store_write_fails() {
        use hakus_secrets::{KeyringStore, SecretsError};
        use std::sync::Arc;
        ...
        assert!(
            message.contains(&hakus_config::quote_os_path(store.path())),
            "{message}"
        );
    ...
    fn auth_diagnostic_reports_paths_and_presence_without_values() {
        ...
        let _home = ScopedEnvVar::set("HAKUS_HOME", &home.to_string_lossy());
        let _backend = ScopedEnvVar::set("HAKUS_SECRET_BACKEND", "file");
        ...
        assert!(
            output.contains(&format!(
                "hakus home: {} (source: HAKUS_HOME (isolated); state: present)",
                hakus_config::quote_os_path(&home)
            )),
            "{output}"
        );
        assert!(
        ...
        assert!(
            output.contains(&format!(
                "settings: {} (present)",
                hakus_config::quote_os_path(&settings_path)
            )),
            "{output}"
        );
        assert!(
        ...
        assert!(
            output.contains(&format!(
                ...
                hakus_config::quote_os_path(&secret_path)
            )),
            "{output}"
        );
        assert!(
        ...
        assert!(
            output.contains("legacy secret store: suppressed by explicit HAKUS_HOME isolation"),
            "{output}"
        );
    ...
    fn auth_status_never_probes_codex_file_and_reports_exact_consent() {
        use hakus_secrets::InMemoryKeyringStore;
        use std::sync::Arc;
        ...
        store.config.providers.openai_codex.external_credentials =
            Some(hakus_config::ExternalCredentialConsentToml::read_only(
                ProviderKind::OpenaiCodex,
                hakus_config::ExternalCredentialSource::CodexCli,
                auth_path.clone(),
            ));
        ...
        assert!(output.contains(&format!(
            "path={}",
            hakus_config::quote_os_path(&auth_path)
        )));
        assert!(output.contains(&format!(
            "consent_version={}",
            hakus_config::EXTERNAL_CREDENTIAL_CONSENT_VERSION
        ...
        assert!(
            changed.contains(&hakus_config::quote_os_path(&auth_path)),
            "{changed}"
        );
    ...
    fn xai_valid_owned_generation_blocks_external_consent_without_storage_probes() {
        ...
        assert!(
            scoped.contains(
                "credential route: Hakus-owned OAuth configured/unprobed (valid generation pointer; storage unprobed)"
            ),
            "{scoped}"
        );
        assert!(scoped.contains("external credentials: blocked by the configured Hakus-owned xAI OAuth generation"), "{scoped}");
        assert!(
            scoped.contains(
                "xAI OAuth generation: configured Hakus-owned pointer (storage unprobed)"
            ),
        ...
        assert!(
            !scoped.contains("active source: Hakus-owned OAuth"),
        ...
        assert!(
            xai_row.contains("Hakus-owned OAuth configured/unprobed"),
            "{xai_row}"
        );
        ...
        assert!(
            get.starts_with("xai: configured (source: Hakus-owned OAuth generation"),
            "{get}"
        );
    ...
    fn xai_invalid_generation_requires_repair_blocks_external_and_keeps_api_key_diagnostics() {
        ...
        assert!(scoped.contains("external credentials: blocked by the invalid Hakus-owned xAI OAuth generation pointer"), "{scoped}");
        assert!(
            scoped.contains("repair: run `hakus auth xai-device`"),
            "{scoped}"
        );
        assert!(
    ...
    fn external_consent_persists_exact_scope_and_api_key_or_revoke_disables_it() {
        ...
        assert!(
            preview.contains(&format!(
                "exact resolved path: {}",
                hakus_config::quote_os_path(&external_path)
            )),
            "{preview}"
        );
        ...
        assert_eq!(
            consent.access,
            hakus_config::ExternalCredentialAccess::ReadOnly
        );
        ...
        run_auth_command_with_secrets(
            ...
            AuthCommand::Set {
                provider: ProviderArg::Xai,
                api_key: Some("xai-hakus-owned-key".to_string()),
                api_key_stdin: false,
            },
            &secrets,
        )
        .expect("Hakus-owned API key should supersede external consent");
        assert!(store.config.providers.xai.external_credentials.is_none());
        assert_eq!(
    ...
    fn auth_status_scoped_provider_shows_detailed_info() {
        ...
        for sentinel in [hakus_config::API_KEYRING_SENTINEL, "  __KEYRING__  "] {
    ...
    fn logout_removes_plaintext_provider_keys() {
        ...
        hakus_config::with_xai_oauth_lifecycle_lock(|owned| {
            owned.write(generation, b"xai-generation", false)?;
            owned.write(
                hakus_config::LEGACY_XAI_OAUTH_FILE_NAME,
                b"legacy-xai",
                false,
            )?;
            Ok(())
        })
        .expect("seed Hakus-owned xAI credentials");
    ...
    fn cli_provider_helpers_follow_config_metadata() {
        let registry_kinds: Vec<ProviderKind> = hakus_config::provider::all_providers()
            .iter()
    ...
    fn the_telemetry_flag_documents_itself_in_help() {
        ...
        assert!(
            telemetry_line.contains("HAKUS_TELEMETRY=0 always")
    ...
    fn root_help_describes_product_actions_not_internal_tui_layers() {
        ...
        assert!(
            !help.contains("TUI"),
            "root help must describe what Hakus does, not its internal UI/runtime layers:\n{help}"
        );
    }
    ...
    fn only_one_function_may_locate_and_spawn_the_tui() {
        ...
        // two-process glue has been deleted; the only TUI entry is hakus_tui::run.
    ...
    fn parses_no_project_config_before_subcommand() {
        let cli = parse_ok(&["hakus", "--no-project-config", "exec", "list the files"]);
    ...
    fn no_project_config_after_passthrough_subcommand_is_not_the_dispatcher_flag() {
        ...
        let cli = parse_ok(&["hakus", "exec", "--no-project-config", "hi"]);
        assert!(!cli.no_project_config);
        match cli.command {
            Some(Commands::Exec(args)) => {
    ...
    fn parses_top_level_continue_for_interactive_resume() {
        let cli = parse_ok(&["hakus", "--continue"]);
        ...
        let cases = [
            ...
            (
                ...
                vec![
                    ...
                    "bash",
                    "source <(hakus completion bash)",
                    "~/.local/share/bash-completion/completions/hakus",
                    "fpath=(~/.zfunc $fpath)",
                    "hakus completion fish > ~/.config/fish/completions/hakus.fish",
                    "hakus completion powershell | Out-String | Invoke-Expression",
                ],
