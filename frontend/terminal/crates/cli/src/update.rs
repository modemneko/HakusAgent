...
pub fn run_update(beta: bool, check_only: bool, proxy_arg: Option<String>) -> Result<()> {
    ...
    if check_only {
        ...
        if update_is_needed(channel, current_version, latest_tag)? {
            println!("Update available. Run `hakus update` to install {latest_tag}.");
            println!(
                "Release source: {}",
    ...
    // installed `codew` and pre-0.9.5 `hakus-tui` command paths are
...
enum UpdateReleaseSource {
    ...
    Cnb { base_url: String },
    /// An operator-supplied asset directory (`HAKUS_RELEASE_BASE_URL`).
...
fn cnb_source_candidate(tag_name: &str, os: &str, rust_arch: &str) -> ReleaseSourceCandidate {
    ...
    let binary_name = release_asset_name_for_prefix("hakus", os, rust_arch);
...
The package has been renamed to `hakus`. This update will install the
canonical `hakus` command and refresh any existing `codew` or
`hakus-tui` compatibility command from the same binary beside the legacy
...
original install method:
  ...
  Cargo:
    ...
    cargo install hakus-cli --locked
...
fn update_plan_for_exe(current_exe: &Path) -> UpdatePlan {
    ...
    let primary = installed_command_path(current_exe, "hakus");
    push_unique_path(&mut target_paths, primary);

    for alias in ["codew", "hakus-tui"] {
        let alias_path = installed_command_path(current_exe, alias);
        let migrate_legacy_tui = alias == "hakus-tui"
...
fn release_asset_stem_for(current_exe: &Path, os: &str, rust_arch: &str) -> String {
    let _ = current_exe;
    release_asset_stem_for_prefix("hakus", os, rust_arch)
}
...
fn update_http_client_with_timeout(
    ...
    let mut builder = hakus_release::platform_blocking_http_client_builder();
...
///
/// `HAKUS_USE_CNB_MIRROR` and `HAKUS_RELEASE_BASE_URL` both resolve to
...
fn release_from_asset_base_url(
    ...
    let name = release_asset_name_for_prefix("hakus", os, rust_arch);
    assets.push(Asset {
...
fn fetch_latest_beta_release_from_url(url: &str, proxy: Option<&Proxy>) -> Result<Release> {
    ...
    // GitHub caps this endpoint at 100 releases per page. Hakus uses the
...
fn glibc_compatibility_message(
    ...
    format!(
        "\
Prebuilt Hakus asset `{asset_name}` requires GLIBC_{required}, but {host_line}
...
baseline, or add a musl/static Linux asset. Set HAKUS_SKIP_GLIBC_CHECK=1 to
...
mod tests {
    ...
    const UPDATE_ENV_VARS: &[&str] = &[
        hakus_release::RELEASE_BASE_URL_ENV,
        hakus_release::LEGACY_RELEASE_BASE_URL_ENV,
        hakus_release::DEEPSEEK_RELEASE_BASE_URL_ENV,
        hakus_release::CNB_MIRROR_ENV,
        hakus_release::UPDATE_VERSION_ENV,
        hakus_release::LEGACY_UPDATE_VERSION_ENV,
    ];
    ...
    fn android_loaded_image_resolves_agreed_mapping() {
        ...
        let executable = dir.path().join("hakus");
    ...
    fn android_loaded_image_canonicalizes_symlink_and_sibling_policy() {
        ...
        let canonical_dispatcher = canonical_dir.join("hakus");
        let canonical_tui = canonical_dir.join("hakus-tui");
        let invoked = install_dir.join("hakus");
    ...
    fn android_loaded_image_rejects_relative_or_deleted_paths() {
        ...
        let relative_maps = format!(
            "1000-2000 r-xp 00000000 {:x}:{:x} {} hakus\n",
            device_major,
            device_minor,
            metadata.ino()
        );
    ...
    fn android_loaded_image_rejects_linker_and_symlink_to_linker() {
        ...
        let invoked = dir.path().join("hakus");
    ...
    fn android_linker_name_recognizes_bionic_loader_variants() {
        ...
        assert!(!is_android_linker_name(Path::new("hakus")));
    }
    ...
    fn android_loaded_image_rejects_authority_disagreement() {
        let dir = tempfile::TempDir::new().unwrap();
        let mapped = dir.path().join("mapped-hakus");
        let dladdr = dir.path().join("dladdr-hakus");
    ...
    fn android_loaded_image_recheck_detects_pre_replace_swap() {
        let dir = tempfile::TempDir::new().unwrap();
        let candidate = dir.path().join("hakus");
    ...
    fn android_identity_preflight_prevents_all_paired_replacements() {
        let dir = tempfile::TempDir::new().unwrap();
        let primary = dir.path().join("hakus");
        let sibling = dir.path().join("hakus-tui");
    ...
    #[test]
    fn every_invocation_name_uses_hakus_release_asset() {
        for command in [
            "hakus",
            "hakus.exe",
            "codew",
    ...
    fn test_is_legacy_binary_detection() {
        ...
        assert!(!is_legacy_binary(Path::new("hakus")));
        assert!(!is_legacy_binary(Path::new("hakus-tui")));
    ...
    fn legacy_binary_message_gives_copy_pasteable_migration_steps() {
        ...
        assert!(message.contains("canonical `hakus` command"));
        ...
        assert!(message.contains("npm install -g hakus"));
        ...
        assert!(message.contains("cargo install hakus-cli --locked"));
    ...
    fn legacy_dispatcher_update_targets_canonical_compatibility_commands() {
        ...
        assert_eq!(
            ...
            vec![
                dir.path()
                    .join(format!("hakus{}", std::env::consts::EXE_SUFFIX)),
                dir.path()
                    .join(format!("hakus-tui{}", std::env::consts::EXE_SUFFIX))
            ]
        );
        assert!(plan.asset_stem.starts_with("hakus-"));
        assert!(!plan.asset_stem.starts_with("hakus-tui-"));
    }
    ...
    fn test_release_asset_stem_for_supported_platforms() {
        let cases = [
            ("hakus", "macos", "aarch64", "hakus-macos-arm64"),
            ("hakus", "macos", "x86_64", "hakus-macos-x64"),
            ("hakus", "linux", "x86_64", "hakus-linux-x64"),
            ("hakus", "windows", "x86_64", "hakus-windows-x64"),
            ("hakus", "windows", "aarch64", "hakus-windows-arm64"),
            ("codew", "macos", "aarch64", "hakus-macos-arm64"),
            ("hakus-tui", "linux", "x86_64", "hakus-linux-x64"),
        ];
    ...
    fn v094_three_command_install_updates_every_path_from_primary_bytes() {
        ...
        assert!(plan.asset_stem.starts_with("hakus-"));
        assert!(!plan.asset_stem.contains("hakus-tui"));
    ...
    fn test_asset_matching_accepts_binary_assets_and_rejects_checksums() {
        ...
        assert!(asset_matches_platform(
            "hakus-tui-windows-x64.exe",
            "hakus-tui-windows-x64"
        ));
        assert!(!asset_matches_platform(
            "hakus-tui-windows-x64.exe.sha256",
            "hakus-tui-windows-x64"
        ));
        assert!(!asset_matches_platform(
            "hakus-macos-aarch64.tar.gz",
            "hakus-macos-arm64"
        ));
    }
    ...
    fn select_platform_asset_prefers_bare_binary_over_archive() {
        let release = Release {
            ...
            assets: vec![
                Asset {
                    name: "hakus-macos-arm64.tar.gz".to_string(),
                    browser_download_url: "https://example.invalid/hakus-macos-arm64.tar.gz"
                        .to_string(),
                },
                Asset {
                    name: "hakus-macos-arm64".to_string(),
                    browser_download_url: "https://example.invalid/hakus-macos-arm64"
                        .to_string(),
                },
            ],
        };
        ...
        let asset =
            select_platform_asset(&release, "hakus-macos-arm64").expect("platform asset");

        assert_eq!(asset.name, "hakus-macos-arm64");
    }
    ...
    fn select_platform_asset_falls_back_to_archive_when_bare_binary_is_missing() {
        ...
        assert_eq!(asset.name, "hakus-macos-arm64.tar.gz");
    }
    ...
    #[test]
    fn glibc_compatibility_message_is_hakus_branded_and_actionable() {
        ...
        assert!(message.contains("Prebuilt Hakus asset `hakus-linux-x64`"));
    ...
    fn parse_checksum_manifest_accepts_sha256sum_format() {
        let manifest = "\
2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824  hakus-macos-arm64
E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855  *hakus-windows-x64.exe
";
        let checksums = parse_checksum_manifest(manifest).expect("valid manifest");

        assert_eq!(
            checksums.get("hakus-macos-arm64").map(String::as_str),
            Some("2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824")
        );
        assert_eq!(
            checksums
                .get("hakus-windows-x64.exe")
                .map(String::as_str),
    ...
    fn parse_checksum_manifest_rejects_malformed_lines() {
        let err = parse_checksum_manifest("not-a-hash  hakus-macos-arm64")
    ...
    fn expected_sha256_from_manifest_requires_matching_asset() {
        ...
        let err = expected_sha256_from_manifest(manifest, "hakus-macos-arm64")
            .expect_err("missing asset should fail");
        assert!(
            err.to_string()
                .contains("checksum manifest is missing hakus-macos-arm64"),
            "unexpected error: {err:#}"
        );
    }
    ...
    fn test_replace_binary_creates_and_replaces() {
        let dir = tempfile::TempDir::new().unwrap();
        let target = dir.path().join("hakus-test");
    ...
    fn test_replace_binary_creates_new_file() {
        let dir = tempfile::TempDir::new().unwrap();
        let target = dir.path().join("hakus-new-test");
    ...
    fn mocked_release() -> Release {
        let json = r#"{
          "tag_name": "v0.8.8",
          "assets": [
            { "name": "hakus-linux-x64",          "browser_download_url": "https://example.invalid/hakus-linux-x64" },
            { "name": "hakus-macos-x64",          "browser_download_url": "https://example.invalid/hakus-macos-x64" },
            { "name": "hakus-macos-arm64",        "browser_download_url": "https://example.invalid/hakus-macos-arm64" },
            { "name": "hakus-windows-x64.exe",    "browser_download_url": "https://example.invalid/hakus-windows-x64.exe" },
            { "name": "hakus-windows-x64.exe.sha256", "browser_download_url": "https://example.invalid/hakus-windows-x64.exe.sha256" },
            { "name": "hakus-windows-arm64.exe",  "browser_download_url": "https://example.invalid/hakus-windows-arm64.exe" }
          ]
        }"#;
    ...
    fn mocked_release_selects_dispatcher_asset_for_supported_platforms() {
        ...
        let cases = [
            ("macos", "aarch64", "hakus-macos-arm64"),
            ("macos", "x86_64", "hakus-macos-x64"),
            ("linux", "x86_64", "hakus-linux-x64"),
            ("windows", "x86_64", "hakus-windows-x64.exe"),
            ("windows", "aarch64", "hakus-windows-arm64.exe"),
        ];

        for (os, arch, expected) in cases {
            let stem = release_asset_stem_for(Path::new("/usr/local/bin/hakus"), os, arch);
    ...
    fn mocked_release_selects_primary_asset_when_compatibility_alias_invokes_update() {
        ...
        let stem = release_asset_stem_for(
            Path::new("/usr/local/bin/hakus-tui"),
            "macos",
            "aarch64",
        );
        ...
        assert_eq!(windows_asset.name, "hakus-windows-arm64.exe");
    }
    ...
    fn android_arm64_maps_to_android_release_assets() {
        ...
        assert_eq!(
            release_asset_stem_for_prefix("hakus", "android", "aarch64"),
            "hakus-android-arm64"
        );
        assert_eq!(
            release_asset_stem_for(Path::new("hakus-tui"), "android", "aarch64"),
            "hakus-android-arm64"
        );
        assert_eq!(
    ...
    fn android_release_assets_never_select_linux_arm64() {
        ...
        let stem = release_asset_stem_for_prefix("hakus", "android", "aarch64");
        assert!(
            !stem.contains("linux"),
    ...
    fn mirror_release_uses_base_url_and_platform_assets() {
        ...
        assert_eq!(
            ...
            "https://mirror.example/releases/v0.8.36/hakus-artifacts-sha256.txt"
        );

        let dispatcher =
            select_platform_asset(&release, "hakus-linux-x64").expect("dispatcher asset");
        assert_eq!(
            dispatcher.browser_download_url,
            "https://mirror.example/releases/v0.8.36/hakus-linux-x64"
        );
        assert_eq!(release.assets.len(), 2);
        assert!(
            select_platform_asset(&release, "hakus-tui-linux-x64").is_none(),
    ...
    fn mirror_release_uses_windows_exe_asset_names() {
        ...
        assert_eq!(release.tag_name, "v0.8.36");
        assert!(
            select_platform_asset(&release, "hakus-windows-x64")
                .is_some_and(|asset| asset.name == "hakus-windows-x64.exe")
        );
        assert!(select_platform_asset(&release, "hakus-tui-windows-x64").is_none());
        ...
        assert!(
            select_platform_asset(&arm_release, "hakus-windows-arm64")
                .is_some_and(|asset| asset.name == "hakus-windows-arm64.exe")
        );
    }
    ...
    fn cnb_release_base_url_includes_tag_directory() {
        assert_eq!(
            hakus_release::cnb_release_base_url("0.8.47"),
            "https://cnb.cool/hakus.net/hakus/-/releases/download/v0.8.47"
        );
        assert_eq!(
            hakus_release::cnb_release_base_url("v0.8.47"),
            "https://cnb.cool/hakus.net/hakus/-/releases/download/v0.8.47"
        );
    }
    ...
    fn parse_release_version_accepts_tags_and_build_suffixes() {
        assert_eq!(
            hakus_release::parse_release_version("v0.9.0-beta.1").unwrap(),
            semver::Version::parse("0.9.0-beta.1").unwrap()
        );
        assert_eq!(
            hakus_release::parse_release_version("0.8.45 (abcdef123456)").unwrap(),
    ...
    fn update_fallback_hint_points_china_users_to_cnb_and_asset_mirrors() {
        ...
        assert!(hint.contains(hakus_release::CNB_REPO_URL), "{hint}");
        assert!(
            hint.contains(hakus_release::RELEASE_BASE_URL_ENV),
            "{hint}"
        );
        assert!(
            hint.contains(hakus_release::UPDATE_VERSION_ENV),
            "{hint}"
        );
        assert!(hint.contains("hakus-cli"), "{hint}");
        assert!(!hint.contains("hakus-tui --locked"), "{hint}");
    }
    ...
    fn candidates_for(
        ...
        proactive_source_candidates(fetched, "hakus-linux-x64", os, arch)
    }
    ...
    fn cnb_wins_when_its_manifest_answers_first() {
        ...
        assert_eq!(
            plan.binary_url,
            "https://cnb.cool/hakus.net/hakus/-/releases/download/v9.9.9/hakus-linux-x64",
            "the binary must come from the source whose manifest won"
        );
        assert_eq!(plan.binary_name, "hakus-linux-x64");
        assert!(
            plan.checksums.contains_key("hakus-linux-x64"),
    ...
    fn selection_fails_closed_when_no_source_is_usable() {
        ...
        assert!(
            message.contains("does not list hakus-linux-x64"),
    ...
    fn cnb_candidate_targets_the_exact_tag_and_platform_asset() {
        ...
        assert_eq!(
            candidate.manifest_url,
            "https://cnb.cool/hakus.net/hakus/-/releases/download/v0.9.0-beta.2/hakus-artifacts-sha256.txt"
        );
        assert_eq!(
            candidate.binary_url,
            "https://cnb.cool/hakus.net/hakus/-/releases/download/v0.9.0-beta.2/hakus-linux-x64"
        );
        assert_eq!(candidate.binary_name, "hakus-linux-x64");
    }
    ...
    fn github_candidate_addresses_the_manifest_even_when_the_payload_omits_it() {
        let release = Release {
            ...
            assets: vec![Asset {
                name: "hakus-linux-x64".to_string(),
                browser_download_url: "https://cdn.example/hakus-linux-x64".to_string(),
            }],
        };

        let candidate =
            github_source_candidate(&release, "hakus-linux-x64").expect("github candidate");
        ...
        assert_eq!(
            candidate.binary_url,
            "https://cdn.example/hakus-linux-x64"
        );
    }
    ...
    fn every_single_source_platform_requires_a_checksum_manifest() {
        for (os, arch) in [
            ...
            let asset_stem = release_asset_stem_for_prefix("hakus", os, arch);
            let asset_name = release_asset_name_for_prefix("hakus", os, arch);
            ...
            let message = format!("{err:#}");
            assert!(
                message.contains("does not publish required hakus-artifacts-sha256.txt"),
    ...
    fn a_malformed_single_source_manifest_fails_before_binary_download() {
        ...
        let err = single_source_download_plan(&fetched, "hakus-macos-arm64", None)
    ...
    fn a_single_source_manifest_must_cover_the_exact_platform_binary() {
        let manifest = format!("{}  hakus-linux-x64\n", "a".repeat(64));
        let (manifest_url, request_rx, handle) =
        ...
        let fetched = FetchedRelease {
            release: Release {
                ...
                assets: vec![
                    ...
                    Asset {
                        name: "hakus-windows-x64.exe".to_string(),
        ...
        let err = single_source_download_plan(&fetched, "hakus-windows-x64", None)
            .expect_err("a manifest for another platform must fail closed");
        let message = format!("{err:#}");
        assert!(
            message.contains("does not list hakus-windows-x64.exe"),
            "{message}"
        );
        let request = request_rx.recv().expect("manifest request");
        assert!(request.starts_with("GET /release "), "got {request:?}");
    ...
    fn an_explicit_mirror_remains_pinned_and_verified_from_that_mirror() {
        ...
        let manifest = format!("{}  hakus-macos-arm64\n", sha256_hex(bytes));
        ...
        let plan = single_source_download_plan(&fetched, "hakus-macos-arm64", None)
        ...
        assert_eq!(
            plan.binary_url,
            mirror_asset_url(&base_url, "hakus-macos-arm64")
        );
        verify_downloaded_asset(&plan, bytes)
            .expect("the pinned mirror's checksum must verify its bytes");
        let request = request_rx.recv().expect("manifest request");
        assert!(
            request.starts_with("GET /hakus-artifacts-sha256.txt "),
            "got {request:?}"
        );
        handle.join().expect("test server thread");
    }
    ...
    fn a_github_release_without_this_platform_leaves_cnb_as_the_only_candidate() {
        let fetched = FetchedRelease {
            release: Release {
                ...
                assets: vec![Asset {
                    name: "hakus-macos-arm64".to_string(),
                    browser_download_url: "https://cdn.example/hakus-macos-arm64".to_string(),
                }],
            },
            source: UpdateReleaseSource::GitHub,
        };
    ...
    fn explicit_overrides_take_precedence_over_probing() {
        {
            ...
            set_update_env(hakus_release::UPDATE_VERSION_ENV, "9.9.9");
            ...
            assert_eq!(
                describe_release_source_for_check(&fetched, "hakus-linux-x64", None),
        ...
        {
            let _env = UpdateEnvGuard::clear();
            set_update_env(hakus_release::CNB_MIRROR_ENV, "1");
            set_update_env(hakus_release::UPDATE_VERSION_ENV, "9.9.9");
    ...
    fn a_locked_source_serves_both_the_manifest_and_the_binary() {
        const BINARY: &[u8] = b"\x7fELF hakus linux x64 payload";
        let manifest = format!("{}  hakus-linux-x64\n", sha256_hex(BINARY));
        ...
        let candidate = ReleaseSourceCandidate {
            ...
            binary_name: "hakus-linux-x64".to_string(),
            binary_url: mirror_asset_url(&origin, "hakus-linux-x64"),
        };
        ...
        assert!(
            manifest_request.starts_with("GET /hakus-artifacts-sha256.txt "),
            "got {manifest_request:?}"
        );
        assert!(
            binary_request.starts_with("GET /hakus-linux-x64 "),
            "got {binary_request:?}"
        );
        handle.join().expect("test server thread");
    }
    ...
    fn a_checksum_mismatch_fails_closed_and_names_the_source() {
        let mut plan = DownloadPlan {
            ...
            binary_name: "hakus-linux-x64".to_string(),
            binary_url: "https://cnb.example/hakus-linux-x64".to_string(),
            checksums: HashMap::from([("hakus-linux-x64".to_string(), "a".repeat(64))]),
        };
        ...
        let message = format!("{err:#}");
        assert!(
            message.contains("is missing hakus-linux-x64"),
            "{message}"
        );
    }
    ...
    fn release_sources_describe_themselves_for_status_and_receipts() {
        ...
        assert_eq!(
            cnb.describe(),
            "CNB mirror (https://cnb.cool/hakus.net/hakus/-/releases/download/v9.9.9)"
        );
    ...
    fn fetch_latest_release_from_url_reads_mocked_release_json() {
        let body = br#"{
          "tag_name": "v9.9.9",
          "assets": [
            { "name": "hakus-linux-x64", "browser_download_url": "http://example.invalid/hakus-linux-x64" },
            { "name": "hakus-artifacts-sha256.txt", "browser_download_url": "http://example.invalid/hakus-artifacts-sha256.txt" }
          ]
        }"#;
        ...
        assert!(
            request_lower.contains("user-agent: hakus-updater"),
            "got {request:?}"
        );
        handle.join().expect("test server thread");
    }
