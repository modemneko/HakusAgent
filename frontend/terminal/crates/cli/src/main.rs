...
fn main() -> std::process::ExitCode {
    // Reset SIGPIPE to SIG_DFL so piping hakus output into a command that
    // exits early (e.g. `hakus doctor | head`) terminates the process
    ...
    // Single-binary argv0 dispatch: `codew` is now an alias for `hakus`
    ...
    let _ = std::env::args().next().and_then(|argv0| {
        ...
        let trimmed = base
            ...
            .unwrap_or(base);
        if trimmed == "codew" {
            // No-op: the single `hakus` binary handles both names.
        }
        None::<()>
    });
