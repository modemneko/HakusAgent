//! Hakus account and BYOK credential commands.
//!
...
//! surface signs a CLI profile into the managed Hakus account and stores
...
use hakus_config::{ConfigStore, ProviderKind};
use hakus_secrets::Secrets;
use hakus_secrets::account::{
...
pub(crate) struct CloudArgs {
    /// Hakus account API origin. HTTPS is required except for loopback HTTP.
...
enum CloudCommand {
    ...
    Logout,
    /// Manage provider API keys stored in the signed-in Hakus account.
    Keys(CloudKeysArgs),
...
enum CloudKeysCommand {
    ...
    List,
    /// Save a provider key to the signed-in Hakus account.
    Set(CloudKeySetArgs),
    /// Remove a provider key from the signed-in Hakus account.
...
struct CloudKeySetArgs {
    ...
    #[arg(long, default_value = "Hakus CLI")]
    label: String,
}
...
impl ReqwestTransport {
    fn new(base: Url) -> Result<Self> {
        let client = reqwest::blocking::Client::builder()
            ...
            .user_agent(concat!("hakus/", env!("CARGO_PKG_VERSION")))
            .build()
            .context("failed to initialize the Hakus account HTTP client")?;
        Ok(Self { base, client })
    }
}
...
impl CloudTransport for ReqwestTransport {
    fn execute(&self, request: CloudRequest) -> Result<CloudResponse> {
        let url = self
            ...
            .context("failed to construct the Hakus account request URL")?;
        ...
        let response = builder
            .send()
            .context("could not reach the Hakus service")?;
        let status = response.status().as_u16();
        let mut body = Vec::new();
        response
            .take(MAX_RESPONSE_BYTES + 1)
            .read_to_end(&mut body)
            .context("failed to read the Hakus service response")?;
        if body.len() as u64 > MAX_RESPONSE_BYTES {
            bail!("The Hakus service returned an unexpectedly large response");
        }
...
impl<'a, T: CloudTransport> CloudClient<'a, T> {
    ...
    fn poll_device(
        ...
        loop {
            if started.elapsed() >= timeout {
                bail!(
                    "Hakus account login timed out; run `hakus account login` to try again"
                );
            }
            let response = self.transport.execute(CloudRequest {
                method: HttpMethod::Post,
    ...
    fn load_auth(&self) -> Result<Option<StoredCloudAuth>> {
        self.account_store.load().context(
            "the local Hakus account session is unreadable; run `hakus account logout` and sign in again",
        )
    }
    ...
    fn save_auth(&self, bundle: AuthBundle) -> Result<()> {
        self.account_store
            .save(bundle)
            .context("failed to save the Hakus account session in the local secret store")
    }

    fn clear_auth(&self) -> Result<()> {
        self.account_store
            .clear()
            .context("failed to remove the local Hakus account session")
    }
    ...
    fn me(&self) -> Result<CloudUser> {
        ...
        if me.user.id.trim().is_empty() {
            bail!("The Hakus service returned an account without an ID");
        }
    ...
    fn execute_authenticated(
        ...
        let Some(mut stored) = self.load_auth()? else {
            bail!("Not signed in. Run `hakus account login` first");
        };
        ...
        match refresh.status {
            ...
            401 => {
                self.clear_auth()?;
                bail!("The Hakus account session expired. Run `hakus account login` again");
            }
    ...
    match secure_account_session_secrets() {
        Ok(secrets) => {
            if secrets.backend_name().starts_with("file-based") {
                eprintln!(
                    "warning: OS credential manager unavailable; {CLOUD_ALLOW_FILE_SESSION_STORE_ENV}=1 explicitly enables the local 0600 Hakus secrets file for cloud session tokens"
                );
            }
            Ok(secrets)
        }
        Err(_) => bail!(
            "Hakus account login requires an OS credential manager for session tokens. Configure Keychain, Credential Manager, or Secret Service and try again. Headless users may explicitly opt into the local 0600 secrets file with {CLOUD_ALLOW_FILE_SESSION_STORE_ENV}=1"
        ),
    }
}
...
pub(crate) fn reject_inline_api_key(api_key: Option<&str>) -> Result<()> {
    if api_key.is_some() {
        bail!(
            "`hakus account` does not accept the global `--api-key` flag because command-line values can leak through shell history. Use `account keys set <provider>` for a hidden prompt, `--api-key-stdin`, or `--from-local`"
        );
    }
    Ok(())
}
...
fn run_with<T: CloudTransport, W: Write>(
    ...
    match command {
        CloudCommand::Login(login) => {
            ...
            writeln!(out, "Hakus account sign-in")?;
            ...
            let user = client.me()?;
            write_account(out, "Signed in to Hakus.", profile, api_base, &user)
        }
        CloudCommand::Status => match client.load_auth()? {
            ...
            None => {
                writeln!(out, "Not signed in to Hakus.")?;
                writeln!(out, "Profile: {}", printable(profile))?;
                writeln!(out, "API: {api_base}")?;
                writeln!(out, "Run `hakus account login` to sign in.")?;
                Ok(())
            }
        },
        CloudCommand::Logout => {
            let remote_revoked = client.logout()?;
            writeln!(out, "Removed the local Hakus account session.")?;
            writeln!(out, "Profile: {}", printable(profile))?;
            if !remote_revoked {
                writeln!(
                    out,
        ...
        CloudCommand::Keys(keys) => match keys.command {
            CloudKeysCommand::List => {
                let user = client.me()?;
                write_account(out, "Hakus account keys.", profile, api_base, &user)?;
            ...
            CloudKeysCommand::Set(set) => {
                ...
                writeln!(
                    out,
                    "Saved {} for Hakus account {} (profile {}).",
                    set.provider.slug(),
                    printable(&user.id),
                    printable(profile)
                )?;
            ...
            CloudKeysCommand::Remove { provider } => {
                ...
                writeln!(
                    out,
                    "Removed {} from Hakus account {} (profile {}).",
                    provider.slug(),
                    printable(&user.id),
                    printable(profile)
                )?;
        ...
        CloudCommand::Pull(args) => {
            if !args.dry_run {
                bail!(
                    "Account settings import is not available yet; local config was not changed. Run `hakus account pull --dry-run` to inspect the signed-in account."
                );
            }
            let user = client.me()?;
...
fn validate_api_base(value: &str) -> Result<ValidatedApiBase> {
    let mut url = Url::parse(value.trim()).context("invalid Hakus account API base URL")?;
    if !url.username().is_empty() || url.password().is_some() {
        bail!("Hakus account API base URL must not contain credentials");
    }
    if url.query().is_some() || url.fragment().is_some() {
        bail!("Hakus account API base URL must not contain a query or fragment");
    }
    if !matches!(url.path(), "" | "/") {
        bail!("Hakus account API base URL must be an origin without a path");
    }
    let host = url
        .host_str()
        .ok_or_else(|| anyhow!("Hakus account API base URL must include a host"))?;
    let allowed = url.scheme() == "https" || (url.scheme() == "http" && is_loopback_host(host));
    if !allowed {
        bail!(
            "Hakus account API base URL must use HTTPS (loopback HTTP is allowed for testing)"
        );
    }
    url.set_path("/");
...
fn validate_verification_url(
    ...
    let url =
        Url::parse(value).context("The Hakus service returned an invalid verification URL")?;
    if value != url.as_str() {
        bail!("The Hakus service returned an unsafe verification URL");
    }
    let host = url.host_str().ok_or_else(|| {
        anyhow!("The Hakus service returned a verification URL without a host")
    })?;
    ...
    let api = Url::parse(api_base).context("invalid Hakus account API base URL")?;
    let canonical_api = api.scheme() == "https"
        && api.host_str() == Some("api.hakus.net")
        && api.port_or_known_default() == Some(443);
    let loopback_api = api.host_str().is_some_and(is_loopback_host);
    if canonical_api {
        if url.scheme() != "https"
            || !host.eq_ignore_ascii_case("app.hakus.net")
            || url.port_or_known_default() != Some(443)
        {
            bail!("The Hakus service returned an untrusted verification origin");
        }
        ...
        bail!(
            "Browser login is only enabled for the canonical Hakus account API or a loopback test API"
        );
    }
...
fn validate_user_code(code: &str) -> Result<()> {
    ...
    {
        bail!("The Hakus service returned an invalid user code");
    }
    Ok(())
}
...
fn validate_device_code(code: &str) -> Result<()> {
    ...
    {
        bail!("The Hakus service returned an invalid device authorization response");
    }
    Ok(())
}
...
fn json_body(value: &impl Serialize) -> Result<Vec<u8>> {
    serde_json::to_vec(value).context("failed to encode Hakus account request")
}
...
fn parse_json_body<T: DeserializeOwned>(body: &[u8]) -> Result<T> {
    serde_json::from_slice(body).context("The Hakus service returned an invalid JSON response")
}
...
fn response_error(response: &CloudResponse) -> anyhow::Error {
    ...
    match code {
        Some(code) => anyhow!(
            "Hakus account request failed (HTTP {}, code {code})",
            response.status
        ),
        None => anyhow!(
            "Hakus account request failed (HTTP {})",
            response.status
        ),
    }
}
