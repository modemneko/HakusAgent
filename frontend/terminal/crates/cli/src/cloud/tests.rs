...
fn parses_cloud_command_matrix_and_rejects_inline_keys() {
    assert!(matches!(
        command(&["hakus", "account", "status"]),
        CloudCommand::Status
    ));
    assert!(matches!(
        command(&["hakus", "cloud", "login", "--no-open"]),
...
fn api_base_requires_https_or_literal_loopback_http() {
    assert_eq!(
        validate_api_base("https://api.hakus.net/")
            .unwrap()
            .display,
        "https://api.hakus.net"
    );
    ...
    assert!(validate_api_base("http://api.hakus.net").is_err());
...
fn verification_urls_are_pinned_to_the_app_or_loopback() {
    ...
    const API: &str = "https://api.hakus.net";
    assert!(
        validate_verification_url("https://app.hakus.net/cli/authorize", API, CODE, false,)
            .is_ok()
    );
    assert!(
        validate_verification_url(
            "https://app.hakus.net/cli/authorize?user_code=ABCD-EFGH-JKLM",
            API,
            CODE,
            true,
        )
    ...
    for unsafe_url in [
        ...
        "https://user@app.hakus.net/cli/authorize",
        "https://app.hakus.net/cli/authorize#continue",
        "https://app.hakus.net/cli/authorize/extra",
        "https://app.hakus.net/cli/other/../authorize",
        "https://app.hakus.net/cli/%61uthorize",
        "https://app.hakus.net/cli/authorize?next=https%3A%2F%2Fattacker.example",
        "https://app.hakus.net/cli/authorize?user_code=ABCD-EFGH-JKLM&next=evil",
    ] {
        assert!(
...
fn user_codes_and_key_inputs_match_the_server_contract() {
    ...
    assert_eq!(
        validate_label("  Hakus\tCLI  ").unwrap(),
        "Hakus CLI"
    );
    ...
    let transport = FakeTransport::new(vec![
        response(
            ...
            json!({
                ...
                "verificationUri": "https://app.hakus.net/cli/authorize",
                "verificationUriComplete": "https://app.hakus.net/cli/authorize?user_code=ABCD-EFGH-JKLM",
                "expiresIn": 600,
                "interval": 1
            }),
        ),
...
fn cloud_sessions_are_isolated_by_profile_and_api_origin() {
    let (secrets, _) = test_secrets();
    let transport = FakeTransport::new(vec![]);
    let default = CloudClient::new(&transport, &secrets, "default", "https://api.hakus.net");
    let work = CloudClient::new(&transport, &secrets, "work", "https://api.hakus.net");
...
fn status_refreshes_once_on_unauthorized_and_never_displays_tokens() {
    ...
    CloudClient::new(&transport, &secrets, "default", "https://api.hakus.net")
        .save_auth(auth(
            "access-old-secret",
            "refresh-old-secret",
            "acct-refresh",
...
fn account_pull_dry_run_is_truthful_and_read_only() {
    ...
    run_with(
        command(&["hakus", "account", "pull", "--dry-run"]),
        "default",
        "https://api.hakus.net",
        &config,
        &secrets,
...
fn non_terminal_refresh_responses_preserve_the_local_session() {
    for status in [403, 429, 500, 503] {
        ...
        let client = CloudClient::new(&transport, &secrets, "default", "https://api.hakus.net");
        client
            .save_auth(auth(
                "access-old-secret",
                "refresh-still-valid",
...
fn set_list_and_remove_use_account_routes_without_secret_output() {
    ...
    for cmd in [
        ...
        command(&["hakus", "cloud", "keys", "list"]),
        command(&["hakus", "cloud", "keys", "remove", "openai"]),
    ] {
        run_with(
            cmd,
            "default",
    ...
    assert!(output.contains("Hakus account acct-keys"));
...
fn from_local_uses_config_without_printing_or_requiring_an_inline_key() {
    ...
    CloudClient::new(&transport, &secrets, "work", "https://api.hakus.net")
...
fn logout_recovers_from_a_corrupt_local_session_record() {
    ...
    let _keep_temp = temp;
    let (secrets, store) = test_secrets();
    let slot = cloud_auth_slot("default", "https://api.hakus.net");
...
fn logout_clears_obsolete_or_wrong_origin_session_records() {
    let canonical_api_base = "https://api.hakus.net";
...
fn cloud_auth_slot_does_not_embed_profile_or_origin() {
    let slot = cloud_auth_slot("private-profile", "https://api.hakus.net");
    assert!(!slot.contains("private-profile"));
    assert!(!slot.contains("api.hakus.net"));
    assert_ne!(
        slot,
        cloud_auth_slot("other-profile", "https://api.hakus.net")
    );
}
...
fn account_login_timeout_fails_the_command() {
    ...
    // against a stub server: `error: Hakus account login timed out` now
    // exits 1.
    let (temp, config) = test_config();
    let _keep_temp = temp;
    let (secrets, _) = test_secrets();
