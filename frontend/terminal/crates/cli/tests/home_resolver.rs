...
use hakus_config::{
    HAKUS_APP_DIR, CONFIG_FILE_NAME, LEGACY_APP_DIR, hakus_home, default_config_path,
};
use hakus_secrets::FileKeyringStore;
...
impl ProcessEnv {
    fn install(root: &Path, home: &Path, userprofile: &Path, hakus_home: &OsString) -> Self {
        let lock = ENV_LOCK
            .lock()
        ...
        let prior = Self {
            ...
            hakus_home: std::env::var_os("HAKUS_HOME"),
        };
        ...
        unsafe {
            ...
            std::env::set_var("HAKUS_HOME", hakus_home);
        }
...
impl Drop for ProcessEnv {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            restore_var("HAKUS_HOME", self.hakus_home.take());
        }
    }
}
...
fn non_unicode_override_is_one_explicit_isolation_boundary() {
    ...
    use hakus_state::default_state_db_path;
    ...
    assert_eq!(hakus_home().expect("config home"), explicit);
    assert_eq!(
        default_config_path().expect("config path"),
