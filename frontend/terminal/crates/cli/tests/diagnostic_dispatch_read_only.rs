...
//! (`auth status --diagnostic`). The single `hakus` binary has no sibling
...
fn dispatcher_diagnostics_are_in_process_and_read_only() {
    ...
    for (args, expects_entries_json, expects_auth_diagnostic) in [
        ...
        // The diagnostic runs entirely in-process: the single `hakus` binary
        // dispatches through `run_tui_in_process` -> `hakus_tui::run`. No
        ...
        let output = Command::new(hakus_binary())
            .args(args)
            .env_clear()
        ...
        if expects_auth_diagnostic {
            ...
            assert!(
                stdout.contains(&format!(
                    "hakus home: {}",
                    hakus_config::quote_os_path(&hakus_home)
                )),
                "{stdout}"
            );
            assert!(
                stdout.contains(&format!(
                    "config: {}",
                    hakus_config::quote_os_path(&hakus_home.join("config.toml"))
                )),
                "{stdout}"
            );
            assert!(
                stdout.contains(&format!(
                    "settings: {}",
                    hakus_config::quote_os_path(&hakus_home.join("settings.toml"))
                )),
                "{stdout}"
            );
            assert!(
            ...
            assert!(
                stdout.contains(
                    "legacy secret store: suppressed by explicit HAKUS_HOME isolation"
                ),
                "{stdout}"
            );
        ...
        assert!(
            !primary_home.exists(),
            "dispatcher {args:?} must not create a primary Hakus home or migrated state"
        );
        assert!(
            !hakus_home.exists(),
            "dispatcher {args:?} must not create an explicit HAKUS_HOME"
        );
    }
}
