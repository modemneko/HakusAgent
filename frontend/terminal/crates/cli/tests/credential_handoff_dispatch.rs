...
fn terminal_refusal_precedes_config_reads_and_secret_migration() {
    ...
    let malformed_hakus_home = malformed.path().join("hakus-home");
    fs::create_dir_all(&malformed_hakus_home).expect("create explicit Hakus home");
    let malformed_config = malformed_hakus_home.join("config.toml");
    ...
    let mut command = isolated_command(&malformed_home, Some(&malformed_hakus_home));
    ...
    assert!(
        !malformed_hakus_home.join("secrets").exists(),
    ...
    let primary_home = sealed_home.join(".hakus");
...
fn initialization_failures_are_source_free() {
    ...
    let hakus_home = fixture.path().join(format!("hakus-home-{SENTINEL}"));
    fs::create_dir_all(&hakus_home).expect("create Hakus home");
    fs::write(
    ...
    let mut command = isolated_command(&sealed_home, Some(&hakus_home));
    let output = command
        .args([
            "--api-key",
            SENTINEL,
...
fn installed_unix_dispatcher_settles_a_closed_pipe_cleanly() {
    let fixture = TempDir::new().expect("fixture root");
    let sealed_home = fixture.path().join("home");
    let hakus_home = fixture.path().join("hakus-home");
    let mut command = isolated_command(&sealed_home, Some(&hakus_home));
    let output = command
        .args([
            "--api-key",
...
fn isolated_command(home: &Path, hakus_home: Option<&Path>) -> Command {
    ...
    command
        ...
        .stdin(Stdio::null())
        .stderr(Stdio::piped());
    if let Some(hakus_home) = hakus_home {
        command.env("HAKUS_HOME", hakus_home);
    }
    command
}
