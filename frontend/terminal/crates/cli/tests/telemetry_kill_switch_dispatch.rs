...
//!
//! The single `hakus` binary resolves dispatcher overrides, states the
//! telemetry floor in its environment, and then calls `hakus_tui::run`.
...
/// `HAKUS_TELEMETRY=0` beats `--telemetry true`, end to end.
///
...
fn env_off_beats_cli_on_end_to_end() {
    ...
    assert!(
        ...
        "`HAKUS_TELEMETRY=0` must beat `--telemetry true` before the runtime arms"
    );
}
...
fn config_set_true_reenables_a_historical_decline() {
    ...
    let state_path = hakus_home.join("setup_state.json");
    state
        .save_to(&state_path)
...
fn dispatch_and_read_telemetry_with_endpoint(
    ...
    if let Some(value) = telemetry_env {
        command.env("HAKUS_TELEMETRY", value);
    }
    let output = command.output().expect("run hakus dispatcher");
    assert!(
        output.status.success(),
    ...
    let telemetry_dir = hakus_home.join("telemetry");
...
fn hakus_binary() -> &'static str {
    env!("CARGO_BIN_EXE_hakus")
}
