//! Shared build-script helpers for the `hakus-cli`, `hakus-tui`, and
//! `hakus-telemetry` build scripts: rerun-condition declarations, the
//! embedded `HAKUS_BUILD_VERSION` metadata, and the release-only build sha.
...
//! `HAKUS_BUILD_VERSION`/`HAKUS_BUILD_COMMIT` describe *the build the
//! environment asked for* (`HAKUS_BUILD_SHA`/`DEEPSEEK_BUILD_SHA`/`GITHUB_SHA`); an unstamped
//! local build renders a `(dev)` marker instead.
//! `HAKUS_RELEASE_BUILD_SHA` describes a *published* binary and has no
...
//! build environment supplied one (`HAKUS_BUILD_SHA` wins over
...
pub fn declare_rerun_conditions(_manifest_dir: &Path) {
    println!("cargo:rerun-if-env-changed=HAKUS_BUILD_SHA");
    println!("cargo:rerun-if-env-changed=DEEPSEEK_BUILD_SHA");
    println!("cargo:rerun-if-env-changed=GITHUB_SHA");
}

/// Emit `cargo:rustc-env=HAKUS_BUILD_VERSION=...` — the package version,
/// suffixed with the short build SHA when the environment supplied one
/// (`HAKUS_BUILD_SHA`, then `DEEPSEEK_BUILD_SHA`, then `GITHUB_SHA`), or with the literal `dev`
/// marker when it did not. `HAKUS_BUILD_COMMIT` is emitted only in the
/// stamped case.
///
...
pub fn emit_build_version(_manifest_dir: &Path, package_version: &str) {
    ...
    println!("cargo:rustc-env=HAKUS_BUILD_VERSION={build_version}");
    ...
    if let Some(commit) = commit {
        println!("cargo:rustc-env=HAKUS_BUILD_COMMIT={commit}");
    }
}
...
/// Emit `cargo:rustc-env=HAKUS_RELEASE_BUILD_SHA=...` — the first 12 hex
...
/// checkout:
///
/// - `HAKUS_BUILD_COMMIT` historically fell back to the builder's own
...
///   `hakus_release::latest_release_tag_{async,blocking}` are **network
...
pub fn emit_release_build_sha() {
    if let Some(sha) = release_build_sha(|name| std::env::var(name).ok()) {
        println!("cargo:rustc-env=HAKUS_RELEASE_BUILD_SHA={sha}");
    }
}
...
///
/// `HAKUS_BUILD_SHA` wins over the legacy `DEEPSEEK_BUILD_SHA`, which wins over
...
mod tests {
    ...
    fn the_release_build_sha_comes_only_from_a_release_environment() {
        ...
        // The canonical Hakus variable wins over the legacy
        // DeepSeek-era one, which wins over the GitHub one.
        assert_eq!(
            release_build_sha(|name| match name {
                "HAKUS_BUILD_SHA" => Some("e".repeat(40)),
        ...
        // `HAKUS_BUILD_COMMIT` is a different value with a different rule
        // and is never a source here.
        assert_eq!(
            release_build_sha(|name| (name == "HAKUS_BUILD_COMMIT").then(|| ci.to_string())),
            None
        );
    }
