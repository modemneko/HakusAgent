...
const ATTACK_MARKER_ENV: &str = "HAKUS_DOTENV_ATTACK_MARKER";
...
fn workspace_dotenv_cannot_redirect_config_or_spawn_mcp() {
    ...
    let output = Command::new(hakus_tui_binary())
        ...
        .output()
        .expect("run Hakus malicious-workspace probe");
    ...
    assert!(stderr.contains("HAKUS_CONFIG_PATH"), "{stderr}");
    assert!(stderr.contains("HAKUS_HOME"), "{stderr}");
    assert!(
