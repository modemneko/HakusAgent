...
//!
//! Everything here drives the real `hakus-tui` binary inside a sealed
//! `HOME`/`HAKUS_HOME`, with a loopback recorder standing in for the
//! telemetry endpoint. The unit tests in `hakus-telemetry` prove the
...
//! nor `HAKUS_TELEMETRY=0` was ever read by a process that would have sent.
//!
...
use hakus_config::{SetupState, TELEMETRY_NOTICE_VERSION};
...
struct Fixture {
    ...
    // this module launches the full Hakus binary, and low-resource CI
...
impl Fixture {
    fn new() -> Self {
        ...
        let hakus_home = root.path().join("hakus-home");
        let workspace = root.path().join("workspace");
        for dir in [&home, &hakus_home, &workspace] {
    ...
    /// terminal, but the decision it records lives on this `HAKUS_HOME` and
    ...
    fn record_notice(&self, opt_in: bool) {
        ...
        state
            .save_to(&self.hakus_home.join("setup_state.json"))
            .expect("write setup state");
    }

    fn setup_state_path(&self) -> PathBuf {
        self.hakus_home.join("setup_state.json")
    }

    fn telemetry_root(&self) -> PathBuf {
        self.hakus_home.join("telemetry")
    }

    fn command(&self) -> Command {
        let mut command = Command::new(hakus_tui_binary());
        command
            ...
            .env("HAKUS_HOME", &self.hakus_home)
            .env("HAKUS_SECRET_BACKEND", "file")
            .env("HAKUS_MEMORY", "false")
        ...
        if let Some(endpoint) = &self.endpoint {
            command.env("HAKUS_TELEMETRY_ENDPOINT", endpoint);
        }
        command
    }
    ...
    fn run_completions(&self) -> Output {
        ...
        let output = command.output().expect("run hakus-tui completions");
        assert!(
    ...
    fn written_files(&self) -> Vec<PathBuf> {
        let mut out = Vec::new();
        for base in [&self.home, &self.hakus_home, &self.workspace] {
...
fn hakus_tui_binary() -> PathBuf {
    if let Some(path) = option_env!("CARGO_BIN_EXE_hakus-tui") {
        return PathBuf::from(path);
    }
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_hakus-tui") {
        return PathBuf::from(path);
    }
    ...
    path.push(format!("hakus-tui{}", std::env::consts::EXE_SUFFIX));
    path
}
...
///
/// No environment variable is set here on purpose. `HAKUS_TELEMETRY=0` and
...
async fn telemetry_disabled_by_env_sends_zero_requests() {
    ...
    command
        ...
        .output()
        .expect("run hakus-tui completions");

    assert_no_batches(&server, "`HAKUS_TELEMETRY=0`").await;
    assert!(
        !fixture.telemetry_root().exists(),
...
async fn an_unparseable_telemetry_env_value_sends_zero_requests() {
    ...
    assert_no_batches(&server, "`HAKUS_TELEMETRY=maybe`").await;
    assert!(
        !fixture.telemetry_root().exists(),
...
async fn forced_off_run_preserves_a_consenting_users_state() {
    ...
    let mut command = fixture.command();
    command
        .env("HAKUS_TELEMETRY", "not-a-bool")
        .args([
            "--config",
            fixture.config_path.to_str().expect("config path"),
            "completions",
...
/// nothing.
///
/// `HAKUS_TELEMETRY=0` used to resolve as an *answer*, so it took the
...
async fn a_run_scoped_kill_switch_preserves_a_consenting_users_state() {
    ...
    for value in ["0", "off", "false"] {
        ...
        assert!(
            !root.join("disabled").exists(),
            "`HAKUS_TELEMETRY={value}` tombstoned a machine nobody opted out"
        );
        assert_eq!(
            snapshot(&root),
            before,
            "`HAKUS_TELEMETRY={value}` touched a consenting user's telemetry state"
        );
    }
...
///
/// Every bound in `hakus-telemetry`'s schema is a property of how a payload
/// is *built*: closed enums, `u32`s, `ProviderKind::as_str()`,
/// `reduce_panic_site`. None of that survives the round trip, because `flush`
/// re-reads `buffer.jsonl` and deserializes it — and `$HAKUS_HOME` is a
...
async fn a_hostile_buffer_line_never_reaches_a_batch() {
    ...
    let mut child = command.spawn().expect("spawn hakus-tui exec");
    ...
    let status = child
        ...
        .expect("wait for hakus-tui exec")
        .expect("hakus-tui exec must exit");
    let output = Output {
        status,
...
/// The documented mid-session opt-out — `hakus config set telemetry false`,
...
/// Ctrl-C must not wait on a lock a second Hakus process is holding.
///
...
fn plant_sentinels(fixture: &Fixture, base_url: &str) {
    ...
    std::fs::write(
        fixture.hakus_home.join("mcp.json"),
...
fn run_exec(fixture: &Fixture, prompt: &str) -> Output {
    ...
    let status = match child.wait_timeout(EXEC_TIMEOUT).expect("wait for exec") {
        ...
        None => {
            let _ = child.kill();
            panic!("hakus-tui exec did not exit within {EXEC_TIMEOUT:?}");
        }
    };
    Output {
        status,
