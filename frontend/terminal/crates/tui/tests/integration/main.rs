...
//! binaries each re-linked the full `hakus-tui` graph — one harness keeps the
//! same test names (`integration::adaptive_evidence_acceptance::...`) so
//! `cargo test -p hakus-tui adaptive_evidence_acceptance` still filters.
...
// The legacy text tool-call parser lives in hakus-core now; keep the
// `crate::tool_parser` path the suites use.
use hakus_core::tool_parser;
