...
use hakus_secrets::{FileKeyringStore, KeyringStore};
...
fn doctor_text_leaves_a_sealed_home_untouched() {
    ...
    assert!(stdout.contains("hakus Doctor"), "stdout:\n{stdout}");
}
...
fn doctor_json_rejects_kimi_code_claude_alias_with_machine_readable_guidance() {
    ...
    let hakus_home = fixture.path().join("isolated-hakus-home");
...
"#;
    ...
    assert!(
        !hakus_home.exists(),
        "doctor must not create HAKUS_HOME state"
    );
}
...
fn setup_status_leaves_a_sealed_home_untouched() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Hakus Status"), "stdout:\n{stdout}");
}
...
fn diagnostics_read_home_legacy_settings_without_migrating_them() {
    for args in [
        ...
        let primary_home = home.join(".hakus");
        ...
        assert!(
            !primary_home.exists(),
            "diagnostic {args:?} must not create a primary Hakus home"
        );
    }
}
...
fn doctor_json_does_not_inherit_an_ambient_legacy_secret_from_an_explicit_home() {
    ...
    assert!(
        !hakus_home.exists(),
        "doctor must not create an isolated Hakus home or secret store"
    );
}
...
fn doctor_text_probe_uses_a_legacy_key_without_migrating_it() {
    ...
    fs::create_dir_all(&workspace).expect("workspace");
    let legacy = home.join(".deepseek").join("secrets").join("secrets.json");
    let primary = home.join(".hakus").join("secrets").join("secrets.json");
    FileKeyringStore::new(&legacy)
...
#[test]
fn doctor_json_stash_honors_an_explicit_hakus_home() {
    let fixture = TempDir::new().expect("fixture root");
    let workspace = fixture.path().join("workspace");
    let home = fixture.path().join("home");
    let hakus_home = fixture.path().join("isolated-hakus-home");
    fs::create_dir_all(&workspace).expect("workspace");
    let ambient_stash = home.join(".hakus").join("composer_stash.jsonl");
    ...
    assert!(
        ...
        "doctor must not inspect an ambient stash outside explicit HAKUS_HOME"
    );
    assert_eq!(
...
fn run_sealed_diagnostic<const N: usize>(args: [&str; N]) -> Output {
    ...
    let hakus_home = fixture.path().join("sealed-hakus-home");
    ...
    assert!(
        !hakus_home.exists(),
        "diagnostic {args:?} must not create HAKUS_HOME or a secrets store at {}",
        hakus_home.display()
    );
    output
}
...
/// root so this test isolates Hakus's own state contract.
