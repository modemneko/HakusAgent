//! End-to-end shape lock for the per-model-call `token_usage` event on the
//! `hakus exec --output-format stream-json` stream (#52 / FINISH-0.9.4).
//!
//! A `wiremock` OpenAI-compatible endpoint stands in for the provider. Two
//! cases pin the contract:
//!
//! - usage reported by the provider -> exactly one `token_usage` event per
...
//! - usage absent from the provider stream -> no `token_usage` event at all
...
fn run_exec_stream_json(server: &MockServer) -> Vec<Value> {
    ...
    command
        ...
        .env(
            "HAKUS_CONFIG_PATH",
            home.path().join(".hakus").join("config.toml"),
        )
        .env(
        ...
        .env("HAKUS_BASE_URL", server.uri())
        .env("DEEPSEEK_MODEL", TEST_MODEL)
        .env("HAKUS_MODEL", TEST_MODEL)
        .env("RUST_LOG", "warn")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());

    std::fs::create_dir_all(home.path().join(".hakus")).expect("create hakus config dir");
        ...
        .expect("wait for hakus-tui")
    {
        ...
        None => {
            ...
            panic!(
                "hakus-tui exec timed out after {RUN_TIMEOUT:?}\nstdout:\n{}\nstderr:\n{}",
...
async fn turn_usage_event_is_emitted_with_reported_fields_and_stream_contract_holds() {
    ...
    for event in &events {
        assert_eq!(event["schema"], "hakus.exec-stream");
        assert_eq!(event["schema_version"], 1);
    }

    // Exactly one per-call usage receipt, numbered from 1.
    let usage_events = events_of_type(&events, "token_usage");
    assert_eq!(
        usage_events.len(),
        1,
        "expected one token_usage event: {events:#?}"
    );
    ...
    let usage_object = usage.as_object().expect("token_usage object");
    ...
    let usage_pos = types.iter().position(|t| *t == "token_usage");
    assert!(
        content_pos.is_some_and(|c| usage_pos.is_some_and(|u| c < u)),
        "token_usage must follow the content it accounts for: {types:?}"
    );
...
async fn turn_usage_event_is_skipped_when_provider_reports_no_usage() {
    ...
    assert!(
        events_of_type(&events, "token_usage").is_empty(),
        "no token_usage event without provider-reported usage: {events:#?}"
    );
