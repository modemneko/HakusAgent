...
//!
//! Benchmark evidence (Terminal-Bench 2.1, hakus 0.9.4): five tasks were
...
//! and `hakus exec` exited 1, and Harbor raised
...
fn run_exec(workspace: &Path, home: &Path, base_url: &str) -> std::process::Output {
    std::fs::create_dir_all(home.join(".hakus")).expect("config directory");
    std::fs::create_dir_all(home.join(".deepseek")).expect("legacy config directory");
    std::fs::write(
        home.join(".hakus/config.toml"),
        "allow_shell = true\n\n[retry]\nenabled = false\n",
    )
    ...
    command
        ...
        .env("HAKUS_CONFIG_PATH", home.join(".hakus/config.toml"))
        .env("DEEPSEEK_CONFIG_PATH", home.join(".deepseek/config.toml"))
        .env("DEEPSEEK_API_KEY", "ci-test-key-not-real")
        .env("DEEPSEEK_BASE_URL", base_url)
        .env("HAKUS_BASE_URL", base_url)
        .env("DEEPSEEK_MODEL", MODEL)
        .env("HAKUS_MODEL", MODEL)
        .env("RUST_LOG", "warn")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
...
fn binary() -> PathBuf {
    std::env::var_os("CARGO_BIN_EXE_hakus-tui")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/debug/hakus-tui")
        })
}
...
fn run_with_timeout(mut command: Command, timeout: Duration) -> std::process::Output {
    let mut child = command.spawn().expect("spawn hakus exec");
