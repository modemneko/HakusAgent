...
impl HarnessBuilder {
    pub fn new(program: impl Into<PathBuf>) -> Self {
        ...
        let env = HashMap::from([("HAKUS_TELEMETRY".to_string(), "0".to_string())]);
        Self {
    ...
    pub fn spawn(self) -> Result<Harness> {
        ...
        if let Some(home) = self.seal_home.as_deref() {
            ...
            let hakus_config = home.join(".hakus").join("config.toml");
            ...
            builder = builder
                ...
                .env("HAKUS_CONFIG_PATH", hakus_config.to_string_lossy())
...
impl Harness {
    ...
    ///
    /// `QA_TUI_BIN` overrides the `hakus-tui` resolution entirely — cargo
    ...
    pub fn cargo_bin(name: &str) -> PathBuf {
        ...
        if name == "hakus-tui"
            && let Some(path) = option_env!("CARGO_BIN_EXE_hakus-tui")
        {
            return PathBuf::from(path);
        }
...
pub fn make_sealed_workspace() -> Result<SealedWorkspace> {
    ...
    std::fs::create_dir_all(home.join(".hakus")).context("mkdir home/.hakus")?;
