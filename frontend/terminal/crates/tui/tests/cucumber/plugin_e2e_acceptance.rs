...
async fn plugin_module_does_not_break_binary_load() {
    let output = Command::new(hakus_tui_binary())
        .arg("--version")
        .output()
        .expect("hakus-tui --version should start");

    assert!(
        output.status.success(),
        "hakus-tui --version failed\nstderr:\n{}",
        String::from_utf8_lossy(&output.stderr)
    );

    let version = String::from_utf8_lossy(&output.stdout);
    assert!(
        version.contains("hakus"),
        "version output should mention hakus, got: {version}"
    );
}
...
fn write_reviewed_bundle_fixture(workspace: &std::path::Path) -> PathBuf {
    ...
    let bundle = workspace.join(".hakus/plugins/demo");
...
async fn plugin_toml_binary_lifecycle_skill_and_stdio_mcp_acceptance() {
    ...
    let mcp_log = workspace.home().join(".hakus/plugin-acceptance.log");
    let (base_url, shutdown_tx, model_thread) = spawn_hermetic_model_server();
    let mut tui = Harness::builder(Harness::cargo_bin("hakus-tui"))
        ...
        .env("HAKUS_DISABLE_MODELS_DEV_FETCH", "1")
        .env("NO_ANIMATIONS", "1")
        .env("RUST_LOG", "warn")
        .args([
            "--workspace",
    ...
    let state = std::fs::read_to_string(workspace.home().join(".hakus/plugins/state.json"))
