...
fn offline_hakus_workspace_containing(world: &mut ToolLifecycleWorld, step: &Step) {
...
async fn user_asks(world: &mut ToolLifecycleWorld, prompt: String) {
    ...
    let output = run_hakus_exec(world, &server, &prompt);
...
fn hakus_should_send_user_request_to_mocked_llm(world: &mut ToolLifecycleWorld) {
...
fn hakus_should_send_tool_result_back_to_mocked_llm(world: &mut ToolLifecycleWorld) {
    let request = world
        .requests
        .iter()
...
fn hakus_should_send_tool_error_back_to_mocked_llm(world: &mut ToolLifecycleWorld) {
    let request = world
        .requests
        .iter()
        .find(|request| request_contains_tool_result(request))
...
fn hakus_should_send_malformed_argument_error_back_to_mocked_llm(
    world: &mut ToolLifecycleWorld,
) {
    let request = world
        .requests
...
fn hakus_should_send_real_tool_error_back_to_mocked_llm(world: &mut ToolLifecycleWorld) {
    let request = world
        .requests
        .iter()
        .find(|request| request_contains_tool_result(request))
...
fn hakus_should_send_empty_tool_result_back_to_mocked_llm(world: &mut ToolLifecycleWorld) {
    let request = world
        .requests
        .iter()
        .find(|request| request_contains_tool_result(request))
...
fn run_hakus_exec(
    ...
    std::fs::create_dir_all(home.join(".hakus")).expect("create hakus home config dir");
...
fn run_with_timeout(mut command: Command, timeout: Duration) -> std::process::Output {
    ...
    let status = match child.wait_timeout(timeout).expect("wait for hakus-tui") {
        Some(status) => status,
        None => {
            let _ = child.kill();
            let _ = child.wait();
            ...
            panic!(
                "hakus-tui exec timed out after {timeout:?}\nstdout:\n{}\nstderr:\n{}",
