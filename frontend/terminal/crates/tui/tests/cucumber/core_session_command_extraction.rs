...
//!
//! This runs the binary through `hakus-tui eval` and verifies that the
...
fn clean_hakus_evaluation_workspace(world: &mut CoreSessionExtractionWorld) {
...
fn eval_harness_runs_shell_command(world: &mut CoreSessionExtractionWorld) {
    ...
    let output = Command::new(hakus_tui_binary())
        ...
        .arg(record_dir.path())
        .output()
        .expect("hakus-tui eval should start");

    assert!(
        output.status.success(),
        "hakus-tui eval failed\nstderr:\n{}",
...
async fn hakus_eval_runs_after_extraction() {
