...
fn clean_hakus_evaluation_workspace(world: &mut EvalSmokeWorld) {
...
fn binary_exits_without_crashing(world: &mut EvalSmokeWorld) {
    ...
    assert!(
        ...
        "hakus-tui eval exited with unexpected code {exit_code} (expected 0 or 1)"
    );
...
fn assert_no_signal_crash(status: &ExitStatus) {
    ...
    assert!(
        ...
        "hakus-tui eval was killed by signal {} (crash?)",
