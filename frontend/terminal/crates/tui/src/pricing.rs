...
pub(crate) fn billing_surface_for_route(
    ...
    match provider {
        ...
        // Ollama Cloud publishes plan/account terms, not a Hakus-owned
...
fn invalid_catalog_pricing_audit(
    offering: &hakus_config::catalog::CatalogOffering,
) -> Option<TurnCostAudit> {
    offering
        .cost
        .as_ref()
        .is_some_and(|cost| !hakus_config::pricing::catalog_cost_is_valid(cost))
...
enum VerifiedOffering {
    ...
    Usable(hakus_config::catalog::CatalogOffering),
...
/// was actually served on (see [`hakus_config::catalog::base_url_fingerprint`]).
...
fn verified_catalog_offering(
    ...
    // not provider prices, and `https://api.hakus.net/session` 503
...
fn capabilities_only_offering(
    mut offering: hakus_config::catalog::CatalogOffering,
) -> hakus_config::catalog::CatalogOffering {
    offering.cost = None;
    offering
}
...
mod tests {
    ...
    fn malformed_catalog_row_has_an_explicit_runtime_reason() {
        let offering = hakus_config::catalog::CatalogOffering {
            provider: "openrouter".to_string(),
            wire_model_id: "openai/gpt-5.5".to_string(),
            cost: Some(hakus_config::models_dev::ModelsDevCost {
                input: Some(f64::NAN),
                output: Some(30.0),
    ...
    fn models_dev_live_cost_is_capabilities_only_and_falls_back_to_bundled_rates() {
        ...
        crate::provider_lake::set_live_snapshot(
            hakus_config::catalog::CatalogSnapshot {
                offerings: vec![hakus_config::catalog::CatalogOffering {
                    ...
                    source: hakus_config::catalog::CatalogSource::Live {
    ...
    fn verified_provider_live_rates_win_over_bundled_docs() {
        ...
        let now = Utc::now();
        let fetched_at = u64::try_from(now.timestamp()).expect("timestamp");
        let fingerprint = hakus_config::catalog::base_url_fingerprint(
