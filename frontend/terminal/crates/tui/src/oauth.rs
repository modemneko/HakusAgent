...
//! consent grant. Hakus never refreshes or rewrites that external file.
//!
//! # Security
//!
...
pub fn missing_auth_message() -> String {
    format!(
        "OpenAI Codex OAuth credentials are unavailable.\n\
         \n\
         Hakus checks OPENAI_CODEX_ACCESS_TOKEN and CODEX_ACCESS_TOKEN automatically.\n\
         Access to the Codex CLI file is disabled by default. After `codex login`, grant read-only access explicitly with:\n\
         `hakus auth external-consent --provider openai-codex --mode read-only --path {}`\n\
...
mod tests {
    ...
    fn missing_auth_message_explains_disabled_default_and_explicit_consent() {
        ...
        assert!(message.contains(&hakus_config::quote_os_path(&auth_file_path())));
