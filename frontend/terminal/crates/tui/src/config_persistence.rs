...
{
    hakus_config::mutate_config_document(path, mutate)
}
...
pub(crate) fn write_config_toml_atomic(path: &Path, body: &str) -> anyhow::Result<()> {
    hakus_config::create_config_document(path, body)
}
...
pub(crate) fn set_document_value(
    ...
    hakus_config::set_config_document_value(doc, segments, value)
}
...
pub(crate) fn unset_document_value(
    ...
    hakus_config::unset_config_document_value(doc, segments)
}
...
pub(crate) fn persist_hotbar_bindings(
    config_path: Option<&Path>,
    bindings: &[hakus_config::HotbarBindingToml],
) -> anyhow::Result<PathBuf> {
    let path = config_toml_path(config_path)?;
    mutate_config_document(&path, |doc| {
...
mod tests {
    ...
    impl EnvGuard {
        fn new(home: &Path) -> Self {
            ...
            Self {
                ...
                _hakus_home: crate::test_support::EnvVarGuard::remove("HAKUS_HOME"),
    ...
    fn persist_status_items_writes_tui_section_to_config_toml() {
        let temp_root = temp_root("hakus-statusline-persist");
    ...
    #[test]
    fn config_toml_path_uses_hakus_home_for_fresh_installs() {
        let temp_root = temp_root("hakus-config-path-fresh");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn config_toml_path_preserves_legacy_config_when_it_exists() {
        let temp_root = temp_root("hakus-config-path-legacy");
    ...
    #[test]
    fn config_toml_path_ignores_legacy_config_when_hakus_home_is_explicit() {
        let temp_root = temp_root("hakus-config-path-explicit-home");
        let explicit_home = temp_root.join("isolated-hakus");
        let legacy_config = temp_root.join(".deepseek").join("config.toml");
        fs::create_dir_all(legacy_config.parent().unwrap()).unwrap();
        fs::write(&legacy_config, "").unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    #[test]
    fn config_toml_path_prefers_hakus_env_over_legacy_env() {
        let temp_root = temp_root("hakus-config-path-env");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        ...
        unsafe {
            env::set_var("HAKUS_CONFIG_PATH", &preferred);
    ...
    fn config_toml_path_keeps_missing_env_target_authoritative() {
        let temp_root = temp_root("hakus-config-path-missing-env-fallback");
        let home_config = temp_root.join(".hakus").join("config.toml");
    ...
    fn persist_status_items_preserves_existing_unrelated_keys() {
        let temp_root = temp_root("hakus-statusline-preserve");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn persist_bool_key_preserves_comments() {
        let temp_root = temp_root("hakus-persist-comments");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn persist_table_bool_key_updates_existing_memory_enabled() {
        let temp_root = temp_root("hakus-persist-memory-update");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn persist_memory_enabled_round_trips_through_config_load() {
        let temp_root = temp_root("hakus-persist-memory-roundtrip");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn persist_custom_provider_writes_named_openai_compatible_table() {
        let temp_root = temp_root("hakus-custom-provider-persist");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);

        let path = temp_root.join(".hakus").join("config.toml");
        ...
        let dispatcher = hakus_config::ConfigStore::load(Some(written))
    ...
    fn persist_custom_provider_rejects_builtin_or_invalid_names() {
        let temp_root = temp_root("hakus-custom-provider-invalid");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        let path = temp_root.join(".hakus").join("config.toml");
    ...
    fn persist_local_custom_provider_records_keyless_auth() {
        let temp_root = temp_root("hakus-custom-provider-local-keyless");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        let path = temp_root.join(".hakus").join("config.toml");
    ...
    fn persist_hotbar_bindings_writes_primary_config_path_for_fresh_installs() {
        let temp_root = temp_root("hakus-hotbar-persist-fresh");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        ...
        assert_eq!(path, temp_root.join(".hakus").join("config.toml"));
        let body = fs::read_to_string(&path).expect("written file should be readable");
    ...
    fn persist_default_hotbar_bindings_round_trips_for_hotbar_on() {
        ...
        let temp_root = temp_root("hakus-hotbar-on-defaults");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);

        let defaults = hakus_config::default_hotbar_bindings_toml();
        assert_eq!(defaults.len(), hakus_config::HOTBAR_SLOT_COUNT as usize);
        ...
        let resolved = parsed.resolve_hotbar_bindings(&hakus_config::DEFAULT_HOTBAR_ACTIONS);
        assert_eq!(
            resolved.bindings,
            hakus_config::default_hotbar_bindings()
        );
    }

    #[test]
    fn persist_hotbar_bindings_preserves_comments_and_replaces_existing_tables() {
        let temp_root = temp_root("hakus-hotbar-persist-comments");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
...
"#,
    ...
    fn persist_hotbar_bindings_writes_empty_array_to_disable_defaults() {
        let temp_root = temp_root("hakus-hotbar-persist-empty");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
...
"#;
    ...
    fn golden_replacing_existing_root_value_only_touches_that_value() {
        let temp_root = temp_root("hakus-golden-root-value");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        let path = temp_root.join(".deepseek").join("config.toml");
    ...
    fn golden_mutations_preserve_unrelated_comments_order_and_quoted_tables() {
        let temp_root = temp_root("hakus-golden-mutations");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        let path = temp_root.join(".deepseek").join("config.toml");
        write_golden_config(&path);
        ...
        persist_hotbar_bindings(
            Some(&path),
            &[hakus_config::HotbarBindingToml {
                slot: 2,
                action: "session.compact".to_string(),
                label: None,
            }],
    ...
    fn set_document_value_inserts_api_key_even_when_a_comment_mentions_it() {
        ...
        // the insert entirely.
        let temp_root = temp_root("hakus-golden-api-key-comment");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        let path = temp_root.join(".deepseek").join("config.toml");
        write_golden_config(&path);
...
"#
    ...
    fn persist_custom_provider_unsets_removed_optional_fields() {
        let temp_root = temp_root("hakus-custom-provider-unset");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        let path = temp_root.join(".hakus").join("config.toml");
    ...
    fn config_writes_land_with_owner_only_permissions() {
        ...
        let temp_root = temp_root("hakus-persist-perms");
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
        let path = temp_root.join(".deepseek").join("config.toml");
        write_golden_config(&path);
    ...
    /// resolves it independently in `hakus-config`'s
    /// `resolve_runtime_options`, which is what `hakus model resolve`
    ...
    fn the_dispatcher_and_the_tui_resolve_the_same_active_model() {
        ...
        for (case, body, expected) in cases {
            let temp_root = temp_root("hakus-model-chain-agreement");
            fs::create_dir_all(&temp_root).unwrap();
            let _guard = EnvGuard::new(&temp_root);
            ...
            let dispatcher = hakus_config::ConfigStore::load(Some(path.clone()))
