...
pub const ALL_MESSAGE_IDS: &[MessageId] = &[
    ...
    MessageId::CmdWechatDescription,
...
mod tests {
    ...
    fn zh_hans_constitution_copy_uses_charter_term() {
        ...
        assert!(setup_intro.contains("Hakus"));
        ...
        // lead names commands, not the product, so asserting "Hakus" here
    ...
    fn provider_description_is_present_for_all_locales() {
        for locale in Locale::shipped_complete() {
            ...
            assert!(
                !description.contains("hakus |"),
                "{} provider description should not name hakus as a backend: {description}",
                locale.tag()
            );
        }
    }
