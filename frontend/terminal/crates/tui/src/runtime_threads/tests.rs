...
use hakus_protocol::agent_mail::{AgentMailDeliveryMode, AgentMailSender, AgentMailStatus};
...
const EVENT_PROCESS_ROLE_ENV: &str = "HAKUS_TEST_EVENT_PROCESS_ROLE";
const EVENT_PROCESS_ROOT_ENV: &str = "HAKUS_TEST_EVENT_PROCESS_ROOT";
const EVENT_PROCESS_THREAD_ENV: &str = "HAKUS_TEST_EVENT_PROCESS_THREAD";
const EVENT_PROCESS_SIGNAL_ENV: &str = "HAKUS_TEST_EVENT_PROCESS_SIGNAL";
const EVENT_PROCESS_START_ENV: &str = "HAKUS_TEST_EVENT_PROCESS_START";
const EVENT_PROCESS_WORKER_ENV: &str = "HAKUS_TEST_EVENT_PROCESS_WORKER";
const EVENT_PROCESS_COUNT_ENV: &str = "HAKUS_TEST_EVENT_PROCESS_COUNT";
...
async fn update_thread_workspace_persists_event_and_evicts_idle_engine() -> Result<()> {
    let manager = test_manager(test_runtime_dir())?;
    let old_workspace = std::env::temp_dir().join("hakus-runtime-old-workspace");
    let new_workspace = std::env::temp_dir().join("hakus-runtime-new-workspace");
    let thread = manager
        .create_thread(CreateThreadRequest {
            model: None,
...
async fn update_thread_workspace_rejects_active_turn() -> Result<()> {
    let manager = test_manager(test_runtime_dir())?;
    let old_workspace = std::env::temp_dir().join("hakus-runtime-active-old");
    let new_workspace = std::env::temp_dir().join("hakus-runtime-active-new");
    let thread = manager
        .create_thread(CreateThreadRequest {
            model: None,
            workspace: Some(old_workspace.clone()),
...
///   HAKUS_MAIL_ACCEPTANCE_DIR=/Volumes/VIXinSSD/CW/artifacts/... \
///     cargo test -p hakus-tui --lib agent_mail_release_acceptance -- --ignored
#[tokio::test]
#[ignore = "release acceptance: run explicitly with HAKUS_MAIL_ACCEPTANCE_DIR"]
async fn agent_mail_release_acceptance_two_task_matrix() -> Result<()> {
    use hakus_protocol::agent_mail::AGENT_MAIL_EVENT_READ;
    ...
    fs::create_dir_all(&workspace)?;
    fs::create_dir_all(&foreign_workspace)?;
    let artifacts = std::env::var("HAKUS_MAIL_ACCEPTANCE_DIR")
        .map(PathBuf::from)
