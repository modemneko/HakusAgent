...
fn readonly_tokens_admitted(trimmed: &str) -> bool {
    ...
    let command_refs = command_tokens
        ...
        .map(String::as_str)
        .collect::<Vec<_>>();
    if is_hakus_readonly_invocation(&command_refs) {
        return true;
    }
...
fn is_hakus_readonly_invocation(tokens: &[&str]) -> bool {
    let Some((command, args)) = tokens.split_first() else {
        return false;
    };
    if !matches!(*command, "hakus" | "codew") {
        return false;
    }
...
fn is_safe_command(command: &str) -> bool {
    ...
    if let Some(start) = primary_token_index(&tokens) {
        let refs = tokens[start..]
            ...
            .map(String::as_str)
            .collect::<Vec<_>>();
        if is_hakus_readonly_invocation(&refs) {
            return true;
        }
    }
...
mod tests {
    ...
    fn test_safe_commands() {
        ...
        assert_eq!(
            analyze_command("hakus --version").level,
            SafetyLevel::Safe
        );
        assert_eq!(analyze_command("hakus --help").level, SafetyLevel::Safe);
        assert_eq!(
    ...
    fn test_eval_invocation_is_blocked_without_substring_false_positive() {
        ...
        assert_ne!(
            analyze_command("cargo run --bin hakus -- eval").level,
            SafetyLevel::Dangerous
        );
    }
    ...
    fn test_eval_substring_is_not_misclassified() {
        ...
        let evaluate_safe = analyze_command("cargo run --bin hakus -- eval").level;
        assert_ne!(
            evaluate_safe,
            SafetyLevel::Dangerous,
