...
//!
//! Storage shape (TOML at `~/.hakus/skills_state.toml`, legacy `~/.deepseek/skills_state.toml`):
//!
//! ```toml
...
impl SkillStateStore {
    ...
    /// other Hakus processes. Long-running Runtime API servers call this
...
fn default_state_path() -> Result<PathBuf> {
    ...
    Ok(hakus_config::hakus_home()
        .context("could not resolve Hakus state directory")?
...
fn persist_disabled(path: &Path, disabled: &BTreeSet<String>) -> Result<()> {
    ...
    hakus_config::persistence::atomic_write(path, body.as_bytes())
...
mod tests {
    ...
    fn cross_process_toggles_serialize_and_merge() {
        const CHILD_PATH: &str = "HAKUS_TEST_SKILL_STATE_PATH";
        const CHILD_NAME: &str = "HAKUS_TEST_SKILL_STATE_NAME";
