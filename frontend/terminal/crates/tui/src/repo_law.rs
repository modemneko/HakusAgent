...
//!
//! `.hakus/constitution.json` invariants were previously advisory prose
...
pub(crate) fn repo_law_plan_decision(
    ...
    let reason = format!(
        "Repo law holds this write: \"{}\" protects {protects} (matched {target}, .hakus/constitution.json)",
        rule.text
    );
...
mod tests {
    ...
    fn block_action_denies_protected_write() {
        ...
        assert!(reason.contains(".hakus/constitution.json"), "{reason}");
    }
