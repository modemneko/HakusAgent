...
use hakus_config::ExternalCredentialReadGrant;
...
/// Read one Hakus-owned credential file through the same no-follow,
...
/// below Hakus's credentials directory before invoking this function.
pub(crate) fn read_hakus_owned_to_string(path: &Path) -> Result<Option<String>> {
    let mut file = match open_secure_regular_file(path, true) {
        ...
        Err(error) => {
            return Err(error).with_context(|| {
                format!(
                    "securely opening Hakus-owned credential file {}",
                    hakus_config::quote_os_path(path)
                )
            });
        }
    ...
    file.by_ref()
        ...
        .with_context(|| {
            format!(
                "reading Hakus-owned credential file {}",
                hakus_config::quote_os_path(path)
            )
        })?;
    if bytes.len() as u64 > MAX_EXTERNAL_CREDENTIAL_BYTES {
        bail!(
            "Hakus-owned credential file {} exceeds the {} byte safety limit",
            hakus_config::quote_os_path(path),
            MAX_EXTERNAL_CREDENTIAL_BYTES
        );
    }
    String::from_utf8(bytes).map(Some).with_context(|| {
        format!(
            "Hakus-owned credential file {} is not valid UTF-8",
            hakus_config::quote_os_path(path)
        )
    })
}
    ...
    if require_owner_only {
            ...
            || metadata.nlink() != 1
        {
            return Err(io::Error::new(
                ...
                "Hakus-owned credential file must be singly linked, owned by this user, and mode 0600 or stricter",
            ));
        }
    }
    Ok(current)
...
fn open_secure_regular_file(path: &Path, require_owner_only: bool) -> io::Result<File> {
    ...
    if require_owner_only {
        ...
        if information.nNumberOfLinks != 1 {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                "Hakus-owned credential file must be singly linked",
            ));
        }
...
pub(crate) fn secure_hakus_owned_windows_path(
    path: &Path,
...
fn verify_windows_owner_only_handle(
    ...
    if owner.is_null() || unsafe { EqualSid(owner, user.sid()) } == 0 {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "Hakus-owned credential file owner is not the current user",
        ));
    }
    if dacl.is_null() {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "Hakus-owned credential file must have an owner-only DACL",
        ));
    }
    let mut count = 0;
    ...
    if count != 1 || entries.is_null() {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "Hakus-owned credential file DACL must grant only one user",
        ));
    }
    ...
    if !current_user_only {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "Hakus-owned credential file DACL is not current-user-only",
        ));
    }
    Ok(())
}
        ...
        if metadata.file_attributes() & FILE_ATTRIBUTE_REPARSE_POINT != 0 {
            return Err(io::Error::new(
                ...
                format!(
                    ...
                    hakus_config::quote_os_path(&current)
                ),
            ));
        }
    }
...
mod tests {
    use super::*;
    use hakus_config::{ExternalCredentialConsentToml, ExternalCredentialSource, ProviderKind};
    ...
    fn owned_read_requires_owner_only_regular_file_and_never_follows_symlinks() {
        ...
        std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o644))
            .expect("loose mode");
        assert!(
            read_hakus_owned_to_string(&path).is_err(),
        ...
        assert_eq!(
            read_hakus_owned_to_string(&path)
        ...
        assert!(read_hakus_owned_to_string(&link).is_err());
    }
    ...
    fn owned_read_is_bounded() {
        ...
        file.set_permissions(std::fs::Permissions::from_mode(0o600))
            .unwrap();
        let error = read_hakus_owned_to_string(&path).expect_err("oversized owned file");
        assert!(error.to_string().contains("safety limit"), "{error:#}");
    }
    ...
    fn secured_owned_windows_fixture(contents: &[u8]) -> (tempfile::TempDir, std::path::PathBuf) {
        let dir = tempfile::tempdir().expect("tempdir");
        secure_hakus_owned_windows_path(dir.path(), true).expect("owner-only directory");
        let path = dir.path().join("owned.json");
        std::fs::write(&path, contents).expect("fixture");
        secure_hakus_owned_windows_path(&path, false).expect("owner-only file");
        (dir, path)
    }
    ...
    fn owned_read_rejects_leaf_reparse_points_on_windows() {
        ...
        if std::os::windows::fs::symlink_file(&path, &link).is_ok() {
            assert!(
                read_hakus_owned_to_string(&link).is_err(),
