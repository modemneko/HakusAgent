...
fn workspace_trust_config_guard(workspace: &Path) -> WorkspaceTrustConfigGuard {
    ...
    let hakus_config_path =
        crate::test_support::EnvVarGuard::set("HAKUS_CONFIG_PATH", config_path.as_os_str());
...
fn write_path_only_test_command(dir: &Path) -> String {
    let command = "hakus-mcp-path-only-test";
    #[cfg(windows)]
...
fn static_mcp_command_uses_expanded_sanitized_stdio_path() {
    ...
    let _path = crate::test_support::EnvVarGuard::set(
        "HAKUS_MCP_PATH_ONLY_DIR",
        temp.path().as_os_str(),
    );
    let _secret = crate::test_support::EnvVarGuard::set(
        "HAKUS_MCP_STATIC_TEST_SECRET",
    ...
    server.env.insert(
        "PATH".to_string(),
        "${HAKUS_MCP_PATH_ONLY_DIR}".to_string(),
    );
    ...
    assert!(
        child_env
            .iter()
            .all(|(key, _)| key != "HAKUS_MCP_STATIC_TEST_SECRET"),
...
fn static_mcp_command_reports_missing_with_server_path_override() {
    let temp = tempfile::tempdir().expect("tempdir");
    let mut server = test_server_config();
    server.command = Some("hakus-mcp-command-that-does-not-exist".to_string());
    server.env.insert(
        "PATH".to_string(),
...
fn static_mcp_command_reports_invalid_path_expansion() {
    let _lock = crate::test_support::lock_test_env();
    let _missing = crate::test_support::EnvVarGuard::remove("HAKUS_MCP_MISSING_PATH_DIR");
    let mut server = test_server_config();
    server.command = Some("hakus-mcp-command".to_string());
    server.env.insert(
        "PATH".to_string(),
        "do-not-leak-${HAKUS_MCP_MISSING_PATH_DIR}-also-secret".to_string(),
    );

    let error = static_mcp_command_availability(&server)
        .expect_err("missing PATH placeholder must fail static validation");
    let error = format!("{error:#}");
    assert!(error.contains("HAKUS_MCP_MISSING_PATH_DIR"));
    assert!(!error.contains("hakus-mcp-command"));
...
fn static_mcp_command_anchors_relative_and_empty_path_to_server_cwd() {
    ...
    let relative_command = "hakus-mcp-relative-path-test";
    ...
    let empty_path_command = "hakus-mcp-empty-path-test";
...
fn static_mcp_command_preserves_literal_name_and_requires_execute_bits() {
    let temp = tempfile::tempdir().expect("tempdir");
    let literal_command = " hakus-mcp-literal-command-test ";
    ...
    let non_executable = temp.path().join("hakus-mcp-non-executable-test");
    write_unix_test_command(&non_executable, 0o644);
    server.command = Some("hakus-mcp-non-executable-test".to_string());
    assert_eq!(
...
fn static_mcp_command_matches_windows_path_and_extension_rules() {
    ...
    let pathext_command = "hakus-mcp-pathext-only-test";
    fs::write(
...
fn mcp_auth_required_error_item_is_model_visible() {
    ...
    assert!(
        item["message"]
            .as_str()
            .expect("message")
            .contains("hakus mcp login nordic-mcp")
    );
}
...
"#,
    ...
    let discovery = crate::plugins::discovery::DiscoveryConfig {
        ...
        user_plugins_dir: plugins_root,
        workspace_plugins_dir: workspace.join(".hakus/plugins-unused"),
        builtin_plugin_dirs: Vec::new(),
...
CALL_MARKER = "${HAKUS_TEST_PLUGIN_CALL_MARKER}"
"#,
    )
    .unwrap();
...
async fn connect_all_reloads_before_snapshotting_new_server_names() {
    ...
    std::fs::write(
        &path,
        r#"{"servers":{"late":{"command":"hakus-test-command-that-does-not-exist"}}}"#,
    )
    .unwrap();
...
async fn discover_snapshot_includes_underlying_spawn_error_in_chain() {
    ...
    fs::write(
        ...
        r#"{
            "mcpServers": {
                "broken": {
                    "command": "hakus-tui-test-this-binary-does-not-exist-9f8e7d6c5b4a",
                    "args": []
                }
            }
        }"#,
