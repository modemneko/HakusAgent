...
//!
//! Everything here uses sealed fixtures (temp `HAKUS_HOME`, test keys in
...
//! - an explicit workspace config (via `HAKUS_CONFIG_PATH`) can select a
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            match &self.prev_home {
                Some(v) => std::env::set_var("HAKUS_HOME", v),
                None => std::env::remove_var("HAKUS_HOME"),
            }
            match &self.prev_config_path {
                Some(v) => std::env::set_var("HAKUS_CONFIG_PATH", v),
                None => std::env::remove_var("HAKUS_CONFIG_PATH"),
            }
        }
    }
}
...
fn sealed_home() -> &'static Path {
    ...
    HOME.get_or_init(|| {
        ...
        // The config lives at $HAKUS_HOME/config.toml (the primary path
        // when HAKUS_HOME is explicit).
        std::fs::write(
...
/// Point `HAKUS_HOME` at the sealed home (optionally also pinning
/// `HAKUS_CONFIG_PATH`). Caller must hold `lock_test_env`.
fn sealed_env(config_path: Option<&Path>) -> HomeGuard {
    let prev_home = std::env::var_os("HAKUS_HOME");
    let prev_config_path = std::env::var_os("HAKUS_CONFIG_PATH");
    // SAFETY: serialised by lock_test_env held by the caller.
    unsafe {
        std::env::set_var("HAKUS_HOME", sealed_home());
        match config_path {
            Some(path) => std::env::set_var("HAKUS_CONFIG_PATH", path),
            None => std::env::remove_var("HAKUS_CONFIG_PATH"),
        }
    }
    HomeGuard {
...
fn workspace_with_config(dir: &Path, provider: &str) -> PathBuf {
    ...
    std::fs::create_dir_all(ws.join(".hakus")).expect("workspace dir");
    std::fs::write(
        ws.join(".hakus").join("config.toml"),
        format!(
            r#"provider = "{provider}"
...
fn explicit_workspace_config_selects_its_route_without_locking_user_global() {
    ...
    let config_path = ws_a.join(".hakus").join("config.toml");
...
fn write_nested_zai_config(base: &std::path::Path) -> std::path::PathBuf {
    ...
    std::fs::create_dir_all(nested.join(".hakus")).expect("nested dir");
    std::fs::write(
        nested.join(".hakus").join("config.toml"),
        r#"provider = "zai"
...
fn nested_repo_does_not_change_readiness() {
    ...
    assert_user_global_survives_workspace("nested", nested.join(".hakus/config.toml"));
}
...
fn symlinked_worktree_does_not_change_readiness() {
    ...
    assert_user_global_survives_workspace("symlinked", symlinked.join(".hakus/config.toml"));
}
...
fn workspace_fleet_selection_affects_only_that_workspace() {
    ...
    let prev = std::env::var_os("HAKUS_HOME");
    // SAFETY: serialised by lock_test_env.
    unsafe { std::env::set_var("HAKUS_HOME", home.path()) };
    ...
    assert!(
        !ws_a.join(".hakus/fleets/team-a.toml").exists(),
