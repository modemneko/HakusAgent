...
fn api_provider_metadata_helpers_follow_config_provider_metadata() {
    ...
    for kind in hakus_config::ProviderKind::ALL {
...
fn provider_config_key_follows_config_provider_metadata() {
    for kind in hakus_config::ProviderKind::ALL
        .into_iter()
        .filter(|kind| *kind != hakus_config::ProviderKind::Deepseek)
    {
        let provider = ApiProvider::from_kind(kind);
        assert_eq!(
...
fn config_loads_sibling_permissions_into_exec_policy_engine() {
    ...
    fs::write(
        dir.path().join(hakus_config::PERMISSIONS_FILE_NAME),
        r#"
[[rules]]
tool = "exec_shell"
command = "cargo test"
"#,
    ...
    let decision = config
        .exec_policy_engine
        .check(hakus_execpolicy::ExecPolicyContext {
            command: "cargo test --workspace",
            cwd: dir.path().to_string_lossy().as_ref(),
            tool: Some("exec_shell"),
            path: None,
            ask_for_approval: hakus_execpolicy::AskForApproval::OnFailure,
            sandbox_mode: None,
        })
...
#[test]
fn load_honors_hakus_home_for_primary_config_path() -> Result<()> {
    let _lock = lock_test_env();
    let dir = tempfile::tempdir()?;
    let hakus_home = dir.path().join("isolated-hakus");
    fs::create_dir_all(&hakus_home)?;
    fs::write(hakus_home.join("config.toml"), "provider = \"zai\"\n")?;
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str());
    let _hakus_config = EnvVarGuard::remove("HAKUS_CONFIG_PATH");
    let _deepseek_config = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");

    let expected = hakus_home.join("config.toml");
...
fn profile_hotbar_override_replaces_entire_user_list() {
    ...
    profiles.insert(
        "compact".to_string(),
        Config {
            hotbar: Some(vec![hakus_config::HotbarBindingToml {
                slot: 2,
    ...
    assert_eq!(
        merged.hotbar,
        Some(vec![hakus_config::HotbarBindingToml {
            slot: 2,
            action: "session.compact".to_string(),
            label: Some("Compact".to_string()),
        }])
...
fn verifier_config_parses_hunt_policy_and_merges_overrides() {
    ...
    assert_eq!(
        ...
        hakus_config::VerifierVerdictPolicy::Hunt
    );

    let merged = merge_config(
        Config {
            verifier: Some(hakus_config::VerifierConfigToml {
                enabled: false,
                verdict_policy: hakus_config::VerifierVerdictPolicy::Hunt,
            }),
            ..Config::default()
        },
        Config {
...
fn workflow_config_defaults_when_omitted_and_overrides_round_trip() {
    ...
    assert_eq!(
        ...
        hakus_config::WorkflowConfigToml::default()
    );
    ...
    let round_tripped: hakus_config::WorkflowConfigToml =
    ...
    let merged = merge_config(
        Config {
            workflow: Some(hakus_config::WorkflowConfigToml::default()),
            ..Config::default()
        },
        Config {
...
fn live_search_provider_update_preserves_environment_precedence() {
    let _guard = lock_test_env();
    let previous_hakus = env::var_os("HAKUS_SEARCH_PROVIDER");
    let previous_deepseek = env::var_os("DEEPSEEK_SEARCH_PROVIDER");
    unsafe {
        env::set_var("HAKUS_SEARCH_PROVIDER", "bocha");
    ...
    unsafe {
        EnvGuard::restore_var("HAKUS_SEARCH_PROVIDER", previous_hakus);
...
fn structural_config_load_keeps_safe_environment_overrides_but_omits_secret_values() {
    ...
    let _home = EnvVarGuard::set("HAKUS_HOME", temp.path().join("home"));
    let _profile = EnvVarGuard::remove("HAKUS_PROFILE");
    let _legacy_profile = EnvVarGuard::remove("DEEPSEEK_PROFILE");
    let _managed = EnvVarGuard::remove("HAKUS_MANAGED_CONFIG_PATH");
    let _legacy_managed = EnvVarGuard::remove("DEEPSEEK_MANAGED_CONFIG_PATH");
    let _requirements = EnvVarGuard::remove("HAKUS_REQUIREMENTS_PATH");
    ...
    let _sandbox_key = EnvVarGuard::set("HAKUS_SANDBOX_API_KEY", "structural-sandbox-secret");
    let _legacy_sandbox_key = EnvVarGuard::set(
        "DEEPSEEK_SANDBOX_API_KEY",
        "legacy-structural-sandbox-secret",
    );
    let _search_key = EnvVarGuard::set("HAKUS_SEARCH_API_KEY", "structural-search-secret");
    let _legacy_search_key =
        EnvVarGuard::set("DEEPSEEK_SEARCH_API_KEY", "legacy-structural-search-secret");
    let _base_url = EnvVarGuard::set("HAKUS_BASE_URL", "https://safe.example:8443/v1");
    let _allow_shell = EnvVarGuard::set("HAKUS_ALLOW_SHELL", "false");
...
fn apply_env_overrides_sets_search_base_url() {
    let _guard = lock_test_env();
    let prev_hakus = env::var_os("HAKUS_SEARCH_BASE_URL");
    let prev_deepseek = env::var_os("DEEPSEEK_SEARCH_BASE_URL");
    unsafe {
        env::remove_var("HAKUS_SEARCH_BASE_URL");
        env::set_var(
    ...
    unsafe {
        EnvGuard::restore_var("HAKUS_SEARCH_BASE_URL", prev_hakus);
...
#[test]
fn hakus_search_base_url_env_wins_over_legacy_alias() {
    let _guard = lock_test_env();
    let prev_hakus = env::var_os("HAKUS_SEARCH_BASE_URL");
    let prev_deepseek = env::var_os("DEEPSEEK_SEARCH_BASE_URL");
    unsafe {
        env::set_var(
            "HAKUS_SEARCH_BASE_URL",
            "https://hakus-search.example/html/",
        );
        env::set_var(
            "DEEPSEEK_SEARCH_BASE_URL",
    ...
    assert_eq!(
        config.search.and_then(|search| search.base_url),
        Some("https://hakus-search.example/html/".to_string())
    );
}

#[test]
fn hakus_prefer_bwrap_env_wins_over_legacy_alias() {
    let _guard = lock_test_env();
    let _primary = EnvVarGuard::set("HAKUS_PREFER_BWRAP", "false");
...
fn legacy_prefer_bwrap_env_remains_a_compatible_alias() {
    let _guard = lock_test_env();
    let _primary = EnvVarGuard::remove("HAKUS_PREFER_BWRAP");
    let _legacy = EnvVarGuard::set("DEEPSEEK_PREFER_BWRAP", "true");
    let mut config = Config::default();
...
struct EnvGuard {
    ...
    _sealed_hakus_home: EnvVarGuard,
    _sealed_hakus_config_path: EnvVarGuard,
    _sealed_deepseek_config_path: EnvVarGuard,
    home: Option<OsString>,
    userprofile: Option<OsString>,
    hakus_home: Option<OsString>,
    hakus_config_path: Option<OsString>,
    deepseek_config_path: Option<OsString>,
    hakus_secret_backend: Option<OsString>,
    ...
    hakus_provider: Option<OsString>,
    hakus_model: Option<OsString>,
    hakus_base_url: Option<OsString>,
...
impl EnvGuard {
    fn new(home: &Path) -> Self {
        ...
        let hakus_home_prev = env::var_os("HAKUS_HOME");
        let hakus_config_prev = env::var_os("HAKUS_CONFIG_PATH");
        let deepseek_config_prev = env::var_os("DEEPSEEK_CONFIG_PATH");
        let hakus_secret_backend_prev = env::var_os("HAKUS_SECRET_BACKEND");
        ...
        let hakus_provider_prev = env::var_os("HAKUS_PROVIDER");
        let hakus_model_prev = env::var_os("HAKUS_MODEL");
        let hakus_base_url_prev = env::var_os("HAKUS_BASE_URL");
        ...
        let sealed_hakus_home = EnvVarGuard::remove("HAKUS_HOME");
        let sealed_hakus_config_path = EnvVarGuard::remove("HAKUS_CONFIG_PATH");
        let sealed_deepseek_config_path = EnvVarGuard::set("DEEPSEEK_CONFIG_PATH", &config_str);
        // Safety: test-only environment mutation guarded by a global mutex.
        unsafe {
            env::remove_var("HAKUS_SECRET_BACKEND");
            ...
            env::remove_var("HAKUS_PROVIDER");
            env::remove_var("HAKUS_MODEL");
            env::remove_var("HAKUS_BASE_URL");
        ...
        Self {
            ...
            _sealed_hakus_home: sealed_hakus_home,
            _sealed_hakus_config_path: sealed_hakus_config_path,
            _sealed_deepseek_config_path: sealed_deepseek_config_path,
            home: home_prev,
            userprofile: userprofile_prev,
            hakus_home: hakus_home_prev,
            hakus_config_path: hakus_config_prev,
            deepseek_config_path: deepseek_config_prev,
            hakus_secret_backend: hakus_secret_backend_prev,
            ...
            hakus_provider: hakus_provider_prev,
            hakus_model: hakus_model_prev,
            hakus_base_url: hakus_base_url_prev,
...
impl Drop for EnvGuard {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            Self::restore_var("HAKUS_HOME", self.hakus_home.take());
            Self::restore_var("HAKUS_CONFIG_PATH", self.hakus_config_path.take());
            Self::restore_var("DEEPSEEK_CONFIG_PATH", self.deepseek_config_path.take());
            Self::restore_var(
                "HAKUS_SECRET_BACKEND",
                self.hakus_secret_backend.take(),
            );
            Self::restore_var(
            ...
            Self::restore_var("HAKUS_PROVIDER", self.hakus_provider.take());
            Self::restore_var("HAKUS_MODEL", self.hakus_model.take());
            Self::restore_var("HAKUS_BASE_URL", self.hakus_base_url.take());
...
fn subagent_max_spawn_depth_defaults_allows_zero_and_clamps() {
    assert_eq!(
        ...
        hakus_config::DEFAULT_SPAWN_DEPTH
    );
    ...
    let high = Config {
        subagents: Some(SubagentsConfig {
            max_depth: Some(hakus_config::MAX_SPAWN_DEPTH_CEILING + 10),
            ..SubagentsConfig::default()
        }),
        ..Config::default()
    };
    assert_eq!(
        ...
        hakus_config::MAX_SPAWN_DEPTH_CEILING
    );
}
...
fn policy_control_waits_for_foreign_test_env_overrides_to_restore() {
    ...
    let reader = {
        ...
        let shell_override = EnvVarGuard::set("HAKUS_ALLOW_SHELL", "false");
...
fn base_url_reads_wait_for_foreign_test_env_overrides_to_restore() {
    ...
    let reader = {
        ...
        let override_guard =
            EnvVarGuard::set("HAKUS_BASE_URL", "https://temporary.test.invalid/v1");
        let reader = std::thread::spawn(move || {
...
fn workspace_trust_reads_existing_projects_table() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-existing-project-trust-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn save_deepseek_key_uses_isolated_file_store_without_plaintext_config() -> Result<()> {
    ...
    let hakus_home = temp_root.path().join("hakus-home");
    let config_path = hakus_home.join("config.toml");
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str());
    let _config_path = EnvVarGuard::set("HAKUS_CONFIG_PATH", config_path.as_os_str());
    let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
    ...
    assert_eq!(
        hakus_secrets::Secrets::auto_detect().get("deepseek")?,
...
fn full_logout_clears_secret_store_slot_and_config_document() -> Result<()> {
    ...
    let _guard = EnvGuard::new(&temp_root);
    let hakus_home = temp_root.join("hakus-home");
    let config_path = hakus_home.join("config.toml");
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str());
    let _config_path = EnvVarGuard::set("HAKUS_CONFIG_PATH", config_path.as_os_str());
    let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
...
fn single_provider_logout_clears_secret_store_slot() -> Result<()> {
    ...
    assert_eq!(
        hakus_secrets::Secrets::auto_detect().get("openrouter")?,
...
fn config_api_key_shadow_warning_names_sources_winner_and_resolution() -> Result<()> {
    ...
    let _config_path = EnvVarGuard::set("HAKUS_CONFIG_PATH", config_path.as_os_str());
    let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
    hakus_secrets::Secrets::auto_detect().set("openrouter", "store-key")?;
    ...
    assert!(
        warning.contains("hakus auth set --provider openrouter"),
...
#[test]
fn whitespace_hakus_home_never_opens_ambient_file_secret_store() -> Result<()> {
    let _lock = lock_test_env();
    let temp_root = tempfile::tempdir()?;
    ...
    let _hakus_home_unset = EnvVarGuard::remove("HAKUS_HOME");
    let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
    let _config_path = EnvVarGuard::set("HAKUS_CONFIG_PATH", &config_path);
    let ambient_store = hakus_secrets::Secrets::file_backed();
    ...
    let _whitespace_home = EnvVarGuard::set("HAKUS_HOME", " \t ");
    let resolved_config_path = hakus_config::resolve_config_path(None)?;
...
fn provider_api_key_config_failure_restores_secret_and_keeps_external_route() -> Result<()> {
    ...
    for prior in [None, Some("prior-xai-secret")] {
        let temp_root = tempfile::tempdir()?;
        let _guard = EnvGuard::new(temp_root.path());
        let hakus_home = temp_root.path().canonicalize()?.join("hakus-home");
        fs::create_dir_all(&hakus_home)?;
        let config_path = hakus_home.join("config.toml");
        fs::create_dir(&config_path)?;
        let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &hakus_home);
        let _config_path = EnvVarGuard::set("HAKUS_CONFIG_PATH", &config_path);
        let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
        let generation = "xai-auth-0123456789abcdef0123456789abcdef.json";
        hakus_config::with_xai_oauth_lifecycle_lock(|store| {
        ...
        assert_eq!(
            fs::read(hakus_home.join("credentials").join(generation))?,
            b"prior-owned-epoch",
...
fn save_key_refuses_plaintext_config_when_isolated_file_store_is_unwritable() -> Result<()> {
    ...
    let hakus_home = temp_root.path().join("hakus-home");
    let config_path = hakus_home.join("config.toml");
    fs::create_dir_all(hakus_home.join("secrets"))?;
    fs::write(
        hakus_home.join("secrets/secrets.json"),
        "not valid json",
    )?;
    #[cfg(unix)]
    fs::set_permissions(
    ...
    assert!(
        message.contains(&hakus_config::quote_os_path(&resolved_config_path)),
        "{message}"
    );
    assert!(
...
#[test]
fn relative_hakus_home_key_save_creates_no_workspace_state() -> Result<()> {
    let _lock = lock_test_env();
    let temp_root = tempfile::tempdir()?;
    let _guard = EnvGuard::new(temp_root.path());
    ...
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &relative_home);
    let _config_path = EnvVarGuard::remove("HAKUS_CONFIG_PATH");
    let _legacy_config_path = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
    let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");

    let error = save_api_key("never-persisted")
        .expect_err("relative HAKUS_HOME must fail before persistence");
    let message = format!("{error:#}");
    assert!(message.contains("HAKUS_HOME"), "{message}");
...
fn provider_neutral_cli_key_wins_after_profile_provider_switch() -> Result<()> {
    ...
    let _cli_key = EnvVarGuard::set("HAKUS_CLI_API_KEY", "explicit-profile-key");
...
fn provider_neutral_cli_key_requires_dispatcher_source_marker() -> Result<()> {
    let _lock = lock_test_env();
    let _source = EnvVarGuard::remove("DEEPSEEK_API_KEY_SOURCE");
    let _cli_key = EnvVarGuard::set("HAKUS_CLI_API_KEY", "untrusted-generic-key");
    let _anthropic_env = EnvVarGuard::remove("ANTHROPIC_API_KEY");
    let mut providers = ProvidersConfig::default();
    providers.anthropic.api_key = Some("saved-anthropic-key".to_string());
    let config = Config {
...
fn has_api_key_uses_root_config_key_for_deepseek_variants() {
    // A concurrent HAKUS_BASE_URL override deliberately unbinds the saved
...
"#,
    ...
    assert!(
        ...
        "provider-scoped hakus key must be stripped: {after}"
    );
    assert!(
...
fn save_api_key_for_preserves_comments_in_provider_tables() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-provider-key-comments-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn standalone_tui_reads_saved_secret_before_ambient_env() -> Result<()> {
    ...
    let temp_root = tempfile::tempdir()?;
    let _guard = EnvGuard::new(temp_root.path());
    let hakus_home = temp_root.path().join("isolated-hakus");
    fs::create_dir_all(&hakus_home)?;
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str());
    let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
...
fn authenticated_local_provider_reads_saved_secret() -> Result<()> {
    ...
    hakus_secrets::Secrets::auto_detect().set("vllm", "saved-local-secret")?;
...
fn named_custom_provider_never_reuses_generic_custom_secret() -> Result<()> {
    ...
    hakus_secrets::Secrets::auto_detect().set("custom", "endpoint-a-secret")?;
...
fn built_in_provider_custom_endpoint_never_reuses_global_credentials() -> Result<()> {
    ...
    hakus_secrets::Secrets::auto_detect().set("openrouter", "saved-official-key")?;
...
fn env_selected_custom_endpoints_do_not_rebind_file_credentials() -> Result<()> {
    ...
    let _source = EnvVarGuard::remove("DEEPSEEK_API_KEY_SOURCE");
    let _cli_key = EnvVarGuard::remove("HAKUS_CLI_API_KEY");
...
fn generic_base_url_override_never_reaches_pinned_child_routes() -> Result<()> {
    ...
    for env_name in ["HAKUS_BASE_URL", "DEEPSEEK_BASE_URL"] {
        ...
        let custom_placeholder = normalize_base_url(
            hakus_config::ProviderKind::Custom
                .provider()
                .default_base_url(),
        );
        assert_eq!(
...
fn provider_scoped_base_url_env_applies_only_to_its_own_route() -> Result<()> {
    ...
    let session_host = "https://session-gateway.example.test/v1";
    let _base = EnvVarGuard::set("HAKUS_BASE_URL", session_host);
...
"#,
    ...
    let _source = EnvVarGuard::set("DEEPSEEK_API_KEY_SOURCE", "cli");
    let _cli_key = EnvVarGuard::set("HAKUS_CLI_API_KEY", "explicit-cli-key");
...
"#,
    ...
    assert_eq!(
        effective.access,
        hakus_config::ExternalCredentialAccess::Disabled
    );
    assert!(!has_api_key_for(&config, ApiProvider::OpenaiCodex));
    assert_eq!(
...
"#,
    ...
    let _base = EnvVarGuard::set("HAKUS_BASE_URL", "https://env-gateway.example.test/v1");
...
fn auth_mode_none_suppresses_config_env_secret_and_oauth_credentials() -> Result<()> {
    ...
    hakus_secrets::Secrets::auto_detect().set("xai", "saved-xai-key")?;
...
fn active_provider_detects_env_only_api_key() -> Result<()> {
    let _lock = lock_test_env();
    let temp_root =
        env::temp_dir().join(format!("hakus-tui-env-only-key-{}", std::process::id()));
    fs::create_dir_all(&temp_root)?;
    let _guard = EnvGuard::new(&temp_root);
...
#[test]
fn default_user_paths_use_hakus_home_for_fresh_installs() -> Result<()> {
    let _lock = lock_test_env();
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
    ...
    assert_eq!(
        ...
        temp_root.join(".hakus").join("config.toml")
    );
    assert_eq!(
        config.mcp_config_path(),
        temp_root.join(".hakus").join("mcp.json")
    );
    assert_eq!(
        config.notes_path(),
        temp_root.join(".hakus").join("notes.txt")
    );
    assert_eq!(
        config.memory_path(),
        temp_root.join(".hakus").join("memory.md")
    );
    assert_eq!(
        config.skills_dir(),
        temp_root.join(".hakus").join("skills")
    );
...
#[test]
fn explicit_hakus_home_isolates_all_config_owned_user_paths() -> Result<()> {
    let _lock = lock_test_env();
    let temp_root = tempfile::tempdir()?;
    let ambient_home = temp_root.path().join("ambient-home");
    ...
    let _home = EnvVarGuard::set("HOME", &ambient_home);
    let _userprofile = EnvVarGuard::set("USERPROFILE", &ambient_home);
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &explicit_home);
...
fn relative_mcp_config_path_falls_back_to_user_global_config() -> Result<()> {
    let _lock = lock_test_env();
    let temp_root = tempfile::tempdir()?;
    let explicit_home = temp_root.path().join("hakus-home");
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &explicit_home);
    let config = Config {
...
#[test]
fn whitespace_hakus_home_keeps_ambient_legacy_config_path_fallbacks() -> Result<()> {
    let _lock = lock_test_env();
    let temp_root = tempfile::tempdir()?;
    let ambient_home = temp_root.path().join("ambient-home");
    let ambient_legacy = ambient_home.join(".deepseek");
...
#[cfg(unix)]
#[test]
fn non_unicode_hakus_home_is_preserved_by_config_owned_user_paths() -> Result<()> {
    ...
    let explicit_home = temp_root
        .path()
        .join(OsString::from_vec(b"hakus-\xff-home".to_vec()));
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &explicit_home);
...
#[test]
fn hakus_config_path_env_wins_over_legacy_env() -> Result<()> {
    let _lock = lock_test_env();
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-config-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
    let preferred = temp_root.join("preferred.toml");
    let legacy = temp_root.join("legacy.toml");
    let _hakus_config = EnvVarGuard::set("HAKUS_CONFIG_PATH", &preferred);
...
#[test]
fn skills_scan_hakus_only_defaults_false_and_parses_true() -> Result<()> {
    assert!(!Config::default().skills_config().scan_hakus_only());

    let config: Config = toml::from_str(
        r#"
[skills]
scan_hakus_only = true
"#,
    )?;

    assert!(config.skills_config().scan_hakus_only());
    Ok(())
}
...
fn relative_config_env_is_a_load_error() -> Result<()> {
    ...
    let temp_root = tempfile::tempdir()?;
    let _guard = EnvGuard::new(temp_root.path());
    let _config_path = EnvVarGuard::set("HAKUS_CONFIG_PATH", ".hakus/config.toml");
    let _legacy_config_path = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
...
fn normalize_model_name_rejects_invalid_or_non_deepseek_ids() {
    ...
    assert!(normalize_model_name("hakus v4").is_none());
...
fn profile_skills_config_merges_individual_fields() {
    ...
    assert_eq!(skills.scan_hakus_only, Some(true));
}
...
fn retired_deepseek_aliases_from_env_are_migrated_before_runtime() -> Result<()> {
    ...
    for (provider, alias, expected_effort) in [
        ...
        // Safety: test-only environment mutation guarded by a global mutex.
        unsafe {
            env::set_var("HAKUS_PROVIDER", provider);
            env::set_var("HAKUS_MODEL", alias);
        }
...
fn nvidia_nim_provider_normalizes_deepseek_v4_pro_alias() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-nim-model-alias-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn nvidia_nim_provider_normalizes_deepseek_v4_flash_alias() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-nim-flash-model-alias-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn mistral_model_env_overrides_vendor_default() {
    ...
    let _generic_model = EnvVarGuard::remove("HAKUS_MODEL");
...
#[test]
fn hakus_model_precedes_mistral_model() {
    let _lock = lock_test_env();
    let _provider_model = EnvVarGuard::set("MISTRAL_MODEL", "mistral-small-latest");
    let _generic_model = EnvVarGuard::set("HAKUS_MODEL", "mistral-medium-latest");
    let _legacy_model = EnvVarGuard::remove("DEEPSEEK_MODEL");
    let mut config = Config {
        provider: Some("mistral".to_string()),
        ..Config::default()
...
fn nvidia_nim_env_accepts_short_nim_base_url_alias() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-nim-base-url-alias-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn nvidia_nim_env_accepts_facade_base_url_forwarding() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-nim-forwarded-base-url-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn xiaomi_mimo_provider_uses_documented_defaults() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-xiaomi-mimo-defaults-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn xiaomi_mimo_custom_env_url_does_not_inherit_ambient_key() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-xiaomi-mimo-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn xiaomi_mimo_env_token_plan_mode_uses_token_plan_key_and_endpoint() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-xiaomi-mimo-token-plan-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn xiaomi_mimo_env_pay_as_you_go_mode_prefers_standard_key() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-xiaomi-mimo-payg-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
// Regression for issue #1714: `hakus --provider openai --model
...
fn openai_facade_custom_url_does_not_inherit_ambient_key() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-openai-forwarded-base-url-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn volcengine_provider_requires_api_key() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-volcengine-auth-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn siliconflow_provider_uses_canonical_defaults() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-siliconflow-defaults-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn ollama_cloud_resolves_saved_provider_key() -> Result<()> {
    ...
    hakus_secrets::Secrets::auto_detect().set("ollama", "ollama-cloud-saved-key")?;
    let config = Config {
        provider: Some("ollama".to_string()),
        providers: Some(ProvidersConfig {
            ollama: ProviderConfig {
                base_url: Some(hakus_config::provider::OLLAMA_CLOUD_BASE_URL.to_string()),
                ..ProviderConfig::default()
            },
            ..ProvidersConfig::default()
        }),
...
fn migrated_ollama_cloud_scope_preserves_legacy_table_and_slot_read_only() -> Result<()> {
    ...
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str());
    let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
    hakus_secrets::Secrets::auto_detect().set("ollama", "legacy-cloud-key")?;
    ...
    assert_eq!(
        ...
        hakus_config::provider::OLLAMA_CLOUD_BASE_URL
    );
    ...
    assert_eq!(
        hakus_secrets::Secrets::auto_detect().get("ollama-cloud")?,
        None,
...
fn ollama_cloud_without_key_fails_with_cloud_guidance() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-ollama-cloud-missing-key-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn ollama_custom_remote_does_not_inherit_cloud_env_key() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-ollama-custom-remote-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
    ...
    hakus_secrets::Secrets::auto_detect().set("ollama", "must-not-cross-routes-either")?;
...
fn deepseek_base_url_env_scopes_to_self_hosted_providers() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-self-hosted-base-url-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn siliconflow_custom_env_url_does_not_inherit_ambient_key() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-siliconflow-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
    ...
    // Safety: test-only environment mutation guarded by a global mutex.
    unsafe {
        env::set_var("HAKUS_PROVIDER", "siliconflow");
...
fn arcee_provider_uses_direct_defaults() -> Result<()> {
    ...
    unsafe {
        env::set_var("HAKUS_PROVIDER", "arcee");
...
fn siliconflow_cn_base_url_env_normalizes_model_aliases() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-siliconflow-cn-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
    ...
    // Safety: test-only environment mutation guarded by a global mutex.
    unsafe {
        env::set_var("HAKUS_PROVIDER", "siliconflow-CN");
        env::set_var("SILICONFLOW_API_KEY", "sf-env-key");
...
fn siliconflow_cn_reads_hyphenated_provider_table_from_config_file() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-siliconflow-cn-table-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn siliconflow_cn_preserves_reported_deepseek_prefixed_v4_route() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-siliconflow-cn-v4-report-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn siliconflow_cn_falls_back_to_shared_siliconflow_table_when_unset() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-siliconflow-cn-fallback-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn siliconflow_cn_env_overrides_write_cn_table_only() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-siliconflow-cn-env-table-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
"#,
    ...
    assert_eq!(
        ...
        credential_raw,
        "Hakus must never read, refresh, or rewrite Kimi CLI credentials"
    );
    Ok(())
}
...
fn codex_external_credentials_are_disabled_by_default_and_managed_fails_before_io() -> Result<()> {
    ...
    let mut managed_consent = hakus_config::ExternalCredentialConsentToml::read_only(
        hakus_config::ProviderKind::OpenaiCodex,
        hakus_config::ExternalCredentialSource::CodexCli,
        auth_path.clone(),
    );
    managed_consent.access = hakus_config::ExternalCredentialAccess::Managed;
    let managed = Config {
        provider: Some(ApiProvider::OpenaiCodex.as_str().to_string()),
        providers: Some(ProvidersConfig {
...
/// Env-var-only path: `HAKUS_BASE_URL=https://api.kimi.com/coding/v1`
/// combined with `HAKUS_PROVIDER=moonshot` must trigger Kimi Code
...
"#,
    ...
    unsafe {
        env::set_var("HAKUS_PROVIDER", "moonshot");
        env::set_var("HAKUS_BASE_URL", "https://api.kimi.com/coding/v1");
    }
...
/// `DEEPSEEK_PROVIDER` env var (still produced by the `hakus
/// --provider moonshot` dispatcher for compat); the test also has a
/// `HAKUS_PROVIDER` twin below for the public env path.
#[test]
fn moonshot_kimi_code_model_overrides_root_deepseek_default() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-kimi-code-root-model-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
/// Same regression as above, but driven by the public `HAKUS_PROVIDER`
/// env var. Documents the recommended user-facing setup path: never
/// `DEEPSEEK_PROVIDER=moonshot`, always `HAKUS_PROVIDER=moonshot`
/// (or `hakus --provider moonshot`, which also resolves through
/// this code path internally).
#[test]
fn moonshot_kimi_code_model_resolves_via_hakus_provider_env() -> Result<()> {
    let _lock = lock_test_env();
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
...
"#,
    ...
    // Safety: test-only env mutation guarded by lock_test_env().
    unsafe { env::set_var("HAKUS_PROVIDER", "moonshot") };
...
/// `HAKUS_PROVIDER` wins when both it and the legacy
/// `DEEPSEEK_PROVIDER` are set, so a user adding the new alias to their
/// shell isn't surprised by a stale legacy export.
#[test]
fn hakus_provider_env_takes_precedence_over_deepseek_provider() -> Result<()> {
    let _lock = lock_test_env();
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
...
fn xai_invalid_owned_generation_blocks_external_and_uses_api_key_fallback() -> Result<()> {
    ...
    fs::write(&external_path, external_raw)?;
    let _home = EnvVarGuard::set("HAKUS_HOME", &root);
...
fn save_api_key_for_openrouter_writes_provider_table() -> Result<()> {
    ...
    let config_path = temp_root.join(".deepseek").join("config.toml");
    let _config_path = EnvVarGuard::set("HAKUS_CONFIG_PATH", config_path.as_os_str());
    let _secret_backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "local");
    let resolved_config_path = hakus_config::resolve_config_path(None)?;
...
fn nvidia_nim_reads_facade_provider_table() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-nim-provider-table-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn nvidia_nim_provider_table_key_overrides_root_deepseek_key() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-nim-root-key-precedence-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn invalid_provider_auth_source_is_not_explicit_configuration() {
    let entry = ProviderConfig {
        auth: Some(hakus_config::ProviderAuthSourceToml {
            source: hakus_config::AuthSourceKind::Command,
            command: Vec::new(),
            timeout_ms: None,
            secret_id: None,
...
fn huggingface_provider_uses_direct_defaults() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-huggingface-defaults-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
    ...
    unsafe {
        env::set_var("HAKUS_PROVIDER", "huggingface");
...
fn huggingface_hf_token_env_api_key_resolves() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-huggingface-hf-token-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn huggingface_missing_key_error_mentions_env_fallbacks() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-huggingface-missing-key-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
fn huggingface_custom_env_urls_do_not_inherit_ambient_keys() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-huggingface-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
...
fn notifications_quiet_and_events_default_off_and_all_enabled() {
    ...
    assert!(notifications.events.turn_completed);
...
fn huggingface_short_custom_env_url_does_not_inherit_ambient_key() -> Result<()> {
    ...
    let nanos = SystemTime::now()
        ...
        .as_nanos();
    let temp_root = env::temp_dir().join(format!(
        "hakus-tui-huggingface-short-env-test-{}-{}",
        std::process::id(),
        nanos
    ));
    fs::create_dir_all(&temp_root)?;
...
"#,
    ...
    // Safety: test-only env mutation guarded by lock_test_env().
    unsafe {
        env::set_var("HAKUS_BASE_URL", "http://127.0.0.1:18185/v1");
        env::set_var("HAKUS_MODEL", "env-legacy-model");
...
/// v0.9.1 kimi-k3 dogfood report: a dogfood user ran `hakus --provider moonshot --model kimi-k3`
/// and the session kept reporting `kimi-k2.7-code`. The `--model` flag reaches
/// this binary as `HAKUS_MODEL`, so the route it produces is asserted here
...
fn cli_model_flag_selects_kimi_k3_on_the_moonshot_platform_route() -> Result<()> {
    ...
    unsafe {
        env::set_var("HAKUS_PROVIDER", "moonshot");
        env::set_var("HAKUS_MODEL", MOONSHOT_KIMI_K3_MODEL);
    }
...
fn readiness_and_inventory_classify_the_resolved_route_not_the_session_host() -> Result<()> {
    ...
    let config_path = temp_root.path().join("config.toml");
    fs::write(&config_path, "provider = \"deepseek\"\n")?;
    let _base = EnvVarGuard::set("HAKUS_BASE_URL", "http://127.0.0.1:11434/v1");
...
fn configured_inactive_provider_reads_its_secret_store_key() -> Result<()> {
    let _lock = lock_test_env();
    let temp = tempfile::tempdir()?;
    let _home = EnvVarGuard::set("HAKUS_HOME", temp.path());
    let _backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
    ...
    hakus_secrets::Secrets::auto_detect().set("moonshot", "kimi-test-credential")?;
    assert!(
        has_api_key_for(&config, ApiProvider::Moonshot),
