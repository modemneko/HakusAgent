...
//! only on `std`, `hakus-paths`, and `shellexpand` plus one another, so they
...
fn try_default_config_path_from_environment() -> anyhow::Result<PathBuf> {
    hakus_config::resolve_config_path(None)
}
...
/// trusting it routed them at a populated `~/.hakus/config.toml` (#5355,
...
pub(crate) fn hakus_home_dir() -> Result<Option<PathBuf>, hakus_paths::PathOverrideError> {
...
fn home_config_path_from_environment() -> Option<PathBuf> {
    match hakus_home_dir() {
        Ok(Some(home)) => return Some(home.join(hakus_config::CONFIG_FILE_NAME)),
        Ok(None) => {}
        Err(error) => {
            tracing::error!(
                error = %error,
                "invalid Hakus home override; refusing to substitute a different config path"
            );
            return None;
        }
    }
...
pub(crate) fn env_config_path() -> Result<Option<PathBuf>, hakus_paths::PathOverrideError> {
    #[cfg(test)]
    {
...
fn env_config_path_unlocked() -> Result<Option<PathBuf>, hakus_paths::PathOverrideError> {
    hakus_paths::config_path_override()
}
...
fn default_user_state_path_from_environment(name: &str) -> Option<PathBuf> {
    match hakus_home_dir() {
        ...
        Err(error) => {
            tracing::error!(
                error = %error,
                "invalid Hakus home override; refusing to substitute a different state root"
            );
            return None;
        }
    }
