...
/// Receipt for the committed Hakus-owned xAI OAuth generation.
#[derive(Debug)]
    ...
    for key in ["GROK_AUTH_PATH", "XAI_AUTH_PATH"] {
        if let Ok(path) = std::env::var(key) {
            ...
            if !p.as_os_str().is_empty() {
                return hakus_config::resolve_external_credential_path(&p).unwrap_or(p);
            }
        }
    }
...
/// Hakus-owned xAI token file. Native login and refresh never target the
/// Grok CLI's file.
pub fn hakus_auth_file_path() -> Result<PathBuf> {
    hakus_config::legacy_xai_oauth_path()
}
...
fn configured_owned_auth_file_path(config: &Config) -> Result<Option<PathBuf>> {
    ...
    match generation {
        Some(generation) => hakus_config::xai_oauth_generation_path(generation).map(Some),
        None => Ok(None),
    }
}

/// Detect the [#5032] bricked-launch state: `[providers.xai]` selects OAuth and
/// points `oauth_credential_generation` at a Hakus-owned credential file
...
pub fn credentials_valid(config: &Config) -> bool {
    // Hakus-owned OAuth bytes are inert until the xAI provider explicitly
    ...
    if let Ok(path) = hakus_auth_file_path()
        && let Ok(Some(mut file)) = load_owned_auth_file(&path)
        && let Some((_, entry)) = select_entry(&mut file)
        && (entry_access_token_is_fresh(&entry)
            || entry
...
/// Load xAI OAuth credentials. Hakus-owned credentials may refresh and
/// rewrite Hakus-owned storage. External credentials are read-only.
...
pub fn get_credentials(config: &Config) -> Result<XaiOAuthCredentials> {
    anyhow::ensure!(
        config.api_provider() == ApiProvider::Xai
            && config
                ...
                .and_then(|entry| entry.auth_mode.as_deref())
                .is_some_and(auth_mode_uses_xai_oauth),
        "Hakus-owned xAI OAuth credentials are inactive until the xAI route explicitly selects OAuth"
    );
    if let Some(owned_path) = configured_owned_auth_file_path(config)? {
        return get_owned_credentials(&owned_path);
    }
    let owned_path = hakus_auth_file_path()?;
    ...
    let (scope, entry) = select_entry(&mut file).ok_or_else(|| {
        anyhow::anyhow!(
            "xAI OAuth credentials at {} have no usable entry. Run `grok login` again or use `hakus auth xai-device` for Hakus-owned storage.",
            hakus_config::quote_os_path(grant.path())
        )
    })?;
    if !entry_access_token_is_fresh(&entry) {
        bail!(
            "xAI OAuth access token in {} is expired. Read-only consent never refreshes or rewrites another CLI's credentials. Run `grok login` again or use `hakus auth xai-device`.",
            hakus_config::quote_os_path(grant.path())
        );
    }
    let token = entry
...
fn get_owned_credentials(path: &Path) -> Result<XaiOAuthCredentials> {
    let directory = hakus_config::xai_oauth_credentials_dir()?;
    anyhow::ensure!(
        path.parent() == Some(directory.as_path()),
        "Hakus-owned xAI OAuth path escaped the credentials directory"
    );
    let name = path
        .file_name()
        .and_then(|name| name.to_str())
        .context("Hakus-owned xAI OAuth path must have a UTF-8 basename")?;
    anyhow::ensure!(
        name == hakus_config::LEGACY_XAI_OAUTH_FILE_NAME
            || hakus_config::is_valid_xai_oauth_generation(name),
        "Hakus-owned xAI OAuth path has an invalid basename"
    );
...
fn get_owned_credentials_locked<F>(
    store: &hakus_config::XaiOAuthCredentialStore,
    name: &str,
    refresh_access: F,
    ...
    F: FnOnce(&str, &str, &str) -> Result<TokenResponse>,
{
    ...
    let mut file = load_owned_auth_file_from_store(store, name)?.ok_or_else(|| {
        anyhow::anyhow!(
            "Hakus-owned xAI OAuth credentials were not found at {}. Run `hakus auth xai-device` again.",
            hakus_config::quote_os_path(&path)
        )
    })?;
    let (scope, mut entry) = select_entry(&mut file).ok_or_else(|| {
        anyhow::anyhow!(
            "Hakus-owned xAI OAuth credentials at {} have no usable entry. Run `hakus auth xai-device` again.",
            hakus_config::quote_os_path(&path)
        )
    })?;
    ...
    let refresh = entry
        ...
        .context(
            "xAI OAuth access token expired and no refresh_token is stored. \
             Run `grok login` or `hakus auth xai-device` again.",
        )?;
    let issuer = entry
        .oidc_issuer
        .clone()
...
///
/// Public residual entry point for CLI/TUI wiring (`hakus auth` /
...
pub async fn device_code_login() -> Result<PendingXaiDeviceLogin> {
    ...
    let open_browser = std::env::var_os("HAKUS_XAI_OAUTH_NO_BROWSER").is_none();
...
pub fn activate_device_login(
    ...
    hakus_config::with_xai_oauth_lifecycle_lock(move |store| {
...
fn activate_device_login_locked(
    ...
    let generation = format!(
        "{}{}{}",
        hakus_config::XAI_OAUTH_GENERATION_PREFIX,
        uuid::Uuid::new_v4().simple(),
        hakus_config::XAI_OAUTH_GENERATION_SUFFIX
    );
    hakus_config::validate_xai_oauth_generation(&generation)?;
    ...
    let activation = hakus_config::mutate_config_document(&config_path, |document| {
        ...
        if let Some(previous) = previous_generation.as_deref() {
            hakus_config::validate_xai_oauth_generation(previous).with_context(|| {
        ...
        let previous_owned_name = match previous_generation.as_deref() {
            ...
            None if store
                .read_to_string(hakus_config::LEGACY_XAI_OAUTH_FILE_NAME)?
                .is_some() =>
            {
                Some(hakus_config::LEGACY_XAI_OAUTH_FILE_NAME.to_string())
            }
            None => None,
        };
    ...
    if let Some(config) = live_config {
        config.mark_hakus_owned_xai_oauth(generation.clone());
    }
    ...
    eprintln!(
        "Signed in. Hakus-owned credentials activated at {}.",
        hakus_config::quote_os_path(&auth_path)
    );
...
pub fn clear_dangling_xai_oauth_generation(config_path: Option<&Path>) -> Result<()> {
    ...
    hakus_config::mutate_config_document(&config_path, |document| {
        hakus_config::unset_config_document_value(
            document,
            &["providers", key_inside, "oauth_credential_generation"],
        )?;
...
pub fn missing_auth_message() -> String {
    format!(
        "xAI OAuth credentials not found.\n\
         Options:\n\
         1. Run `hakus auth xai-device` for Hakus-owned OAuth storage\n\
         2. To read an existing Grok CLI login without changing it, run \
         `hakus auth external-consent --provider xai --mode read-only --path {}`\n\
         3. Or use API-key auth: export XAI_API_KEY=... / \
         hakus auth set --provider xai",
        hakus_config::quote_os_path(&auth_file_path())
    )
}
...
fn load_owned_auth_file(path: &Path) -> Result<Option<AuthFile>> {
    let Some(raw) = crate::external_credentials::read_hakus_owned_to_string(path)? else {
        return Ok(None);
    };
...
fn load_external_auth_file(
    grant: &hakus_config::ExternalCredentialReadGrant,
) -> Result<AuthFile> {
    ...
    store
        ...
        .with_context(|| {
            format!(
                ...
                hakus_config::quote_os_path(&store.directory().join(name))
            )
        })?;
    #[cfg(test)]
...
mod tests {
    ...
    fn loads_fresh_token_from_grok_auth_json() {
        ...
        let _home_guard = crate::test_support::EnvVarGuard::set("HAKUS_HOME", &root);
    ...
    fn disabled_external_grok_credentials_cause_zero_external_io() {
        ...
        let owned_home = root.join("hakus-owned");
        let _home_guard = crate::test_support::EnvVarGuard::set("HAKUS_HOME", &owned_home);
        let _path_guard = crate::test_support::EnvVarGuard::set("GROK_AUTH_PATH", &path);
        let config = Config {
            provider: Some(ApiProvider::Xai.as_str().to_string()),
            providers: Some(crate::config::ProvidersConfig {
    ...
    fn expired_external_grok_grant_reads_as_missing_key_despite_refresh_token() {
        ...
        .unwrap();
        let _home_guard =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", root.join("hakus-owned"));
        let _path_guard = crate::test_support::EnvVarGuard::set("GROK_AUTH_PATH", &path);
    ...
    #[test]
    fn native_login_storage_is_hakus_owned() {
        let _guard = crate::test_support::lock_test_env();
        let dir = TempDir::new().unwrap();
        let grok_path = dir.path().join("external-grok-auth.json");
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", dir.path());
        let _grok = crate::test_support::EnvVarGuard::set("GROK_AUTH_PATH", &grok_path);

        let owned = hakus_auth_file_path().expect("Hakus-owned auth path");
    ...
    fn seed_legacy_owned_credentials() -> PathBuf {
        hakus_config::with_xai_oauth_lifecycle_lock(|store| {
            ...
            store.path_for(hakus_config::LEGACY_XAI_OAUTH_FILE_NAME)
        })
    ...
    fn concurrent_refreshes_share_one_rotated_epoch() {
        ...
        let home = dir
            ...
            .join("owned-home");
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", &home);
...
"#,
        ...
        .unwrap();
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", &home);
        let consent = hakus_config::ExternalCredentialConsentToml::read_only(
            hakus_config::ProviderKind::Xai,
            hakus_config::ExternalCredentialSource::GrokCli,
            external_path.clone(),
        );
        ...
        assert!(hakus_config::is_valid_xai_oauth_generation(generation));
    ...
    fn missing_file_message_mentions_oauth_paths() {
        ...
        assert!(msg.contains("Hakus-owned OAuth storage"), "{msg}");
