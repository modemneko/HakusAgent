...
const FAILURE_EVENT_PREFIX: &str = concat!(
    "<hakus:runtime_event kind=\"subagent_failed\" priority=\"high\" visibility=\"internal\">\n",
...
const WAITING_EVENT_PREFIX: &str = concat!(
    "<hakus:runtime_event kind=\"waiting_for_subagents\" visibility=\"internal\">\n",
...
const WAITING_EVENT_SUFFIX: &str = concat!(
    ...
    "waiting strategy. The runtime will deliver <hakus:subagent.done> sentinels ",
...
const CHILD_COMPLETION_EVENT_SUFFIX: &str = "</hakus:runtime_event>";
const CHILD_COMPLETION_SECTION: &str = "\n--- child sub-agent completion ---\n";
const SHELL_COMPLETION_EVENT_PREFIX: &str = concat!(
    "<hakus:runtime_event kind=\"background_shell_completion\" visibility=\"internal\">\n",
...
const RESTORED_COMPLETION_HEADER: &str = "[Hakus restored sub-agent checkpoint]";
const RESTORED_COMPLETIONS_HEADER: &str = "[Hakus restored sub-agent checkpoints]";
const RESTORED_RUNNING_HEADER: &str = "[Hakus restored sub-agent runtime checkpoint]";
const RESTORED_TOPOLOGY_HEADER: &str = "[Hakus restored Agent topology checkpoint]";

const AGENT_TOPOLOGY_EVENT_PREFIX: &str =
    "<hakus:runtime_state kind=\"agent_topology\" schema=\"v1\" visibility=\"internal\">\n";
...
const DONE_SENTINEL_START: &str = "<hakus:subagent.done>";
const DONE_SENTINEL_END: &str = "</hakus:subagent.done>";
...
fn agent_topology_checkpoint_message(snapshots: &[SubAgentResult]) -> Message {
    ...
    let payload = AgentTopologyCheckpoint {
        schema: "hakus.agent_topology.v1",
    ...
    let json = serde_json::to_string(&payload).unwrap_or_else(|_| {
        "{\"schema\":\"hakus.agent_topology.v1\",\"authority\":\"runtime_unavailable\"}"
            .to_string()
    });
    Message {
...
fn parse_agent_topology_checkpoint(message: &Message) -> Option<SavedAgentTopologyCheckpoint> {
    ...
    if checkpoint.schema != "hakus.agent_topology.v1" {
        return None;
    }
...
mod tests {
    ...
    fn completion_payload(agent_id: &str, status: &str, summary: &str) -> String {
        format!(
            "{summary}\n<hakus:subagent.done>{{\"agent_id\":\"{agent_id}\",\"name\":\"Tide\",\"agent_type\":\"implementer\",\"status\":\"{status}\",\"summary_location\":\"previous_line\"}}</hakus:subagent.done>"
        )
    }
    ...
    fn restore_projection_replaces_completion_control_plane_and_is_idempotent() {
        ...
        assert!(!display.contains("<hakus:runtime_event"));
        assert!(!display.contains("<hakus:subagent.done>"));
    ...
    fn restore_projection_accepts_failed_error_location_sentinel() {
        let raw = subagent_completion_runtime_message(concat!(
            ...
            "<hakus:subagent.done>{\"agent_id\":\"agent_failed\",",
    ...
    fn failed_completion_uses_high_priority_runtime_event_and_restores_safely() {
        let payload = concat!(
            ...
            "\"error_location\":\"previous_line\"}</hakus:subagent.done>",
        );
    ...
    fn restore_projection_fails_safe_for_malformed_owned_completion() {
        let raw = runtime_handoff_message(subagent_completion_runtime_text(
            "Partial child result\n<hakus:subagent.done>{not-json}</hakus:subagent.done>",
        ));
    ...
    fn restore_projection_sanitizes_nested_child_completion_envelope() {
        let nested = concat!(
            ...
            "<hakus:subagent.done>{\"agent_id\":\"agent_nested\",",
