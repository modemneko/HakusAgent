...
mod tests {
    ...
    // #3240: a fresh install must resolve the history file under `.hakus`,
    // never the legacy `.deepseek` dir, so normal use doesn't recreate it.
    #[test]
    fn fresh_install_uses_hakus_not_legacy() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let path = history_path_with_home(Some(tmp.path().to_path_buf()))
            .expect("path resolves with a home dir");
        assert_eq!(path, tmp.path().join(".hakus").join(HISTORY_FILE_NAME));
        assert!(
    ...
    // Once a `.hakus` history exists it wins over any legacy file.
    #[test]
    fn hakus_history_preferred_over_legacy() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let primary = tmp.path().join(".hakus").join(HISTORY_FILE_NAME);
        let legacy = tmp.path().join(".deepseek").join(HISTORY_FILE_NAME);
