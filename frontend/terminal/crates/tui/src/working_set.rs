...
impl WorkingSet {
    ...
    /// `HAKUS_CACHE_MAXIMAL` env toggle (`1`/`true`/`on`/`yes`). The env
    ...
    fn cache_maximal_enabled(&self) -> bool {
        ...
        match std::env::var("HAKUS_CACHE_MAXIMAL") {
            Ok(v) => matches!(
...
mod tests {
    ...
    // don't touch the process-wide `HAKUS_CACHE_MAXIMAL` env var (which
