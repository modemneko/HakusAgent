...
pub(crate) const APPROX_BYTES_PER_TOKEN: usize = hakus_config::APPROX_BYTES_PER_TOKEN;
...
pub enum SegmentProvenance {
    ...
    /// A file under `$HAKUS_HOME` that replaced a bundled constant, plus the
...
fn classify(index: usize, text: &str, sources: &PreviewSources<'_>) -> SegmentProvenance {
    ...
    if text.contains("<hakus_user_constitution") {
...
mod tests {
        ...
        let overridden = preview(
            ...
            &PreviewSources {
                base_prompt: Some(BasePromptSource::ConfigOverride {
                    path: "/home/u/.hakus/prompts/constitution.md".to_string(),
                }),
    ...
    fn constitution_block_provenance_never_invents_a_path() {
        let prompt = blocks(&[
            "# Constitution\n\n<hakus_user_constitution source=\"user-global\">\nx\n</hakus_user_constitution>",
        ]);
        ...
        let known = preview(
            &prompt,
            &PreviewSources {
                user_constitution_path: Some(Path::new("/home/u/.hakus/constitution.json")),
                ..PreviewSources::default()
            },
        );
        assert!(
    ...
    fn home_paths_are_masked_in_display_only() {
        let raw = "constitution at /Users/someone/.hakus/constitution.json";
        let prompt = blocks(&[raw]);
        let preview = preview(
            &prompt,
            &PreviewSources {
        ...
        assert!(preview.segments[0].text.contains("~/.hakus"));
