...
fn update_renderer_canonicalizes_safe_release_tags() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "Update available. Run `hakus update` to install.".to_string(),
        ]
    );
}
...
fn cli_defaults_doctor_offline_and_keeps_json_incompatible_with_live_flags() {
    let cli =
        crate::Cli::try_parse_from(["hakus-tui", "doctor"]).expect("parse default doctor");
    ...
    let cli = crate::Cli::try_parse_from(["hakus-tui", "doctor", "--probe-search"])
    ...
    for output_flag in ["--json", "--context-json"] {
        for live_flag in [
            ...
            assert!(
                crate::Cli::try_parse_from(["hakus-tui", "doctor", output_flag, live_flag,])
                    .is_err(),
...
#[test]
fn explicit_hakus_home_owns_every_default_user_path() {
    let _lock = crate::test_support::lock_test_env();
    let temp = tempfile::tempdir().expect("temp home");
    let home = temp.path().join("isolated-hakus-home");
    ...
    let _automations = crate::test_support::EnvVarGuard::remove("HAKUS_AUTOMATIONS_DIR");
    let _legacy_automations = crate::test_support::EnvVarGuard::remove("DEEPSEEK_AUTOMATIONS_DIR");
    let _tasks = crate::test_support::EnvVarGuard::remove("HAKUS_TASKS_DIR");
    let _legacy_tasks = crate::test_support::EnvVarGuard::remove("DEEPSEEK_TASKS_DIR");
    let _runtime = crate::test_support::EnvVarGuard::remove("HAKUS_RUNTIME_DIR");
...
fn explicit_relative_config_matches_the_canonical_loader_path() {
    ...
    let expected = hakus_config::resolve_config_path(Some(relative.to_path_buf()))
...
fn human_and_json_backend_reports_never_include_secret_file_contents() {
    ...
    let diagnostic = hakus_secrets::diagnose_secret_backend();
