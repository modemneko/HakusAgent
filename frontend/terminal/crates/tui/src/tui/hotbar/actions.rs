...
impl Default for HotbarRecommendationOptions {
    fn default() -> Self {
        Self {
            max_total: usize::from(hakus_config::HOTBAR_SLOT_COUNT),
            max_eligible_per_category: usize::MAX,
            include_required_args: false,
        }
    }
...
pub fn recommended_hotbar_bindings(
    app: &App,
    options: HotbarRecommendationOptions,
) -> Vec<hakus_config::HotbarBindingToml> {
    recommend_hotbar_actions(app, options)
        .into_iter()
        .take(usize::from(hakus_config::HOTBAR_SLOT_COUNT))
        .enumerate()
        .map(|(idx, entry)| hakus_config::HotbarBindingToml {
...
fn default_hotbar_position(action_id: &str) -> Option<usize> {
    hakus_config::DEFAULT_HOTBAR_ACTIONS
        .iter()
...
impl AppHotbarAction {
    ...
    fn recommendation(&self) -> HotbarRecommendation {
        if hakus_config::DEFAULT_HOTBAR_ACTIONS.contains(&self.id) {
...
impl HotbarAction for SlashHotbarAction {
    ...
    fn metadata(&self, locale: Locale) -> HotbarActionMetadata {
        let recommendation = if hakus_config::DEFAULT_HOTBAR_ACTIONS.contains(&self.id.as_str())
        {
            HotbarRecommendation::Default
        } else {
...
mod tests {
    ...
    fn default_hotbar_actions_have_registered_default_metadata() {
        ...
        for id in hakus_config::DEFAULT_HOTBAR_ACTIONS {
            let action = registry
                .get(id)
    ...
    fn hotbar_recommendations_limit_eligible_actions_by_category() {
        ...
        for default_id in hakus_config::DEFAULT_HOTBAR_ACTIONS {
            assert!(
                recommendations
                    .iter()
    ...
    fn control_plane_commands_are_bound_and_bare_dispatch_is_read_only() {
        use hakus_lane::control::OPERATIONS;
        use hakus_lane::{ControlAuthority, ControlSurface, TargetKind};
    ...
    fn reasoning_cycle_is_inert_while_a_turn_is_running() {
        ...
        let _hakus_home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", tmp.path().join(".hakus"));
        let _deepseek_config = crate::test_support::EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
        let _hakus_config = crate::test_support::EnvVarGuard::remove("HAKUS_CONFIG_PATH");
