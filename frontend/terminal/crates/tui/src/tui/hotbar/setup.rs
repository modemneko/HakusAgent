...
pub struct HotbarSetupView {
    ...
    selected_slot: u8,
    original_bindings: BTreeMap<u8, hakus_config::HotbarBindingToml>,
    draft_bindings: BTreeMap<u8, hakus_config::HotbarBindingToml>,
...
impl HotbarSetupView {
    ...
    #[must_use]
    #[cfg(test)]
    pub fn binding_for_slot(&self, slot: u8) -> Option<&hakus_config::HotbarBindingToml> {
    ...
    pub fn select_slot(&mut self, slot: u8) -> bool {
        if !(1..=hakus_config::HOTBAR_SLOT_COUNT).contains(&slot) {
            self.validation_errors = vec![tr_hotbar_setup(
                self.locale,
                MessageId::HotbarSetupSlotOutOfRange,
                &[
                    ("{slot}", slot.to_string()),
                    ("{max}", hakus_config::HOTBAR_SLOT_COUNT.to_string()),
                ],
            )];
            return false;
        }
    ...
    #[must_use]
    pub fn save_bindings(&self) -> Vec<hakus_config::HotbarBindingToml> {
    ...
    fn move_slot(&mut self, delta: isize) {
        let len = usize::from(hakus_config::HOTBAR_SLOT_COUNT);
    ...
    fn slots_line(&self) -> Line<'static> {
        let slots = (1..=hakus_config::HOTBAR_SLOT_COUNT)
            .map(|slot| {
                let label = self
                    .draft_bindings
                    .get(&slot)
