...
use hakus_config::catalog::{CatalogOffering, CatalogSnapshot};
use hakus_config::provider::{CredentialAcquisition, WireFormat};
use hakus_config::route::{PricingSku, RequestProtocol};
use hakus_config::{
...
enum Stage {
    ...
    /// Hakus-owned device OAuth flow; neither path may impersonate the other.
    XaiAuthChoice,
    KeyEntry,
...
///
/// Only the two endpoints Hakus can classify are offered. A hand-edited
...
pub struct ProviderDashboardRow {
    ...
    external_credential_status: Option<hakus_config::ExternalCredentialConsentStatus>,
    pub is_active: bool,
    has_key: bool,
...
impl ProviderDashboardRow {
    ...
    fn from_config_with_provider_id(
        ...
        let compatibility_kind = (provider == ApiProvider::DeepseekCN)
            .then_some(hakus_config::ProviderKind::Deepseek);
...
fn bundled_reasoning_catalog() -> &'static CatalogSnapshot {
    ...
    CATALOG.get_or_init(|| CatalogSnapshot {
        ...
        offerings: hakus_config::catalog::bundled_catalog_offerings(),
    })
}
...
pub(crate) fn external_consent_target_for_provider(
    ...
    hakus_config::ProviderKind,
    hakus_config::ExternalCredentialSource,
    std::path::PathBuf,
)> {
    ...
    let path = hakus_config::resolve_external_credential_path(path).ok()?;
...
fn codex_external_file_is_fresh(path: &std::path::Path) -> bool {
    ...
    // never refreshed by Hakus, so an opaque token must be treated as stale.
    false
}
...
impl ProviderPickerView {
    ...
    fn render_provider_detail(&self, area: Rect, buf: &mut Buffer, row: &ProviderDashboardRow) {
        ...
        if let Some(status) = row.external_credential_status.as_ref() {
            ...
            let owner_path = self
                ...
                .replace("{path}", &hakus_config::quote_os_path(&status.path));
            ...
            let semantics = match status.access {
                ...
                hakus_config::ExternalCredentialAccess::Managed => {
    ...
    fn render_key_entry(&self, area: Rect, buf: &mut Buffer) {
        ...
        let mut hint_lines = if codex_oauth {
            vec![
                ...
                Line::from(Span::styled(
                    "CLI: hakus auth external-consent --provider openai-codex; no token is stored here.",
                    Style::default().fg(palette::TEXT_MUTED),
                )),
            ]
        } else if saved_credential && self.api_key_input.trim().is_empty() {
    ...
    fn render_external_consent_confirm(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph::new(vec![
            ...
            Line::from(format!(
                "{revoke_label}: hakus auth external-revoke --provider {}",
                provider.as_str()
            )),
        ])
        .wrap(Wrap { trim: false })
...
mod tests {
    ...
    fn protected_self_hosted_row_requires_its_configured_auth_mode() {
        let _env_lock = crate::test_support::lock_test_env();
        let temp = tempfile::tempdir().expect("isolated credential home");
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", temp.path());
    ...
    fn sensenova_preset_fills_published_openai_host() {
        ...
        assert_eq!(
            ...
            hakus_config::SENSENOVA_BASE_URL
        );
        assert_eq!(
            picker.custom_provider_model,
            hakus_config::SENSENOVA_DEFAULT_MODEL
        );
        assert_eq!(
            picker.custom_provider_api_key_env,
            hakus_config::SENSENOVA_API_KEY_ENV
        );
        let rendered = render_text(&picker, 100, 20);
    ...
    fn template_list_mouse_selects_row_and_second_click_activates() {
        ...
        assert_eq!(
            ...
            hakus_config::SENSENOVA_TEMPLATE_ID
        );
    ...
    fn modelstudio_family_key_marks_all_variants_configured() {
        ...
        let _home = crate::test_support::EnvVarGuard::set("HOME", tmp.path());
        let _userprofile = crate::test_support::EnvVarGuard::set("USERPROFILE", tmp.path());
        let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", tmp.path());
        let _backend = crate::test_support::EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
    ...
    fn onboarding_catalog_honors_typed_credentials_for_every_builtin_provider() {
        use hakus_config::provider::CredentialAcquisition;
        ...
        let _hakus_home =
            EnvVarGuard::set("HAKUS_HOME", home.path().to_string_lossy().as_ref());
        let _codex_home = EnvVarGuard::set("CODEX_HOME", home.path().to_string_lossy().as_ref());
        let _secret_backend = EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
    ...
    fn openai_codex_key_entry_is_oauth_only() {
        ...
        assert!(matches!(
            ...
            ViewAction::EmitAndClose(ViewEvent::ProviderPickerExternalConsentConfirmed {
                ...
                consent_provider: hakus_config::ProviderKind::OpenaiCodex,
                source: hakus_config::ExternalCredentialSource::CodexCli,
                ..
            })
        ));
    }
    ...
    fn xai_auth_choice_keeps_api_key_device_oauth_and_external_reuse_distinct() {
        ...
        assert!(rendered.contains("Hakus-owned storage"));
        picker.handle_key(key(KeyCode::Char('2')));
        assert!(matches!(
            picker.handle_key(key(KeyCode::Enter)),
            ViewAction::EmitAndClose(ViewEvent::ProviderPickerXaiOAuthRequested)
    ...
    fn inactive_external_consents_are_visible_without_io_and_never_enter_routing_inventory() {
        ...
        let owned_home = temp.path().join("hakus-owned");

        let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", &owned_home);
        let _codex_path =
        ...
        for provider in [ApiProvider::OpenaiCodex, ApiProvider::Xai] {
            ...
            assert!(
                visible.contains("revoke: hakus auth external-revoke"),
                "{visible}"
            );
            assert!(
