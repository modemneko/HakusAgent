...
fn parse_inline_spans(line: &str, base_style: Style, link_style: Style) -> Vec<InlineToken> {
    ...
    while !rest.is_empty() {
            ...
            && let Some(end) = rest[1..].find('*')
        {
            ...
            // identifier like `hakus_tui`, not italic markup).
...
mod tests {
    ...
    fn underscores_inside_identifiers_render_as_literal_text() {
        ...
        // markdown parser ate the underscore in `hakus_tui` because
        ...
        let cases = [
            "crate hakus_tui handles approvals",
    ...
    fn table_pipes_inside_inline_code_stay_in_the_cell() {
        let src = "| Check | Result |\n\
                   |---|---|\n\
                   | `strings ~/.cargo/bin/hakus-tui | grep -c \"legacy marker\"` | 0 matches |\n";
        let parsed = parse(src);
        ...
        assert_eq!(
            rows[1],
            &vec![
                "`strings ~/.cargo/bin/hakus-tui | grep -c \"legacy marker\"`".to_string(),
                "0 matches".to_string(),
            ]
        );
        ...
        let data_line = rendered_lines
            .iter()
            .find(|line| line.contains("strings ~/.cargo/bin/hakus-tui"))
