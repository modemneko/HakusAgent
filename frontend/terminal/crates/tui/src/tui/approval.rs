...
/// hakus-execpolicy (next to `AskForApproval`); re-exported here so
/// `crate::tui::approval::ApprovalMode` keeps working.
pub use hakus_execpolicy::ApprovalMode;
...
impl ApprovalRequest {
    ...
    /// `.hakus/constitution.json` ask rule forces review.
    #[must_use]
    ...
    pub fn ask_rule_preview(&self) -> Option<String> {
        ...
        let permissions = hakus_config::PermissionsToml {
...
fn description_is_repo_law_prompt(description: &str) -> bool {
    description.starts_with("Repo law holds this write:")
        && description.contains(".hakus/constitution.json")
}
