...
pub(crate) fn recover_engine_event_disconnect(app: &mut App) -> bool {
    ...
    app.add_message(HistoryCell::Error {
        message: "Engine stopped before completing the turn. Check ~/.hakus/crashes and retry."
            .to_string(),
...
pub(crate) fn launch_worktree_spec(
    ...
    requested: &str,
) -> Result<hakus_lane::WorktreeProvision> {
...
pub(crate) async fn provision_launch_worktree(
    ...
    let provisioned =
        tokio::task::spawn_blocking(move || hakus_lane::provision_worktree(&spec))
            .await
...
pub(crate) fn persist_rules_from_approval(
    app: &mut App,
    config: &mut Config,
    rules: &[hakus_config::ToolAskRule],
) {
    let action = rules.first().map(|rule| rule.action);
    match hakus_config::ConfigStore::load(app.config_path.clone()).and_then(|mut store| {
        let added = match action {
            Some(hakus_execpolicy::PermissionAction::Ask) => store.append_ask_rules(rules)?,
            Some(hakus_execpolicy::PermissionAction::Allow) => {
                store.append_allow_rules(rules)?
            }
            Some(hakus_execpolicy::PermissionAction::Deny) => {
        ...
        Ok((added, path)) if added > 0 => {
            let action = match action {
                Some(hakus_execpolicy::PermissionAction::Allow) => "allow",
                _ => "ask",
            };
            app.status_message = Some(format!(
...
pub(crate) fn restore_loaded_session_provider(
    ...
    app.provider_chain = provider
        .kind()
        .map(|kind| hakus_config::ProviderChain::new(kind, &config.fallback_providers))
