...
pub(crate) fn startup_version_check_source(config: &UpdateConfig) -> StartupVersionCheckSource {
    resolve_version_check_source(config, hakus_release::suppression_reason())
}
...
pub(crate) fn resolve_version_check_source(
    config: &UpdateConfig,
    suppression: Option<hakus_release::SuppressionReason>,
...
pub(crate) fn update_check_cache_path() -> Option<PathBuf> {
    hakus_config::hakus_home()
        .ok()
        .map(|home| hakus_release::check::cache_path_in(&home))
}
...
pub(crate) async fn cached_version_hint(
    ...
    let now = hakus_release::check::now_unix();
    if let Some(path) = cache_path
        && let Some(entry) = hakus_release::UpdateCheckCache::load(path)
    ...
    if let Some(path) = cache_path
        && latest_tag.is_some()
        && let Err(err) = hakus_release::UpdateCheckCache::now(latest_tag.clone()).store(path)
    {
...
pub(crate) async fn latest_tag_from_startup_source(
    ...
    match source {
        ...
        StartupVersionCheckSource::ReleaseResolver => {
            ...
            let body = hakus_release::fetch_release_json_async(
                hakus_release::LATEST_RELEASE_URL,
                "latest release",
            )
            .await
            .ok()?;
...
pub(crate) fn release_mirror_env_configured() -> bool {
    let version = hakus_release::update_version_from_env()
        .unwrap_or_else(|| env!("CARGO_PKG_VERSION").to_string());
    hakus_release::release_base_url_from_env(&version).is_some()
}

pub(crate) async fn latest_tag_from_configured_update_uri(
    update_uri: &str,
) -> Result<Option<String>> {
    let body = hakus_release::fetch_release_json_async(update_uri, "configured latest release")
        .await?;
