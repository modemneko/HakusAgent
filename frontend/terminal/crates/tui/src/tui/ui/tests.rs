...
fn canonical_git_run_and_web_actions_use_semantic_history_cells() {
    ...
    handle_tool_call_started(
        ...
        "Web-search",
        "Web",
        &serde_json::json!({"action": "search", "query": "Hakus"}),
    );
    let active = app.active_cell.as_ref().expect("active cell");
    assert!(matches!(
        active.entries()[0],
...
/// 30-second sticky warning blaming "another Hakus process" that does
    ...
    apply_coordination_detail_projection(
        ...
        CoordinationDetailProjection {
            process_lock_note: Some(
                "another Hakus process (pid 4242) owns delegated coordination for /ws: busy"
                    .to_string(),
            ),
            ..base
        },
...
impl ConfigPathEnvGuard {
    fn new() -> Self {
        ...
        let hakus_config_path =
            crate::test_support::EnvVarGuard::set("HAKUS_CONFIG_PATH", &config_path);
        let deepseek_config_path =
            crate::test_support::EnvVarGuard::set("DEEPSEEK_CONFIG_PATH", &config_path);
        Self {
            _hakus_config_path: hakus_config_path,
...
struct SettingsHomeGuard {
    ...
    _hakus_home: crate::test_support::EnvVarGuard,
    _hakus_config_path: crate::test_support::EnvVarGuard,
    _deepseek_config_path: crate::test_support::EnvVarGuard,
    _hakus_provider: crate::test_support::EnvVarGuard,
...
impl SettingsHomeGuard {
    fn new() -> Self {
        ...
        Self {
            ...
            _hakus_home: crate::test_support::EnvVarGuard::set(
                "HAKUS_HOME",
                &hakus_home,
            ),
            _hakus_config_path: crate::test_support::EnvVarGuard::remove(
                "HAKUS_CONFIG_PATH",
            ),
            ...
            _hakus_provider: crate::test_support::EnvVarGuard::remove("HAKUS_PROVIDER"),
...
fn terminal_origin_reset_resets_scroll_region_origin_without_destructive_clear() {
    ...
    // every TurnCompleted. The cleared back-buffer plus a single ratatui clear
...
fn credential_paste_content_never_enters_traces_or_rendered_status() {
    ...
    let _secret_backend = crate::test_support::EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
...
"#,
    ...
    let bindings = vec![hakus_config::HotbarBindingToml {
        slot: 1,
    ...
    let parsed: hakus_config::ConfigToml =
...
fn hotbar_setup_save_error_leaves_live_config_and_file_unchanged() {
    ...
    let mut app = create_test_app();
    app.config_path = Some(config_path.clone());
    let original_bindings = vec![hakus_config::HotbarBindingToml {
        slot: 2,
    ...
    let attempted_bindings = vec![hakus_config::HotbarBindingToml {
        slot: 1,
        action: "mode.agent".to_string(),
        label: None,
    }];
...
async fn session_denied_cache_auto_deny_explains_the_cached_rejection() {
    ...
    let audit_path = home._tmp.path().join(".hakus").join("audit.log");
    let mut app = create_test_app();
...
fn setup_checkpoint_waits_for_onboarding_and_skip_flag() {
    ...
    assert!(app.view_stack.is_empty());
    let state = hakus_config::SetupState::load()
    ...
    assert_eq!(
        ...
        hakus_config::ConstitutionChoice::Deferred
    );
    assert_eq!(
        state.status(hakus_config::SetupStep::Constitution),
        hakus_config::StepStatus::Deferred
    );
    assert!(
        !open_setup_checkpoint_if_due(&mut app, &config, false),
...
fn setup_runtime_preset_apply_persists_settings_config_and_state() {
    ...
    let mut state = hakus_config::SetupState {
        runtime_posture_source: hakus_config::RuntimePostureSource::Confirmed,
        ..Default::default()
    };
    state.set_step(
    ...
    let saved_state = hakus_config::SetupState::load()
        .expect("load setup state")
        .expect("setup state exists");
    assert_eq!(
        saved_state.status(hakus_config::SetupStep::TrustSandbox),
        hakus_config::StepStatus::Verified
    );
    assert_eq!(
        saved_state.runtime_posture_source,
        hakus_config::RuntimePostureSource::Confirmed
    );
}
...
fn setup_runtime_preset_rolls_back_durable_and_live_state_when_state_save_fails() {
    ...
    let state_path = hakus_config::SetupState::path().expect("state path");
    ...
    let error = apply_setup_runtime_preset(
        ...
        hakus_config::SetupState::default(),
    )
...
fn setup_high_trust_cannot_override_project_approval_policy() {
    ...
    let workspace = config_path
        ...
        .join("project");
    let project_dir = workspace.join(hakus_config::HAKUS_APP_DIR);
...
fn apply_loaded_session_never_restores_background_shell_event_as_composer_draft() {
    ...
    let shell_completion = Message {
        ...
        content: vec![
            ContentBlock::Text {
                text: concat!(
                    "<hakus:runtime_event kind=\"background_shell_completion\" ",
                    "visibility=\"internal\">\n",
...
fn apply_loaded_session_projects_subagent_handoff_without_retry_draft_or_user_cell() {
    ...
    let payload = concat!(
        ...
        "<hakus:subagent.done>{\"agent_id\":\"agent_resume\",\"name\":\"Tide\",",
        "\"agent_type\":\"implementer\",\"status\":\"completed\",",
        "\"summary_location\":\"previous_line\"}</hakus:subagent.done>",
    );
    ...
    let persisted_handoff = Message {
        ...
        content: vec![
            ContentBlock::Text {
                text: format!(
                    "<hakus:runtime_event kind=\"subagent_completion\" visibility=\"internal\">\n\
...
async fn successful_native_xai_oauth_activation_completes_onboarding() {
    let config_env = ConfigPathEnvGuard::new();
    let hakus_home = config_env
        ._tmp
        .path()
        .canonicalize()
        .expect("canonical temp root")
        .join("xai-success-home");
    let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", &hakus_home);
    let mut app = create_test_app();
    app.config_path = Some(config_env.config_path());
    app.onboarding = OnboardingState::Provider;
    app.onboarding_needs_api_key = true;
    ...
    let switched = apply_hakus_owned_xai_login(
        &mut app,
        &mut engine.handle,
        &mut config,
        pending,
...
async fn xai_api_key_confirmation_saves_only_the_selected_xai_slot() {
    let config_env = ConfigPathEnvGuard::new();
    let hakus_home = config_env._tmp.path().canonicalize().expect("temp home");
...
fn first_run_ollama_choice_survives_restart_without_rewriting_config() {
    ...
    let result = state
        .steps
        .get(&hakus_config::SetupStep::ProviderModel)
...
fn failed_first_run_route_persistence_keeps_provider_setup_active() {
    let _lock = crate::test_support::lock_test_env();
    let tmp = TempDir::new().expect("settings tempdir");
    let bad_home = tmp.path().join("hakus-home-file");
    ...
    let _hakus_home =
        crate::test_support::EnvVarGuard::set("HAKUS_HOME", bad_home.as_os_str());
    let _deepseek_config_path = crate::test_support::EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
    let _hakus_config_path = crate::test_support::EnvVarGuard::remove("HAKUS_CONFIG_PATH");
    let _hakus_provider = crate::test_support::EnvVarGuard::remove("HAKUS_PROVIDER");
...
async fn provider_switch_model_override_updates_target_provider_model_slot() {
    ...
    let state = hakus_config::SetupState::load()
        ...
        .expect("setup state");
    assert_eq!(
        state.status(hakus_config::SetupStep::ProviderModel),
        hakus_config::StepStatus::Verified
    );
...
async fn immediate_submit_custom_provider_missing_key_preflight_shows_auth_next_step() {
    let _lock = crate::test_support::lock_test_env();
    let _missing_key =
        crate::test_support::EnvVarGuard::remove("HAKUS_TEST_MISSING_LM_STUDIO_KEY");
    let mut config =
        named_custom_session_config("lm-studio", "http://127.0.0.1:1234/v1", "local-model");
    let provider = config
        .providers
    ...
    provider.api_key_env = Some("HAKUS_TEST_MISSING_LM_STUDIO_KEY".to_string());
    let mut app = create_test_app();
    app.set_provider_identity(ApiProvider::Custom, "lm-studio");
    app.set_model_selection("local-model".to_string());
    app.input = "preserve 用户 input".to_string();
...
fn auto_routed_turn_compaction_uses_selected_route_not_stale_app_route() {
    ...
    app.model = "auto".to_string();
    app.api_provider = ApiProvider::Deepseek;
    app.active_route_limits = Some(hakus_config::route::RouteLimits {
...
fn hotbar_dispatches_bound_slot_and_ignores_empty_slot() {
    ...
    let config = Config {
        hotbar: Some(hakus_config::default_hotbar_bindings_toml()),
        ..Config::default()
    };
    app.onboarding = OnboardingState::None;
...
fn subagent_completion_status_reads_done_sentinel() {
    let result = r#"done
<hakus:subagent.done>{"agent_id":"agent_x","status":"completed"}</hakus:subagent.done>"#;
...
fn subagent_failure_notice_surfaces_receipt_fields() {
    let result = concat!(
        ...
        "<hakus:subagent.done>{\"event\":\"subagent.failed\",",
...
<hakus:subagent.done>{"agent_id":"agent_x","status":"failed"}</hakus:subagent.done>"#;
...
<hakus:subagent.done>{"agent_id":"agent_x","status":"interrupted"}</hakus:subagent.done>"#;
...
<hakus:subagent.done>{"agent_id":"agent_x","status":"budget_exhausted"}</hakus:subagent.done>"#;
    assert_eq!(
...
<hakus:subagent.done>{"agent_id":"agent_x","status":"cancelled"}</hakus:subagent.done>"#;
    assert_eq!(
...
fn startup_notice_accepts_the_v095_single_binary_release_inventory() {
    ...
    assert!(
        REQUIRED_RELEASE_ASSETS
            .iter()
            .all(|asset| !asset.starts_with("hakus-tui-")),
...
fn version_hint_requires_complete_release_assets() {
    ...
    assert!(
        hint.toast_line(hakus_release::InstallMethod::Binary)
        ...
        missing_manifest["assets"]
            ...
            .filter(|asset| {
                asset.get("name").and_then(serde_json::Value::as_str)
                    != Some("hakus-artifacts-sha256.txt")
            })
            .cloned()
            .collect(),
    );
...
fn startup_version_check_is_disabled_in_ci_and_on_opt_out() {
    ...
    for reason in [
        hakus_release::SuppressionReason::ContinuousIntegration("CI"),
        hakus_release::SuppressionReason::OptOut("HAKUS_NO_UPDATE_CHECK"),
    ] {
        assert_eq!(
            resolve_version_check_source(
                &UpdateConfig {
...
async fn a_fresh_cache_answers_the_update_check_without_the_network() {
    let dir = tempfile::tempdir().expect("tempdir");
    let cache = hakus_release::check::cache_path_in(dir.path());
    hakus_release::UpdateCheckCache::now(Some("v9.9.9".to_string()))
        .store(&cache)
        .expect("seed cache");
...
async fn a_cached_up_to_date_answer_produces_no_notice() {
    let dir = tempfile::tempdir().expect("tempdir");
    let cache = hakus_release::check::cache_path_in(dir.path());
    hakus_release::UpdateCheckCache::now(Some("v0.9.4".to_string()))
        .store(&cache)
        .expect("seed cache");
...
fn update_notice_names_the_command_for_the_actual_install_method() {
    ...
    let npm_block = notice.notice_block(hakus_release::InstallMethod::Npm);
    assert!(
        npm_block.contains("npm install -g hakus@latest"),
        "names the npm command: {npm_block:?}"
    );
    assert!(
        npm_block.contains("Do not use `hakus update`"),
        "warns against self-update: {npm_block:?}"
    );
    assert!(
        notice
            .toast_line(hakus_release::InstallMethod::Homebrew)
            .contains("brew upgrade hakus"),
    ...
    assert!(
        notice
            .toast_line(hakus_release::InstallMethod::Binary)
            .contains("hakus update")
    );
}
...
fn update_notice_block_is_persistent_and_actionable() {
    ...
    let block = notice.notice_block(hakus_release::InstallMethod::Binary);
    assert!(
...
fn apply_slash_menu_selection_honors_user_argument_metadata_and_builtin_override() {
    ...
    let commands_dir = tmp.path().join(".hakus").join("commands");
...
fn visible_error_becomes_full_detail_target_ahead_of_adjacent_tool() {
    ...
    app.history = vec![
        ...
        HistoryCell::Error {
            message: "Refusing insecure base URL.\nSet HAKUS_ALLOW_INSECURE_HTTP=1 only for a trusted LAN host."
                .to_string(),
            severity: crate::error_taxonomy::ErrorSeverity::Error,
        },
    ];
    ...
    assert!(body.contains("HAKUS_ALLOW_INSECURE_HTTP=1"), "{body}");
}
...
async fn reselecting_live_model_and_thinking_is_session_local() {
    ...
    assert!(
        state
            .steps
            .contains_key(&hakus_config::SetupStep::ProviderModel),
...
async fn model_picker_startup_default_reports_settings_write_failure() {
    ...
    // The fixture's HAKUS_HOME is a file, so no setup-state root can
...
fn out_of_order_completes_finalize_one_history_cell_per_turn() {
    ...
    // Flush via the explicit helper (mirrors what TurnCompleted does).
    app.flush_active_cell();
...
fn orphan_tool_complete_with_unknown_id_pushes_separate_cell() {
    // A ToolCallFinished with no matching ToolCallStarted — the orphan path.
...
fn approval_prompt_uses_event_input_after_message_complete_drain() {
    ...
    let event_input = serde_json::json!({
        "command": "cargo test -p hakus-tui approval",
        "workdir": "/repo",
    });
    ...
    assert!(content.contains("cargo test -p hakus-tui approval"));
...
async fn approval_decision_persists_ask_rules_to_permissions_file() {
    ...
    let mut config = Config::default();
    let mut engine = mock_engine_handle();
    let rule = hakus_config::ToolAskRule::exec_shell("cargo test");
    ...
    let store = hakus_config::ConfigStore::load(Some(config_path)).expect("load config store");
...
fn recoverable_provider_error_advances_fallback_chain() {
    ...
    let mut app = create_test_app();
    app.api_provider = ApiProvider::Deepseek;
    app.provider_chain = Some(hakus_config::ProviderChain::new(
        hakus_config::ProviderKind::Deepseek,
        &[hakus_config::ProviderKind::Openrouter],
    ));
...
async fn provider_switch_rollback_corrects_setup_receipt_when_persistence_fails() {
    ...
    .await;
    let target_state = hakus_config::SetupState::load()
...
fn env_only_auth_failure_reopens_provider_onboarding() {
    ...
    app.config_path = Some(std::path::PathBuf::from("/tmp/hakus-phase2.toml"));
    app.api_key_env_only = true;
    ...
    assert!(
        status.contains("anthropic")
            ...
            && status.contains("/tmp/hakus-phase2.toml"),
...
fn subagent_completion_notification_uses_summary_line_not_sentinel() {
    let payload = crate::tui::notifications::subagent_terminal_payload(
        ...
        "agent_live",
        "Finished the docs audit.\n<hakus:subagent.done>{}</hakus:subagent.done>",
        &crate::tools::subagent::SubAgentStatus::Completed,
        false,
    ...
    assert!(!payload.render_inline().contains("hakus:subagent.done"));
}
...
fn subagent_cancelled_notification_never_claims_completion() {
    let payload = crate::tui::notifications::subagent_terminal_payload(
        crate::localization::Locale::En,
        "agent_stopped",
        "Cancelled\n<hakus:subagent.done>{\"status\":\"cancelled\"}</hakus:subagent.done>",
        &crate::tools::subagent::SubAgentStatus::Cancelled,
        false,
...
#[test]
fn terminal_input_child_pause_drains_hakus_events_before_editor_handoff() {
    let (tx, rx) = std::sync::mpsc::channel();
    tx.send(TerminalInputMessage::Event(Event::Key(KeyEvent::new(
        KeyCode::Char('x'),
        KeyModifiers::NONE,
