...
pub(crate) fn apply_agent_spawned_status_and_observer(
    ...
    hakus_telemetry::session_counters().bump(hakus_telemetry::Counter::SubagentSpawn);
...
pub(crate) fn apply_coordination_detail_projection(
    ...
    // warning blaming "another Hakus process" would be false (owner
    // report, 2026-08-04) — so it stays off the sticky strip.
    if !projection.process_lock_held {
        let note = projection
            .process_lock_note
            .as_deref()
            .unwrap_or("another Hakus process owns delegated coordination for this workspace");
...
pub(crate) fn apply_engine_error_to_app(
    ...
    ) && app.api_key_env_only
    {
        ...
        let config_path = match crate::config::resolve_load_config_path(app.config_path.clone()) {
            ...
            Ok(None) => "~/.hakus/config.toml".to_string(),
    ...
    if let Some(action) = result.action {
        match action {
            ...
            AppAction::PermissionRulesChanged => {
                match hakus_config::load_permissions_snapshot(app.config_path.clone()) {
                    Ok(snapshot) => {
            ...
            AppAction::OpenTemplateSetup { template_id } => {
                if app.view_stack.top_kind() != Some(ModalKind::ProviderPicker) {
                        ...
                        )
                    {
                        ...
                        let template = hakus_config::provider_setup_template(&template_id);
...
pub(crate) fn apply_workspace_runtime_state(app: &mut App, config: &Config, workspace: PathBuf) {
    ...
    app.skills_scan_hakus_only = config.skills_config().scan_hakus_only();
...
pub(crate) async fn apply_hakus_owned_xai_login(
    app: &mut App,
    engine_handle: &mut EngineHandle,
    config: &mut Config,
...
pub(crate) fn apply_loaded_session_with_goal(
    ...
    .map_err(|reason| {
        format!(
            "saved session provider '{}' could not be resolved from the live config: {reason}. Hakus will not fall back",
