...
impl SoundEvent {
    ...
    pub fn from_notification_kind(kind: NotificationKind) -> SoundEvent {
        match kind {
            NotificationKind::TurnCompleted => SoundEvent::TurnCompleted,
    ...
    pub fn parse(s: &str) -> Option<SoundEvent> {
        match s {
            "turn-complete" => Some(SoundEvent::TurnCompleted),
...
pub fn cue_for(event: SoundEvent) -> SoundCue {
    match event {
        SoundEvent::TurnCompleted => SoundCue::Bell,
...
impl Default for EventSoundPolicy {
    fn default() -> Self {
        Self {
            enabled: false,
            events: vec![SoundEvent::TurnCompleted, SoundEvent::ApprovalNeeded],
...
impl EventSoundPolicy {
    ...
    pub fn decide(&mut self, event: SoundEvent, now_ms: u64) -> SoundDecision {
        ...
        if event == SoundEvent::TurnCompleted && self.completion_sound_active {
...
mod tests {
    ...
    fn cue_table_is_the_documented_mapping() {
        assert_eq!(cue_for(SoundEvent::TurnCompleted), SoundCue::Bell);
    ...
    fn parse_covers_the_documented_event_names() {
        let names = [
            ("turn-complete", SoundEvent::TurnCompleted),
    ...
    fn unlisted_event_is_suppressed_as_not_listed() {
        let mut policy = enabled_policy(vec![SoundEvent::TurnCompleted]);
        for event in all_events() {
            let expected = if event == SoundEvent::TurnCompleted {
    ...
    fn turn_complete_defers_to_active_completion_sound() {
        let mut active = EventSoundPolicy {
            ...
            ..enabled_policy(vec![SoundEvent::TurnCompleted])
        };
        assert_eq!(
            active.decide(SoundEvent::TurnCompleted, 0),
            SoundDecision::Suppress(SuppressReason::TurnCompleteHandledByCompletionSound)
        );

        let mut inactive = enabled_policy(vec![SoundEvent::TurnCompleted]);
        assert_eq!(
            inactive.decide(SoundEvent::TurnCompleted, 0),
            SoundDecision::Play(SoundCue::Bell)
        );
    }
    ...
    fn from_config_ignores_unknown_events_and_parses_kebab_case() {
        ...
        assert_eq!(
            policy.events,
            vec![SoundEvent::TurnCompleted, SoundEvent::ApprovalNeeded]
        );
