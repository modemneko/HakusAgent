...
///
/// `TurnCompleted` re-resolves the same identity and must reproduce every field
...
pub struct SuggestionRouteSnapshot<'a> {
    ...
    /// Actual base URL this turn's client used, from `Event::TurnCompleted`.
...
/// what `Event::TurnCompleted` reports back. Comparing anything else here would
...
mod tests {
    ...
    /// The endpoint `Event::TurnCompleted` would report for this config.
    ///
    ...
    fn config_mutated_before_turn_started_cannot_move_authority_off_route_a() {
        ...
        // --- TurnCompleted: the later suggestion launch is refused. ---
        let snapshot = config_snapshot(&route, &authority, &actual_base_url);
        assert!(
    ...
    fn config_requires_the_endpoint_the_turn_actually_used() {
        ...
        // `Event::TurnCompleted` reported a different endpoint than the one the
