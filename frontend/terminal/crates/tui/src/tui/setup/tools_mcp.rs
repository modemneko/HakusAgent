...
impl SetupToolsMcpFacts {
    pub(super) fn from_app_config(app: &App, config: &Config, hakus_home: &Path) -> Self {
        ...
        let tools_dir = hakus_home.join("tools");
        let tools = tools_dir_inventory(&tools_dir);
        let plugins = plugins_inventory(app, config, hakus_home);
        ...
        let plugins_path_display = display_path(&plugins_dir_for(app, config, hakus_home));
...
fn mcp_inventory(app: &App, project_mcp_path: &Path) -> McpInventoryRow {
    ...
    match crate::mcp::load_config_with_workspace_and_plugins(
        ...
        Err(_) => McpInventoryRow {
            ...
            detail: format!(
                "config unreadable at {} (and project {}); open /mcp or run `hakus doctor` — secrets not shown",
...
fn mcp_snapshot_inventory(
    ...
    if total == 0 {
        return McpInventoryRow {
            ...
            detail: format!(
                "nothing configured yet ({paths}); optional — use /mcp or `hakus mcp init` later"
            ),
...
fn mcp_config_inventory(global: &Path, project: &Path, cfg: &McpConfig) -> McpInventoryRow {
    ...
    detail.push_str("; /mcp or `hakus doctor` for full checks");
    McpInventoryRow {
        status,
        detail,
        scope: McpInventoryScope::Configuration,
...
fn skills_inventory(app: &App) -> InventoryRow {
    ...
    if !dir_exists {
        return InventoryRow {
            status: InventoryStatus::Off,
            detail: format!(
                "nothing configured yet (missing at {path}); optional — /skills or `hakus setup --skills`"
            ),
        };
    }
    if !dir_is_dir {
...
fn tools_dir_inventory(tools_dir: &Path) -> InventoryRow {
    ...
    if !tools_dir.exists() {
        return InventoryRow {
            status: InventoryStatus::Off,
            detail: format!(
                "nothing configured yet (missing at {path}); optional — `hakus setup --tools`"
            ),
        };
    }
    if !tools_dir.is_dir() {
...
fn plugins_inventory(app: &App, config: &Config, hakus_home: &Path) -> InventoryRow {
    let plugins_dir = plugins_dir_for(app, config, hakus_home);
...
fn plugins_dir_for(_app: &App, _config: &Config, hakus_home: &Path) -> PathBuf {
    hakus_home.join("plugins")
}
...
mod tests {
    ...
    fn write_path_only_command(dir: &Path) -> String {
        let command = "hakus-setup-mcp-path-only-test";
        #[cfg(windows)]
    ...
    fn setup_resolves_server_path_while_doctor_stays_structural() {
        ...
        server.command = Some("hakus-setup-mcp-command-that-does-not-exist".to_string());
        assert_eq!(
    ...
    fn on_ramp_text_mentions_safe_commands_and_redacts() {
        let facts = SetupToolsMcpFacts {
            ...
            overall_status: InventoryStatus::Off,
            needs_action: false,
            mcp_path_display: "~/.hakus/mcp.json".into(),
            skills_path_display: "~/.hakus/skills".into(),
            plugins_path_display: "~/.hakus/plugins".into(),
        };
        let text = on_ramp_text(Locale::En, &facts);
        assert!(text.contains("hakus mcp init") || text.contains("/mcp"));
        ...
        assert!(text.contains("~/.hakus/mcp.json"));
