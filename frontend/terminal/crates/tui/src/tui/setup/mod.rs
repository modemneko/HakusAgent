...
impl SetupRuntimeFacts {
    fn from_app_config(app: &App, config: &Config) -> Self {
        ...
        let health = if provider_ready {
            ...
            format!(
                "{}; run codex login, then grant exact read-only access with `hakus auth external-consent --provider openai-codex --mode read-only`, or open /provider",
                readiness.label()
            )
        ...
        let hakus_home = setup_hakus_home_dir();
        let persistence = SetupPersistenceFacts::from_app_config(app, config, &hakus_home);
        let tools_mcp =
            tools_mcp::SetupToolsMcpFacts::from_app_config(app, config, &hakus_home);
...
fn setup_hakus_home_dir() -> std::path::PathBuf {
    hakus_config::hakus_home().unwrap_or_else(|_| {
        crate::config::effective_home_dir().map_or_else(
            || std::path::PathBuf::from(".hakus"),
            |home| home.join(".hakus"),
        )
    })
}
...
impl SetupWizardView {
    ...
    /// Hakus should take. This saves guidance only. Runtime approval,
...
fn project_runtime_override_warning(workspace: &Path, locale: Locale) -> Option<String> {
    let outcome = hakus_config::load_project_config_outcome(workspace);
...
enum DraftProvenance {
    ...
    Guided,
    /// Drafted by the named model, then sanitized and bounded by Hakus.
    Model(String),
...
fn constitution_ratification_text(
    ...
    match locale {
        Locale::Ja => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "{label} があなたのガイド回答から起草し、Hakus が構造検証と境界制限を適用しました。"
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · ユーザー憲法\n{RULE}\n\n{drafted_by}\n\n\
                 これは Hakus があなたと協働するための常設の基準です。優れた憲法のように、使えるほど短く、\
        ...
        Locale::ZhHans => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "由 {label} 根据你的引导式答案起草，并已由 Hakus 完成结构校验与边界限制。"
                ),
            ...
            format!(
                "HAKUS · 用户宪章\n{RULE}\n\n{drafted_by}\n\n\
                 这是 Hakus 与你协作时长期遵循的偏好和规则。内容应保持简短、便于执行，以持久原则为主，并可随时调整。\
        ...
        Locale::ZhHant => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "由 {label} 根據你的引導式答案起草，並已由 Hakus 完成結構驗證與邊界限制。"
                ),
            ...
            format!(
                "HAKUS · 使用者憲法\n{RULE}\n\n{drafted_by}\n\n\
                 這是 Hakus 與你協作的長期準則。像優秀的憲法一樣：足夠簡短因而可用，由持久原則而非詳盡規則構成，並且可以隨你修訂。\
        ...
        Locale::PtBr => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Rascunhado por {label} a partir das suas respostas guiadas, depois validado por schema e limitado pelo Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · CONSTITUIÇÃO DO USUÁRIO\n{RULE}\n\n{drafted_by}\n\n\
                 Esta é a regra permanente de como o Hakus trabalha com você. Como boas constituições, \
        ...
        Locale::Es419 => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Redactado por {label} desde tus respuestas guiadas, luego validado por schema y acotado por Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · CONSTITUCIÓN DEL USUARIO\n{RULE}\n\n{drafted_by}\n\n\
                 Esta es la regla permanente de cómo Hakus trabaja contigo. Como las buenas constituciones, \
        ...
        Locale::Vi => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Được {label} soạn từ câu trả lời hướng dẫn của bạn, rồi được Hakus kiểm tra schema và giới hạn biên."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · HIẾN PHÁP NGƯỜI DÙNG\n{RULE}\n\n{drafted_by}\n\n\
                 Đây là luật thường trực cho cách Hakus làm việc với bạn. Giống các hiến pháp tốt, \
        ...
        Locale::Ko => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "{label}이(가) 당신의 가이드 답변을 바탕으로 초안을 작성했고, Hakus이 구조를 검증하고 범위를 제한했습니다."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · 사용자 헌법\n{RULE}\n\n{drafted_by}\n\n\
                 이것은 Hakus이 당신과 함께 일하는 방식에 대한 상시 규칙입니다. 훌륭한 헌법이 그렇듯, \
        ...
        Locale::Ca => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Redactat per {label} a partir de les teves respostes guiades, després validat per esquema i acotat per Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · CONSTITUCIÓ DE L'USUARI\n{RULE}\n\n{drafted_by}\n\n\
                 Aquesta és la llei permanent de com Hakus treballa amb tu. Com les bones constitucions, \
        ...
        Locale::De => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Entworfen von {label} aus deinen geführten Antworten, dann schema-geprüft und begrenzt durch Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · NUTZERVERFASSUNG\n{RULE}\n\n{drafted_by}\n\n\
                 Dies ist das geltende Gesetz dafür, wie Hakus mit dir arbeitet. Wie die besten Verfassungen \
        ...
        Locale::Fr => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Rédigé par {label} à partir de vos réponses guidées, puis validé par schéma et borné par Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · CONSTITUTION DE L'UTILISATEUR\n{RULE}\n\n{drafted_by}\n\n\
                 Voici la loi permanente qui régit la façon dont Hakus travaille avec vous. Comme les meilleures constitutions, \
        ...
        Locale::Id => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Disusun oleh {label} dari jawaban terpandu Anda, lalu diperiksa skemanya dan dibatasi oleh Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · KONSTITUSI PENGGUNA\n{RULE}\n\n{drafted_by}\n\n\
                 Ini adalah hukum tetap tentang cara Hakus bekerja dengan Anda. Seperti konstitusi terbaik, \
        ...
        Locale::Hi => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "{label} द्वारा आपके गाइडेड उत्तरों से तैयार, फिर Hakus द्वारा स्कीमा-जाँचा और सीमित किया गया।"
                ),
            ...
            format!(
                "HAKUS · उपयोगकर्ता संविधान\n{RULE}\n\n{drafted_by}\n\n\
                 यह Hakus आपके साथ कैसे काम करे, इसका स्थायी कानून है। सर्वोत्तम संविधानों की तरह, \
        ...
        Locale::Ru => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Подготовлено {label} на основе ваших ответов на наводящие вопросы, затем проверено по схеме и ограничено Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · КОНСТИТУЦИЯ ПОЛЬЗОВАТЕЛЯ\n{RULE}\n\n{drafted_by}\n\n\
                 Это постоянный закон о том, как Hakus работает с вами. Как лучшие конституции, \
        ...
        Locale::Uk => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Підготовлено {label} на основі ваших відповідей на навідні запитання, потім перевірено за схемою та обмежено Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · КОНСТИТУЦІЯ КОРИСТУВАЧА\n{RULE}\n\n{drafted_by}\n\n\
                 Це постійний закон про те, як Hakus працює з вами. Як найкращі конституції, \
        ...
        _ => {
            let drafted_by = match provenance {
                DraftProvenance::Model(label) => format!(
                    "Drafted by {label} from your guided answers, then schema-checked and bounded by Hakus."
                ),
                DraftProvenance::Guided => {
            ...
            format!(
                "HAKUS · USER CONSTITUTION\n{RULE}\n\n{drafted_by}\n\n\
                 This is the standing law for how Hakus works with you. Like the best \
...
pub fn persist_user_constitution_choice(
    ...
    let mut transaction = hakus_config::persistence::SetupTransaction::new();
