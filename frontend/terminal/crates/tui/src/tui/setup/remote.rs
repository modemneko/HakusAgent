...
//! | [`RemoteMode::RuntimeApi`] | `hakus serve --http` on loopback, token-authenticated |
...
const RUNTIME_TOKEN_VARS: [&str; 2] = ["HAKUS_RUNTIME_TOKEN", "DEEPSEEK_RUNTIME_TOKEN"];
...
const RUNTIME_HOST_VARS: [&str; 2] = ["HAKUS_RUNTIME_HOST", "DEEPSEEK_RUNTIME_HOST"];

/// Placeholder substituted for every secret in the generated plan preview.
const REDACTED: &str = "<redacted>";

/// The four ways a Hakus runtime can be reached.
...
impl SetupRemoteFacts {
    ...
    pub(super) fn from_app_with_env(app: &App, lookup: &dyn Fn(&str) -> Option<String>) -> Self {
        ...
        let bridge_slugs = registry::BRIDGES
            ...
            .collect::<Vec<_>>();
        let provider_count = hakus_config::ProviderKind::all().len();
...
/// `hakus remote-setup --generate-only` would produce. It calls
...
Every secret is shown as <redacted>; Hakus never reads their values here.\n\n",
    );
...
pub(super) fn on_ramp_text(
    ...
    let command = format!(
        "hakus remote-setup --generate-only --cloud lighthouse --bridge telegram --provider {command_provider} --out ./hakus-deploy/lighthouse-telegram"
    );
...
mod tests {
    ...
    fn a_lan_bind_without_a_token_asks_for_action_rather_than_claiming_ready() {
        let modes = observe_modes(&env(&[("HAKUS_RUNTIME_HOST", "0.0.0.0")]));
        let lan = modes
            .iter()
        ...
        let both = observe_modes(&env(&[
            ("HAKUS_RUNTIME_HOST", "0.0.0.0"),
            ("HAKUS_RUNTIME_TOKEN", "t-secret-value"),
        ]));
        assert_eq!(
            both.iter()
                .find(|fact| fact.mode == RemoteMode::MobileLan)
    ...
    fn hostile_secret_values_never_reach_any_rendered_fact() {
        ...
        let modes = observe_modes(&env(&[
            ("HAKUS_RUNTIME_TOKEN", hostile_token),
            ("HAKUS_RUNTIME_HOST", "10.0.0.5"),
        ...
        assert!(rendered.contains("HAKUS_RUNTIME_TOKEN"));
    ...
    fn plan_preview_is_generated_in_memory_and_fully_redacted() {
        ...
        assert!(preview.contains(&format!("HAKUS_RUNTIME_TOKEN={REDACTED}")));
    }
    ...
    fn plan_preview_writes_nothing_to_disk() {
        ...
        assert!(!std::path::Path::new("./hakus-deploy").exists());
    }
