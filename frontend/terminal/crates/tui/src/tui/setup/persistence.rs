...
impl Default for SetupPersistenceFacts {
    fn default() -> Self {
        Self {
            home_result: "HAKUS_HOME not loaded".to_string(),
...
impl SetupPersistenceFacts {
    pub(super) fn from_app_config(app: &App, config: &Config, hakus_home: &Path) -> Self {
        let home_source = if hakus_config::hakus_home_is_explicit() {
            "explicit"
        } else {
            "default"
        };
        let home_presence = dir_presence(hakus_home);
        let config_path = hakus_config::resolve_config_path(app.config_path.clone())
            .unwrap_or_else(|_| hakus_home.join("config.toml"));
        let state_path = hakus_config::SetupState::path().unwrap_or_else(|_| {
            hakus_home.join(hakus_config::setup_state::SETUP_STATE_FILE_NAME)
        });
        let constitution_path = hakus_config::UserConstitution::path()
            .unwrap_or_else(|_| hakus_home.join("constitution.json"));
        ...
        Self {
            home_result: format!(
                "{home_source} HAKUS_HOME at {} ({home_presence})",
                hakus_home.display()
            ),
