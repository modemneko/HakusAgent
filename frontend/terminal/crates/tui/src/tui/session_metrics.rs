...
//! - **steps**: model calls (`Event::TokenUsage`) plus tool calls
//!   (`Event::ToolCallFinished`) — the agent's step count.
//! - **LLM**: sum of model-call wall time. Uses `TokenUsage::request_ms`
//!   (dispatch → usage receipt) when the engine measured dispatch, else the
//!   stream duration it always reports.
//! - **tools**: sum of tool wall time from `ToolCallStarted` → `ToolCallFinished`
//!   by tool id (the runtime's own clock, taken when the events drain).
//! - **TTFT avg**: mean of `TokenUsage::first_token_ms` over the model calls
//!   that reported one.
...
pub struct SessionMetrics {
    /// Model calls that reported usage (`Event::TokenUsage`).
    pub model_calls: u64,
    /// Tool calls that completed (`Event::ToolCallFinished`).
    pub tool_calls: u64,
