...
//! whole cycle — the second one because a user can easily have two Hakus
...
/// the developer's real `~/.hakus/settings.toml`, so under `cfg(test)` the
...
mod tests {
    ...
    fn a_worker_enrolled_in_the_test_env_scope_completes_a_transaction() {
        ...
        assert!(tmp.path().join(".hakus/settings.toml").exists());
    }

    /// Seal `HOME`/`HAKUS_HOME` onto `tmp`. Caller must already hold
    /// `lock_test_env()`.
    ...
    fn an_unauthorized_thread_neither_writes_nor_waits_for_a_sealed_scope() {
        ...
        assert!(
            !tmp.path().join(".hakus/settings.toml").exists(),
