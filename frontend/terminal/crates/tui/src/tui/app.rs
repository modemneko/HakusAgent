...
use hakus_config::{ProviderChain, route::RouteLimits};
...
/// Lifecycle identity retained until the matching `TurnCompleted` arrives.
///
...
pub(crate) fn bound_agent_activity_text(value: &str) -> String {
    ...
    let redacted = hakus_config::persistence::redact_secrets(&visible);
...
pub struct App {
    ...
    /// that `TurnCompleted { error: .. }` would otherwise emit on top of
    ...
    pub workflow_config: hakus_config::WorkflowConfigToml,
    ...
    /// authoritative TurnCompleted arrives. Stream events already queued for
    ...
    /// Incremented on `TurnCompleted` from the elapsed time of the
    ...
    pub constitution_draft_cell: std::sync::Arc<
        std::sync::Mutex<
            Option<(
                ...
                Result<Box<hakus_config::UserConstitution>, String>,
            )>,
        >,
    >,
...
impl App {
    ...
    fn discover_cached_skills(
        ...
        scan_hakus_only: bool,
        plugins: &crate::plugins::PluginRegistry,
    ) -> Vec<(String, String)> {
        crate::skills::discover_for_workspace_and_dir_with_mode_and_plugins(
            workspace,
            skills_dir,
            crate::skills::SkillDiscoveryMode::from_hakus_only(scan_hakus_only),
            Some(plugins),
        )
        .into_enabled()
        .list()
