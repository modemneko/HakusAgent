...
impl Renderable for ChatWidget {
    fn render(&self, _area: Rect, buf: &mut Buffer) {
        ...
        let area = _area;
        // Repaint the full chat area with the hakus-ink background each
...
mod tests {
    ...
    fn slash_completion_hints_exclude_set_and_deepseek_commands() {
        ...
        assert!(!hints.iter().any(|hint| hint.name == "/hakus"));
    }
    ...
    fn empty_state_shows_startup_context() {
        ...
        app.workspace = PathBuf::from("/tmp/hakus-test-workspace");
        ...
        assert!(rendered.contains("Hakus"));
        assert!(rendered.contains("/tmp/hakus-test-workspace · no git · mcp 2"));
    ...
    fn empty_state_centers_startup_block_by_actual_text_width() {
        ...
        let text_lines = lines
            ...
            .collect::<Vec<_>>();
        let context = "/tmp/hakus-test-workspace · no git · mcp 0";
    ...
    fn underwater_launch_is_visibly_deep_and_preserves_text_cells() {
        ...
        app.low_motion = false;
        app.fancy_animations = true;
        app.workspace = PathBuf::from("hakus-test-workspace");
    ...
    /// #4208: `HAKUS_ASCII_SAFE=1` must narrow every CodeWhale-authored
    ...
    fn repo_law_approval_has_distinct_authority_grammar() {
        let request = crate::tui::approval::ApprovalRequest::new(
            "approval-law",
            "edit_file",
            "Repo law holds this write: \"manifest review\" protects Cargo.toml (matched Cargo.toml, .hakus/constitution.json)",
...
+new\n";
    ...
    fn approval_shell_modal_stays_useful_on_short_terminals() {
        let request = crate::tui::approval::ApprovalRequest::new_with_intent(
            ...
            &serde_json::json!({
                "command": "cd /Volumes/VIXinSSD/hakus; cargo clippy -p hakus-tui --all-targets --locked -- -D warnings 2>&1 | tee /tmp/hakus-clippy.log",
                "cwd": "/Volumes/VIXinSSD/hakus",
            }),
            "exec_shell:cargo-clippy",
            Some("Confirmed - passes in isolation, so this is the documentation gate."),
            std::path::Path::new("/Volumes/VIXinSSD/hakus"),
        );
        let view = crate::tui::approval::ApprovalView::new(request.clone());
        let widget = ApprovalWidget::new(&request, &view);
        let area = Rect::new(0, 0, 80, 20);
