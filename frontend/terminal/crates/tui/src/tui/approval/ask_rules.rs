...
pub(super) fn build_persistent_allow_rules(
    ...
    let Some(workspace) = hakus_execpolicy::normalize_workspace_scope(workspace.as_ref())
    else {
        return Vec::new();
    };
...
fn build_file_write_ask_rules(
    ...
    let workspace = workspace.to_string_lossy();
    let Some(relative) =
        hakus_execpolicy::normalize_workspace_relative_path(path, workspace.as_ref())
