...
fn build_escape(method: Method, in_tmux: bool, msg: &str) -> Vec<u8> {
    match method {
        ...
        Method::Ghostty => {
            ...
            let seq = format!("\x1b]777;notify;hakus;{msg}\x07");
...
impl NotificationGate {
    ...
    pub fn from_config(notif: &crate::config::NotificationsConfig) -> Self {
        Self {
            quiet: notif.quiet,
            turn_completed: notif.events.turn_completed,
    ...
    pub fn allows(self, kind: NotificationKind) -> bool {
        ...
        match kind {
            NotificationKind::TurnCompleted => self.turn_completed,
    ...
    const fn to_bits(self) -> u8 {
        (self.quiet as u8 * Self::QUIET_BIT)
            | (self.turn_completed as u8 * Self::TURN_COMPLETE_BIT)
    ...
    const fn from_bits(bits: u8) -> Self {
        Self {
            ...
            turn_completed: bits & Self::TURN_COMPLETE_BIT != 0,
...
pub fn set_title_prefix(prefix: Option<&str>) {
    ...
    if TITLE_ANIMATION_RUNNING.load(Ordering::SeqCst) {
        let base = title_animation_base()
            .lock()
            .map_or_else(|_| "Hakus".to_string(), |base| base.clone());
...
fn title_animation_base() -> &'static Mutex<String> {
    TITLE_ANIMATION_BASE.get_or_init(|| Mutex::new("Hakus".to_string()))
}
...
pub fn stop_title_animation_quietly() {
    ...
    TITLE_ANIMATION_GENERATION.fetch_add(1, Ordering::SeqCst);
    COMPLETION_MARKER_SHOWN.store(false, Ordering::SeqCst);
    set_terminal_title(&decorate_title("Hakus"));
}
    ...
    let _ = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let args = [
                ...
                "-e".to_string(),
                "display notification theBody with title \"Hakus\" subtitle theSubtitle sound name \"default\"".to_string(),
                "-e".to_string(),
                "end run".to_string(),
                "--".to_string(),
                body,
...
pub fn completed_turn_payload(
    ...
    NotificationPayload::turn_completed(&headline).with_preview(preview.as_deref())
}
...
pub fn subagent_terminal_payload(
    ...
    let result_line = result
        .lines()
        .map(str::trim)
        .find(|line| !line.is_empty() && !line.starts_with("<hakus:subagent.done>"));
...
mod tests {
    ...
    fn title_whale_is_static_when_focused_or_motion_disabled() {
        ...
        assert_eq!(
            title_activity_label("Hakus", Duration::ZERO, true, true),
            "🐳 working…"
        );
        assert_eq!(
            title_activity_label("Hakus", Duration::ZERO, false, false),
            "🐳 working…"
        );
        assert_eq!(
            title_activity_label("Hakus", Duration::ZERO, false, true),
            "🐳 working…"
        );
        assert_eq!(
            title_activity_label("Hakus", Duration::from_millis(800), false, true),
            "🐋 working…"
        );
    }
    ...
    fn title_prefix_decorates_rest_and_completion_titles() {
        ...
        assert_eq!(decorate_title("Hakus"), "[feature/x] Hakus");
        assert_eq!(decorate_title("✓ done"), "[feature/x] ✓ done");
        set_title_prefix(None);
        assert_eq!(decorate_title("Hakus"), "Hakus");
    ...
    fn disabled_category_suppresses_only_that_kind() {
        ...
        assert!(gate.allows(NotificationKind::TurnCompleted));
    ...
    #[test]
    fn osc9_body_format() {
        let out = capture(Method::Osc9, false, "hakus: done", 0, 1);
        assert_eq!(out, b"\x1b]9;hakus: done\x07");
    }
    ...
    fn title_prefix_change_at_rest_repaints_the_resting_title() {
        ...
        COMPLETION_MARKER_SHOWN.store(false, Ordering::SeqCst);
        set_title_prefix(Some("Alpha"));
        assert_eq!(decorate_title(resting_title_body()), "[Alpha] Hakus");
        COMPLETION_MARKER_SHOWN.store(true, Ordering::SeqCst);
        assert_eq!(decorate_title(resting_title_body()), "[Alpha] ✓ done");
        COMPLETION_MARKER_SHOWN.store(false, Ordering::SeqCst);
        set_title_prefix(None);
        assert_eq!(decorate_title(resting_title_body()), "Hakus");
    }
    ...
    fn ghostty_escape_format() {
        ...
        let s = String::from_utf8(out).unwrap();
        assert!(
            s.contains("777;notify;hakus;done"),
    ...
    fn macos_notification_keeps_localized_status_as_subtitle() {
        let payload = NotificationPayload::turn_completed("ターン完了 (1m 5s)")
    ...
    fn macos_notification_truncates_preview() {
        let payload = NotificationPayload::turn_completed("Turn complete")
