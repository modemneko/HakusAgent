...
/// The user-facing operating mode. Defined in hakus-config; re-exported
/// here so `crate::tui::app::types::AppMode` keeps working.
pub use hakus_config::AppMode;

/// Localized, TUI-only presentation of [`AppMode`]. Kept out of
/// hakus-config so the mode type does not depend on the locale packs.
pub trait AppModeUi {
...
impl ReasoningEffort {
    ...
    pub fn normalize_for_route(
        ...
        // real tier distinct for normal turns; only Hakus-only synonyms
...
pub enum InitialInput {
    ...
    ///
    /// Used by `hakus pr <N>` (#451) to drop the model into a session
...
pub(crate) const WORKFLOW_DRAFT_INSTRUCTION_PREFIX: &str = "[hakus.workflow-draft.v1]";
...
pub enum SubmitDisposition {
    ...
    Steer,
    /// Park on `queued_messages` for dispatch after TurnCompleted.
