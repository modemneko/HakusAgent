...
impl App {
    ...
    pub fn new_with_plugin_registry(
        ...
        let xai_dangling_repair_message = if xai_oauth_needs_reauth {
            ...
            Some(
                "⚠ xAI OAuth credentials are missing. Re-authenticate with \
                 `hakus auth xai-device` or the in-app login, or switch providers."
                    .to_string(),
            )
        } else {
            None
        ...
        // model explicitly (`--model`, forwarded as `HAKUS_MODEL`), that
        // request wins. Before this fix the memory won unconditionally, so
        // `hakus --provider moonshot --model kimi-k3` silently kept running
        ...
        // then project-local `.hakus/hooks.toml` (#3026).
        ...
        let skills_scan_hakus_only = config.skills_config().scan_hakus_only();
