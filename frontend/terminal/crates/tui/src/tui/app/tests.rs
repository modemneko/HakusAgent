...
fn explicit_launch_provider_overrides_saved_startup_provider() {
    ...
    .expect("settings");
    let _config_path = EnvVarGuard::set("DEEPSEEK_CONFIG_PATH", &config_path);
    let _provider = EnvVarGuard::set("HAKUS_PROVIDER", "xiaomi-mimo");
...
fn bang_shell_prefix_parses_compact_and_spaced_forms() {
    ...
    assert_eq!(shell_command_from_bang_input("! pwd"), Ok(Some("pwd")));
    assert_eq!(
        shell_command_from_bang_input("  !  cargo test -p hakus-tui sidebar"),
        Ok(Some("cargo test -p hakus-tui sidebar"))
    );
...
fn app_new_detects_missing_api_key_with_default_config() {
    ...
    let config_path = tmp.path().join("config.toml");
    let _config_path = EnvVarGuard::set("DEEPSEEK_CONFIG_PATH", &config_path);
    let _provider_env = EnvVarGuard::remove("HAKUS_PROVIDER");
...
fn first_run_app_starts_on_welcome_when_a_key_is_missing() {
    ...
    let _home = EnvVarGuard::set("HAKUS_HOME", home.path().to_string_lossy().as_ref());
...
#[test]
fn cached_skills_respect_hakus_only_scan_config() {
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let workspace = tmp.path().join("workspace");
    ...
    let hakus_dir = workspace
        .join(".hakus")
        .join("skills")
        .join("hakus-skill");
    std::fs::create_dir_all(&hakus_dir).expect("hakus skill dir");
    std::fs::write(
        hakus_dir.join("SKILL.md"),
        "---\nname: hakus-skill\ndescription: CodeWhale skill\n---\nbody\n",
    )
    .expect("write hakus skill");
    ...
    assert_eq!(app.skills_dir, workspace.join(".hakus").join("skills"));
    assert!(
        app.cached_skills
            .iter()
            .any(|(name, _)| name == "hakus-skill"),
...
#[test]
fn resolve_skills_dir_requires_hakus_skills_to_be_directory() {
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let workspace = tmp.path().join("workspace");
    std::fs::create_dir_all(workspace.join(".hakus")).expect("hakus dir");
    std::fs::write(
        workspace.join(".hakus").join("skills"),
        "not a directory",
    )
    .expect("skills file");
...
#[test]
fn cached_skills_preserve_configured_directory_in_hakus_only_scan() {
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let workspace = tmp.path().join("workspace");

    let hakus_skill_dir = workspace
        .join(".hakus")
        .join("skills")
        .join("workspace-hakus");
    std::fs::create_dir_all(&hakus_skill_dir).expect("workspace hakus skill dir");
    std::fs::write(
        hakus_skill_dir.join("SKILL.md"),
        "---\nname: workspace-hakus\ndescription: Workspace CodeWhale skill\n---\nbody\n",
    )
    .expect("write workspace hakus skill");
    ...
    assert!(
        app.cached_skills
            .iter()
            .any(|(name, _)| name == "workspace-hakus"),
...
#[test]
fn cached_skills_reject_hakus_only_workspace_symlink_escape() {
    let tmp = tempfile::TempDir::new().expect("tempdir");
    let workspace = tmp.path().join("workspace");
    ...
    let link_path = workspace.join(".hakus").join("skills");
...
fn paste_defers_oversized_text_consolidation_until_submit() {
    ...
    assert_eq!(app.cursor_position, app.input.chars().count());
    let pastes_dir = tmp.path().join(".hakus/pastes");
    assert!(
    ...
    let submitted = app.submit_input().expect("expected submitted input");
    assert!(
        submitted.starts_with("@.hakus/pastes/paste-"),
...
fn paste_under_threshold_does_not_consolidate() {
    ...
    assert!(!app.input.starts_with("@.hakus/pastes/"));
...
fn large_multiline_paste_preserves_exact_bytes_through_submit() {
    ...
    // Real dogfood saw paths like `hakus-v091-exact-88a158-ci` arrive as
    ...
    let payload = format!(
        "Mission path: /Volumes/VIXinSSD/CW/worktrees/hakus-v091-exact-88a158-ci\n\
         SHA: 0dfe9170a10e081fe48b23239f22d33260f4fa24\n\
         Branch: codex/v091-local-candidate-20260722\n\
         Paths that must not truncate: hakus-v091-exact-88a158-ci worktrees/hakus-v091-exact-88a158-ci\n\
...
fn work_restore_reconciles_fleet_from_the_restored_workspace() {
    ...
    ledger
        .enqueue(hakus_protocol::fleet::FleetInboxEntry {
            run_id: hakus_protocol::fleet::FleetRunId::from("run-restore"),
...
fn failed_workspace_owner_reconcile_leaves_previous_work_state_intact() {
    ...
    ledger
        .enqueue(hakus_protocol::fleet::FleetInboxEntry {
            run_id: hakus_protocol::fleet::FleetRunId::from("run-regress"),
...
fn legacy_yolo_honors_a_missing_explicit_config_path_without_home_fallback() {
    ...
    let home_config_dir = home.join(hakus_config::HAKUS_APP_DIR);
    ...
    let _hakus_home = EnvVarGuard::remove("HAKUS_HOME");
...
fn app_with_fallback_chain(
    active: ApiProvider,
    fallbacks: &[hakus_config::ProviderKind],
    keyed: &[ApiProvider],
) -> App {
    ...
    let _hakus_home =
        EnvVarGuard::set("HAKUS_HOME", settings_home.path().join(".hakus"));
...
fn advance_fallback_skips_unauthed_middle_provider_and_lands_on_next_ready() {
    ...
    let mut app = app_with_fallback_chain(
        ...
        &[
            ...
            hakus_config::ProviderKind::Together,
        ],
...
fn startup_and_fallback_skip_inactive_external_only_routes_without_io() {
    ...
    let _home = EnvVarGuard::set("HAKUS_HOME", temp.path().join("owned-home"));
...
fn advance_fallback_local_primary_may_fall_back_to_local_sibling() {
    ...
    let mut app = app_with_fallback_chain(
        ApiProvider::Ollama,
        &[hakus_config::ProviderKind::Vllm],
        &[],
    );
...
/// Seal `HOME`/`HAKUS_HOME` onto a temp dir so these tests can assert the
...
fn sealed_settings_home(tmp: &std::path::Path) -> Vec<EnvVarGuard> {
    vec![
        ...
        EnvVarGuard::set("HAKUS_HOME", tmp.join(".hakus")),
        EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH"),
        EnvVarGuard::remove("HAKUS_CONFIG_PATH"),
    ]
}
...
fn failed_startup_default_write_is_reported_not_swallowed() {
    ...
    // below it fails.
    let blocked_home = tmp.path().join("hakus-home-file");
    ...
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &blocked_home);
    let _deepseek_config = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
    let _hakus_config = EnvVarGuard::remove("HAKUS_CONFIG_PATH");
    let _writes = crate::tui::startup_defaults::allow_writes_in_tests();
...
/// developer's real `~/.hakus/settings.toml` during `cargo test`.
#[test]
...
async fn a_late_startup_default_failure_is_returned_not_only_logged() {
    ...
    assert!(
        !message.contains(".hakus") && !message.contains(tmp.path().to_str().unwrap()),
...
/// launch. A dogfood user ran `hakus --provider moonshot --model kimi-k3`
...
fn an_explicit_launch_model_outranks_the_remembered_provider_model() {
    ...
    let _hakus_config_path = EnvVarGuard::remove("HAKUS_CONFIG_PATH");
    ...
    let _no_flag = EnvVarGuard::remove("HAKUS_MODEL");
    ...
    // `--model` reaches this binary as HAKUS_MODEL. It must win.
    let _model_flag = EnvVarGuard::set("HAKUS_MODEL", "kimi-k3");
