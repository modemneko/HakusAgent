...
//! encoded as PNG and persisted under `~/.hakus/clipboard-images/` so the
...
impl TerminalClipboardContext {
    fn detect() -> Self {
        ...
        let ssh_clipboard = std::env::var_os("HAKUS_SSH_CLIPBOARD");
...
impl ClipboardHandler {
    ...
    ///
    /// `workspace` is used as a fallback location when `~/.hakus/` cannot
...
/// `~/.hakus/clipboard-images/` so the path is stable across worktrees and
...
fn clipboard_images_dir_for_home(workspace: &Path, home: Option<&Path>) -> PathBuf {
    if let Some(home) = home {
        return home.join(".hakus").join("clipboard-images");
    }
...
mod tests {
    ...
    #[test]
    fn clipboard_images_dir_uses_hakus_home_directory() {
        let home = tempfile::tempdir().unwrap();
        let workspace = tempfile::tempdir().unwrap();

        assert_eq!(
            clipboard_images_dir_for_home(workspace.path(), Some(home.path())),
            home.path().join(".hakus").join("clipboard-images")
        );
    }
...
"#,
    ...
    fn tmux_load_buffer_w_reaches_attached_client_with_default_passthrough_disabled() {
        ...
        let nonce = std::time::SystemTime::now()
            ...
            .as_nanos();
        let socket = format!("hakus-clipboard-{}-{nonce}", std::process::id());
