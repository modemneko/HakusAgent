...
//! across both scopes: user-global (`$HAKUS_HOME/fleets/`) and folder
//! (`.hakus/fleets/`). Rows show name, scope badge, and operator summary —
...
fn legacy_profile_file_count(workspace: &std::path::Path) -> usize {
    ...
    let ws_dir = workspace.join(".hakus").join("agents");
...
mod tests {
    ...
    fn list_renders_both_scopes_and_marks_selection() {
        ...
        unsafe { std::env::set_var("HAKUS_HOME", sealed_home()) };
    ...
    fn delete_requires_confirmation_and_removes_the_file() {
        ...
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", home.path());
        let ws = tempfile::TempDir::new().unwrap();
...
members = []"#,
    ...
    fn migration_banner_appears_only_when_needed_and_migrates() {
        ...
        let agents = ws.path().join(".hakus/agents");
