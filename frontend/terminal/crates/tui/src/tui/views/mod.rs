...
pub enum ViewEvent {
    ...
    HotbarSetupSaved {
        bindings: Vec<hakus_config::HotbarBindingToml>,
    },
    /// Emitted by the constitution-first setup shell when a staged setup-state
    /// record should be committed atomically to `$HAKUS_HOME/setup_state.json`.
    ...
    SetupConstitutionCommitRequested {
        constitution: hakus_config::UserConstitution,
        state: hakus_config::SetupState,
        message: String,
    },
...
impl ViewStack {
    ...
    pub fn push<V: ModalView + 'static>(&mut self, view: V) {
        ...
        tracing::debug!(target: "hakus_tui::view_stack", action = "push", kind = ?kind, depth = self.views.len(), "view pushed");
    }
    ...
    pub fn push_boxed(&mut self, view: Box<dyn ModalView>) {
        ...
        tracing::debug!(target: "hakus_tui::view_stack", action = "push_boxed", kind = ?kind, depth = self.views.len(), "view pushed");
    }

    pub fn pop(&mut self) -> Option<Box<dyn ModalView>> {
        let popped = self.views.pop();
        if let Some(view) = popped.as_ref() {
            tracing::debug!(target: "hakus_tui::view_stack", action = "pop", kind = ?view.kind(), depth = self.views.len(), "view popped");
        }
        popped
    }
        ...
        match action {
            ...
            ViewAction::Close => {
                if let Some(view) = self.views.pop() {
                    tracing::debug!(target: "hakus_tui::view_stack", action = "close", kind = ?view.kind(), depth = self.views.len(), "view closed via action");
                }
            }
            ...
            ViewAction::EmitAndClose(event) => {
                events.push(event);
                if let Some(view) = self.views.pop() {
                    tracing::debug!(target: "hakus_tui::view_stack", action = "emit_and_close", kind = ?view.kind(), depth = self.views.len(), "view closed via action");
                }
            }
        }
        events
    ...
    pub fn new_for_app(app: &App) -> Self {
        ...
        let mut rows = vec![
            ...
            ConfigRow {
                ...
                value: hakus_config::ProviderSetupTemplate::settings_value(),
                editable: true,
                scope: ConfigScope::Saved,
            },
            ConfigRow {
            ...
            ConfigRow {
                ...
                value: config
                    ...
                    .unwrap_or_else(|| hakus_config::FleetExecConfig::default().max_spawn_depth)
                    .to_string(),
                editable: false,
                scope: ConfigScope::Saved,
            },
...
fn config_choice_label(locale: Locale, key: &str, value: &str) -> String {
    let label = match (key, value) {
        ...
        ("status_indicator", "cw") => "Hakus mark".to_string(),
...
mod tests {
    ...
    fn create_test_app() -> App {
        ...
        let isolated_config_path = std::env::temp_dir().join(format!(
            "hakus-config-view-test-{}-{config_id}.toml",
            std::process::id()
        ));
...
"#,
        ...
        assert!(
            row.value
                .contains(&hakus_config::quote_os_path(&auth_path)),
            "{}",
            row.value
        );
        assert!(
...
"#,
    ...
    fn config_view_experimental_features_show_effective_state_and_overrides() {
        let temp_root = std::env::temp_dir().join(format!(
            "hakus-experimental-config-view-test-{}",
            std::process::id()
        ));
