...
fn view_with_overrides() -> FleetRosterView {
    ...
    if let Some(reviewer) = members.iter_mut().find(|m| m.id == "reviewer") {
        ...
        reviewer.source = PathBuf::from(".hakus/agents/reviewer.toml");
...
fn detail_lines_carry_overlay_source_for_project_members() {
    ...
    assert!(
        text.contains("custom overlay (.hakus/agents/reviewer.toml)"),
        "{text}"
    );
...
fn roster_loads_config_members_through_the_shared_merge() {
    ...
    profiles.insert(
        ...
        hakus_config::FleetProfile {
            slot: hakus_config::FleetSlot::from_name("scout"),
            role: hakus_config::FleetRole {
                name: "scout".to_string(),
                description: Some("Writes docs.".to_string()),
                instructions: None,
            },
            loadout: hakus_config::FleetLoadout::Fast,
            model: None,
            provider: None,
...
fn detail_pane_reports_shadowed_lower_layers() {
    ...
    view.shadowed.push(crate::fleet::roster::ShadowedProfile {
        ...
        shadowed_source: PathBuf::from("/home/op/.hakus/agents/reviewer.toml"),
        winner_origin: ProfileOrigin::Workspace,
        winner_source: PathBuf::from(".hakus/agents/reviewer.toml"),
    });
    let reviewer = view.members.iter().find(|m| m.id == "reviewer").unwrap();
    let text = member_detail_lines_with_session(reviewer, None, &view.shadowed, view.locale)
        .iter()
    ...
    assert!(
        text.contains("project · .hakus/agents/reviewer.toml (wins)"),
        "detail names the winning layer: {text}"
    );
    assert!(
        text.contains("personal · /home/op/.hakus/agents/reviewer.toml (ignored)"),
