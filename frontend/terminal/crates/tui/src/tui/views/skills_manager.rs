...
mod tests {
    ...
    fn remove_requires_confirm_then_emits() {
        ...
        let _home = IsolatedHome::new(&tmp);
        let workspace = tmp.path().join("ws");
        let skill_dir = workspace.join(".hakus").join("skills").join("demo");
    ...
    fn selected_skill_row_uses_charter_pointer_and_selection_bg() {
        ...
        let _home = IsolatedHome::new(&tmp);
        let mut app = app_in(&tmp);
        app.skills_dir = tmp.path().join("home").join(".hakus").join("skills");
    ...
    fn import_replace_confirm_is_scoped_to_import_target() {
        ...
        write_skill_pkg(
            &workspace.join(".hakus").join("skills").join("shared"),
            "shared",
            "owned-project",
        );
        write_skill_pkg(
