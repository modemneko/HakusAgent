...
use hakus_workflow::fleet_composition::{
...
const PROFILE_DIR: &str = ".hakus/agents";
...
const ROLES: [Choice; 9] = [
    ...
    Choice {
        ...
        description: Cow::Borrowed(
            "Define the posture yourself in a workspace agent TOML profile under .hakus/agents/.",
        ),
    },
];
...
const THINKING_CHOICES: &[Choice] = &[
    ...
    Choice {
        ...
        summary: Cow::Borrowed("Let Hakus choose"),
...
pub struct FleetSetupSnapshot {
    ...
    /// Resolved personal profile directory (`$HAKUS_HOME/agents`), or the
...
impl FleetSetupView {
    ...
    fn review_policy_summary(&self) -> String {
        format!(
            "Workers run without a token cap by default · {}s api, {}s heartbeat. Launch with Fleet → exec; /fleet workers (or /subagents) shows sub-agents in the current interactive session; /fleet status and hakus fleet status both read the persistent .hakus/fleet.jsonl ledger.",
...
mod tests {
    ...
    fn snapshot() -> FleetSetupSnapshot {
        FleetSetupSnapshot {
            workspace: PathBuf::from("/tmp/hakus-test-workspace"),
    ...
    /// test reads the developer's real `$HAKUS_HOME/agents`.
