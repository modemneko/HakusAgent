...
//!
//! - [`NotificationKind::TurnCompleted`] — the localized "Turn complete"
...
const FALLBACK_HEADLINE: &str = "Hakus";
...
impl NotificationKind {
    ...
    pub const fn allows_preview(self) -> bool {
        matches!(self, Self::TurnCompleted | Self::SubagentTerminal)
    }
}
...
impl NotificationPayload {
    ...
    #[must_use]
    pub fn turn_completed(headline: &str) -> Self {
        Self::new(NotificationKind::TurnCompleted, headline, None)
    }
...
mod tests {
    ...
    fn every_kind(text: &str) -> Vec<NotificationPayload> {
        vec![
            NotificationPayload::turn_completed(text).with_preview(Some(text)),
    ...
    fn bounds_are_char_based_not_byte_based() {
        let payload = NotificationPayload::turn_completed(&"日".repeat(200));
    ...
    fn preview_is_kind_gated_not_caller_gated() {
        let on = NotificationPayload::turn_completed("Turn complete")
    ...
    fn absolute_paths_are_reduced_to_basename() {
        let payload = NotificationPayload::turn_completed("Turn complete").with_preview(Some(
    ...
    fn control_bytes_and_newlines_are_collapsed() {
        let payload = NotificationPayload::turn_completed("Turn\x1b[31m complete\n\nsecond line");
        assert_eq!(payload.headline(), "Turn complete second line");
    }

    #[test]
    fn empty_input_still_yields_a_headline() {
        let payload = NotificationPayload::turn_completed("   \n  ");
