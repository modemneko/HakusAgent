...
pub fn build_entries_with_plugins(
    ...
    let skills = skills::discover_for_workspace_and_dir_with_mode_and_plugins(
        workspace,
        skills_dir,
        skills::SkillDiscoveryMode::from_hakus_only(skills_scan_hakus_only),
        Some(plugins),
    )
    .into_enabled();
...
mod tests {
    ...
    #[test]
    fn command_palette_skills_respect_hakus_only_scan() {
        let tmp = TempDir::new().expect("tempdir");
        let workspace = tmp.path().join("workspace");
        ...
        let hakus_skill_dir = workspace
            ...
            .join("skills")
            .join("hakus-skill");
        std::fs::create_dir_all(&hakus_skill_dir).expect("create hakus skill dir");
        std::fs::write(
        ...
        let entries = build_entries(
            Locale::En,
            workspace.join(".hakus").join("skills").as_path(),
            true,
            workspace.as_path(),
            Path::new("mcp.json"),
            None,
        ...
        assert!(skill_labels.contains(&"$hakus-skill"));
    ...
    fn command_palette_includes_only_active_reviewed_plugin_skills() {
        ...
        let tmp = TempDir::new().expect("tempdir");
        let _home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", tmp.path().join("home"));
        let workspace = tmp.path().join("workspace");
    ...
    fn command_palette_includes_workspace_user_commands() {
        let tmp = TempDir::new().expect("tempdir");
        let workspace = tmp.path().join("workspace");
        let commands_dir = workspace.join(".hakus").join("commands");
