...
//!
//! The flow asks only for what Hakus genuinely needs before the user can
...
pub fn mark_onboarded() -> std::io::Result<PathBuf> {
    let path = default_marker_path().ok_or_else(|| {
        std::io::Error::new(
            ...
            "Hakus home directory not found",
        )
    })?;
...
mod tests {
    ...
    #[test]
    fn fresh_install_marker_path_uses_hakus_not_legacy() {
        let tmp = tempfile::tempdir().expect("tempdir");
    ...
    #[test]
    fn hakus_marker_wins_over_legacy_marker() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let primary = tmp.path().join(".hakus").join(ONBOARDED_MARKER_FILE);
        let legacy = tmp.path().join(".deepseek").join(ONBOARDED_MARKER_FILE);
    ...
    #[test]
    fn explicit_hakus_home_marker_survives_restart_resolution() {
        let _env_lock = crate::test_support::lock_test_env();
        let tmp = tempfile::tempdir().expect("tempdir");
        let ambient_home = tmp.path().join("ambient profile");
        let isolated_home = tmp.path().join("isolated Hakus state");
        ...
        let _hakus_home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", &isolated_home);
        ...
        assert!(
            !ambient_home.join(".hakus").exists(),
