...
use hakus_config::model_reference::ModelReferenceCard;
use hakus_config::pricing::OfferingPricing;
...
fn push_configured_provider_model(
    ...
    let Some(entry) = config.provider_config_for(provider) else {
        return;
    };
    ...
    // User-curated `[providers.<id>].models` list — every entry becomes a
    // picker row (unknown ids render as custom rows), so one provider can
    // carry several switchable models without catalog support.
...
fn offering_for_row(row: &ModelPickerRow) -> Option<hakus_config::catalog::CatalogOffering> {
