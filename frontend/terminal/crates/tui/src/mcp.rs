...
impl ReviewedPluginMcpSource {
    ...
    fn catalog_is_current(&self) -> bool {
        ...
        // source and the Hakus-owned stage before publishing any entry.
        crate::plugins::registry::verify_plugin_component_authority(
            &self.authority,
            self.required_capability(),
        )
...
impl McpPool {
    ...
    /// `.hakus/mcp.json`. Project servers override same-name global
                ...
                if let Some(conn) = pool.connections.get(name) {
                    ...
                    if snapshot.connected {
                        ...
                        hakus_telemetry::session_counters()
                            .bump(hakus_telemetry::Counter::McpServerConnected);
                    }
                    snapshot.tools = conn
                        .tools()
                        .iter()
