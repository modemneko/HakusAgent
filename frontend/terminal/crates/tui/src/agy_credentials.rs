...
//! `antigravityUnifiedStateSync.oauthToken`. Hakus may read that one
//! value from that exact file only after `hakus auth external-consent`.
//! The database is opened read-only and never written, refreshed, or
//! copied into the process environment. Hakus-owned
...
use hakus_config::{ExternalCredentialReadGrant, ExternalCredentialSource};
...
///
/// 1. Hakus-owned `ANTIGRAVITY_API_KEY` (config table or environment)
...
pub(crate) fn antigravity_oauth_token_from_grant(
    ...
    let metadata = file
        .metadata()
        .with_context(|| format!("statting {}", hakus_config::quote_os_path(path)))?;
...
/// The stored value is opaque to Hakus. Accept only shapes observed in
...
mod tests {
    ...
    fn grant_for(path: &Path) -> ExternalCredentialReadGrant {
        hakus_config::ExternalCredentialConsentToml::read_only(
            hakus_config::ProviderKind::Antigravity,
    ...
    fn wrong_grant_source_is_rejected() {
        ...
        let grant = hakus_config::ExternalCredentialConsentToml::read_only(
            hakus_config::ProviderKind::Antigravity,
