...
impl HookContext {
    ...
    pub fn to_env_vars(&self) -> HashMap<String, String> {
        ...
        if let Some(ref id) = self.tool_call_id {
            env.insert("HAKUS_TOOL_CALL_ID".to_string(), id.clone());
        ...
        if let Some(ref session_id) = self.session_id {
            env.insert("HAKUS_SESSION_ID".to_string(), session_id.clone());
...
impl HookExecutor {
    ...
    /// correlation for anything that grouped records by `HAKUS_SESSION_ID`
...
/// become "Hakus aborted the tool call".
...
mod tests {
    ...
    fn trust_workspace_for_project_hooks(workspace: &Path, config_path: &Path) -> EnvVarGuard {
        let guard = EnvVarGuard::set("HAKUS_CONFIG_PATH", config_path);
...
"#;
    ...
    fn turn_end_payload_contains_post_turn_observer_fields() {
        let context = HookContext::new()
            .with_session_id("sess_test")
            .with_workspace(PathBuf::from("/tmp/hakus"))
            .with_mode("agent")
        ...
        assert_eq!(payload["workspace"], "/tmp/hakus");
    ...
    fn observer_hook_receives_eof_instead_of_inheriting_terminal_stdin() {
        const INNER_ENV: &str = "HAKUS_TEST_HOOK_EOF_INNER";
        const TEST_NAME: &str =
...
"#,
    ...
    fn load_with_project_appends_project_hooks_after_global() {
        ...
        let project_dir = dir.path().join(".hakus");
        std::fs::create_dir_all(&project_dir).expect("mkdir .hakus");
        std::fs::write(
...
"#,
    ...
    fn load_with_project_ignores_project_hooks_until_workspace_trusted() {
        let _lock = lock_test_env();
        let dir = tempfile::tempdir().expect("tempdir");
        let _config = EnvVarGuard::set("HAKUS_CONFIG_PATH", dir.path().join("config.toml"));
        let _legacy_config = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
        let project_dir = dir.path().join(".hakus");
        std::fs::create_dir_all(&project_dir).expect("mkdir .hakus");
        std::fs::write(
...
"#,
    ...
    fn session_id_is_stable_across_every_event_and_survives_a_rebind() {
        // One TUI session, one `HAKUS_SESSION_ID`. The legacy
        ...
        for event in crate::hooks::ALL_HOOK_EVENTS {
            ...
            assert_eq!(
                env.get("HAKUS_SESSION_ID"),
                Some(&session_id),
                "event `{}` reported a different Hakus session id",
                event.as_str()
            );
            assert_eq!(
    ...
    fn tool_call_id_is_exported_for_correlation() {
        ...
        let without = HookContext::new()
            .with_tool_name("exec_shell")
            .to_env_vars();
        assert!(!without.contains_key("HAKUS_TOOL_CALL_ID"));
