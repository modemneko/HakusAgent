...
/// How much a hook's result can change what Hakus does next.
///
/// This is the steering allowlist. "Observer" is a statement about
/// Hakus's control flow only — an observer hook is still an arbitrary
...
pub enum HookSteering {
    ...
    ContributesShellEnv,
    /// stdout is ignored and the result cannot change Hakus's behavior.
    Observer,
}
...
impl HookEvent {
    ...
    /// Whether a hook result for this event can change Hakus's own
...
impl HooksConfig {
    /// Load global hooks merged with project-local `.hakus/hooks.toml` (#3026).
    ///
    ...
    /// Plugin files are read only from Hakus's immutable staged snapshot;
    ...
    pub fn load_with_project_and_plugins(
        ...
        let project_path = workspace.join(".hakus").join("hooks.toml");
...
"#,
    ...
    /// The steering allowlist. Exactly three events can change what Hakus
    ...
    /// Observer-only is a claim about Hakus's control flow, not about the
