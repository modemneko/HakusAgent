...
//! - `HAKUS_MODELS_DEV_URL` — base URL (appends `/catalog.json`) or a full
//!   `*.json` URL.
//! - `HAKUS_MODELS_DEV_PATH` — local file path; skips the network.
//! - `HAKUS_DISABLE_MODELS_DEV_FETCH` — when truthy, never hits the network.
...
use hakus_config::models_dev::{MODELS_DEV_CATALOG_URL, ModelsDevCatalog};
use hakus_config::persistence::atomic_write;
...
pub const ENV_MODELS_DEV_URL: &str = "HAKUS_MODELS_DEV_URL";
/// Env: load catalog JSON from a local path (skips network).
pub const ENV_MODELS_DEV_PATH: &str = "HAKUS_MODELS_DEV_PATH";
/// Env: disable network fetch entirely (`1`/`true`/`yes`/`on`).
pub const ENV_DISABLE_FETCH: &str = "HAKUS_DISABLE_MODELS_DEV_FETCH";
...
/// are: `resolve_state_dir` lives in `hakus-config`, which is compiled as a
...
/// their real `~/.hakus/catalog` and rendered against whatever models they
/// last fetched (#5359).
#[must_use]
pub fn cache_path() -> Option<PathBuf> {
    ...
    hakus_config::resolve_state_dir("catalog")
        .ok()
...
/// Force a refresh: prefer `HAKUS_MODELS_DEV_PATH`, else network fetch.
///
...
pub(crate) fn offerings_from_json_for_test(
    body: &str,
) -> Result<Vec<hakus_config::catalog::CatalogOffering>, String> {
...
mod tests {
    ...
    use hakus_config::catalog::CatalogSource;
    ...
    ///
    /// `resolve_state_dir` lives in `hakus-config`, which is a plain
    ...
    fn guarded_cache_path_follows_the_sealed_home() {
        ...
        let _guard = EnvVarGuard::set("HAKUS_HOME", home.path());

        let path = cache_path().expect("cache path");

        assert!(
            path.starts_with(home.path()),
            "sealed HAKUS_HOME was ignored: {}",
            path.display()
        );
    }
    ...
    fn publish_from_path_updates_provider_lake() {
        ...
        let _home = EnvVarGuard::set("HAKUS_HOME", dir.path().join("home"));
    ...
    fn stale_disk_cache_still_publishes() {
        ...
        let _home = EnvVarGuard::set("HAKUS_HOME", &home);
