...
use hakus_config::route::{LogicalModelRef, RouteRequest, RouteResolver};
...
pub(crate) fn credential_state_for_provider(
    ...
    if uses_kimi_imported_token {
        // Kimi remains API-key-only until Hakus has its own registered
...
pub(crate) fn route_is_valid_for_model(
    ...
    let compatibility_kind =
        (provider == ApiProvider::DeepseekCN).then_some(hakus_config::ProviderKind::Deepseek);
...
mod tests {
    ...
    fn disabled_external_imports_are_not_probed_by_readiness() {
        let _lock = crate::test_support::lock_test_env();
        let temp = tempfile::tempdir().expect("oauth fixture root");
        let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", temp.path());
        ...
        let _cli_key =
            crate::test_support::EnvVarGuard::set("HAKUS_CLI_API_KEY", "explicit-cli-key");
            ...
            providers: Some(crate::config::ProvidersConfig {
                xai: crate::config::ProviderConfig {
                    auth: Some(hakus_config::ProviderAuthSourceToml {
                        ...
                        command: Vec::new(),
                        timeout_ms: None,
                        secret_id: Some("hakus/xai".to_string()),
                    }),
                    ..Default::default()
                },
                ..Default::default()
...
"#,
        ...
        .expect("write config");
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", temp.path());
        let _provider = crate::test_support::EnvVarGuard::set("HAKUS_PROVIDER", "deepseek");
        ...
        let _model =
            crate::test_support::EnvVarGuard::set("HAKUS_MODEL", "anthropic/private-model");
