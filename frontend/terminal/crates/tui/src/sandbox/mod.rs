...
impl SandboxManager {
    ...
    fn prepare_seatbelt(spec: &CommandSpec) -> ExecEnv {
        ...
        env.insert("HAKUS_SANDBOX".to_string(), "seatbelt".to_string());
    ...
    fn prepare_bwrap(&self, spec: &CommandSpec) -> ExecEnv {
        ...
        let mut env = spec.env.clone();
        env.insert("HAKUS_SANDBOX".to_string(), "bwrap".to_string());
...
mod tests {
    ...
    #[test]
    #[cfg(target_os = "macos")]
    fn sandbox_child_env_exports_hakus_marker_and_legacy_alias() {
        ...
        assert_eq!(
            env.env.get("HAKUS_SANDBOX").map(String::as_str),
            Some("seatbelt")
        );
        assert_eq!(
    ...
    fn test_parity_manager_default_no_bwrap() {
        ...
        #[cfg(all(target_os = "linux", not(target_env = "ohos")))]
        {
            let primary_marker = env.env.get("HAKUS_SANDBOX");
    ...
    fn test_parity_manager_with_bwrap() {
        ...
        #[cfg(all(target_os = "linux", not(target_env = "ohos")))]
        {
            if crate::sandbox::bwrap::is_available() {
                ...
                assert_eq!(env.sandbox_type, SandboxType::None);
                assert!(!env.env.contains_key("HAKUS_SANDBOX"));
