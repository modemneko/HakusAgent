...
impl SandboxPolicy {
    ...
    pub fn posture_label_with_enforcement(&self, enforcement: SandboxEnforcement) -> String {
        if enforcement == SandboxEnforcement::ExternalBackend {
            ...
            return format!(
                "{requested_policy} policy (external execution backend configured; filesystem/network isolation unverified by Hakus)"
            );
        }
...
mod tests {
    ...
    fn enforcement_labels_distinguish_local_external_and_unavailable() {
        ...
        assert!(
            external.contains("isolation unverified by Hakus"),
            "{external}"
        );
        assert!(
