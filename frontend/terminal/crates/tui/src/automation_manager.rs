...
///
/// Stored under `~/.hakus/automations/triggers/{trigger_id}.json`.
...
mod tests {
    ...
    #[test]
    fn default_automations_dir_honors_hakus_home_as_hard_override() {
        ...
        // $HAKUS_HOME IS the home dir (no ".hakus" appended); the
    ...
    #[test]
    fn default_automations_dir_prefers_deepseek_automations_dir_over_hakus_home() {
        let _lock = crate::test_support::lock_test_env();
        let tmp = tempfile::TempDir::new().unwrap();
        // SAFETY: serialised by lock_test_env.
        unsafe {
            std::env::set_var("DEEPSEEK_AUTOMATIONS_DIR", tmp.path());
            std::env::set_var("HAKUS_HOME", "/should/not/be/used");
        }
