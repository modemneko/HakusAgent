...
fn offline_doctor_loads_never_materialize_secret_environment_overrides() {
    ...
    let _profile = crate::test_support::EnvVarGuard::remove("HAKUS_PROFILE");
    let _legacy_profile = crate::test_support::EnvVarGuard::remove("DEEPSEEK_PROFILE");
    let _managed = crate::test_support::EnvVarGuard::remove("HAKUS_MANAGED_CONFIG_PATH");
    let _legacy_managed = crate::test_support::EnvVarGuard::remove("DEEPSEEK_MANAGED_CONFIG_PATH");
    let _requirements = crate::test_support::EnvVarGuard::remove("HAKUS_REQUIREMENTS_PATH");
    ...
    let _allow_shell = crate::test_support::EnvVarGuard::set("HAKUS_ALLOW_SHELL", "false");
    ...
    for probe in ["--probe-api", "--probe-local"] {
        let cli = Cli::try_parse_from(["hakus-tui", "--config", &config_arg, "doctor", probe])
...
fn doctor_does_not_expand_mcp_environment_placeholders() {
    ...
    let _missing = crate::test_support::EnvVarGuard::remove("HAKUS_DOCTOR_MCP_MISSING_PATH");
    let mut server = make_server(Some("hakus-mcp-command"), &[], None);
    server.env.insert(
        "PATH".to_string(),
        "do-not-leak-${HAKUS_DOCTOR_MCP_MISSING_PATH}-also-secret".to_string(),
    );
    ...
    assert!(!serialized.contains("HAKUS_DOCTOR_MCP_MISSING_PATH"));
}
#[test]
...
fn doctor_startup_skips_workspace_dotenv_loading() {
    ...
    for args in [
        vec!["hakus", "doctor"],
        vec!["hakus", "doctor", "--json"],
        vec!["hakus", "doctor", "--context-json"],
        vec!["hakus", "doctor", "--probe-mcp"],
        vec!["hakus", "doctor", "--check-updates"],
    ] {
...
fn explicit_doctor_api_probes_may_load_workspace_dotenv_credentials() {
    ...
    for args in [
        ["hakus", "doctor", "--probe-api"],
        ["hakus", "doctor", "--probe-local"],
    ] {
        let phase = Cell::new(0);
        let (_cli, command) = prepare_cli_startup(
            parse_cli(&args),
...
fn resolve_api_key_source_does_not_open_standalone_secret_store() {
    ...
    let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str());
    ...
    let secret_path = hakus_home.join("secrets").join("secrets.json");
...
fn resolve_api_key_source_does_not_probe_system_keyring() {
    ...
    let _home = crate::test_support::EnvVarGuard::set(
        "HAKUS_HOME",
        temp.path().join("hakus-home").as_os_str(),
    );
    let _backend = crate::test_support::EnvVarGuard::set("HAKUS_SECRET_BACKEND", "system");
    let _deepseek_key = crate::test_support::EnvVarGuard::remove("DEEPSEEK_API_KEY");
    let _deepseek_source = crate::test_support::EnvVarGuard::remove("DEEPSEEK_API_KEY_SOURCE");
...
fn credential_diagnostic_treats_sentinel_as_unprobed_store_not_config() {
    ...
    assert!(!hakus_home.join("secrets/secrets.json").exists());
...
fn unavailable_sentinel_routes_stay_distinct_from_unknown_and_unprobed() {
    ...
    let _home =
        crate::test_support::EnvVarGuard::set("HAKUS_HOME", temp.path().join("hakus-home"));
