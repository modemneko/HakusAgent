...
use hakus_config::provider::WireFormat;
use hakus_config::route::{
    LogicalModelRef, ReadyRouteCandidate, RouteLimits, RouteRequest, RouteResolver,
};
use hakus_config::{auth_mode_disables_api_key, is_upstream_auth_header};
...
const ALLOW_INSECURE_HTTP_ENV: &str = "HAKUS_ALLOW_INSECURE_HTTP";
...
fn client_user_agent(api_provider: ApiProvider) -> &'static str {
    ...
    // Hakus identity rather than impersonating a Kimi CLI.
...
fn push_file_backed_model_bound_secrets(values: &mut Vec<String>) {
    ...
    // The isolated regression below opts in with a temporary HAKUS_HOME,
    ...
    // resolver.
    let secrets = hakus_secrets::Secrets::file_backed_read_only();
...
fn redact_model_bound_text(text: &str, exact_secret_values: &[String]) -> String {
    ...
    for secret in exact_secret_values {
        redacted = redacted.replace(secret, hakus_config::persistence::REDACTED);
    }
    hakus_config::persistence::redact_secrets(&redacted)
}
    ...
    if base_url.starts_with("http://") {
        anyhow::bail!(
            "Refusing insecure base URL '{display_base_url}'.\n\
             ...
             set the env var `{ALLOW_INSECURE_HTTP_ENV}=1` in the shell that runs hakus and re-run.\n\
             \n\
             Example: `{ALLOW_INSECURE_HTTP_ENV}=1 hakus` (note the underscores).",
        );
    }
...
/// Returns true when HAKUS_FORCE_HTTP1 (legacy alias: DEEPSEEK_FORCE_HTTP1)
...
impl DeepSeekClient {
    ...
    pub fn new(config: &Config) -> Result<Self> {
        ...
        let model_aware = api_provider.metadata().is_some_and(|provider| {
            provider.wire_policy() == hakus_config::provider::WirePolicy::ModelAware
        });
        if model_aware {
    ...
    fn from_parts(
        ...
        // when H2 stalls. When HAKUS_FORCE_HTTP1 is set, both clients are
    ...
    fn build_http_client_with_auth_mode(
        ...
        if pin_http1 {
            if force_http1_from_env() && !force_http1 {
                logging::info("HAKUS_FORCE_HTTP1=1 — pinning HTTP client to HTTP/1.1");
            }
...
fn provider_wire_format_for_config(
    ...
    api_provider
        .kind()
        .and_then(|kind| {
            hakus_config::provider::provider_for_kind(kind)
                .wire_policy()
                .fixed()
        })
        .unwrap_or_else(|| {
    ...
    pub async fn fetch_catalog_delta(&self) -> Result<ProviderCatalogDelta, CatalogRefreshError> {
        ...
        let offerings: Vec<CatalogOffering> = if provider == "openrouter" {
            ...
            named_gateway_catalog_offerings_from_body(
                &body,
                hakus_config::ProviderKind::Telecomjs,
                &provider,
                &fingerprint,
                fetched_at,
            )?
            ...
            named_gateway_catalog_offerings_from_body(
                &body,
                hakus_config::ProviderKind::Edenai,
                &provider,
                &fingerprint,
                fetched_at,
            )?
...
pub(crate) fn record_provider_response(provider: crate::config::ApiProvider, status: u16) {
    ...
    if let Some(counter) = hakus_telemetry::counters::http_status_counter(status) {
...
fn retry_reason_label_and_human(err: &LlmError) -> (&'static str, String) {
    ...
    if matches!(err, LlmError::NetworkError(_) | LlmError::Timeout(_)) {
        hakus_telemetry::session_counters()
            .bump_error(hakus_telemetry::ErrorCounter::NetworkError);
    }
    match err {
...
/// one roster. Hakus's `OpencodeGo` route is intentionally Chat-only, so
...
fn named_gateway_catalog_offerings_from_body(
    body: &str,
    kind: hakus_config::ProviderKind,
    provider: &str,
    fingerprint: &str,
    fetched_at: u64,
    ...
    let bundled = hakus_config::catalog::bundled_catalog_offerings();
...
fn openrouter_to_catalog_offering(
    ...
    use hakus_config::models_dev::{ModelsDevCost, ModelsDevLimit, ModelsDevModalities};
...
mod tests {
    ...
    use hakus_protocol::runtime::DynamicToolSpec;
    use serde_json::json;
    ...
    fn openrouter_pricing_maps_cache_write_per_token_to_per_million() {
        ...
        let pricing = hakus_config::pricing::OfferingPricing::from_catalog_offering(&priced)
            .expect("priced offering");
        let write = hakus_config::pricing::TokenUsage {
        ...
        let unwritten =
            hakus_config::pricing::OfferingPricing::from_catalog_offering(&unwritten)
                .expect("priced offering");
        assert_eq!(unwritten.estimate_cost(&write), None);
        assert_eq!(
            unwritten.unpriced_used_classes(&write),
            vec![hakus_config::pricing::TokenClass::CacheWrite]
        );
    }
    ...
    fn deferred_dynamic_request_tool() -> Tool {
        let registry = ToolRegistryBuilder::new()
            ...
            .build(ToolContext::new(
                std::env::temp_dir().join("hakus-k3-deferred-capture"),
            ));
        registry
            .to_api_tools()
            .into_iter()
    ...
    fn model_bound_request_redacts_configured_secrets_and_bare_active_key() {
        ...
        assert!(content.contains(hakus_config::persistence::REDACTED));
    ...
    fn model_bound_request_redacts_inactive_file_store_and_environment_secrets() {
        ...
        let _hakus_home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", &hakus_home);
        let _secret_backend =
            crate::test_support::EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
        ...
        hakus_secrets::Secrets::file_backed()
    ...
    #[test]
    fn whitespace_hakus_home_does_not_load_ambient_redaction_secrets() {
        let _env_lock = crate::test_support::lock_test_env();
        let tmp = tempfile::tempdir().expect("tempdir");
        ...
        let _hakus_home_unset = crate::test_support::EnvVarGuard::remove("HAKUS_HOME");
        let _secret_backend =
            crate::test_support::EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
        hakus_secrets::Secrets::file_backed()
            .set("arcee", "ambient-redaction-secret-sentinel")
            .expect("seed ambient file secret store");
        let _whitespace_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", " \t ");
        let mut values = Vec::new();
    ...
    fn short_chat_tool_payload_is_redacted_before_wire_serialization() {
        ...
        assert!(serialized.contains(hakus_config::persistence::REDACTED));
    }
    ...
    fn configured_secret_redaction_reaches_all_protocol_bodies() {
        ...
        for (route, body) in [
            ...
            assert!(
                body.contains(hakus_config::persistence::REDACTED),
    ...
    async fn retrieved_turn_loop_spillover_is_sanitized_before_model_wire() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let spillover_root = tmp.path().join(".hakus").join("tool_outputs");
        ...
        assert!(wire.contains(hakus_config::persistence::REDACTED));
    }
    ...
    fn wire_adapter_does_not_persist_sessionless_sha_spillover() {
        ...
        let prior = crate::tools::truncate::set_test_spillover_root(Some(
            tmp.path().join(".hakus").join("tool_outputs"),
        ));
        struct Restore(Option<std::path::PathBuf>);
        impl Drop for Restore {
            fn drop(&mut self) {
    ...
    #[test]
    fn moonshot_uses_hakus_user_agent_not_kimi_cli_identity() {
        let user_agent = client_user_agent(ApiProvider::Moonshot);

        assert!(user_agent.contains("hakus/"));
    ...
    async fn telecomjs_live_catalog_keeps_cross_provider_metadata_unknown() {
        let server = MockServer::start().await;
        let ambiguous_id = hakus_config::catalog::bundled_catalog_offerings()
            .into_iter()
            .find(|offering| {
    ...
    async fn static_rows_survive_a_live_refresh_failure() {
        ...
        let fresh_live: Vec<CatalogOffering> = cache
            ...
            .unwrap_or_default();
        let snapshot = hakus_config::catalog::CatalogCompiler::new()
    ...
    impl AllowInsecureHttpEnvGuard {
        fn capture() -> Self {
            ...
            // the HAKUS-first fallback chain behind a test's back.
    ...
    fn outbound_seam_clamps_every_dialect_to_the_exact_route_envelope() {
        ...
        let _canonical =
            crate::test_support::EnvVarGuard::set("HAKUS_MAX_OUTPUT_TOKENS", "384000");
