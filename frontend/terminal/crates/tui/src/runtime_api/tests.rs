...
fn runtime_session_fallback_retains_non_unicode_explicit_home_boundary() {
    ...
    let explicit = tmp.path().join(std::ffi::OsString::from_vec(
        b"hakus-\xff-home".to_vec(),
    ));
    let _home = EnvVarGuard::set("HOME", &home);
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &explicit);
...
fn runtime_tui_settings_reject_legacy_modes_and_do_not_save_env_overlays() -> Result<()> {
    ...
    let settings_dir = tmp.path().join(".hakus");
...
fn workspace_status_reports_head_and_dirty_counts() -> Result<()> {
    ...
    run_test_git(
        ...
        &[
            ...
            "-c",
            "user.email=hakus@example.invalid",
            "commit",
            "-m",
            "init",
        ],
...
#[test]
fn runtime_token_environment_prefers_the_hakus_name() {
...
fn token_from_cookie_header_decodes_percent_encoded_token() {
    assert_eq!(
        token_from_cookie_header(Some(
            "theme=dark; hakus_runtime_token=abc%20ABC%2B%2F%3F%3A%3D%26%25"
        )),
        Some("abc ABC+/?:=&%".to_string())
    );
    assert_eq!(
        token_from_cookie_header(Some("hakus_runtime_token=bad%ZZ")),
        None
    );
}
...
struct TestServerOverrides {
    ...
    fleet_hakus_binary: Option<String>,
...
> {
    let root = std::env::temp_dir().join(format!("hakus-config-api-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
...
fn write_fake_fleet_binary(root: &Path, marker: &Path) -> Result<PathBuf> {
    ...
    let binary = root.join("fake-hakus");
    fs::write(
        &binary,
        format!(
...
fn write_fake_fleet_binary(root: &Path, marker: &Path) -> Result<PathBuf> {
    ...
    // Hakus binary. A `.cmd` fake introduces an extra `cmd.exe` wrapper
    ...
    let source = root.join("fake-hakus.rs");
    let binary = root.join("fake-hakus.exe");
    let helper = format!(
        r##"fn main() {{
...
"##,
    ...
    let output = std::process::Command::new(rustc)
        .arg("--edition=2024")
        .arg("--crate-name=hakus_fleet_test_helper")
        .arg(&source)
        .arg("-o")
        .arg(&binary)
        .output()
...
async fn health_and_tasks_endpoints_work() -> Result<()> {
    ...
    assert_eq!(health["service"], "hakus-runtime-api");
...
async fn mcp_tools_endpoint_is_passive_until_connect_requested() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-mcp-tools-api-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    fs::create_dir_all(&root)?;
...
async fn runtime_token_guard_protects_v1_routes() -> Result<()> {
    ...
    let cookie_token = client
        ...
        .header(
            header::COOKIE,
            format!("hakus_runtime_token={}", url_query_component(&token)),
        )
        .send()
        .await?
        .error_for_status()?;
    ...
    let hakus_header = client
        .get(format!("http://{addr}/v1/threads/summary"))
        .header("x-hakus-runtime-token", &token)
        .send()
        .await?
        .error_for_status()?;
    assert_eq!(hakus_header.status(), StatusCode::OK);
...
async fn web_bootstrap_sets_strict_cookie_once_and_preserves_v1_auth() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-web-api-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let workspace = root.join("workspace");
    ...
    let icon = client
        .get(format!("http://{addr}/assets/hakus-192.png"))
        .send()
        .await?;
    ...
    let wrong = client
        .get(format!(
            "http://{addr}/__hakus/bootstrap/cwwb_00000000000000000000000000000000"
        ))
        .send()
        .await?;
    ...
    let exchange = client
        .get(format!("http://{addr}/__hakus/bootstrap/{nonce}"))
        .send()
        .await?;
    ...
    let set_cookie = exchange
        ...
        .to_string();
    assert!(set_cookie.starts_with("hakus_web_session=cwws_"));
...
async fn web_assets_are_absent_outside_web_mode() -> Result<()> {
    ...
    for path in [
        "/",
        "/assets/hakus-web.css",
        "/assets/hakus-web.js",
        "/assets/hakus-192.png",
    ] {
...
async fn fleet_status_runtime_api_exposes_state_and_actions() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-fleet-api-{}", Uuid::new_v4()));
    let workspace = root.join("workspace");
    fs::create_dir_all(&workspace)?;
    ...
    let fake_hakus = write_fake_fleet_binary(&root, &restarted_marker)?;
    ...
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root_token_mobile_workspace_and_subagents(
            ...
            Some(fake_hakus.display().to_string()),
        )
        .await?
    else {
        return Ok(());
...
async fn agent_runs_runtime_api_exposes_persisted_worker_receipts() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-agent-runs-api-{}", Uuid::new_v4()));
    let workspace = root.join("workspace");
    fs::create_dir_all(workspace.join(".hakus/state"))?;
    ...
    fs::write(
        workspace.join(".hakus/state/subagents.v1.json"),
...
async fn compatibility_stream_exposes_and_resolves_user_input_without_answer_echo() -> Result<()> {
    ...
    let engine_task = tokio::spawn(async move {
        ...
        harness
            .tx_event
            .send(EngineEvent::ToolCallFinished {
                id: "input_compat".to_string(),
                name: "request_user_input".to_string(),
...
async fn session_resume_thread_returns_400_when_saved_custom_provider_was_removed() -> Result<()> {
    let root = std::env::temp_dir().join(format!(
        "hakus-session-removed-provider-{}",
        Uuid::new_v4()
    ));
    let sessions_dir = root.join("sessions");
    fs::create_dir_all(&sessions_dir)?;
...
async fn spawn_server_with_saved_sessions(
    ...
    let root = std::env::temp_dir().join(format!("hakus-session-routes-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let workspace = root.join("workspace");
    fs::create_dir_all(&workspace)?;
...
async fn runtime_info_reports_bind_state() -> Result<()> {
    ...
    let info: serde_json::Value = client
        ...
        .json()
        .await?;
    assert_eq!(info["service"], "hakus-runtime-api");
    assert_eq!(info["runtime_api_version"], "1.0");
    assert_eq!(info["hakus_version"], info["version"]);
    let commit = info["hakus_commit"]
        .as_str()
...
async fn mobile_page_is_available_only_when_enabled() -> Result<()> {
    ...
    assert!(html.contains("Hakus Mobile"));
...
async fn mobile_page_serves_shell_when_auth_enabled() -> Result<()> {
    ...
    let bearer = client
        ...
        .await?
        .error_for_status()?;
    assert!(bearer.text().await?.contains("Hakus Mobile"));
...
async fn mobile_insecure_mode_allows_page_and_v1_routes_without_token() -> Result<()> {
    ...
    let page = client
        ...
        .await?
        .error_for_status()?;
    assert!(page.text().await?.contains("Hakus Mobile"));
...
async fn skills_endpoint_exposes_safe_plugin_provenance_and_shared_toggle() -> Result<()> {
    ...
    let plugin_config = crate::plugins::discovery::DiscoveryConfig {
        ...
        workspace_plugins_dir: workspace.join(".hakus/plugins"),
...
#[test]
fn resolve_skills_dir_respects_hakus_only_scan() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let workspace = tmp.path();
    let agents_skills = workspace.join(".agents").join("skills");
    let hakus_skills = workspace.join(".hakus").join("skills");
    fs::create_dir_all(&agents_skills).expect("create agents skills dir");
    fs::create_dir_all(&hakus_skills).expect("create hakus skills dir");
    ...
    let expected = fs::canonicalize(&hakus_skills).expect("canonical hakus skills");
    assert_eq!(resolved, expected);
}

#[test]
fn resolve_skills_dir_preserves_explicit_dir_in_hakus_only_scan() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let workspace = tmp.path().join("workspace");
    let hakus_skills = workspace.join(".hakus").join("skills");
...
#[cfg(unix)]
#[test]
fn resolve_skills_dir_rejects_hakus_only_symlink_escaping_workspace() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let _env_lock = crate::test_support::lock_test_env();
    let _home = crate::test_support::EnvVarGuard::set("HOME", tmp.path());
    let _userprofile = crate::test_support::EnvVarGuard::set("USERPROFILE", tmp.path());
    ...
    let dothakus = workspace_root.join(".hakus");
    fs::create_dir_all(&dothakus).expect("create .hakus");
    let bad_link = dothakus.join("skills");
    std::os::unix::fs::symlink(&escape_target, &bad_link).expect("symlink");
...
async fn set_config_rejects_unknown_key_with_bad_request() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-config-unknown-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn set_config_validates_max_history_input() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-config-maxhist-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn set_config_validates_subagents_enabled_input() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-config-subenabled-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn set_config_validates_subagents_max_depth_input() -> Result<()> {
    ...
    // values with a descriptive 400 error.
    let root = std::env::temp_dir().join(format!("hakus-config-subdepth-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn set_config_with_config_path_writes_to_specified_file() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("hakus-config-path-persist-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    ...
    // clamping (Fix #1).
    let over_ceiling = u64::from(hakus_config::MAX_SPAWN_DEPTH_CEILING) + 10;
    ...
    let expected = format!(
        "max_depth = {}",
        u64::from(hakus_config::MAX_SPAWN_DEPTH_CEILING)
    );
    assert!(
...
async fn reload_config_endpoint_returns_success() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-config-reload-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
async fn api_surfaces_only_configured_model_for_custom_provider_route() -> Result<()> {
    let root = std::env::temp_dir().join(format!(
        "hakus-config-custom-provider-model-{}",
        Uuid::new_v4()
    ));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
...
async fn reload_config_reads_from_config_path_and_updates_in_memory_state() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("hakus-config-reload-path-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
    ...
    assert_eq!(
        initial_depth,
        u64::from(hakus_config::DEFAULT_SPAWN_DEPTH),
...
async fn switch_provider_without_model_arg_preserves_user_per_provider_model() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-switch-no-model-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(
        &config_file,
...
async fn switch_provider_with_explicit_model_arg_persists_model() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-switch-with-model-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(
        &config_file,
...
async fn switch_provider_rejects_unknown_provider_id() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-switch-unknown-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
...
async fn switch_provider_rejects_legacy_deepseek_cn_alias() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-switch-cn-alias-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# empty\n")?;
...
async fn switch_provider_empty_model_string_treated_as_no_override() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("hakus-switch-empty-model-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(
        &config_file,
...
async fn switch_provider_persists_provider_key_on_disk() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("hakus-switch-provider-disk-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(
        &config_file,
...
async fn reload_config_refreshes_mcp_config_path() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("hakus-config-mcp-refresh-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
...
async fn set_config_with_persist_false_does_not_write_to_disk() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-config-nopersist-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
...
async fn set_config_subagents_max_depth_below_ceiling_not_clamped() -> Result<()> {
    ...
    let root =
        std::env::temp_dir().join(format!("hakus-config-depth-noclamp-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
    ...
    let ceiling = u64::from(hakus_config::MAX_SPAWN_DEPTH_CEILING);
    let (status, body) = post_set_config(
        &client,
        &addr,
        "subagents_max_depth",
...
async fn set_config_subagents_enabled_false_persists() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-config-subfalse-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
...
async fn reload_config_with_malformed_file_returns_error() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-config-malformed-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
...
async fn reload_config_applies_multiple_persisted_keys() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-config-multi-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
...
async fn set_config_response_contains_all_expected_fields() -> Result<()> {
    ...
    let root = std::env::temp_dir().join(format!("hakus-config-shape-{}", Uuid::new_v4()));
    fs::create_dir_all(&root)?;
    let config_file = root.join("custom-config.toml");
    fs::write(&config_file, "# initial\n")?;
...
async fn cors_layer_advertises_exact_supported_headers_and_never_an_extra() -> Result<()> {
    ...
    let allowed = client
        ...
        .header(
            ...
            "authorization, content-type, accept, x-hakus-runtime-token, x-deepseek-runtime-token",
        )
        .send()
        .await?;
...
async fn thread_goal_crud_and_invalid_transition() -> Result<()> {
    ...
    let mut accrued: hakus_protocol::ThreadGoal = serde_json::from_value(created.clone())?;
...
fn fleet_receipt_json_pass_result_has_no_failure_fields() {
    use hakus_protocol::fleet::{FleetReceipt, FleetRunId, FleetTaskResult};
...
fn fleet_receipt_json_receipt_artifact_sets_evidence_available() {
    ...
    let receipt = FleetReceipt {
        ...
        artifacts: vec![FleetArtifactRef {
            ...
            path: PathBuf::from(".hakus/fleet/run-e/task-e/worker-e/receipt.json"),
...
async fn fleet_receipt_api_list_and_get_round_trip() -> Result<()> {
    ...
    use hakus_protocol::fleet::{FleetScore, FleetTaskResult};

    let root = std::env::temp_dir().join(format!("hakus-receipt-api-{}", Uuid::new_v4()));
    let workspace = root.join("workspace");
    fs::create_dir_all(&workspace)?;
...
async fn memory_list_returns_empty_for_fresh_store() -> Result<()> {
    ...
    fs::create_dir_all(&home)?;
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &home);
...
async fn mcp_server_management_crud() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-mcp-mgmt-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    fs::create_dir_all(&root)?;
...
async fn mcp_server_management_create_requires_command_or_url() -> Result<()> {
    let root =
        std::env::temp_dir().join(format!("hakus-mcp-mgmt-validation-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    fs::create_dir_all(&root)?;
...
async fn mcp_server_management_redacts_credentials() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-mcp-mgmt-redact-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    fs::create_dir_all(&root)?;
...
async fn runtime_info_advertises_mcp_server_management() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-mcp-capability-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
    let Some((addr, _runtime_threads, handle)) =
        spawn_test_server_with_root(root, sessions_dir).await?
    else {
...
/// Create a minimal skill package under `root_dir/.hakus/skills/<name>`.
...
fn create_managed_skill(root_dir: &std::path::Path, name: &str) -> Result<(PathBuf, String)> {
    let skill_dir = root_dir.join(".hakus").join("skills").join(name);
...
async fn skill_lifecycle_audit_returns_receipt_for_installed_skill() -> Result<()> {
    ...
    assert_eq!(entry["source_kind"], "hakus_managed");
...
async fn skill_lifecycle_endpoints_require_auth_when_token_is_set() -> Result<()> {
    let root = std::env::temp_dir().join(format!("hakus-skill-auth-{}", Uuid::new_v4()));
    let sessions_dir = root.join("sessions");
