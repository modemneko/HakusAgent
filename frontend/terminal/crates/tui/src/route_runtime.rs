...
fn classify_provider_route_preflight_next_step(identity_key: &str, reason: &str) -> Option<String> {
    ...
    {
        return Some(format!(
            "Run `codex login`, then retry {identity_key}; Hakus reads that official CLI login without modifying it."
        ));
    }
...
pub(crate) fn validate_unpinned_model_provider(
    ...
    let Some(owner) = hakus_config::known_foreign_model_owner(kind, model, base_url) else {
        return Ok(());
    };
    Err(format!(
...
mod tests {
    ...
    fn provider_route_preflight_missing_key_error_surfaces_reason_and_auth_step() {
        let err = anyhow::anyhow!(
            "Custom provider 'lm-studio' API key not found. Run 'hakus auth set --provider custom'."
        );
    ...
    fn provider_route_preflight_codex_oauth_errors_surface_the_right_next_step() {
        ...
        assert!(missing_formatted.contains(
            "Next step: Run `codex login`, then retry openai-codex; Hakus reads that official CLI login without modifying it."
        ));
    ...
    fn direct_moonshot_k3_route_uses_documented_1m_limits_with_provenance() {
        ...
        assert!(candidate.applied_limit_overrides().contains(
            &hakus_config::route::SourcedLimitOverride {
                field: hakus_config::route::LimitField::OutputTokens,
                value: Some(1_048_576),
                source: hakus_config::route::OverrideSource::DocumentedRouteOutputMaximum,
            }
        ));
        assert_eq!(
            crate::route_budget::route_context_window_tokens(
