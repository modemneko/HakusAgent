...
/// Append an audit event to `$HAKUS_HOME/audit.log` (or the default
/// `~/.hakus/audit.log` when no explicit CodeWhale home is configured).
///
...
///
/// The log was append-only with no bound at all: a real `~/.hakus/audit.log`
...
fn default_audit_path() -> anyhow::Result<PathBuf> {
    Ok(hakus_config::hakus_home()?.join("audit.log"))
}

/// Where audit events are written, for surfaces that point a person at the
/// full record (for example `/permissions`). `None` when no Hakus home
