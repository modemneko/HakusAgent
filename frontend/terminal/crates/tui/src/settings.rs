...
impl TuiPrefs {
    ...
    /// Save TUI preferences to `~/.hakus/tui.toml` (or a legacy file when
    ...
    fn save_to_path(&self, path: &Path) -> Result<()> {
        ...
        let body = if path.exists() {
            ...
            hakus_config::merge_and_preserve_comments(&serialized, &raw).unwrap_or_else(|e| {
...
pub struct Settings {
    ...
    /// Show the pre-session launch menu. When false, Hakus enters a new
    ...
    /// - `"cw"` (default): static typographic Hakus mark.
...
impl Settings {
    ...
    /// cross-process file lock, which covers a second Hakus process on the
    /// same home directory.
    ///
    ...
    pub fn available_settings() -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            (
                "theme",
                "UI theme: a compiled name or custom:<name> from the Hakus themes directory",
            ),
            (
                "background_color",
...
///    the `hakus_config::config_document` pattern. The process mutex says
///    nothing about a second Hakus process (a second TUI, `hakus exec`,
...
/// wedge another Hakus instance out of its settings.
...
/// Hakus process, an editor, a `cat` — can observe a half-written file and
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    //
    // The in-process mutex says nothing about a *second* Hakus process on
    // the same home directory — a second TUI, `hakus exec`, the runtime HTTP
    ...
    // on the helper below. It inherits the sealed `HOME`/`HAKUS_HOME`
    ...
    const CHILD_ROLE_ENV: &str = "HAKUS_TEST_SETTINGS_CHILD_ROLE";
    ...
    const CHILD_SIGNAL_ENV: &str = "HAKUS_TEST_SETTINGS_CHILD_SIGNAL";
    /// Where the child writes what it observed, for the parent to assert on.
    const CHILD_RESULT_ENV: &str = "HAKUS_TEST_SETTINGS_CHILD_RESULT";
    ...
    fn settings_cross_process_child_helper() {
        ...
        let inherited_home = std::env::var_os("HAKUS_HOME")
            .expect("settings child needs an inherited Hakus home");
        let _state_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", inherited_home);
    ...
    fn seal_settings_home_for_test(home: &Path) -> Vec<crate::test_support::EnvVarGuard> {
        ...
        vec![
            ...
            EnvVarGuard::set("HAKUS_HOME", home.join(".hakus")),
    ...
    fn tilix_and_terminator_cap_redraws_without_disabling_motion() {
        ...
        for (var, val) in [
            ...
            unsafe {
                ...
                #[cfg(windows)]
                std::env::set_var("WT_SESSION", "hakus-test");
            }
            let mut settings = animated_settings();
            assert!(!settings.low_motion, "default is animated");
            settings.apply_env_overrides();
    ...
    fn legacy_startup_modes_migrate_without_losing_permission_intent() {
        ...
        let _hakus_home = EnvVarRestore::set("HAKUS_HOME", &hakus_home);
    ...
    #[test]
    fn settings_path_defaults_to_hakus_home_for_new_writes() {
        let _g = config_path_test_guard();
        let tmp = tempfile::tempdir().expect("tempdir");
        let _config_override = EnvVarRestore::remove("DEEPSEEK_CONFIG_PATH");
        let _hakus_home = EnvVarRestore::set("HAKUS_HOME", tmp.path().join(".hakus"));
        let _home = EnvVarRestore::set("HOME", tmp.path());

        let got = Settings::path().expect("settings path");

        assert_eq!(got, tmp.path().join(".hakus").join("settings.toml"));
    }

    #[test]
    fn settings_path_prefers_hakus_home_even_when_legacy_exists() {
        let _g = config_path_test_guard();
        let tmp = tempfile::tempdir().expect("tempdir");
    ...
    fn settings_load_ignores_legacy_deepseek_home_without_explicit_home() {
        ...
        let _config_override = EnvVarRestore::remove("DEEPSEEK_CONFIG_PATH");
        let _hakus_home = EnvVarRestore::remove("HAKUS_HOME");
        let _home = EnvVarRestore::set("HOME", tmp.path());
        ...
        assert!(
            ...
            "settings display should surface the canonical hakus path:\n{display}"
        );
    }
    ...
    fn settings_load_ignores_platform_legacy_fallback_without_explicit_home() {
        ...
        let _config_override = EnvVarRestore::remove("DEEPSEEK_CONFIG_PATH");
        let _hakus_home =
            EnvVarRestore::set("HAKUS_HOME", primary.parent().expect("primary parent"));
    ...
    #[test]
    fn settings_load_ignores_legacy_files_when_hakus_home_is_explicit() {
        let _g = config_path_test_guard();
        let tmp = tempfile::tempdir().expect("tempdir");
        ...
        assert_eq!(
            loaded.theme, "system",
            "explicit HAKUS_HOME must not inherit ambient legacy settings"
        );
        assert_eq!(
        ...
        assert!(
            ...
            "ambient legacy settings must not be migrated into explicit HAKUS_HOME"
        );
    }
    ...
    #[test]
    fn tui_prefs_path_defaults_to_hakus_home_for_new_writes() {
        let _g = config_path_test_guard();
        let tmp = tempfile::tempdir().expect("tempdir");
        let _config_override = EnvVarRestore::remove("DEEPSEEK_CONFIG_PATH");
        let _hakus_home = EnvVarRestore::set("HAKUS_HOME", tmp.path().join(".hakus"));
        ...
        assert_eq!(got, tmp.path().join(".hakus").join("tui.toml"));
    }

    #[test]
    fn tui_prefs_path_ignores_legacy_home_when_hakus_home_is_explicit() {
        let _g = config_path_test_guard();
        let tmp = tempfile::tempdir().expect("tempdir");
        let explicit_home = tmp.path().join("isolated-hakus");
        let legacy_dir = tmp.path().join(".deepseek");
    ...
    #[test]
    fn tui_prefs_path_uses_home_hakus_subdir_by_default() {
        let _g = config_path_test_guard();
        let tmp = tempfile::tempdir().expect("tempdir");
        let _config_override = EnvVarRestore::remove("DEEPSEEK_CONFIG_PATH");
        let _hakus_home = EnvVarRestore::set("HAKUS_HOME", tmp.path().join(".hakus"));
