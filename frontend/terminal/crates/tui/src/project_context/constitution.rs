//! `.hakus/constitution.json` — the Hakus-specific repo authority and
//! prioritization policy. This module owns discovery (workspace upward to the
//! git root), parsing, the rendered `<hakus_repo_constitution>` authority
...
/// Hakus-specific repo authority/prioritization policy.
const REPO_CONSTITUTION_RELATIVE_PATH: &[&str] = &[".hakus", "constitution.json"];

/// `schema_version` understood by this build of the constitution loader.
const SUPPORTED_CONSTITUTION_SCHEMA: u32 = 1;

/// Hakus-specific repo authority/prioritization policy, loaded from
/// `.hakus/constitution.json`. All fields are optional so a minimal file
...
impl RepoConstitution {
    ...
    fn render_block(&self, source: &Path) -> String {
        ...
        format!(
            "<hakus_repo_constitution source=\"{}\">\nHakus-specific repo authority policy (local law: subordinate to the global Constitution and the current user request, but above memory and old handoffs; WHALE.md is ignored and should be migrated, not treated as law).\n\n{}</hakus_repo_constitution>",
            source.display(),
            body.trim_end()
        )
    }
...
/// Discover and render `.hakus/constitution.json` from `workspace` or, if
