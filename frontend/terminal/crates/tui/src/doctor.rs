...
impl DoctorPathReport {
    pub(crate) fn resolve(config_override: Option<&Path>) -> Result<Self> {
        let home = hakus_paths::hakus_home()
            .map_err(anyhow::Error::new)?
            .context("could not resolve the canonical Hakus state root")?;
        let config = match config_override {
            Some(path) => hakus_config::resolve_config_path(Some(path.to_path_buf()))
                .context("could not normalize the explicit config path")?,
            None => hakus_config::resolve_config_path(None)
                .unwrap_or_else(|_| home.join(hakus_config::CONFIG_FILE_NAME)),
        };
        let settings = crate::settings::Settings::path()
            .context("could not resolve the canonical settings path")?;
        let sessions = hakus_config::resolve_state_dir("sessions")
...
pub(crate) fn secret_backend_human_lines(
    diagnostic: &hakus_secrets::SecretBackendDiagnostic,
) -> Vec<String> {
    use hakus_secrets::{
...
pub(crate) async fn doctor_search_probe(
    ...
    let client = match crate::tls::reqwest_client_builder()
        ...
        .user_agent(concat!("hakus-doctor/", env!("CARGO_PKG_VERSION")))
        .build()
    {
        Ok(client) => client,
        Err(_) => {
...
pub(crate) fn doctor_search_probe_lines(report: &DoctorSearchProbeReport) -> Vec<String> {
    ...
    match report {
        DoctorSearchProbeReport::NotChecked => vec![
            ...
            "Run `hakus doctor --probe-search` to test the selected provider authority."
                .to_string(),
        ],
...
fn doctor_update_report<E>(
    ...
    match hakus_release::compare_release_versions(current_version, &latest) {
...
fn doctor_update_report_lines(report: &DoctorUpdateReport) -> Vec<String> {
    match report {
        DoctorUpdateReport::NotChecked => vec![
            ...
            "Run `hakus doctor --check-updates` to opt in.".to_string(),
        ],
        ...
        DoctorUpdateReport::ReleaseMetadataInvalid => vec![
            ...
            "Run `hakus update --check` to retry.".to_string(),
        ],
...
pub(crate) async fn print_update_report(probes: DoctorProbeRequest) {
    ...
    let report = if probes.should_check_updates() {
        doctor_update_report(
            current_version,
            hakus_release::latest_release_tag_async(hakus_release::ReleaseChannel::Stable)
                .await,
        )
    } else {
