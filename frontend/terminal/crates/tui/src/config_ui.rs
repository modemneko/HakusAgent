...
/// One editable `[providers.<id>]` entry. Empty `api_key`/`base_url` clears
/// the stored value; `models` is the user-curated picker list.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Default)]
#[serde(rename_all = "snake_case")]
pub struct ProviderEditableEntry {
    #[schemars(
        title = "API key",
        description = "Provider credential. Leave empty to clear."
    )]
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub api_key: Option<String>,
    #[schemars(
        title = "Base URL",
        description = "Optional endpoint override (OpenAI-compatible). Leave empty for the provider default."
    )]
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub base_url: Option<String>,
    #[schemars(
        title = "Models",
        description = "Model ids for this provider, surfaced in the /model picker. Unknown ids render as custom rows."
    )]
    #[serde(default)]
    pub models: Vec<String>,
}

/// Provider management: edit credentials of built-in providers (by id:
/// deepseek, openai, ...) and define custom OpenAI-compatible providers by
/// adding a new id.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Default)]
#[serde(rename_all = "snake_case")]
pub struct ProvidersSection {
    #[schemars(
        title = "Providers",
        description = "Provider id → credentials + model list. Built-in ids edit that provider; any new id becomes a custom OpenAI-compatible provider (set Base URL)."
    )]
    #[serde(default)]
    pub providers: BTreeMap<String, ProviderEditableEntry>,
}
...
pub struct SettingsSection {
    ...
    #[schemars(
        ...
        description = "Theme slug from the fixed Hakus themes directory; used only when theme is custom."
    )]
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub fn parse_mode(arg: Option<&str>) -> Result<ConfigUiMode, String> {
    ...
    // of the hakus-tui navy chrome out of the box. Power users can
...
pub fn build_document(app: &App, config: &Config) -> Result<ConfigUiDocument> {
    ...
    Ok(ConfigUiDocument {
        providers: build_providers_section(config),
...
/// Snapshot `[providers.*]` (built-ins + named custom tables) into the
/// editable document section. Entries that carry nothing are omitted.
fn build_providers_section(config: &Config) -> ProvidersSection {
    let mut providers = BTreeMap::new();
    fn push_entry(
        providers: &mut BTreeMap<String, ProviderEditableEntry>,
        id: &str,
        entry: &crate::config::ProviderConfig,
    ) {
        let api_key = entry.api_key.clone().filter(|v| !v.trim().is_empty());
        let base_url = entry.base_url.clone().filter(|v| !v.trim().is_empty());
        let models: Vec<String> = entry
            ...
            .filter(|m| !m.is_empty())
            .collect();
        if api_key.is_none() && base_url.is_none() && models.is_empty() {
            return;
        }
        providers.insert(
            id.to_string(),
            ProviderEditableEntry {
                api_key,
                base_url,
                models,
            },
    ...
    for provider in crate::config::ApiProvider::all() {
        if let Some(entry) = config.provider_config_for(*provider) {
            push_entry(&mut providers, provider.as_str(), entry);
        }
    }
    // Named custom providers live in the flatten map under their literal id.
    if let Some(tables) = config.providers.as_ref().map(|p| &p.custom) {
        for (id, entry) in tables {
            push_entry(&mut providers, id, entry);
        }
    }
...
/// Apply the providers section: mutate the in-memory config and, when
/// persisting, write `[providers.<id>].{api_key,base_url,models}` into
/// config.toml. Built-in ids hit their typed slot; any other id is written
/// as a named custom table.
fn apply_providers_section(
    ...
    for (id, entry) in &section.providers {
        if !valid_provider_id(id) {
            bail!("provider id `{id}` must be letters, digits, '-', '_', or '.'");
        }
        // In-memory mirror first, so the live session sees the change even
        // before the on-disk reload.
        let cleaned = crate::config::ProviderConfig {
            api_key: entry.api_key.clone().filter(|v| !v.trim().is_empty()),
            base_url: entry.base_url.clone().filter(|v| !v.trim().is_empty()),
            models: entry
                .models
                .iter()
                .map(|m| m.trim().to_string())
        ...
        match crate::config::ApiProvider::parse(id) {
            Some(provider) => {
                let slot = config.provider_config_for_mut(provider);
                slot.api_key = cleaned.api_key.clone();
                slot.base_url = cleaned.base_url.clone();
                slot.models = cleaned.models.clone();
            }
            None => {
                let tables = config
                    .providers
                    ...
                    .entry(id.clone())
                    .or_default();
                tables.api_key = cleaned.api_key.clone();
                tables.base_url = cleaned.base_url.clone();
                tables.models = cleaned.models.clone();
            }
        }
        if persist {
            let path =
                crate::config_persistence::config_toml_path(app.config_path.as_deref())?;
            let api_key = cleaned.api_key.clone();
            let base_url = cleaned.base_url.clone();
            let models = cleaned.models.clone();
            hakus_config::mutate_config_document(&path, |doc| {
                let base = ["providers", id.as_str()];
                match api_key.as_deref() {
                    Some(key) => hakus_config::set_config_document_value(
                        doc,
                        &[base[0], base[1], "api_key"],
                        key.to_string(),
                    )?,
                    None => {
                ...
                match base_url.as_deref() {
                    Some(url) => hakus_config::set_config_document_value(
                        doc,
                        &[base[0], base[1], "base_url"],
                        url.to_string(),
                    )?,
                    None => {
                        hakus_config::unset_config_document_value(
                ...
                if models.is_empty() {
                    hakus_config::unset_config_document_value(
                        doc,
                        &[base[0], base[1], "models"],
                    )?;
                } else {
                    let array = toml_edit::Value::Array(
                        models.iter().map(|m| toml_edit::Value::from(m.as_str())).collect(),
                    );
            ...
            notes.push(format!("provider `{id}` credentials/models saved"));
        }
    }
    Ok(notes)
}
...
pub fn build_schema() -> Value {
    ...
    schema["title"] = Value::String("Hakus Config".to_string());
...
pub fn apply_document(
    ...
    // Providers section runs before the generic key loop so provider
    // credentials are on disk by the time the config reload below runs.
    let provider_summary = {
        let notes = apply_providers_section(&doc.providers, app, config, persist)?;
        if notes.is_empty() {
            None
        } else {
            Some(notes.join(", "))
    ...
    // Providers summary goes last so it survives the final-message cut.
...
mod tests {
    ...
    fn persisted_config_ui_reasoning_updates_the_startup_precedence_layer() {
        ...
        let temp_root = tempfile::tempdir().expect("isolated Hakus home");
        let hakus_home = temp_root.path().join(".hakus");
        fs::create_dir_all(&hakus_home).expect("settings dir");
        fs::write(
        ...
        let _home = EnvVarGuard::set("HAKUS_HOME", &hakus_home);
...
"#,
    ...
    fn build_document_reflects_background_color_from_settings() {
        ...
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let temp_root = std::env::temp_dir().join(format!(
            "hakus-config-ui-background-color-{}-{}",
            std::process::id(),
            nanos
        ));
        fs::create_dir_all(temp_root.join(".deepseek")).expect("config dir");
...
"##,
    ...
    fn build_document_accepts_every_shipped_locale_from_settings() {
        ...
        let hakus_home = temp_root.path().join(".hakus");
        fs::create_dir_all(&hakus_home).expect("settings dir");
        let settings_path = hakus_home.join("settings.toml");
    ...
    fn custom_theme_round_trips_through_typed_config_document() {
        ...
        let temp_root = tempfile::tempdir().expect("isolated Hakus home");
        let hakus_home = temp_root.path().join(".hakus");
        let themes_dir = hakus_home.join("themes");
...
"#,
    ...
    fn schema_contains_typed_enums() {
        ...
        assert_eq!(schema["title"], serde_json::json!("Hakus Config"));
        assert!(
            schema["description"]
                .as_str()
...
"#,
    ...
    #[test]
    fn providers_section_persists_credentials_and_models() {
        let _lock = lock_test_env();
        let dir = tempfile::tempdir().expect("isolated config dir");
        ...
        doc.providers.providers.insert(
            "deepseek".to_string(),
            ProviderEditableEntry {
                api_key: Some("sk-test-123".to_string()),
                base_url: None,
                models: vec!["deepseek-v4-pro".into(), "my-custom-model".into()],
            },
        );
        doc.providers.providers.insert(
            "my-gateway".to_string(),
            ProviderEditableEntry {
                api_key: Some("sk-gw".to_string()),
                base_url: Some("https://gw.example.com/v1".to_string()),
                models: vec!["gateway-model-a".into()],
            },
        );

        let outcome = apply_document(doc, &mut app, &mut config, true).expect("apply");
        assert!(outcome.final_message.contains("deepseek"));
        assert!(outcome.final_message.contains("my-gateway"));

        let raw = std::fs::read_to_string(&config_file).expect("config.toml written");
        assert!(raw.contains("[providers.deepseek]"));
        assert!(raw.contains("sk-test-123"));
        assert!(raw.contains("\"my-custom-model\""));
        assert!(raw.contains("[providers.my-gateway]"));
        assert!(raw.contains("https://gw.example.com/v1"));

        // Reload from disk: built-in slot + named custom table both carry it.
        let reloaded = build_providers_section(
            &Config::load(Some(config_file.clone()), None).expect("reload"),
        );
        let deepseek = reloaded.providers.get("deepseek").expect("deepseek entry");
        assert_eq!(deepseek.api_key.as_deref(), Some("sk-test-123"));
        assert_eq!(deepseek.models, vec!["deepseek-v4-pro", "my-custom-model"]);
        let gateway = reloaded.providers.get("my-gateway").expect("custom entry");
        assert_eq!(gateway.base_url.as_deref(), Some("https://gw.example.com/v1"));
        assert_eq!(gateway.models, vec!["gateway-model-a"]);
    }
}
