...
pub trait Command: Send + Sync {
    ...
    fn contextual_handler(
        &self,
    ) -> Option<hakus_command_contract::handler::CommandHandler<CommandResult>> {
        None
    }
}
...
pub(crate) struct ContextualCommand {
    info: &'static CommandInfo,
    handler: Option<hakus_command_contract::handler::CommandHandler<CommandResult>>,
...
impl ContextualCommand {
    ...
    pub(crate) const fn contextual(
        info: &'static CommandInfo,
        handler: hakus_command_contract::handler::CommandHandler<CommandResult>,
    ) -> Self {
        Self {
            info,
    ...
    where
        C: hakus_command_contract::metadata::RegisterCommand<CommandResult>,
    {
        let portable = C::info();
