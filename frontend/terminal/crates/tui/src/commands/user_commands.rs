//! User-defined slash commands from `~/.hakus/commands/<name>.md` and
//! workspace-local `<workspace>/.hakus/commands/<name>.md`.
//!
...
pub(crate) fn workflow_dirs(workspace: Option<&Path>) -> Vec<PathBuf> {
    let mut dirs = Vec::new();
    if let Some(ws) = workspace {
        dirs.push(ws.join(".hakus").join("workflows"));
    }
    let home = crate::config::effective_home_dir().unwrap_or_else(|| PathBuf::from("~"));
    dirs.push(home.join(".hakus").join("workflows"));
    dirs
}
...
fn synthesize_workflow_command(name: &str, description: &str, path: &Path) -> String {
    ...
    format!(
        "---\ndescription: {description}\nusage: /{name} [args...]\narguments: forwarded to the workflow run's args\n---\nStart the saved workflow `{name}` now: call the `workflow` tool with action=\"start\", source_path=\"{path}\", and args built from this argument text: $ARGUMENTS\nIf the argument text is empty, start the run without args. Report the run_id, monitor with the workflow tool's status action, and when the run settles present its receipt summary (status, phases, failures, artifacts). The durable report lands under .hakus/reports/<run_id>.md.",
        path = path.display(),
    )
}
...
mod tests {
    ...
    #[test]
    fn test_global_commands_dir_contains_hakus_commands() {
        ...
        assert!(
            parts
                .windows(2)
                .any(|pair| pair == [".hakus", "commands"]),
            "expected .hakus/commands components in path, got: {}",
            dir.display()
        );
    }
    ...
    fn load_user_commands_scans_workspace_local_dir() {
        ...
        let ws = tmp.path();
        let cmds_dir = ws.join(".hakus").join("commands");
    ...
    fn workspace_local_shadows_global_by_name() {
        ...
        assert_eq!(
            ...
            "workspace-local (.hakus) must shadow later dirs"
        );
    }
    ...
    fn new_user_command_clears_stale_paused_state() {
        ...
        let tmp = TempDir::new().unwrap();
        let ws = tmp.path().to_path_buf();
        let commands_dir = ws.join(".hakus").join("commands");
        write_command(
            &commands_dir,
            "pause-scan",
            "---\ndescription: Scan repos\npausable: true\n---\nscan",
