...
//!
//! Uses the Hakus changelog embedded at compile time. With no argument,
...
const HAKUS_CHANGELOG: &str = include_str!("../../../../CHANGELOG.md");
...
pub fn change(app: &mut App, version: Option<&str>) -> CommandResult {
    let section = if let Some(ver) = version {
        let ver = ver.trim();
        if ver.is_empty() {
            extract_latest_changelog_section(HAKUS_CHANGELOG)
        } else {
            extract_changelog_section_by_version(HAKUS_CHANGELOG, ver)
        }
    } else {
        extract_latest_changelog_section(HAKUS_CHANGELOG)
    };
    ...
    let latest_section = match section {
        ...
        None => {
            let msg = if let Some(ver) = version {
                let ver = ver.trim();
                if ver.is_empty() {
                    "Could not find a version section in the bundled Hakus changelog. \
                     Expected a line starting with `## [`."
                        .to_string()
                } else {
                    format!("Could not find version \"{ver}\" in the bundled Hakus changelog.")
                }
            } else {
                "Could not find a version section in the bundled Hakus changelog. \
                 Expected a line starting with `## [`."
    ...
    let prev_hint = if let Some(prev_ver) = previous_version_hint(HAKUS_CHANGELOG, version) {
...
fn inline_changelog_section(section: &str) -> String {
    ...
    format!(
        "{truncated}\n\
\n\
[... {} characters omitted from the bundled Hakus changelog]",
...
";
    ...
    fn change_uses_bundled_release_notes_without_workspace_changelog() {
        ...
        let expected = extract_latest_changelog_section(HAKUS_CHANGELOG)
    ...
    fn change_in_non_english_also_sends_translation_action() {
        for (locale, _label) in [
            ...
            if let Some(AppAction::SendMessage(prompt)) = &result.action {
                ...
                let prev_ver = extract_previous_version_number(HAKUS_CHANGELOG)
    ...
    fn change_in_non_english_without_api_key_uses_explicit_fallback() {
        ...
        let _hakus_provider = EnvVarGuard::remove("HAKUS_PROVIDER");
...
Older release.\n";
    ...
    fn change_with_explicit_version_includes_previous_hint() {
        ...
        let explicit = extract_previous_version_number(HAKUS_CHANGELOG)
            .expect("bundled changelog should have a previous release");
        let expected_prev =
            extract_previous_version_number_after_version(HAKUS_CHANGELOG, &explicit)
