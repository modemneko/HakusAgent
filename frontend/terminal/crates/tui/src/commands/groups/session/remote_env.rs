...
const HOSTED_WORK_URL: &str = "https://app.hakus.net/work";
...
mod tests {
    ...
    fn open_builds_encoded_launcher_url_without_embedded_credentials() {
        ...
        assert_eq!(
            url,
            "https://app.hakus.net/work?repo=Hmbown%2FCodeWhale&branch=feature%2Fmobile%26cloud-%7Burl%7D"
        );
        assert!(
            result
                .message
    ...
    fn supported_https_and_ssh_origins_normalize_to_namespace_and_repo() {
        for (origin, expected) in [
            ...
            ("https://cnb.cool/whale/hakus.git", "whale/hakus"),
            (
                "ssh://git@cnb.cool:2222/whale/hakus.git",
                "whale/hakus",
            ),
            ("git@cnb.cool:whale/hakus.git", "whale/hakus"),
        ] {
            assert_eq!(
    ...
    fn opening_requires_a_symbolic_branch() {
        ...
        let identity = Command::new("git")
            .arg("-C")
            .arg(temp.path())
            .args(["config", "user.name", "Hakus Test"])
            .status()
        ...
        let identity = Command::new("git")
            .arg("-C")
            .arg(temp.path())
            .args(["config", "user.email", "test@hakus.invalid"])
            .status()
