...
fn exported_markdown_contains_active_transcript(world: &mut SessionCommandWorld) {
    ...
    assert!(content.contains("# Hakus conversation export"));
...
fn hakus_defers_session_loaded_receipt(world: &mut SessionCommandWorld) {
    assert_eq!(
...
fn hakus_rejects_empty_fork(world: &mut SessionCommandWorld) {
    assert_eq!(
...
fn hakus_reports_one_session_pruned(world: &mut SessionCommandWorld) {
    let message = world
        .last_message
        .as_deref()
...
fn hakus_triggers_context_compaction(world: &mut SessionCommandWorld) {
    assert_eq!(
        world.last_result_is_error,
        Some(false),
...
fn hakus_triggers_context_purge(world: &mut SessionCommandWorld) {
    assert_eq!(
        world.last_result_is_error,
        Some(false),
...
fn hakus_sends_session_relay_instruction_focused_on(
...
fn hakus_rejects_unknown_session_command(world: &mut SessionCommandWorld) {
    assert_eq!(
        world.last_result_is_error,
        Some(true),
