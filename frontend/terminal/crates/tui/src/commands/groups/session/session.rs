...
mod tests {
    ...
    fn test_save_with_default_path_uses_managed_sessions_dir() {
        let tmpdir = TempDir::new().unwrap();
        let _lock = crate::test_support::lock_test_env();
        // Set HAKUS_HOME so the managed sessions directory lands inside the
        ...
        let hakus_home = EnvVarGuard::set("HAKUS_HOME", &home);
        let previous_hakus_home = hakus_home.previous();
        let mut app = create_test_app_with_tmpdir(&tmpdir);
        ...
        assert_eq!(std::env::var_os("HAKUS_HOME"), previous_hakus_home);
    }
