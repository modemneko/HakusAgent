...
fn render_permissions(plugin: &LoadedPlugin) -> String {
    ...
    format!(
        "filesystem_roots=[{filesystem}] network_hosts=[{network}] (exact allowlist for Hakus-managed remote requests; redirects stay same-origin) lifecycle_mutation={} stdio_runtime=[{stdio_authority}]",
