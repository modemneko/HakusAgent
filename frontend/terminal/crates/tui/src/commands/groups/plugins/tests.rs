...
fn write_bundle(root: &Path) {
    let bundle = root.join(".hakus/plugins/demo");
...
fn write_mcp_review_bundle(root: &Path) {
    let bundle = root.join(".hakus/plugins/review-mcp");
...
fn bare_plugin_command_opens_unified_extensions_modal() {
    ...
    let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", root.path().join("home"));
...
fn list_show_validate_are_read_only_and_label_legacy_tools() {
    let _lock = crate::test_support::lock_test_env();
    let root = TempDir::new().unwrap();
    let hakus_home = root.path().join("home");
    ...
    .unwrap();
    let state_path = hakus_home.join("plugins/state.json");
...
fn suggest_ranks_installed_plugins_without_trusting_or_enabling_them() {
    ...
    assert!(!hakus_home.join("plugins/state.json").exists());
...
fn write_mixed_bundle(root: &Path) {
    let bundle = root.join(".hakus/plugins/mixed");
    fs::create_dir_all(bundle.join("skills/hello")).unwrap();
...
fn install_update_uninstall_verbs_drive_the_guided_trust_flow() {
    ...
    runtime.block_on(async {
        ...
        assert!(hakus_home.join("plugins/installed-demo").exists());
        ...
        let raw = fs::read_to_string(hakus_home.join("plugins/state.json")).unwrap();
...
fn kimi_managed_import_is_read_only_until_hash_bound_approval() {
    ...
    assert!(!hakus_home.join("plugins/kimi-demo").exists());
    assert!(!hakus_home.join("plugins/state.json").exists());
...
fn export_verb_writes_agent_plugins_bundle() {
    ...
    assert!(
        root.path()
            .join(".hakus/plugins/demo/plugin.toml")
            .exists()
    );
}
