...
fn marketplace_state_path(hakus_home: &Path) -> std::path::PathBuf {
    hakus_home.join("plugins/marketplaces.json")
}
...
fn marketplace_add_list_show_remove_roundtrip() {
    ...
    assert!(marketplace_state_path(&hakus_home).exists());
    ...
    let before = fs::read_to_string(marketplace_state_path(&hakus_home)).unwrap();
    plugins(&mut app, Some("marketplace list"));
    plugins(&mut app, Some("marketplace show kimi"));
    let after = fs::read_to_string(marketplace_state_path(&hakus_home)).unwrap();
    assert_eq!(
        before, after,
...
fn marketplace_add_rejects_symlinks_and_bad_documents() {
    ...
    let store_path = marketplace_state_path(&hakus_home);
...
fn marketplace_codex_installed_by_default_never_auto_installs() {
    ...
    assert!(!hakus_home.join("plugins/defaulted-thing").exists());
