...
const WORKFLOW_CONFIRM_INSTRUCTION_PREFIX: &str = "[hakus.workflow-confirm.v1]";
...
fn workflow_status(app: &App, run_id: &str) -> CommandResult {
    ...
    if runs.len() > 20 {
        lines.push(format!(
            "… {} older runs in .hakus/workflow-runs.jsonl",
            runs.len() - 20
        ));
    }
...
mod tests {
    ...
    fn workflow_status_and_cancel_answer_from_the_host_without_a_model_turn() {
        ...
        assert!(!dir.path().join(".hakus/workflow-runs.jsonl").exists());
