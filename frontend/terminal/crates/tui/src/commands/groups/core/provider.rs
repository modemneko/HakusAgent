...
pub(in crate::commands) fn provider_setup_action_for_name(raw: &str) -> Result<AppAction, String> {
    ...
    if let Some(template) = hakus_config::provider_setup_template(raw) {
        match template.apply {
            hakus_config::ProviderSetupApply::FirstClass(kind) => {
                return Ok(AppAction::OpenProviderSetup {
                    provider: Some(ApiProvider::from_kind(kind)),
                });
            }
            hakus_config::ProviderSetupApply::Compatible
            | hakus_config::ProviderSetupApply::Unpublished => {
