...
fn migrate_text(locale: Locale) -> String {
    ...
    match hakus_config::UserConstitution::migrate_file(&path) {
        Err(err) => match locale {
            Locale::ZhHans => format!("迁移失败，文件未更改：{err:#}"),
            _ => format!("Migration failed; the file is unchanged: {err:#}"),
        },
        Ok(hakus_config::MigrationOutcome::AlreadyCurrent {
        ...
        Ok(hakus_config::MigrationOutcome::Migrated { receipt, .. }) => {
        ...
        Ok(hakus_config::MigrationOutcome::Rejected(rejection)) => match locale {
            Locale::ZhHans => format!(
...
fn rollback_text(locale: Locale) -> String {
    ...
    match hakus_config::UserConstitution::rollback_file(&path) {
...
enum UserConstitutionStatus {
    ...
    Loaded {
        path: PathBuf,
        constitution: Box<hakus_config::UserConstitution>,
    },
    PathError {
        error: String,
    },
...
fn user_constitution_stack_status(
    ...
    let text = match load {
        ...
        UserConstitutionStatus::PathError { .. } => match locale {
            Locale::ZhHans => "不可用；HAKUS_HOME 错误",
            _ => "unavailable; HAKUS_HOME error",
        },
    };
    text.to_string()
}
...
- /constitution repo：查看 .hakus/constitution.json 仓库本地准则。
...
- /constitution repo: inspect repo-local .hakus/constitution.json law.
...
fn no_repo_law_text(locale: Locale) -> &'static str {
    match locale {
        Locale::ZhHans => "此工作区未找到仓库本地协作准则 .hakus/constitution.json。",
        _ => "No repo-local constitution found at .hakus/constitution.json for this workspace.",
    }
}
...
fn path_error_preview_text(locale: Locale, error: &str) -> String {
    match locale {
        Locale::ZhHans => format!("无法为用户全局协作准则解析 HAKUS_HOME：\n\n{error}"),
        _ => {
            format!("Could not resolve HAKUS_HOME for the user-global constitution:\n\n{error}")
        }
    }
}
...
.hakus/constitution.json 是仓库本地协作准则。它属于某个工作区，并作为独立的仓库协作准则块渲染。

AGENTS.md 和项目说明是项目规则/实现指导。它们可以描述构建命令、仓库规范和本地流程；按优先级顺序，它们低于当前用户请求和内置基础准则，高于用户全局长期偏好、记忆和交接。

WHALE.md 已忽略。将普通项目说明迁移到 AGENTS.md，将 Hakus 专属权限策略迁移到 .hakus/constitution.json。
...
.hakus/constitution.json is repo-local law. It belongs to a workspace and is rendered as a separate repo constitution block.
...
WHALE.md is ignored. Move ordinary project instructions to AGENTS.md and Hakus-specific authority policy to .hakus/constitution.json.
    ...
    fn for_locale(locale: Locale) -> Self {
        match locale {
            Locale::ZhHans => Self {
                ...
                repo_action: "/constitution repo 会在存在时显示 .hakus/constitution.json 本地准则。",
            ...
            _ => Self {
                ...
                repo_action: "/constitution repo shows .hakus/constitution.json local law when present.",
...
mod tests {
    ...
    fn constitution_repair_explains_invalid_file_and_opens_setup() {
        ...
        let tmp = tempdir().expect("tempdir");
        let home = tmp.path().join("hakus-home");
        std::fs::create_dir_all(&home).expect("home");
        std::fs::write(home.join("constitution.json"), "{not valid json").expect("invalid file");
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", home.as_os_str());
        let mut app = test_app();
        app.ui_locale = Locale::En;
    ...
    fn constitution_preview_renders_structured_block() {
        ...
        assert!(result.message.is_none());
        let body = pop_pager_body(&mut app);
        assert!(body.contains("<hakus_user_constitution"));
        assert!(body.contains("Maintains release lanes."));
    }

    /// Seal `HAKUS_HOME` to a temp dir and write a constitution there.
    fn sealed_home(
        ...
        let home = tmp.path().join("hakus-home");
        std::fs::create_dir_all(&home).expect("home");
        let guard = crate::test_support::EnvVarGuard::set("HAKUS_HOME", home.as_os_str());
        constitution
    ...
    fn with_suggestion() -> UserConstitution {
        ...
        .with_recommendation(&hakus_config::ConstitutionRecommendation {
            clauses: vec![hakus_config::ConstitutionClause::suggested(
                "c1",
    ...
    fn suggestions_are_listed_but_never_previewed_as_law() {
        ...
        assert!(preview.contains("<hakus_user_constitution"));
        assert!(
    ...
    fn migrate_reports_a_receipt_and_rollback_restores() {
        ...
        let home = tmp.path().join("hakus-home");
        std::fs::create_dir_all(&home).expect("home");
        let _home_guard = crate::test_support::EnvVarGuard::set("HAKUS_HOME", home.as_os_str());
    ...
    fn constitution_explanation_uses_zh_hans_copy() {
        ...
        assert!(body.contains(".hakus/constitution.json"));
