...
//! in `hakus-lane` and calls the same executor `hakus lane …` calls,
//! so the slash surface, its hotbar action, and the CLI produce identical
//! availability, target selection, outcomes, and receipts.

use hakus_lane::control::{execute_lane_control, operations_for_domain};
...
fn split_verb(arg: Option<&str>) -> (&str, Option<&str>) {
    let Some(rest) = arg.map(str::trim).filter(|value| !value.is_empty()) else {
        ...
        // `hakus lane list`. Listing is read-only, so a one-key hotbar
...
fn help_text() -> String {
    let mut out = String::from(
        "Usage: /lane [list|status <lane-id>|interrupt <lane-id>|restart <lane-id>|resume <lane-id>]\n\n\
         ...
         durable Lane registry under $HAKUS_HOME/lanes/, the same records `hakus lane` \
         ...
         recorded — `hakus lane list` reconciles. Interrupt is submitted to an off-loop \
...
mod tests {
    ...
    fn help_lists_every_verb_with_its_authority_and_cli_twin() {
        ...
        assert!(
            help.contains("$HAKUS_HOME/lanes/"),
    ...
    fn bare_lane_is_read_only_so_the_hotbar_cannot_mutate_state() {
        ...
        assert_eq!(
            ...
            hakus_lane::ControlAuthority::Read,
