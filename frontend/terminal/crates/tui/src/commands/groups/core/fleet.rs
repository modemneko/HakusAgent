...
//! `hakus-lane`, exactly as `hakus fleet …` does (#1888, #4022).
//!
...
//! durable Fleet ledger, and a run started by `hakus fleet run` never
//! appeared. The session view is still reachable as `/fleet workers` (and
//! `/subagents`), now labelled as what it is.

use hakus_lane::control::operations_for_domain;
...
fn help_text() -> String {
    let mut out = String::from(
        "Usage: /fleet [roster|setup|fleets|list|status|workers|interrupt <worker-id>|resume <run-id>]\n\n\
         ...
         /fleet list, status, interrupt, and resume act on the durable .hakus/fleet.jsonl \
         ledger for this workspace — the same records `hakus fleet` reads and writes. \
