...
fn bug_report_diagnostics_hint() -> &'static str {
    "Before filing, first check whether this looks like a model issue or an environment/tool issue: \
     ...
     If you have a local JSONL log, run `hakus session-diagnostics <path>` and include the redacted category summary. \
     Include the Hakus version, OS/terminal, the tool name, and redacted timestamps or log handles when available. \
...
mod tests {
    ...
    fn feedback_bug_opens_bug_template_url_without_prefilled_body() {
        ...
        assert!(message.contains("hakus session-diagnostics <path>"));
