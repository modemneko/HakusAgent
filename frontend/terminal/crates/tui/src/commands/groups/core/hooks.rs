...
//! actually configured in `~/.hakus/config.toml`'s `[hooks]`
...
fn events() -> CommandResult {
    ...
    out.push_str(
        "Available hook events (use one of these as `event = \"...\"` in your `[[hooks.hooks]]` entry).\n\
         Hooks are a TUI runtime feature: `hakus exec`, the CLI dispatcher, the app-server,\n\
...
fn list(app: &App) -> CommandResult {
    ...
    if config.hooks.is_empty() && config.problems.is_empty() {
        return CommandResult::message(
            "No hooks configured. Add a `[[hooks.hooks]]` entry to `~/.hakus/config.toml` to define one.",
        );
    }
...
mod tests {
    ...
    fn events_listing_states_scope_and_the_steering_allowlist() {
        ...
        assert!(body.contains("hakus exec"), "{body}");
