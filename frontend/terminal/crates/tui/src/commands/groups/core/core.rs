...
pub fn profile_switch(_app: &mut App, arg: Option<&str>) -> CommandResult {
    let profile_name = match arg {
        ...
        _ => {
            return CommandResult::error(
                "Usage: /profile <name>\n\nSwitch to a named config profile. Profiles are defined in ~/.hakus/config.toml under [profiles] sections.",
            );
        }
    };
    CommandResult::with_message_and_action(
...
/// Show Hakus documentation, community, managed-app, and provider links.
pub fn hakus_links(app: &mut App) -> CommandResult {
    let locale = app.ui_locale;
    ...
    let _ = writeln!(
        message,
        "{} `https://hakus.net/{site_locale}/docs`",
        tr(locale, MessageId::LinksDocumentation)
    );
    let _ = writeln!(
        message,
        "{} `https://hakus.net/{site_locale}/community`",
    ...
    for provider in hakus_config::provider::providers_sorted_for_display() {
        ...
        if provider.kind() == hakus_config::ProviderKind::Moonshot {
            let _ = writeln!(
                message,
                "{}",
...
mod tests {
    ...
    fn test_help_links_topic_shows_aliases() {
        ...
        assert!(msg.contains("Show Hakus, community, and provider links"));
    ...
    fn test_hakus_links() {
        let mut app = create_test_app();
        let result = hakus_links(&mut app);
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
        assert!(msg.contains("Hakus & community"));
        assert!(msg.contains("https://hakus.net/en/docs"));
        assert!(msg.contains("https://hakus.net/en/community"));
        assert!(msg.contains("https://github.com/Hmbown/CodeWhale"));
        assert!(msg.contains("https://app.hakus.net"));
        ...
        assert!(!msg.contains("https://hakus.dev/docs/providers"));
        assert!(result.action.is_none());
    }
    ...
    fn provider_link_metadata_marks_custom_routes_as_configuration_owned() {
        let links =
            hakus_config::provider::provider_for_kind(hakus_config::ProviderKind::Custom)
                .credential_help();

        assert_eq!(
            links.acquisition,
            hakus_config::provider::CredentialAcquisition::Configuration
        );
    ...
    fn project_links_follow_the_available_public_site_locale() {
        ...
        let msg = hakus_links(&mut app)
            .message
            .expect("links should return a message");

        assert!(msg.contains("`https://hakus.net/zh/docs`"));
        assert!(msg.contains("`https://hakus.net/zh/community`"));
        assert!(msg.contains("`https://app.hakus.net`"));
    }
    ...
    fn test_home_dashboard_includes_all_sections() {
        ...
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
        assert!(msg.contains("Hakus"));
        assert!(!msg.contains("hakus Home Dashboard"));
    ...
    fn test_home_dashboard_quick_actions_reflect_links_and_config_and_hide_removed_commands() {
        ...
        assert!(msg.contains("/links       - Hakus, community & provider links"));
