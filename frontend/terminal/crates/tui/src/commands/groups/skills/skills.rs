...
fn discover_visible_skills(app: &App) -> SkillRegistry {
    crate::skills::discover_for_workspace_and_dir_with_mode_and_plugins(
        &app.workspace,
        &app.skills_dir,
        crate::skills::SkillDiscoveryMode::from_hakus_only(app.skills_scan_hakus_only),
...
fn skill_discovery_mode_label(mode: crate::skills::SkillDiscoveryMode) -> &'static str {
    match mode {
        ...
        crate::skills::SkillDiscoveryMode::CodeWhaleOnly => "hakus-only",
    }
}
...
/// local cache (`~/.hakus/cache/skills/`). Pass `inspect` to show local
...
/// local cache (`~/.hakus/cache/skills/<name>/`).
///
...
fn needs_approval_message(host: &str) -> String {
    format!(
        "Network policy requires approval for {host}.\n\
         Add it to your allow list with `/network allow {host}` (or set [network].default = \"allow\" in ~/.hakus/config.toml), then retry."
    )
}
...
fn network_denied_message(host: &str) -> String {
    format!(
        "Network policy denied access to {host}.\n\
         Remove the deny entry from ~/.hakus/config.toml under [network] or contact your administrator."
    )
}
...
fn registry_fetch_error_hint(err: &anyhow::Error) -> Option<&'static str> {
    ...
    {
        Some(
            "Hint: DNS lookup failed. Check internet/DNS connectivity, or override the registry URL in [skills] of ~/.hakus/config.toml.",
        )
        ...
        || msg.contains("handshake")
    {
        ...
        Some(
            "Hint: registry URL returned 404. Verify the registry URL in [skills] of ~/.hakus/config.toml.",
        )
...
mod tests {
    ...
    #[test]
    fn test_list_skills_respects_hakus_only_scan() {
        let tmpdir = TempDir::new().unwrap();
        let _home = IsolatedHome::new(&tmpdir);
        ...
        .unwrap();
        let hakus_skill_dir = tmpdir
            .path()
            .join(".hakus")
            .join("skills")
            .join("hakus-skill");
        std::fs::create_dir_all(&hakus_skill_dir).unwrap();
        std::fs::write(
        ...
        let mut app = create_test_app_with_tmpdir(&tmpdir);
        app.skills_dir = tmpdir.path().join(".hakus").join("skills");
        app.skills_scan_hakus_only = true;
        let result = list_skills(&mut app, Some(""));
        let msg = result.message.unwrap();

        assert!(msg.contains("/hakus-skill"), "got: {msg}");
        assert!(!msg.contains("/claude-skill"), "got: {msg}");
    }

    #[test]
    fn test_skills_inspect_reports_hakus_only_scan_mode() {
        let tmpdir = TempDir::new().unwrap();
        let _home = IsolatedHome::new(&tmpdir);
        let claude_skill_dir = tmpdir
            .path()
        ...
        let normalized = msg.replace('\\', "/");
        assert!(
            normalized.contains("Discovery mode: hakus-only"),
            "got: {msg}"
        );
        assert!(normalized.contains("hakus-skill"), "got: {msg}");
