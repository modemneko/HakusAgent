...
fn search_provider_display(config: &Config, locale: crate::localization::Locale) -> String {
    ...
    let source = match resolved.source {
        ...
        SearchProviderSource::EnvOverride => "HAKUS_SEARCH_PROVIDER".to_string(),
    };
...
/// full every-provider wipe, use `hakus auth logout`; for single-provider
/// key replacement, use `hakus auth clear --provider <id>` and
/// `hakus auth set --provider <id>`.
            ...
            CommandResult::with_message_and_action(
                format!(
                    "Cleared API key for {provider_name}. \
                     Use `hakus auth clear --provider <id>` to clear a different provider."
                ),
...
mod tests {
    ...
    impl EnvGuard {
        fn new(home: &Path) -> Self {
            ...
            let vars = vec![
                ...
                EnvVarGuard::remove("HAKUS_ALLOW_SHELL"),
                ...
                EnvVarGuard::remove("HAKUS_SEARCH_PROVIDER"),
    ...
    fn config_default_model_cannot_replace_a_non_deepseek_live_route() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-tui-provider-scoped-default-model-test-{}",
            std::process::id()
        ));
    ...
    fn config_reasoning_effort_uses_codex_provider_labels() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-tui-codex-effort-config-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn config_fancy_animations_keeps_ghostty_full_motion() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-tui-ghostty-fancy-config-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn config_command_cannot_bypass_project_shell_constraint() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-project-shell-control-test-{}",
            std::process::id()
        ));
        ...
        fs::create_dir_all(workspace.join(hakus_config::HAKUS_APP_DIR)).unwrap();
        fs::write(
            workspace
    ...
    fn config_command_cannot_bypass_project_or_environment_approval() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-external-approval-control-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(temp_root.join(".deepseek")).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn config_command_subagents_depth_save_clamps_to_ceiling() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-subagents-depth-save-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let config_path = temp_root.join("custom-config.toml");
...
"#,
    ...
    fn config_context_window_query_shows_override_and_effective_source() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-context-window-query-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
...
"#,
    ...
    fn config_command_provider_url_token_plan_persists_provider_base_url() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-provider-url-save-app-path-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
    ...
    fn explicit_default_background_override_survives_theme_preview() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-tui-background-override-test-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .expect("clock")
    ...
    fn session_only_background_override_survives_theme_preview() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-tui-session-background-test-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .expect("clock")
    ...
    fn config_approval_mode_save_persists_top_level_policy() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-approval-policy-save-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(&temp_root).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn config_approval_policy_can_return_to_saved_tui_permission_default() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-approval-policy-tui-default-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(temp_root.join(".deepseek")).unwrap();
        let _guard = EnvGuard::new(&temp_root);
    ...
    fn config_approval_policy_full_access_adopts_tui_posture_and_releases_root_override() {
        let temp_root = env::temp_dir().join(format!(
            "hakus-approval-policy-full-access-test-{}",
            std::process::id()
        ));
        fs::create_dir_all(temp_root.join(".deepseek")).unwrap();
        let _guard = EnvGuard::new(&temp_root);
