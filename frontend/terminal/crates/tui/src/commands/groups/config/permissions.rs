...
use hakus_config::{PermissionsFileState, PermissionsSnapshot, ToolAskRule};
use hakus_execpolicy::PermissionAction;
...
fn remove_permission(app: &App, parts: &[&str]) -> CommandResult {
    ...
    let removed =
        match hakus_config::remove_permission_rule(app.config_path.clone(), index, parts[3]) {
            Ok(rule) => rule,
            Err(error) => return operation_error(app, &error),
        };
...
fn load_snapshot(app: &App) -> anyhow::Result<PermissionsSnapshot> {
    hakus_config::load_permissions_snapshot(app.config_path.clone())
}
...
fn format_snapshot(app: &App, snapshot: &PermissionsSnapshot) -> String {
    ...
    let path = hakus_config::quote_os_path(snapshot.path());
...
fn format_posture_explainer(app: &App) -> String {
    ...
    let audit_path = crate::audit::audit_log_path()
        .map(|path| hakus_config::quote_os_path(&path))
        .unwrap_or_else(|| "$HAKUS_HOME/audit.log".to_string());
    text.push_str(
...
fn rule_applies_in_workspace(rule: &ToolAskRule, workspace: &std::path::Path) -> bool {
    ...
    let Some(rule_workspace) = hakus_execpolicy::normalize_workspace_scope(rule_workspace)
    else {
        return false;
    };
    let Some(workspace) = hakus_execpolicy::normalize_workspace_scope(&workspace) else {
        return false;
    };
...
"#,
        ...
        let displayed_permissions_path =
            hakus_config::resolve_permissions_path(Some(config_path.clone()))
        ...
        assert!(message.contains(&hakus_config::quote_os_path(
    ...
    fn list_preserves_missing_empty_and_malformed_diagnostics() {
        ...
        assert!(malformed_message.contains(&hakus_config::quote_os_path(
            &displayed_permissions_path
        )));
    ...
    fn remove_requires_preview_token_then_emits_live_reload_action() {
        ...
        let parsed: hakus_config::PermissionsToml =
