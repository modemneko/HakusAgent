...
//! the TUI.
//!
//! The updater itself is not reimplemented here. `hakus update` (see
...
pub(super) enum UpdaterPlan {
    /// Run this `hakus` binary's `update` subcommand.
    Run(PathBuf),
    /// A package manager owns the binary; print its command instead.
    Managed(InstallMethod),
    /// Self-update would be correct, but no `hakus` CLI is reachable —
    /// e.g. a bare `hakus-tui` build with no sibling CLI.
    NoUpdater { exe: PathBuf },
}

/// The binary name that carries the `update` subcommand.
const UPDATER_BIN_STEM: &str = "hakus";
...
pub(super) fn resolve_updater(
    ...
    // A `hakus-tui` build has no `update` subcommand, but the CLI that
...
mod tests {
    ...
    fn a_package_managed_install_gets_its_managers_command_not_an_updater_run() {
        for method in [
            ...
            let plan = resolve_updater(
                Some(Path::new("/opt/whatever/hakus")),
                method,
                &|_: &Path| true,
            );
    ...
    fn a_tui_only_build_without_a_sibling_cli_falls_back_to_instructions() {
        let exe = Path::new("/usr/local/bin/hakus-tui");
        ...
        assert!(message.contains("hakus update"), "{message}");
        assert!(message.contains("restart CodeWhale"), "{message}");
        assert!(message.contains("hakus-tui"), "{message}");
    }

    #[test]
    fn a_tui_build_next_to_the_cli_runs_the_sibling_updater() {
        let plan = resolve_updater(
            Some(Path::new("/usr/local/bin/hakus-tui")),
            InstallMethod::Binary,
            &|path: &Path| path == Path::new("/usr/local/bin/hakus"),
        );
        assert_eq!(
            plan,
            UpdaterPlan::Run(PathBuf::from("/usr/local/bin/hakus"))
        );
    }
    ...
    fn a_cli_install_runs_itself_including_the_windows_extension() {
        assert_eq!(
            resolve_updater(
                Some(Path::new("/usr/local/bin/hakus")),
                InstallMethod::Binary,
                &|_: &Path| false
            ),
            UpdaterPlan::Run(PathBuf::from("/usr/local/bin/hakus"))
        ...
        assert_eq!(
            resolve_updater(
                Some(Path::new("C:/tools/hakus.exe")),
                InstallMethod::Binary,
                &|_: &Path| false
            ),
            UpdaterPlan::Run(PathBuf::from("C:/tools/hakus.exe"))
        );
    }
    ...
    fn a_windows_tui_build_finds_the_sibling_cli_with_its_extension() {
        assert_eq!(
            resolve_updater(
                Some(Path::new("C:/tools/hakus-tui.exe")),
                InstallMethod::Binary,
                &|path: &Path| path == Path::new("C:/tools/hakus.exe"),
            ),
            UpdaterPlan::Run(PathBuf::from("C:/tools/hakus.exe"))
        );
    }
    ...
    fn the_check_preamble_states_what_install_would_do() {
        let preamble = install_preamble(Path::new("/usr/local/bin/hakus"));
        assert!(
            preamble.contains("/usr/local/bin/hakus update"),
            "{preamble}"
        );
