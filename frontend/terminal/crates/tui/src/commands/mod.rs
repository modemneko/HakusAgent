...
#[cfg(test)]
impl hakus_command_contract::metadata::RegisterCommand<CommandResult> for Feat015TestCommand {
    fn info() -> &'static hakus_command_contract::metadata::CommandInfo {
        static INFO: hakus_command_contract::metadata::CommandInfo =
            hakus_command_contract::metadata::CommandInfo {
                name: "feat015ctx",
                aliases: &[],
                usage: "/feat015ctx",
    ...
    fn handler() -> hakus_command_contract::handler::CommandHandler<CommandResult> {
        hakus_command_contract::handler::CommandHandler::Contextual(feat015_contextual)
    }
}
...
fn feat015_contextual(
    contexts: hakus_command_contract::handler::CommandContexts<'_>,
    arg: Option<&str>,
) -> CommandResult {
    use hakus_command_contract::handler::ContextParts;
...
pub fn execute(cmd: &str, app: &mut App) -> CommandResult {
    ...
    if let Some(command_object) = registry().get(command.as_str()) {
        ...
        if let Some(handler) = command_object.contextual_handler() {
            ...
            return match handler {
                hakus_command_contract::handler::CommandHandler::Pure(pure_fn) => {
                    pure_fn(command_arg)
                }
                hakus_command_contract::handler::CommandHandler::Contextual(contextual) => {
    ...
    match command.as_str() {
        ...
        "doctor" => CommandResult::error(
            "The /doctor command is a CLI diagnostic. Run `hakus doctor` or `hakus doctor --json`; use `/setup` in the TUI for readiness and verification.",
        ),
...
mod tests {
    ...
    fn user_command_shadows_builtin_before_group_dispatch() {
        ...
        let commands_dir = temp.path().join(".hakus").join("commands");
