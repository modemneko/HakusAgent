...
//! ## On-disk format
//!
//! `~/.hakus/composer_stash.jsonl` — one JSON object per line:
//!
//! ```jsonl
...
/// directory or falls back outside an explicit `HAKUS_HOME` boundary. It
...
pub(crate) struct DiagnosticStashReport {
    /// Candidate stash path, when the Hakus home could be resolved.
    ...
    let primary = match hakus_config::hakus_home() {
        ...
        Err(error) => {
            return DiagnosticStashReport {
                ...
                count: 0,
                error: Some(format!(
                    "could not resolve the Hakus stash path: {error}"
                )),
            };
        }
    };
...
";
    ...
    fn diagnostic_stash_honors_an_explicit_home_without_legacy_fallback() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let primary = tmp.path().join("isolated-hakus").join(STASH_FILE_NAME);
