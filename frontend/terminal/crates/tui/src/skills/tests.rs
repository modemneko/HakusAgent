...
fn workspace_prompt_omits_disabled_skills_without_configured_directory() {
    ...
    let _hakus_home =
        crate::test_support::EnvVarGuard::set("HAKUS_HOME", home.join(".hakus"));
...
#[test]
fn hakus_only_mode_ignores_cross_tool_skill_dirs() {
    let tmpdir = TempDir::new().unwrap();
    let workspace = tmpdir.path().join("workspace");
    let home = tmpdir.path().join("home");
    let configured_dir = home.join(".hakus").join("skills");
    ...
    assert!(names.contains(&"from-hakus"));
    assert!(names.contains(&"configured-hakus"));
    assert!(
...
#[test]
fn hakus_only_mode_still_honors_explicit_configured_dir() {
    let tmpdir = TempDir::new().unwrap();
    let workspace = tmpdir.path().join("workspace");
    let home = tmpdir.path().join("home");
...
#[test]
fn hakus_only_mode_rejects_workspace_hakus_symlink_escape() {
    let tmpdir = TempDir::new().unwrap();
    let workspace = tmpdir.path().join("workspace");
    let home = tmpdir.path().join("home");
    let escape_target = tmpdir.path().join("escape-target");
    std::fs::create_dir_all(workspace.join(".hakus")).unwrap();
    ...
    assert!(
        ...
        "CodeWhale-only mode must not follow workspace .hakus/skills outside the workspace"
    );
}
...
/// `.agents/skills/` and a global skill in `~/.hakus/skills/`
...
fn discover_finds_both_workspace_and_global_skills() {
    ...
    assert!(
        ...
        "global-alpha from ~/.hakus/skills must be discovered: {names:?}",
    );
}
...
fn global_skill_roots_come_from_the_os_home_only() {
    ...
    // explicit `$HAKUS_HOME`), never an account/GitHub handle. A wrong
    // home once produced `Failed to read /Users/<handle>/.hakus/skills/
    ...
    let workspace = tmpdir.path().join("workspace");
    std::fs::create_dir_all(home.join(".hakus").join("skills")).unwrap();
    std::fs::create_dir_all(&workspace).unwrap();
    let _home = crate::test_support::EnvVarGuard::set("HOME", &home);
    let _userprofile = crate::test_support::EnvVarGuard::set("USERPROFILE", &home);
