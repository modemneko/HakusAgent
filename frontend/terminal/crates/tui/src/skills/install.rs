...
//!   an explicit migration message. Hakus can install individual
...
fn reqwest_client() -> reqwest::Client {
    hakus_release::platform_http_client_builder()
        .build()
...
///
/// Lives at `~/.hakus/cache/skills/` so it's separate from user-installed
...
pub fn default_cache_skills_dir() -> PathBuf {
    crate::config::effective_home_dir().map_or_else(
        || PathBuf::from("/tmp/hakus/cache/skills"),
        |p| p.join(".hakus").join("cache").join("skills"),
    )
}
...
pub enum InstallError {
    ...
    SymlinkRejected,
    #[error(
        "Claude Code plugin archive contains multiple SKILL.md entries; Hakus installs one SKILL.md bundle at a time and does not run plugin.json/custom-command runtimes. Install or migrate an individual skills/<name> directory instead"
    )]
    ClaudePluginBundle,
