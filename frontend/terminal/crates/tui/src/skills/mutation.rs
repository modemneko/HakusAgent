...
/// workspace/home anchor, `.hakus`, or `skills` component.
fn validate_owned_target_chain(
    anchor: &Path,
    skills_dir: &Path,
    require_existing: bool,
) -> Result<()> {
    let expected = anchor.join(".hakus").join("skills");
    ...
    let hakus_dir = anchor.join(".hakus");
    let hakus_exists = checked_real_directory(&hakus_dir)?;
    if !hakus_exists {
        if require_existing {
            bail!(
...
fn prepare_owned_target(
    ...
    let hakus_dir = anchor.join(".hakus");
    if !checked_real_directory(&hakus_dir)? {
        create_owned_directory(&hakus_dir)?;
    }
...
mod tests {
    ...
    fn resolve_owned_target_project_and_global() {
        ...
        assert_eq!(project, workspace.join(".hakus").join("skills"));
        assert_eq!(global, home.join(".hakus").join("skills"));
    }
    ...
    fn owned_target_resolution_rejects_symlinked_parent_and_root_in_both_scopes() {
        for target in [SkillTargetScope::Project, SkillTargetScope::Global] {
            ...
            std::os::unix::fs::symlink(&outside_parent, anchor.join(".hakus")).unwrap();
            let err = resolve_owned_target(&workspace, Some(&home), target).unwrap_err();
            assert!(err.to_string().contains("symlinked"), "got: {err}");

            fs::remove_file(anchor.join(".hakus")).unwrap();
            fs::create_dir(anchor.join(".hakus")).unwrap();
            let outside_root = tmp.path().join("outside-root");
            fs::create_dir_all(&outside_root).unwrap();
            std::os::unix::fs::symlink(&outside_root, anchor.join(".hakus").join("skills"))
                .unwrap();
            let err = resolve_owned_target(&workspace, Some(&home), target).unwrap_err();
            assert!(err.to_string().contains("symlinked"), "got: {err}");
        }
    ...
    async fn install_rejects_symlinked_owned_root_without_external_writes() {
        ...
        fs::create_dir_all(workspace.join(".hakus")).unwrap();
        fs::create_dir_all(&home).unwrap();
        fs::create_dir_all(&outside).unwrap();
        fs::write(outside.join("SENTINEL"), "untouched").unwrap();
        std::os::unix::fs::symlink(&outside, workspace.join(".hakus").join("skills")).unwrap();
    ...
    fn import_rejects_symlinked_owned_parent_without_external_writes() {
        ...
        std::os::unix::fs::symlink(&outside_parent, workspace.join(".hakus")).unwrap();
    ...
    fn remove_rejects_symlinked_owned_root_without_external_deletion() {
        ...
        std::os::unix::fs::symlink(&outside_root, workspace.join(".hakus").join("skills"))
            .unwrap();
    ...
    fn import_external_copies_into_project_owned() {
        ...
        fs::create_dir_all(workspace.join(".hakus").join("skills")).unwrap();
        write_skill(
            &workspace.join(".claude").join("skills"),
            "from-claude",
            "import-me",
    ...
    fn import_exact_duplicate_is_already_present() {
        ...
        let content = "---\nname: shared\ndescription: d\n---\nbody\n";
        let owned = workspace.join(".hakus").join("skills").join("shared");
    ...
    fn trust_writes_v2_digest_binding() {
        ...
        let network = NetworkPolicy::default();
        let c = ctx(&workspace, &home, &network);
        let root = workspace.join(".hakus").join("skills");
    ...
    fn import_rejects_when_owned_dir_differs_from_canonical() {
        ...
        let owned_root = workspace.join(".hakus").join("skills");
    ...
    fn dual_scope_same_name_requires_explicit_scope() {
        ...
        write_skill(&home.join(".hakus").join("skills"), "dup", "global");
