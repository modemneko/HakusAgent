...
#[test]
fn bundled_integration_skills_use_current_hakus_commands_and_paths() {
    for (name, body) in [("mcp-builder", MCP_BUILDER_BODY), ("feishu", FEISHU_BODY)] {
        assert!(
            body.contains("hakus mcp"),
    ...
    assert!(SKILL_CREATOR_BODY.contains("<workspace>/.hakus/skills"));
    assert!(SKILL_CREATOR_BODY.contains("~/.hakus/skills"));
    assert!(SKILL_INSTALLER_BODY.contains("~/.hakus/skills"));
...
fn contributor_onboarding_ships_at_generation_8_and_keeps_its_refusals() {
    ...
    assert!(body.contains("./target/release/hakus exec --help"));
