...
impl SkillRootCatalog {
    ...
    pub fn build(
        ...
        let project_owned = workspace.join(".hakus").join("skills");
        push_descriptor(
            &mut roots,
            &mut precedence,
        ...
        if let Some(home) = home_dir {
            ...
            let global_owned = home.join(".hakus").join("skills");
            push_descriptor(
                &mut roots,
                &mut precedence,
            ...
            let cache = home.join(".hakus").join("cache").join("skills");
            push_descriptor(
                &mut roots,
                &mut precedence,
            ...
            push_descriptor(
                ...
                SkillRootAccess::WritableOwned,
                SkillScope::Global,
                PathBuf::from("/tmp/hakus/skills"),
                true,
                true,
                "global-hakus-fallback",
                true,
            );
        }
            ...
            match mode {
                ...
                super::SkillDiscoveryMode::CodeWhaleOnly => {
                    ...
                    if root.kind == SkillRootKind::CodeWhaleProject
                        && !hakus_project_root_is_inside_workspace(workspace, &root.path)
                    {
                        continue;
                    }
                }
...
#[must_use]
pub fn hakus_workspace_skills_dir(workspace: &Path) -> Option<PathBuf> {
    let skills_dir = workspace.join(".hakus").join("skills");
    hakus_project_root_is_inside_workspace(workspace, &skills_dir).then_some(skills_dir)
}
...
fn hakus_project_root_is_inside_workspace(workspace: &Path, skills_dir: &Path) -> bool {
...
mod tests {
    ...
    fn runtime_compatible_preserves_historical_workspace_order() {
        ...
        write_dir(&workspace.join(".hakus").join("skills"));
        write_dir(&workspace.join(".codex").join("skills"));
        write_dir(&home.join(".hakus").join("skills"));
    ...
    #[test]
    fn owned_roots_are_writable_and_hakus_only() {
        let tmp = TempDir::new().unwrap();
        let workspace = tmp.path().join("ws");
        let home = tmp.path().join("home");
        write_dir(&workspace.join(".agents").join("skills"));
    ...
    fn safe_display_path_prefers_home_then_workspace() {
        ...
        let path = home.join(".hakus").join("skills");
        assert_eq!(
            safe_display_path(&path, Some(&workspace), Some(&home)),
            "~/.hakus/skills"
        );
        let project = workspace.join(".hakus").join("skills");
        assert_eq!(
            safe_display_path(&project, Some(&workspace), Some(&home)),
            "<workspace>/.hakus/skills"
        );
    }
}
