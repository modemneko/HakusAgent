...
impl SkillDiscoveryMode {
    #[must_use]
    pub fn from_hakus_only(value: bool) -> Self {
        if value {
            Self::CodeWhaleOnly
        } else {
            Self::Compatible
...
/// 6. `<workspace>/.hakus/skills` — CodeWhale workspace skills.
/// 7. [`agents_global_skills_dir`] — agentskills.io global.
/// 8. `~/.claude/skills` — Claude-ecosystem global (#902).
/// 9. `~/.hakus/skills` — CodeWhale global, primary install target.
...
pub(crate) use roots::hakus_workspace_skills_dir;
#[cfg(test)]
