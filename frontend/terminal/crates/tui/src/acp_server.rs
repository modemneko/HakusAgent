    ...
    while let Some(mut call) = calls.next() {
        ...
        match prepared.admission {
            ...
            AcpToolAdmission::Block(reason) => {
                let content = format!("Blocked by Hakus policy: {reason}");
                write_tool_call_update(writer, session_id, &call, "failed", Some(&content)).await?;
                result_messages.push(tool_result_message(&call.id, content, true));
                continue;
            }
...
impl AcpServer {
    ...
    fn new_session(&mut self, params: Value) -> std::result::Result<Value, AcpError> {
        ...
        let session_id = format!("hakus-{}", uuid::Uuid::new_v4());
...
fn resolve_acp_route_facts(
    ...
    Option<hakus_config::route::RouteLimits>,
...
/// `hakus exec`. Tool availability remains owned by the request catalog;
...
fn build_acp_system_prompt(
    ...
    crate::prompts::system_prompt_for_mode_with_context_skills_session_and_approval_for_host(
        ...
        crate::prompts::PromptSessionContext {
            ...
            skills_scan_hakus_only: config.skills_config().scan_hakus_only(),
...
async fn write_tool_call_update_with_blocks<W>(
    ...
    status: &str,
    content: Option<&str>,
    rich_blocks: &[hakus_tools::ToolResultContentBlock],
) -> Result<()>
where
    W: AsyncWrite + Unpin,
{
    ...
    if content.is_some() || !rich_blocks.is_empty() {
        ...
        blocks.extend(rich_blocks.iter().map(|block| match block {
            hakus_tools::ToolResultContentBlock::Image { mime_type, data } => json!({
                "type": "content",
...
fn acp_auth_methods(config: &Config) -> Value {
    ...
    json!([
        {
            "id": "hakus-terminal-auth",
            "name": "Set Hakus API key",
            "description": format!("Run Hakus's terminal credential setup for the {provider} provider."),
            "type": "terminal",
...
mod tests {
    ...
    async fn tool_update_emits_typed_acp_image_content() {
        ...
        write_tool_call_update_with_blocks(
            ...
            &[hakus_tools::ToolResultContentBlock::Image {
    ...
    fn initialize_advertises_baseline_acp_agent() {
        ...
        assert_eq!(result["agentInfo"]["name"], "hakus");
    ...
    fn config_with_policy_rule(rule: hakus_execpolicy::ToolAskRule) -> Config {
        Config {
    ...
    async fn acp_hook_rewrite_is_reprepared_and_policy_is_re_evaluated() {
        let dir = tempfile::tempdir().expect("tempdir");
        let deny_rewritten_write = hakus_execpolicy::ToolAskRule {
            action: hakus_execpolicy::PermissionAction::Deny,
            ..hakus_execpolicy::ToolAskRule::file_path("write_file", "rewritten.txt")
        };
        let config = config_with_tool_call_hook(
    ...
    fn acp_admission_folds_typed_rules_then_headless_safety_floor() {
        ...
        let allow = hakus_execpolicy::ToolAskRule::file_path("write_file", "allowed.txt")
        ...
        let ask = hakus_execpolicy::ToolAskRule::file_path("write_file", "allowed.txt");
        let (_, admission) =
        ...
        let deny = hakus_execpolicy::ToolAskRule {
            action: hakus_execpolicy::PermissionAction::Deny,
            ..hakus_execpolicy::ToolAskRule::file_path("write_file", "allowed.txt")
        };
        let (_, admission) =
        ...
        let command = "rm -rf ~/";
        let shell_allow = hakus_execpolicy::ToolAskRule::exec_shell(command)
    ...
    fn acp_admission_auto_review_and_repo_law_override_typed_allow() {
        ...
        let law_dir = dir.path().join(".hakus");
        ...
        for (path, expected_block) in [("wire.rs", true), ("CHANGELOG.md", false)] {
            let allow = hakus_execpolicy::ToolAskRule::file_path("write_file", path)
                .into_exact_workspace_allow(workspace.clone());
