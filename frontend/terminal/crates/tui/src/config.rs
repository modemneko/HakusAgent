//! Configuration loading and defaults for hakus.
...
#[cfg(test)]
pub(crate) use hakus_config::API_KEYRING_SENTINEL;
pub(crate) use hakus_config::{ConfigApiKeyValueKind, classify_config_api_key_value};
...
pub enum ApiProvider {
    ...
    /// [`hakus_config::ProviderKind::Custom`] and routes via the OpenAI Chat
...
impl ApiProvider {
    ...
    pub fn parse(value: &str) -> Option<Self> {
        ...
        hakus_config::ProviderKind::parse(value).map(Self::from_kind)
    }
    ...
    #[must_use]
    pub fn metadata(self) -> Option<&'static dyn hakus_config::provider::Provider> {
    ...
    pub fn env_vars(self) -> &'static [&'static str] {
        self.metadata().map_or(
            hakus_config::ProviderKind::Deepseek
                .provider()
                .env_vars(),
    ...
    #[must_use]
    pub fn credential_help(self) -> hakus_config::provider::CredentialHelp {
        self.metadata().map_or_else(
            || {
                hakus_config::provider::provider_for_kind(
                    hakus_config::ProviderKind::Deepseek,
                )
                .credential_help()
            },
            hakus_config::provider::Provider::credential_help,
        )
    }
    ...
    const KIND_LOOKUP: [Option<hakus_config::ProviderKind>; 48] = [
        Some(hakus_config::ProviderKind::Deepseek),
        None, // DeepseekCN
        Some(hakus_config::ProviderKind::DeepseekAnthropic),
        Some(hakus_config::ProviderKind::NvidiaNim),
        Some(hakus_config::ProviderKind::Openai),
        Some(hakus_config::ProviderKind::Atlascloud),
        Some(hakus_config::ProviderKind::WanjieArk),
        Some(hakus_config::ProviderKind::Volcengine),
        Some(hakus_config::ProviderKind::Openrouter),
        Some(hakus_config::ProviderKind::Orcarouter),
        Some(hakus_config::ProviderKind::XiaomiMimo),
        Some(hakus_config::ProviderKind::Novita),
        Some(hakus_config::ProviderKind::Fireworks),
        Some(hakus_config::ProviderKind::Siliconflow),
        Some(hakus_config::ProviderKind::SiliconflowCN),
        Some(hakus_config::ProviderKind::Arcee),
        Some(hakus_config::ProviderKind::Moonshot),
        Some(hakus_config::ProviderKind::Sglang),
        Some(hakus_config::ProviderKind::Vllm),
        Some(hakus_config::ProviderKind::Ollama),
        Some(hakus_config::ProviderKind::OllamaCloud),
        Some(hakus_config::ProviderKind::Huggingface),
        Some(hakus_config::ProviderKind::Together),
        Some(hakus_config::ProviderKind::Qianfan),
        Some(hakus_config::ProviderKind::OpenaiCodex),
        Some(hakus_config::ProviderKind::Anthropic),
        Some(hakus_config::ProviderKind::Openmodel),
        Some(hakus_config::ProviderKind::Zai),
        Some(hakus_config::ProviderKind::Stepfun),
        Some(hakus_config::ProviderKind::Minimax),
        Some(hakus_config::ProviderKind::MinimaxAnthropic),
        Some(hakus_config::ProviderKind::Deepinfra),
        Some(hakus_config::ProviderKind::Sakana),
        Some(hakus_config::ProviderKind::LongCat),
        Some(hakus_config::ProviderKind::OpencodeGo),
        Some(hakus_config::ProviderKind::OpencodeZen),
        Some(hakus_config::ProviderKind::Meta),
        Some(hakus_config::ProviderKind::Xai),
        Some(hakus_config::ProviderKind::Mistral),
        Some(hakus_config::ProviderKind::Google),
        Some(hakus_config::ProviderKind::Antigravity),
        Some(hakus_config::ProviderKind::Telecomjs),
        Some(hakus_config::ProviderKind::Edenai),
        Some(hakus_config::ProviderKind::ModelstudioTokenPlan),
        Some(hakus_config::ProviderKind::ModelstudioTokenPlanAnthropic),
        Some(hakus_config::ProviderKind::ModelstudioCodingPlan),
        Some(hakus_config::ProviderKind::ModelstudioCodingPlanAnthropic),
        Some(hakus_config::ProviderKind::Custom),
    ];
    ...
    #[must_use]
    pub fn kind(self) -> Option<hakus_config::ProviderKind> {
        Self::KIND_LOOKUP[self as usize]
    }

    /// Construct from a config-level `ProviderKind`.
    #[must_use]
    pub fn from_kind(kind: hakus_config::ProviderKind) -> Self {
...
pub(crate) fn opencode_go_chat_model_id(model: &str) -> Option<&'static str> {
    hakus_config::opencode_go_chat_model_id(model)
}
...
/// Hakus-only / local providers Models.dev does not represent, or when
...
pub fn model_completion_names_for_provider(provider: ApiProvider) -> Vec<&'static str> {
    match provider {
        ...
        ApiProvider::OpencodeZen => hakus_config::route::opencode_zen_picker_models(),
...
pub struct ToolsConfig {
    ...
    ///
    /// Defaults to `~/.hakus/tools/` when `None`.
    #[serde(default)]
...
pub struct SubagentsConfig {
    ...
    /// [`hakus_config::DEFAULT_SPAWN_DEPTH`]; any value is clamped to
    /// [`hakus_config::MAX_SPAWN_DEPTH_CEILING`]. Fleet workers are
...
fn default_update_check_interval_hours() -> u64 {
    hakus_release::check::DEFAULT_CHECK_INTERVAL_HOURS
}
...
pub struct Config {
    ...
    /// the terminal window title (the `Hakus` / `reasoning…` / `done.`
    ...
    pub fallback_providers: Vec<hakus_config::ProviderKind>,
    ...
    /// Provider-specific credentials and defaults shared with the `hakus` facade.
    #[serde(default)]
    ...
    /// dispatch layers use the built-in defaults from `hakus_config`.
    #[serde(default)]
    pub hotbar: Option<Vec<hakus_config::HotbarBindingToml>>,
    ...
    #[serde(default)]
    pub fleet: Option<hakus_config::FleetConfigToml>,

    /// Workflow automatic-launch, approval, isolation, and activity
    /// persistence knobs (#4128). When absent, consumers use
    /// [`hakus_config::WorkflowConfigToml::default`] via
    /// [`Self::workflow_config`].
    #[serde(default)]
    pub workflow: Option<hakus_config::WorkflowConfigToml>,

    /// Sub-agent model overrides.
    #[serde(default)]
    pub subagents: Option<SubagentsConfig>,

    /// Runtime API server tuning (`hakus serve --http`). Currently only
    ...
    /// `HAKUS_BASE_URL`/`DEEPSEEK_BASE_URL` value that
...
/// `HAKUS_BASE_URL` / `DEEPSEEK_BASE_URL` override.
///
...
pub enum ToolOverride {
    ...
    Script {
        /// Path to the script (absolute, or relative to `~/.hakus/tools/`).
        path: String,
...
pub struct SkillsConfig {
    ...
    /// When true, skill discovery scans only Hakus-owned skill roots
    ...
    #[serde(default, alias = "scanHakusOnly")]
    pub scan_hakus_only: Option<bool>,
}
...
impl SkillsConfig {
    ...
    #[must_use]
    pub fn scan_hakus_only(&self) -> bool {
        self.scan_hakus_only.unwrap_or(false)
    }
}

/// `[network]` table — mirrors `hakus_config::NetworkPolicyToml` so the live
...
pub struct ProviderConfig {
    ...
    #[serde(alias = "authMode")]
    pub auth_mode: Option<String>,
    /// Validated basename of the active Hakus-owned xAI OAuth generation.
    /// The file always lives below Hakus's private credentials directory.
    ...
    pub auth: Option<hakus_config::ProviderAuthSourceToml>,
    ...
    pub external_credentials: Option<hakus_config::ExternalCredentialConsentToml>,
    ...
    /// User-curated model list surfaced in the `/model` picker (unknown ids
    /// render as custom rows). `model` stays the single active choice.
    #[serde(default)]
...
fn approval_policy_env_is_set() -> bool {
    let read = || {
        std::env::var_os("HAKUS_APPROVAL_POLICY").is_some()
...
fn allow_shell_env_is_set() -> bool {
    let read = || {
        std::env::var_os("HAKUS_ALLOW_SHELL").is_some()
...
impl Config {
    ...
    pub(crate) fn external_credential_consent_status(
        &self,
        provider: ApiProvider,
    ) -> Option<hakus_config::ExternalCredentialConsentStatus> {
        let (kind, source, path) = match provider {
            ...
            ApiProvider::Deepseek => (
                hakus_config::ProviderKind::Deepseek,
                hakus_config::ExternalCredentialSource::DshCli,
                hakus_config::default_dsh_credentials_path(),
            ),
            ApiProvider::DeepseekAnthropic => (
                hakus_config::ProviderKind::DeepseekAnthropic,
                hakus_config::ExternalCredentialSource::DshCli,
                hakus_config::default_dsh_credentials_path(),
            ),
            _ => return None,
        ...
        let active_kind = self
            .api_provider()
            .kind()
            .unwrap_or(hakus_config::ProviderKind::Deepseek);
        let consent = self
            .provider_config_for(provider)
            .and_then(|entry| entry.external_credentials.as_ref());
        Some(hakus_config::external_credential_consent_status(
            consent,
            kind,
            source,
            &path,
    ...
    pub(crate) fn runtime_preset_blocker(
        ...
        let project_controls_runtime = || {
            ...
            let parsed_controls =
                hakus_config::load_project_config(workspace).is_some_and(|project| {
                    project.approval_policy.as_deref().is_some_and(|policy| {
                        hakus_config::project_approval_policy_is_allowed(
                            approval_baseline,
                            policy,
                        )
                    }) || project.sandbox_mode.as_deref().is_some_and(|sandbox| {
                        hakus_config::project_sandbox_mode_is_allowed(
    ...
    pub(crate) fn approval_policy_control(
        ...
        if !workspace_is_home {
            ...
            let approval_baseline = self.approval_policy.as_deref().or(saved_approval_baseline);
            if hakus_config::load_project_config(workspace)
                .and_then(|project| project.approval_policy)
                .is_some_and(|policy| {
                    hakus_config::project_approval_policy_is_allowed(approval_baseline, &policy)
                })
            {
    ...
    pub fn search_provider_resolution(&self) -> SearchProviderResolution {
        if let Ok(raw) = std::env::var("HAKUS_SEARCH_PROVIDER")
    ...
    fn legacy_ollama_cloud_route_configured(&self) -> bool {
        ...
        base_url.is_some_and(|base_url| {
            hakus_config::provider::migrates_legacy_ollama_cloud_route(
                hakus_config::ProviderKind::Ollama,
                &base_url,
            )
        })
    }
    ...
    pub(crate) fn resolve_provider_identity(
        ...
        if !has_exact_custom_table && key.eq_ignore_ascii_case(ApiProvider::Custom.as_str()) {
            ...
            if !self.selects_literal_custom_provider() {
                ...
                return Err(format!(
                    "legacy session records only the generic `custom` provider kind, but the live config does not select exactly one valid named custom route (selected '{}', valid named routes: {}). Restore the original single `[providers.<name>]` route or repair the saved provider identity; Hakus will not guess or fall back",
        ...
        let entry = self
            ...
            .ok_or_else(|| {
                format!(
                    "saved session requires custom provider '{exact_key}', but `[providers.{exact_key}]` is missing from the live config. Restore that exact table and retry; Hakus will not fall back"
                )
            })?;
        if !entry.is_openai_compatible_custom() {
            return Err(format!(
                "saved session requires custom provider '{exact_key}', but `[providers.{exact_key}]` must set `kind = \"openai-compatible\"`. Fix the live config and retry; Hakus will not fall back"
            ));
        }
        let base_url = entry
            .base_url
            ...
            .ok_or_else(|| {
                format!(
                    "saved session requires custom provider '{exact_key}', but `[providers.{exact_key}]` has no `base_url`. Fix the live config and retry; Hakus will not fall back"
                )
            })?;
        let parsed = reqwest::Url::parse(base_url).map_err(|err| {
            format!(
                "saved session requires custom provider '{exact_key}', but `[providers.{exact_key}].base_url` is invalid: {err}. Fix the live config and retry; Hakus will not fall back"
            )
        })?;
        if !matches!(parsed.scheme(), "http" | "https") || parsed.host_str().is_none() {
            return Err(format!(
                "saved session requires custom provider '{exact_key}', but `[providers.{exact_key}].base_url` must be an http(s) URL with a host. Fix the live config and retry; Hakus will not fall back"
            ));
        }
    ...
    fn resolve_exact_provider_identity(
        ...
        if id.is_empty() {
            return Err(
                "persisted provider route has an empty exact provider id; Hakus will not guess or fall back"
                    .to_string(),
            );
        }
        let has_exact_custom_table = self
        ...
        if id.eq_ignore_ascii_case(ApiProvider::Custom.as_str()) && !has_exact_custom_table {
            return Err(format!(
                "persisted provider route requires exact custom provider '{id}', but `[providers.{id}]` is missing from the live config. Restore that exact table and retry; Hakus will not fall back"
            ));
        }
        ...
        if identity.provider == ApiProvider::Custom && identity.persisted_id() != Some(id) {
            return Err(format!(
                "persisted provider route requires exact custom provider '{id}', but the live config only provides the legacy root-level custom route. Restore `[providers.{id}]` and retry; Hakus will not fall back"
            ));
        }
        Ok(identity)
    }
    ...
    pub(crate) fn resolve_persisted_provider_identity(
        ...
        let Some(kind) = kind else {
            return id.map_or_else(
                || {
                    Err(
                        "persisted provider route has neither a provider kind nor an exact provider id; Hakus will not guess or fall back"
                            .to_string(),
                    )
                },
        ...
        let Some(mut provider) = ApiProvider::parse(kind) else {
            ...
            {
                return Err(format!(
                    "persisted provider route has legacy identity '{kind}' but exact provider id '{id}'; repair the mismatched fields because Hakus will not guess or fall back"
                ));
            }
            return match id {
        ...
        if provider == ApiProvider::Custom {
            if let Some(id) = id {
                ...
                if identity.provider != ApiProvider::Custom {
                    return Err(format!(
                        "persisted provider route declares generic kind 'custom' but exact provider id '{id}' resolves as built-in '{}'; use the matching built-in kind or restore `[providers.{id}]`. Hakus will not guess or fall back",
        ...
        {
            return Err(format!(
                "persisted provider route declares built-in kind '{}' but exact provider id '{id}' names a different route; repair the mismatched fields because Hakus will not guess or fall back",
                provider.as_str()
            ));
        }
        ...
        {
            return Err(format!(
                "persisted provider route requires built-in '{}', but an exact `[providers.{}]` custom route shadows the same selector. Rename the custom route or update the saved provider kind/id pair; Hakus will not guess or fall back",
                provider.as_str(),
                provider.as_str()
            ));
        }
    ...
    fn validate_legacy_literal_custom_route(&self) -> std::result::Result<(), String> {
        if self.has_literal_custom_provider_table() {
            return Err(
                "legacy `provider = \"custom\"` is ambiguous because `[providers.custom]` is also present. Move the route to one named `[providers.<name>]` table and update the saved provider identity; Hakus will not guess or fall back"
                    .to_string(),
            );
        }
    ...
    fn validate_legacy_literal_custom_root_route(&self) -> std::result::Result<(), String> {
        ...
        if !self.selects_literal_custom_provider() {
            return Err(format!(
                "legacy session records only the generic `custom` provider kind, but the live config selects '{}'. Only an unchanged legacy config with `provider = \"custom\"` and root-level `base_url`/`default_text_model` can load this session; Hakus will not guess or fall back",
                if selected.is_empty() {
                    "<unset>"
                } else {
                    selected
        ...
        let base_url = self
            ...
            .filter(|base_url| !base_url.is_empty())
            .ok_or_else(|| {
                "legacy `provider = \"custom\"` requires a non-empty root-level `base_url` to load a saved session; Hakus will not use the custom-provider placeholder or fall back"
                    .to_string()
            })?;
        let parsed = reqwest::Url::parse(base_url).map_err(|err| {
            format!(
                "legacy `provider = \"custom\"` has an invalid root-level `base_url`: {err}. Fix the live config and retry; Hakus will not fall back"
            )
        })?;
        if !matches!(parsed.scheme(), "http" | "https") || parsed.host_str().is_none() {
            return Err(
                "legacy `provider = \"custom\"` requires a root-level `base_url` with an http(s) scheme and host; Hakus will not fall back"
                    .to_string(),
            );
        }
        ...
        let model = self
            ...
            .ok_or_else(|| {
                "legacy `provider = \"custom\"` requires a non-empty root-level `default_text_model` to load a saved session; Hakus will not guess or fall back"
                    .to_string()
            })?;
        if model.eq_ignore_ascii_case("auto") || normalize_custom_model_id(model).is_none() {
            return Err(
                "legacy `provider = \"custom\"` requires one explicit, valid root-level `default_text_model` (not `auto`) to load a saved session; Hakus will not guess or fall back"
                    .to_string(),
            );
        }
    ...
    /// Hakus-owned OAuth storage supersedes any dormant Grok CLI consent.
    pub(crate) fn mark_hakus_owned_xai_oauth(&mut self, generation: String) {
    ...
    pub fn http_headers(&self) -> HashMap<String, String> {
        ...
        if auth_mode_disables_api_key(self.auth_mode_for_provider(provider).as_deref()) {
            headers.retain(|name, _| !hakus_config::is_upstream_auth_header(name));
        }
        headers
    }
    ...
    pub fn default_model(&self) -> String {
        ...
        match provider {
            ...
            ApiProvider::Custom => hakus_config::ProviderKind::Custom
                .provider()
                .default_model(),
        }
        .to_string()
    ...
    /// 3. the generic `HAKUS_BASE_URL` / `DEEPSEEK_BASE_URL` override, but
    ...
    /// `HAKUS_BASE_URL` is documented as "base URL for the active
    ...
    /// The generic `HAKUS_BASE_URL` / `DEEPSEEK_BASE_URL` override, but
    ...
    pub(crate) fn external_credential_read_grant(
        &self,
        provider: ApiProvider,
        source: hakus_config::ExternalCredentialSource,
        suggested_path: &Path,
    ) -> Result<hakus_config::ExternalCredentialReadGrant> {
        if provider != self.api_provider() {
            anyhow::bail!(
        ...
        let kind = provider
            .metadata()
            .map(hakus_config::provider::Provider::kind)
        ...
        let consent = self
            ...
            .with_context(|| {
                format!(
                    "External credentials owned by {} are disabled for {}. To allow read-only access to this exact file, run:\n  hakus auth external-consent --provider {} --mode read-only --path {}",
                    source.as_str(),
                    provider.display_name(),
                    kind.as_str(),
                    hakus_config::quote_os_path(suggested_path)
                )
            })?;
        consent
    ...
    fn deepseek_api_key_with_secret_store_mode(&self, read_only: bool) -> Result<String> {
        ...
        //   hakus --provider deepseek --api-key ark-... --base-url ... --model auto
        if matches!(provider, ApiProvider::Deepseek | ApiProvider::DeepseekCN)
                ...
                .is_some_and(provider_config_uses_kimi_imported_token)
        {
            ...
            anyhow::bail!(
                "Kimi CLI credential import is unsupported. Hakus does not impersonate or reuse Kimi OAuth clients; configure an API key from {} instead.",
                credential_help
                    .credential_url
        ...
        // xAI OAuth prefers Hakus-owned device-login storage. An existing
        ...
        // only after exact read-only consent. Hakus never refreshes or
        ...
        // over ambient env so `hakus auth set` fixes stale shell exports.
        ...
        // the TUI. Standalone `hakus-tui` launches must see the identical
        ...
        {
            let path = hakus_config::default_dsh_credentials_path();
        ...
        if provider == ApiProvider::Antigravity && !custom_endpoint {
            let grant = self
                .external_credential_read_grant(
                    provider,
                    hakus_config::ExternalCredentialSource::AgyCli,
                    &hakus_config::default_agy_credentials_path(),
                )
                .ok();
        ...
        match provider {
            ApiProvider::Deepseek | ApiProvider::DeepseekCN => anyhow::bail!(
                "DeepSeek API key not found.\n\
                 ...
                 2. Save it (works in every folder, no OS prompts):\n\
                        hakus auth set --provider deepseek\n\
                 \n\
                 Alternatives:\n\
                   ...
                   • api_key = \"<your-key>\"  in ~/.hakus/config.toml\n\
                   • already configured DeepSeek Harness? grant read-only access:\n\
                        hakus auth external-consent --provider deepseek --mode read-only"
            ),
            ApiProvider::SiliconflowCn => anyhow::bail!(
                "SiliconFlow China API key not found. Get a key: {}. Run 'hakus auth set --provider siliconflow-CN', \
                 set {}, or add [{}] api_key in ~/.hakus/config.toml. \
            ...
            ApiProvider::Moonshot => {
                ...
                if moonshot_base_url_is_exact_kimi_code(&self.deepseek_base_url()) {
                    anyhow::bail!(
                        "Kimi Code membership-plan API key not found. Get a plan key: {}. This route uses api.kimi.com/coding/v1 and does not import Kimi CLI credentials. Run 'hakus auth set --provider moonshot', set {}, or add [{}] api_key.",
                        credential_help
                            .credential_url
                ...
                anyhow::bail!(
                    "Moonshot/Kimi API key not found. Get a key: {}. Run 'hakus auth set --provider moonshot', \
                     set {}, or add [{}] api_key. \
            ...
            ApiProvider::Xai => {
                ...
                anyhow::bail!(
                    "xAI API key not found. Get a key: https://console.x.ai/\n\
                     Run 'hakus auth set --provider xai', set XAI_API_KEY, or add \
                     [providers.xai] api_key.\n\
                     OAuth alternative: run `hakus auth xai-device` for \
                     Hakus-owned storage and set [providers.xai] auth_mode = \"oauth\"."
                );
            }
            ...
            ApiProvider::Ollama => {
                ...
                anyhow::bail!(
                    "Ollama Cloud API key not found. Get a key: {}. Run 'hakus auth set --provider ollama', set OLLAMA_API_KEY, or add [providers.ollama] api_key in ~/.hakus/config.toml.",
                    help.credential_url
                        .unwrap_or(hakus_config::provider::OLLAMA_CLOUD_API_KEY_URL)
                )
            }
                    ...
                    .filter(|name| !name.is_empty())
                {
                    ...
                    None => anyhow::bail!(
                        "Custom provider '{provider_name}' has no auth configured.\n\
                         ...
                         [providers.{provider_name}] in ~/.hakus/config.toml."
                    ),
                }
            }
    ...
    pub fn memory_path(&self) -> PathBuf {
        ...
        if self.memory_backend() == MemoryBackend::Native {
            ...
            // path (`$HAKUS_HOME/memory.md`), and the native store lives
    ...
    /// [`hakus_config::DEFAULT_SPAWN_DEPTH`]. `0` is a valid value that
    /// blocks the `agent` tool at this runtime depth. Any value is clamped to
    /// [`hakus_config::MAX_SPAWN_DEPTH_CEILING`] so the operator's choice
    ...
    pub fn subagent_max_spawn_depth(&self) -> u32 {
        self.subagents
            ...
            .unwrap_or(hakus_config::DEFAULT_SPAWN_DEPTH)
            .min(hakus_config::MAX_SPAWN_DEPTH_CEILING)
    }
    ...
    /// `HAKUS_STREAM_IDLE_TIMEOUT_SECS` env var (legacy alias:
    ...
    #[must_use]
    pub fn fleet_config(&self) -> hakus_config::FleetConfigToml {
    ...
    #[must_use]
    pub fn workflow_config(&self) -> hakus_config::WorkflowConfigToml {
    ...
    ) -> hakus_config::HotbarConfigResolution {
        hakus_config::resolve_hotbar_bindings(self.hotbar.as_deref(), known_action_ids)
    }
...
mod paths;
use paths::{
    canonicalize_or_keep, hakus_home_dir, default_config_path, default_managed_config_path,
...
pub(crate) fn workspace_trust_config_candidate_paths() -> Vec<PathBuf> {
        ...
        if !crate::test_support::guarded_environment_provides_state_paths() {
            return vec![
                crate::test_support::unsealed_test_state_root()
                    .join(hakus_config::CONFIG_FILE_NAME),
            ];
        }
    }
    ...
    match hakus_home_dir() {
        Ok(Some(hakus_home)) => return vec![hakus_home.join("config.toml")],
        Ok(None) => {}
        Err(error) => {
            tracing::error!(
                error = %error,
                "invalid Hakus home override; refusing workspace-trust fallback"
            );
            return Vec::new();
        }
    }
...
///
/// The file intentionally omits `api_key`; onboarding or `hakus auth set`
...
pub fn ensure_config_file_exists(path: Option<PathBuf>) -> Result<Option<PathBuf>> {
    ...
    ensure_parent_dir(&config_path)?;
    let content = format!(
        r#"# hakus Configuration
# Get your API key from https://platform.deepseek.com
# Save it with: hakus auth set --provider deepseek
...
# update_uri = "https://internal.mirror.example/hakus/releases/latest"
"#
    );
...
/// Read the `DEEPSEEK_BASE_URL` / `HAKUS_BASE_URL` env var that the CLI
...
fn env_base_url_override() -> Option<String> {
    hakus_env_var("HAKUS_BASE_URL", "DEEPSEEK_BASE_URL")
        .ok()
...
/// Resolve an env var, preferring the `HAKUS_*` form over the
...
fn apply_env_overrides_unlocked(config: &mut Config, policy: ConfigEnvironmentPolicy) {
    if let Ok(value) = hakus_env_var("HAKUS_PROVIDER", "DEEPSEEK_PROVIDER") {
    ...
    if let Ok(value) = hakus_env_var("HAKUS_BASE_URL", "DEEPSEEK_BASE_URL") {
    ...
    if policy.permits_secret_bearing_values()
        && let Ok(value) = std::env::var("HAKUS_HTTP_HEADERS")
    ...
    if let Some(value) = hakus_env_var("HAKUS_MODEL", "DEEPSEEK_MODEL")
        .ok()
        .or_else(|| {
    ...
    if let Ok(value) =
        std::env::var("HAKUS_SKILLS_DIR").or_else(|_| std::env::var("DEEPSEEK_SKILLS_DIR"))
    {
        config.skills_dir = Some(value);
    }
    if let Ok(value) =
        std::env::var("HAKUS_MCP_CONFIG").or_else(|_| std::env::var("DEEPSEEK_MCP_CONFIG"))
    {
        config.mcp_config_path = Some(value);
    }
    if let Ok(value) =
        std::env::var("HAKUS_NOTES_PATH").or_else(|_| std::env::var("DEEPSEEK_NOTES_PATH"))
    {
        config.notes_path = Some(value);
    }
    if let Ok(value) =
        std::env::var("HAKUS_MEMORY_PATH").or_else(|_| std::env::var("DEEPSEEK_MEMORY_PATH"))
    {
        config.memory_path = Some(value);
    }
    if let Ok(value) =
        std::env::var("HAKUS_MEMORY").or_else(|_| std::env::var("DEEPSEEK_MEMORY"))
    {
        let on = matches!(
    ...
    if let Ok(value) =
        std::env::var("HAKUS_ALLOW_SHELL").or_else(|_| std::env::var("DEEPSEEK_ALLOW_SHELL"))
    {
        config.allow_shell = Some(value == "1" || value.eq_ignore_ascii_case("true"));
    }
    if let Ok(value) = std::env::var("HAKUS_APPROVAL_POLICY")
        .or_else(|_| std::env::var("DEEPSEEK_APPROVAL_POLICY"))
    {
        config.approval_policy = Some(value);
    }
    if let Ok(value) =
        std::env::var("HAKUS_SANDBOX_MODE").or_else(|_| std::env::var("DEEPSEEK_SANDBOX_MODE"))
    {
        config.sandbox_mode = Some(value);
    }
    if let Ok(value) = std::env::var("HAKUS_YOLO").or_else(|_| std::env::var("DEEPSEEK_YOLO")) {
        config.yolo = Some(value == "1" || value.eq_ignore_ascii_case("true"));
    }
    if let Ok(value) =
        std::env::var("HAKUS_VERBOSITY").or_else(|_| std::env::var("DEEPSEEK_VERBOSITY"))
    {
        config.verbosity = Some(value);
    }
    if let Ok(value) = std::env::var("HAKUS_SANDBOX_BACKEND")
        .or_else(|_| std::env::var("DEEPSEEK_SANDBOX_BACKEND"))
    {
        config.sandbox_backend = Some(value);
    }
    if let Ok(value) = hakus_env_var("HAKUS_PREFER_BWRAP", "DEEPSEEK_PREFER_BWRAP") {
        let primary_is_set = std::env::var("HAKUS_PREFER_BWRAP")
            .ok()
            .is_some_and(|value| !value.trim().is_empty());
        ...
        if !primary_is_set && legacy_is_set {
            tracing::warn!(
                "DEEPSEEK_PREFER_BWRAP is deprecated; use HAKUS_PREFER_BWRAP (the legacy alias is removed in 0.10.0)"
            );
        }
        config.prefer_bwrap = Some(value == "1" || value.eq_ignore_ascii_case("true"));
    }
    if let Ok(value) =
        std::env::var("HAKUS_SANDBOX_URL").or_else(|_| std::env::var("DEEPSEEK_SANDBOX_URL"))
    {
        config.sandbox_url = Some(value);
    }
    if policy.permits_secret_bearing_values()
        && let Ok(value) = std::env::var("HAKUS_SANDBOX_API_KEY")
            .or_else(|_| std::env::var("DEEPSEEK_SANDBOX_API_KEY"))
    {
        config.sandbox_api_key = Some(value);
    }
    if let Ok(value) = std::env::var("HAKUS_MANAGED_CONFIG_PATH")
        .or_else(|_| std::env::var("DEEPSEEK_MANAGED_CONFIG_PATH"))
    {
        config.managed_config_path = Some(value);
    }
    if policy.permits_secret_bearing_values()
        && let Ok(value) = std::env::var("HAKUS_SEARCH_API_KEY")
    ...
    if let Ok(value) = hakus_env_var("HAKUS_SEARCH_BASE_URL", "DEEPSEEK_SEARCH_BASE_URL") {
        config
            .search
            .get_or_insert_with(SearchConfig::default)
            .base_url = Some(value);
    }
    if let Ok(value) = std::env::var("HAKUS_REQUIREMENTS_PATH")
        .or_else(|_| std::env::var("DEEPSEEK_REQUIREMENTS_PATH"))
    {
        config.requirements_path = Some(value);
    }
    if let Ok(value) = std::env::var("HAKUS_MAX_SUBAGENTS")
...
fn base_url_is_custom_for_provider(provider: ApiProvider, base_url: &str) -> bool {
    let kind = provider
        .kind()
        .unwrap_or(hakus_config::ProviderKind::Deepseek);
    hakus_config::provider_preserves_custom_base_url_model(kind, base_url)
}
...
pub(crate) fn moonshot_base_url_is_exact_kimi_code(base_url: &str) -> bool {
    hakus_config::provider::is_exact_kimi_code_route(
        hakus_config::ProviderKind::Moonshot,
        base_url,
    )
}
...
pub(crate) fn moonshot_base_url_is_exact_direct_platform(base_url: &str) -> bool {
    hakus_config::provider::is_exact_moonshot_platform_route(
        hakus_config::ProviderKind::Moonshot,
        base_url,
    )
}
...
pub(crate) fn is_exact_xai_grok_4_6_route(
    ...
    provider == ApiProvider::Xai
        && hakus_config::provider::is_exact_xai_platform_route(
            hakus_config::ProviderKind::Xai,
            base_url,
        )
...
pub(crate) fn is_exact_zai_chat_route(provider: ApiProvider, base_url: &str) -> bool {
    provider == ApiProvider::Zai
        && hakus_config::provider::is_exact_zai_chat_route(
            hakus_config::ProviderKind::Zai,
            base_url,
        )
}
...
pub(crate) fn minimax_base_url_is_supported_direct(base_url: &str) -> bool {
    hakus_config::provider::is_exact_minimax_chat_route(
        hakus_config::ProviderKind::Minimax,
        base_url,
    ) || hakus_config::provider::is_exact_minimax_anthropic_route(
        hakus_config::ProviderKind::MinimaxAnthropic,
        base_url,
    )
}
...
/// metadata in `hakus-config`: that remains correct for Moonshot's own
...
pub(crate) fn credential_help_for_provider_route(
    provider: ApiProvider,
    base_url: &str,
) -> hakus_config::provider::CredentialHelp {
    provider.kind().map_or_else(
        || provider.credential_help(),
        |kind| hakus_config::provider::credential_help_for_route(kind, base_url),
    )
}
...
fn load_sibling_exec_policy_engine(config_path: Option<&Path>) -> Result<ExecPolicyEngine> {
    ...
    let permissions_path = hakus_config::permissions_path_for_config_path(config_path);
    ...
    let permissions: hakus_config::PermissionsToml = toml::from_str(&raw).map_err(|_| {
        anyhow::anyhow!(
            "Failed to parse permissions file {}; file contents were omitted",
            hakus_config::quote_os_path(&permissions_path)
        )
    })?;
...
fn merge_skills_config(
    ...
    match (base, override_cfg) {
        ...
        (Some(base), Some(override_cfg)) => Some(SkillsConfig {
            ...
            scan_hakus_only: override_cfg
                .scan_hakus_only
                .or(base.scan_hakus_only),
        }),
    }
}
...
fn merge_provider_config(base: ProviderConfig, override_cfg: ProviderConfig) -> ProviderConfig {
    ProviderConfig {
        ...
        models: if override_cfg.models.is_empty() {
            base.models
        } else {
            override_cfg.models
        },
...
/// Hakus does not define (`[general]` / `[sandbox]`). TOML silently drops
...
fn warn_on_misplaced_top_level_keys(raw: &str) -> Option<String> {
    ...
    // Sections Hakus does not recognize but users nest settings under.
    ...
    Some(format!(
        "Ignoring {} — Hakus has no `[general]` or `[sandbox]` section, so these \
...
fn apply_managed_overrides(config: &mut Config) -> Result<()> {
    ...
    if prior_route != merged_route || config_defines_base_url_for_effective_route(&managed, &merged)
    {
        ...
        // `HAKUS_BASE_URL` fallback for every route — including pinned
...
fn strip_external_credential_consent(config: &mut Config) {
        ...
        .filter(|provider| *provider != ApiProvider::Custom)
    {
        ...
        if external.as_ref().is_some_and(|consent| {
            consent.access != hakus_config::ExternalCredentialAccess::Disabled
        }) {
            *external = None;
        }
    }
...
fn write_config_file_secure(path: &Path, content: &str) -> Result<()> {
    hakus_config::create_config_document(path, content)
}
...
pub enum SavedCredential {
    ...
    /// [`hakus_secrets::Secrets::backend_name`] at write time so the toast
    /// text can name the actual backend (`"system keyring"`,
    /// `"file-based (~/.hakus/secrets/)"`).
    ...
    /// Stored in the Hakus config file only under `cfg(test)` so unit tests
...
/// `HAKUS_CONFIG_PATH`/`DEEPSEEK_CONFIG_PATH` override can point at a
/// workspace-scoped document (`<repo>/.hakus/config.toml`, plaintext and
...
fn credential_config_path() -> anyhow::Result<PathBuf> {
    ...
    if !hakus_config::config_path_is_workspace_scoped(&resolved) {
        return Ok(resolved);
    }
...
/// both an isolated `HAKUS_HOME` and an explicit backend, preventing unit
        ...
        match prior_secret.as_ref() {
            Ok(prior) => match secrets.set(secret_slot, trimmed) {
                Ok(()) => {
                    ...
                    hakus_config::scrub_plaintext_api_keys_from_config_backup(&path)?;
...
fn plaintext_credential_fallback_refused(
    ...
    anyhow::anyhow!(
        "Secret storage {operation} failed: {failure}. Refusing to write the API key in plaintext to {}. Fix the configured secret backend and retry; Hakus did not change that file.",
        hakus_config::quote_os_path(config_path)
    )
}
...
/// isolated `HAKUS_HOME` and an explicit backend, so unit tests can never
/// touch the developer's real credential store.
#[cfg(not(test))]
fn credential_secret_store() -> Option<hakus_secrets::Secrets> {
    Some(hakus_secrets::Secrets::auto_detect())
}

#[cfg(test)]
fn credential_secret_store() -> Option<hakus_secrets::Secrets> {
    let isolated_home = hakus_paths::hakus_home_is_explicit();
    let explicit_backend = std::env::var_os("HAKUS_SECRET_BACKEND")
        .or_else(|| std::env::var_os("DEEPSEEK_SECRET_BACKEND"))
        .is_some_and(|value| !value.is_empty());
    (isolated_home && explicit_backend).then(hakus_secrets::Secrets::auto_detect)
}
...
fn user_global_config_json() -> Option<serde_json::Value> {
    ...
    let path = hakus_config::default_config_path().ok()?;
    ...
    let doc: hakus_config::ConfigToml = toml::from_str(&text).ok()?;
...
pub fn has_api_key_for(config: &Config, provider: ApiProvider) -> bool {
    ...
    if provider == ApiProvider::Antigravity && !config.provider_uses_custom_endpoint(provider) {
        let path = hakus_config::default_agy_credentials_path();
        if config
            .external_credential_read_grant(
                provider,
                hakus_config::ExternalCredentialSource::AgyCli,
...
/// `[providers.<name>] api_key = "..."` to `~/.hakus/config.toml`.
...
pub(crate) fn save_api_key_for_identity(
    ...
    if identity.provider == ApiProvider::Xai {
        return hakus_config::with_xai_oauth_revocation_transaction(|| {
...
fn save_api_key_for_identity_unlocked(
    ...
    if provider == ApiProvider::OpenaiCodex {
        anyhow::bail!(
            "OpenAI Codex uses OAuth. Run `codex login`, then grant exact read-only access with `hakus auth external-consent --provider openai-codex --mode read-only`, or set OPENAI_CODEX_ACCESS_TOKEN for this process; Hakus does not store an API key for this provider."
        );
    }
        ...
        match prior_secret.as_ref() {
            Ok(prior) => match secrets.set(secret_slot, api_key) {
                Ok(()) => {
                    ...
                    hakus_config::scrub_plaintext_api_keys_from_config_backup(&config_path)?;
                    let backend = secrets.backend_name().to_string();
                    log_sensitive_event(
                        "credential.save",
                        json!({
...
pub(crate) fn persist_external_credential_consent_for_at(
    ...
    provider: ApiProvider,
    consent_provider: hakus_config::ProviderKind,
    source: hakus_config::ExternalCredentialSource,
    path: &Path,
) -> Result<PathBuf> {
    ...
    let path = hakus_config::resolve_external_credential_path(path)?;
    ...
    crate::config_persistence::mutate_config_document(&config_path, |doc| {
        ...
        crate::config_persistence::set_document_value(
            ...
            i64::from(hakus_config::EXTERNAL_CREDENTIAL_CONSENT_VERSION),
        )
    })
    .with_context(|| {
        format!(
    ...
    entry.auth_mode = Some("oauth".to_string());
    entry.external_credentials = Some(hakus_config::ExternalCredentialConsentToml::read_only(
        consent_provider,
        source,
        path,
    ));
...
fn provider_env_api_key(provider: ApiProvider) -> Option<String> {
    let from_env = if provider == ApiProvider::Huggingface {
        std::env::var("HUGGINGFACE_API_KEY")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .or_else(|| {
    ...
    // HakusAgent repo bridge: fall back to the repository root config.yaml
    // `api_keys` map (walked up from the CWD) when no env var had the key.
    from_env.or_else(|| hakus_config::hakus_agent_yaml_api_key(provider.as_str()))
}
...
fn provider_secret_store_api_key_with_mode(
    ...
    // Secret-store regressions opt in with an isolated HAKUS_HOME and an
    // explicit backend, matching the secrets crate's own test discipline.
    #[cfg(test)]
    if !hakus_paths::hakus_home_is_explicit()
        || std::env::var_os("HAKUS_SECRET_BACKEND").is_none()
    {
        return None;
    }

    let secrets = if read_only {
        hakus_secrets::Secrets::auto_detect_read_only()
    } else {
...
/// `hakus auth set` and nothing changes, because the stale plaintext
    ...
    provider_secret_store_api_key_with_mode(config, provider, true).map(|_| {
        ...
        format!(
            "both {config_source} in the config file and secret-store slot \"{slot}\" \
             ...
             `hakus auth set --provider {id}` to move the key into the secret store \
...
///
/// The `hakus` dispatcher forwards `--model` to this binary as
/// `HAKUS_MODEL` (with the legacy `DEEPSEEK_MODEL` alias), so an explicit
...
pub(crate) fn explicit_launch_model_override() -> Option<String> {
    hakus_env_var("HAKUS_MODEL", "DEEPSEEK_MODEL")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}
...
pub(crate) fn explicit_launch_provider_override() -> Option<String> {
    hakus_env_var("HAKUS_PROVIDER", "DEEPSEEK_PROVIDER")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}
...
fn missing_provider_api_key_message(provider: ApiProvider) -> Result<String> {
    let credential_hint = provider
        ...
        .unwrap_or_default();
    Ok(format!(
        "{} API key not found.{} Run 'hakus auth set --provider {}', set {}, or add [{}] api_key in ~/.hakus/config.toml.",
        provider.display_name(),
        credential_hint,
        provider.as_str(),
        provider.env_vars_label(),
...
/// secret store.
///
/// The full-wipe logout path (`hakus-tui --logout`, `auth logout`)
...
/// workspace-scoped `.hakus/config.toml`.
#[cfg(test)]
mod credential_scope_tests {
    ...
    /// `.hakus/config.toml` (a checkout the user works in), saving an
    /// API key must write the user-global config under the isolated
    /// `HAKUS_HOME`, never the project file. The `.git` marker stands in
    ...
    fn api_key_save_rescopes_workspace_config_to_user_global() -> Result<()> {
        ...
        let project_dir = workspace.join(".hakus");
        ...
        let _home = EnvVarGuard::set("HAKUS_HOME", user_home.as_os_str());
        let _config = EnvVarGuard::set("HAKUS_CONFIG_PATH", project_config.as_os_str());
        ...
        // regression guards.
        let _backend = EnvVarGuard::remove("HAKUS_SECRET_BACKEND");
