//! Hakus TUI library — single-binary entry point.
...
#[cfg(test)]
mod test_support;
// TLS bootstrap and platform client builders live in hakus-release;
// `crate::tls::*` keeps resolving for every caller.
use hakus_release::tls;
mod todo_snapshot;
...
#[command(
    ...
    bin_name = "hakus-tui",
    author,
    version = env!("HAKUS_BUILD_VERSION"),
    about = "Hakus terminal coding agent",
...
struct Cli {
    ...
    /// Skip loading project-level config from $WORKSPACE/.hakus/config.toml
...
enum Commands {
    ...
    Setup(SetupArgs),
    /// Generate a remote Hakus agent deploy bundle (cloud + chat bridge)
    ...
    /// Connect third-party harnesses through Hakus (currently: DeepSeek Harness `dsh`)
    Integrations {
        #[command(subcommand)]
...
#[command(after_help = "\
Examples:
  hakus exec \"explain this function\"
  hakus exec --auto \"list crates/ with ls\"
  hakus exec --auto --output-format stream-json \"fix the failing test\"

Plain `hakus exec` is a one-shot model response. Use `--auto` for
...
const HAKUS_TOOL_SURFACE_ENV: &str = "HAKUS_TOOL_SURFACE";
...
fn exec_tool_surface_from_env() -> Option<ExecToolSurface> {
    std::env::var(HAKUS_TOOL_SURFACE_ENV)
        .ok()
        .and_then(|value| {
            if should_warn_unknown_exec_tool_surface(&value) {
                eprintln!(
                    "warning: unrecognized {HAKUS_TOOL_SURFACE_ENV}; leaving exec tool surface unchanged. Use `shell-only`, `full`, or `native-tools`."
                );
            }
...
enum FleetCommand {
    ...
    Inspect {
        /// Worker id printed by `hakus fleet run`
        worker_id: String,
    },
    ...
    Resume {
        /// Run id printed by `hakus fleet run`
        run_id: String,
...
struct FleetAlertDryRunArgs {
    ...
    #[arg(long, default_value = "HAKUS_FLEET_SLACK_WEBHOOK")]
    slack_webhook_env: String,
    /// Environment variable containing the generic webhook URL
    #[arg(long, default_value = "HAKUS_FLEET_WEBHOOK_URL")]
    ...
    #[arg(long, default_value = "HAKUS_FLEET_PAGERDUTY_ROUTING_KEY")]
...
fn spawn_signal_cleanup_task() {
    ...
    tokio::spawn(async move {
        ...
        if !CLEANED_UP.swap(true, std::sync::atomic::Ordering::SeqCst) {
            ...
            // would let a second Hakus process sharing HAKUS_HOME hang
...
fn telemetry_session_end() -> hakus_telemetry::Event {
    let counters = hakus_telemetry::session_counters();
    hakus_telemetry::Event::SessionEnd {
        duration_bucket: hakus_telemetry::DurationBucket::from_secs(
            TELEMETRY_SESSION_START
                .get()
                .map_or(0, |start| start.elapsed().as_secs()),
        ),
        exit_class: hakus_telemetry::exit_class(),
        cold_start_bucket: crate::startup_trace::cold_start_ms()
            .map(hakus_telemetry::ColdStartBucket::from_millis),
...
fn record_signal_session_end() {
    hakus_telemetry::set_exit_class(hakus_telemetry::ExitClass::Signal);
    hakus_telemetry::record_blocking(telemetry_session_end());
}
...
fn resolve_exec_resume_session_id(args: &ExecArgs, workspace: &Path) -> Result<Option<String>> {
    ...
    latest_session_id_for_workspace(workspace)?.map_or_else(
        || {
            bail!(
                "No saved sessions found for workspace {}. Use `hakus sessions` to list sessions, or pass `hakus exec --resume <SESSION_ID> ...`.",
                workspace.display()
            )
        },
        |id| Ok(Some(id)),
...
fn resolve_exec_resume_route(
    ...
    if !explicit_provider {
        ...
        let identity = config
            ...
            .with_context(|| {
                format!(
                    "saved session provider '{}' is unavailable; Hakus will not fall back",
...
struct ServeArgs {
    ...
    /// Also reads `HAKUS_CORS_ORIGINS` (comma-separated), then
    ...
    /// `HAKUS_RUNTIME_TOKEN` when omitted, then `DEEPSEEK_RUNTIME_TOKEN`
    /// as an alias.
...
enum McpCommand {
    ...
    Validate,
    /// Register this Hakus binary as a local MCP stdio server.
    ///
    /// This adds a config entry that runs `hakus serve --mcp` (stdio protocol).
    /// For the HTTP/SSE runtime API, use `hakus serve --http` directly instead.
    #[command(
        name = "add-self",
        long_about = "Register this Hakus binary as a local MCP stdio server.\n\nAdds a config entry to ~/.hakus/mcp.json that launches `hakus serve --mcp`\nvia the stdio transport. Other Hakus sessions (or any MCP client) can then\ndiscover and call tools exposed by this server.\n\nUse `hakus serve --http` instead if you need the HTTP/SSE runtime API."
    )]
    AddSelf {
        /// Server name in mcp.json (default: "hakus")
        #[arg(long, default_value = "hakus")]
        name: String,
...
pub(crate) enum IntegrationsCommand {
    /// Official DeepSeek Harness (`dsh`) connected through Hakus
    Dsh {
        #[command(subcommand)]
...
pub(crate) enum DshIntegrationCommand {
    ...
    Plan {
        ...
        profile: String,
        /// Mirror Hakus full access as DSH danger-full-access (only when Hakus itself runs with full access)
        #[arg(long, default_value_t = false)]
        allow_full_access: bool,
        /// Record the Hakus palette (skin) decision for the bundle profile; applied via DSH's `overrideTokens`, never through the overlay
        #[arg(long, default_value_t = false)]
        skin: bool,
    },
    /// Write the overlay and receipt under $HAKUS_HOME/integrations/dsh
    Connect {
        #[arg(long, default_value = "web")]
        profile: String,
        #[arg(long, default_value_t = false)]
    ...
    /// Re-derive the overlay from the current Hakus route
    Update {
        #[arg(long)]
        profile: Option<String>,
        #[arg(long, default_value_t = false)]
    ...
    /// Run dsh with the Hakus overlay; extra args go to the dsh app
    Launch {
    ...
    Enable,
    /// Delete Hakus-owned files only; $DSH_HOME is never touched
    Remove {
        #[arg(long, default_value_t = false)]
        yes: bool,
    },
    /// Documented DSH plugin path: install the Hakus bundle into a dedicated `hakus` DSH profile via `dsh plugin add` (pnpm required)
    InstallBundle {
    ...
    /// `dsh plugin --profile hakus remove hakus-dsh-bundle`, then delete only Hakus-owned bundle files
    RemoveBundle {
        #[arg(long, default_value_t = false)]
        yes: bool,
    },
...
const HAKUS_MAIN_STACK_BYTES: usize = 16 * 1024 * 1024;
...
fn run_with_args(args: Vec<String>) -> Result<()> {
    ...
    // SIGPIPE ignored, which turns short pipelines such as `hakus doctor |
    ...
    std::panic::set_hook(Box::new(move |panic_info| {
        ...
        hakus_telemetry::set_exit_class(hakus_telemetry::ExitClass::Panic);
        if let Some(site) = panic_info
            .location()
            .map(|loc| hakus_telemetry::reduce_panic_site(loc.file(), loc.line(), loc.column()))
        {
    ...
    // #5098: project-scope fleet agent profiles (`.hakus/agents/*.toml`)
    ...
    let runtime_thread = std::thread::Builder::new()
        .name("hakus-main".to_string())
        .stack_size(HAKUS_MAIN_STACK_BYTES)
        .spawn(move || run_async_main(cli, command, plugin_discovery, plugin_registry))
        .context("Failed to start the Hakus runtime thread")?;
    match runtime_thread.join() {
        ...
        Err(payload) => {
            ...
            Err(anyhow!("Hakus runtime thread panicked: {message}"))
        }
    }
}
...
/// tokio's 2 MiB default while only the `hakus-main` owner thread above
/// received `HAKUS_MAIN_STACK_BYTES`. The engine does not run on that owner
...
pub(crate) fn build_runtime() -> Result<tokio::runtime::Runtime> {
    tokio::runtime::Builder::new_multi_thread()
        ...
        .build()
        .context("Failed to build the Hakus Tokio runtime")
}
...
/// them the same.
fn telemetry_surface(command: Option<&Commands>) -> hakus_telemetry::Surface {
    use hakus_telemetry::Surface;
    match command {
...
fn telemetry_session_source(command: Option<&Commands>) -> hakus_telemetry::SessionSource {
    use hakus_telemetry::SessionSource;
    match command {
...
/// `telemetry = false` in the config file nor `HAKUS_TELEMETRY=0` was ever
...
/// `HAKUS_TELEMETRY`, which `EnvRuntimeOverrides::load()` picks up — and
/// re-reading `HAKUS_TELEMETRY` inside the telemetry crate would fork
/// `parse_bool`, the `DEEPSEEK_TELEMETRY` alias, and the floor into a second
/// source of truth.
fn arm_telemetry_with_setup(
    config_path: Option<PathBuf>,
    surface: hakus_telemetry::Surface,
    source: hakus_telemetry::SessionSource,
    setup_override: Option<&hakus_config::SetupState>,
) {
    let Ok(store) = hakus_config::ConfigStore::load(config_path) else {
        return;
    };
    let resolved = store
        .config
        .resolve_runtime_options(&hakus_config::CliRuntimeOverrides::default());
    let setup = if let Some(setup) = setup_override {
        setup.clone()
    } else {
        let Some(setup) = hakus_telemetry::load_setup_state_for_decision() else {
    ...
    let hakus_telemetry::TelemetryDecision::Enabled(consent) =
        hakus_telemetry::decide(&resolved, &setup, surface)
    else {
        return;
    };
    hakus_telemetry::init(consent.with_config_path(Some(store.path().to_path_buf())));
    let _ = TELEMETRY_SESSION_START.set(std::time::Instant::now());
    hakus_telemetry::record(hakus_telemetry::Event::SessionStart { source });
}
...
pub(crate) fn apply_tui_telemetry_decision(
    ...
    setup: &hakus_config::SetupState,
) {
    arm_telemetry_with_setup(
        pending.config_path.clone(),
        hakus_telemetry::Surface::Tui,
...
/// `hakus config list`. A configured endpoint remains buffered for the
/// next interactive session; an explicitly empty endpoint writes its local
/// dry-run batch immediately.
async fn finish_telemetry(outcome: &Result<()>, surface: hakus_telemetry::Surface) {
    if !hakus_telemetry::is_armed() {
        return;
    }
    ...
    if outcome.is_err()
        && hakus_telemetry::exit_class() == hakus_telemetry::ExitClass::Clean
    {
        hakus_telemetry::set_exit_class(hakus_telemetry::ExitClass::Error);
    }
    hakus_telemetry::record(telemetry_session_end());
    if surface == hakus_telemetry::Surface::Cli {
        let _ =
            hakus_telemetry::persist_local_blocking(hakus_telemetry::CLI_PERSIST_TIMEOUT);
        return;
    }
    ...
    let _ = tokio::time::timeout(
        hakus_telemetry::SHUTDOWN_FLUSH_TIMEOUT,
        tokio::task::spawn_blocking(|| {
            hakus_telemetry::shutdown_blocking(hakus_telemetry::SHUTDOWN_FLUSH_TIMEOUT)
        }),
    )
    .await;
}
...
async fn run_async_main_inner(
    ...
    let telemetry_notice_plan = if surface == hakus_telemetry::Surface::Tui {
    ...
    let should_arm_before_dispatch = surface != hakus_telemetry::Surface::Tui
...
async fn run_async_main_dispatch(
    ...
    if let Some(command) = command {
        return match command {
            ...
            Commands::Exec(args) => {
                ...
                // Honour HAKUS_BASE_URL / DEEPSEEK_BASE_URL forwarded by
                // the CLI dispatcher from --base-url.
                if let Ok(env_url) = std::env::var("HAKUS_BASE_URL")
    ...
    // for follow-up messages. Use `hakus exec` for explicit non-interactive
    ...
    // Handle session resume. Plain `hakus` starts fresh: interrupted
    ...
    let resume_session_id = if cli.continue_session {
        ...
        // "plain `hakus` starts fresh" behaviour is unchanged unless the
...
///
/// Deliberately scoped to the plain interactive launch. `hakus "do X"`
/// (top-level prompt) and `hakus exec` are not covered: silently prefixing
...
/// `.env` must not be able to redirect `HAKUS_HOME`, config/profile files,
...
fn warn_on_workspace_dotenv_result() {
    match load_workspace_dotenv_credentials() {
        Ok(Some(report)) if !report.ignored.is_empty() => {
            eprintln!(
                "Hakus ignored non-credential settings in {}: {}. Use config.toml, CLI flags, or the launching shell for control settings.",
                report.path.display(),
        ...
        Err(error) => {
            ...
            eprintln!("Hakus did not load workspace .env: {error}");
        }
    }
}
...
fn load_workspace_dotenv_credentials_from_path(path: &Path) -> Result<WorkspaceDotenvReport> {
    ...
    for entry in entries {
        ...
        // exists inside Hakus, and later startup code treats this process
...
fn is_workspace_dotenv_credential_key(key: &str) -> bool {
    hakus_config::provider::providers_sorted_for_display()
        .into_iter()
...
async fn run_fleet_command(workspace: &Path, config: &Config, args: FleetArgs) -> Result<()> {
    ...
    use hakus_lane::{ControlOperation, ControlSurface};
    use hakus_protocol::fleet::{FleetAlertEventClass, FleetArtifactKind, FleetRunId};

    // Every label and every row below comes from the shared Fleet control
    // surface, so `hakus fleet …` and `/fleet …` cannot drift in how they
    ...
    fn emit_fleet_receipt(receipt: &hakus_lane::ControlReceipt) -> Result<()> {
        if receipt.is_error() {
    ...
    // creates `.hakus/fleet.jsonl` as a side effect, so a later probe would
    ...
    // made `hakus fleet status` in a ledgerless workspace print
    ...
    if let Some(operation) = match &args.command {
        ...
        if !availability.is_available() {
            return emit_fleet_receipt(&hakus_lane::ControlReceipt::unavailable(
                descriptor,
                ControlSurface::Cli,
                availability,
            ));
    ...
    match args.command {
        ...
        FleetCommand::Run(args) => {
            ...
            println!(
                "manager loop running; use `hakus fleet status`, `inspect`, `interrupt`, or `stop --all` from another terminal."
            );
            let mut executor = FleetExecutor::new(workspace);
            let hakus_binary = fleet::executor::configured_hakus_binary();
            let status = manager
                .run_to_completion(
                    &report.run_id,
                    max_workers,
        ...
        FleetCommand::Restart { worker_id } => {
            ...
            println!(
                "manager loop running for restarted run {}; use `hakus fleet status`, `inspect`, `interrupt`, or `stop --all` from another terminal.",
                report.run_id.0
            );
            let mut executor = FleetExecutor::new(workspace);
            let hakus_binary = fleet::executor::configured_hakus_binary();
...
fn tools_readme_template() -> &'static str {
    "# Local tools\n\n\
     ...
     `hakus-tui setup --status` and surfaced in `hakus-tui doctor`.\n\n\
     When `[tools.plugin_dir]` is set in config.toml (or when the default\n\
     `~/.hakus/tools/` directory exists), they are auto-discovered and\n\
...
fn tools_example_script() -> &'static str {
    "#!/usr/bin/env sh\n\
     ...
     # usage: example [name]\n\
     printf 'hakus-tui local tool ok: %s\\n' \"${1:-world}\"\n"
}
...
fn plugins_readme_template() -> &'static str {
    "# Local plugins\n\n\
     Each Hakus plugin bundle lives in its own subdirectory with a\n\
     versioned `plugin.toml`. User bundles live here; workspace bundles live\n\
     under `<workspace>/.hakus/plugins/`. Both are discovered read-only,\n\
     ...
     in `plugin.toml`.\n\n\
     Hakus activates declarative Skills, MCP servers, Commands, Agent\n\
     ...
     disabled-and-untrusted review path; none grants automatic trust. Hakus\n\
...
fn plugin_example_manifest_template() -> &'static str {
    "schema_version = 1\n\n\
     ...
     name = \"example\"\n\
     version = \"0.1.0\"\n\
     description = \"Starter Hakus plugin bundle\"\n\n\
     [skills]\n\
     path = \"skills\"\n"
}
...
/// Resolve the user-supplied CORS origins for `hakus serve --http`.
///
/// Sources, in priority order (later sources extend earlier ones):
/// 1. `--cors-origin URL` flags (repeatable)
/// 2. `HAKUS_CORS_ORIGINS` env var (comma-separated),
...
fn resolve_cors_origins(config: &Config, flag_origins: &[String]) -> Vec<String> {
    ...
    if let Ok(env_value) =
        std::env::var("HAKUS_CORS_ORIGINS").or_else(|_| std::env::var("DEEPSEEK_CORS_ORIGINS"))
    {
...
fn deepseek_home_dir() -> PathBuf {
    hakus_config::hakus_home().unwrap_or_else(|_| {
        crate::config::effective_home_dir()
            .map_or_else(|| PathBuf::from(".hakus"), |h| h.join(".hakus"))
    })
}
...
fn run_setup(
    ...
    println!(
        "{}",
        "Hakus Setup".truecolor(aqua_r, aqua_g, aqua_b).bold()
    );
    ...
    if run_mcp {
        ...
        println!(
            "    Next: edit the file, then run `hakus mcp list` or `hakus mcp tools`."
        );
    }
...
fn run_setup_status(
    ...
    println!(
        "{}",
        "Hakus Status".truecolor(aqua_r, aqua_g, aqua_b).bold()
    );
    println!("{}", "===============".truecolor(sky_r, sky_g, sky_b));
    ...
    match credential.source {
        ...
        ApiKeySource::OAuth => println!(
            "  {} oauth: Hakus-owned route selected (token availability not probed)",
            "·".dimmed()
        ),
    ...
    println!();
    println!("Run `hakus doctor --json` for a machine-readable check.");
    Ok(())
}
...
async fn run_doctor(
    ...
    println!("  hakus-tui: {}", env!("HAKUS_BUILD_VERSION"));
    ...
    let secret_backend = hakus_secrets::diagnose_secret_backend();
    println!();
    ...
    if active_root != &code_home {
        println!(
            "  note: legacy {} found; start Hakus once to trigger safe migration where available.",
    ...
    let session_recovery = doctor_session_recovery_report(
        &code_home,
        &legacy_home,
        hakus_config::hakus_home_is_explicit(),
    );
    ...
    if doctor_should_probe_auth(config) && live_api_requested {
        ...
        match connectivity_result {
            ...
            Err(e) => {
                ...
                if error_msg.contains("401") || error_msg.contains("Unauthorized") {
                    println!(
                        "    Invalid API key. Check `hakus auth status`, DEEPSEEK_API_KEY, or config.toml"
                    );
        ...
        if endpoint_is_local {
            ...
            println!(
                "    Run `hakus doctor --probe-local` to opt in; the request may start a local service."
            );
        } else {
            println!(
            ...
            println!("    Run `hakus doctor --probe-api` to opt in.");
        }
    }
    ...
    match crate::mcp::load_config_with_workspace_and_plugins(&mcp_config_path, workspace, plugins) {
        Ok(cfg) if cfg.servers.is_empty() => {
            ...
            if !mcp_config_path.exists() && !project_mcp_config_path.exists() {
                println!("    Run `hakus mcp init` or add `.hakus/mcp.json`.");
            }
        }
        Ok(cfg) => {
            println!(
            ...
            for (name, server) in &cfg.servers {
                ...
                if !server.is_enabled() {
                    ...
                    println!(
                        "      process/protocol/backend: not checked; `hakus mcp validate` explicitly starts and initializes configured servers"
                    );
                }
            }
            if probes.should_probe_mcp() {
                ...
                println!(
                    "    Use hakus doctor --probe-mcp to opt in to live process/protocol checks; it may start configured servers."
                );
            }
        }
        Err(_) => {
    ...
    {
        println!("    Run `hakus setup --skills` (or add --local for ./skills).");
    }
    ...
    if tools_dir.exists() {
        ...
        println!("    Run `hakus setup --tools` to scaffold a starter dir.");
    }
    ...
    if plugins_dir.exists() {
        ...
        println!("    Run `hakus setup --plugins` to scaffold a starter dir.");
    }
    ...
    // PDF text extraction is an optional integration. Hakus itself stays
...
fn print_doctor_legacy_state_report(
        ...
        && session_recovery.status != DoctorSessionRecoveryStatus::Isolated
    {
        ...
        println!(
            "    Start Hakus once to trigger safe migration where available, then rerun `hakus doctor`."
        );
    }
...
fn print_doctor_session_recovery_report(
    ...
    match report.status {
        DoctorSessionRecoveryStatus::Isolated => {
            println!(
                "  {} legacy sessions: ambient ~/.deepseek/sessions was not inspected because HAKUS_HOME is set",
                "·".dimmed()
            );
            println!(
                "    This preserves the explicit home boundary. To inspect the default home, use a separate shell with HAKUS_HOME unset and rerun `hakus doctor`."
            );
        }
        DoctorSessionRecoveryStatus::NoLegacySessions => {
            println!(
        ...
        | DoctorSessionRecoveryStatus::MigrationIncomplete => {
            ...
            if report.recoverable_file_count > DOCTOR_SESSION_RECOVERY_HUMAN_SAMPLE_LIMIT {
                println!(
                    "    · {} more filename(s); `hakus doctor --json` includes a bounded metadata-only sample",
            ...
            println!(
                "      2. Close other Hakus processes, then run `hakus sessions`; migration adds only missing files, never overwrites primary files, and leaves legacy originals in place."
            );
            println!(
                "      3. Rerun `hakus doctor`. If filenames remain, keep both backups and report only the listed source/destination names."
            );
        }
        DoctorSessionRecoveryStatus::ScanFailed => {
            println!(
            ...
            println!(
                "    Keep both session directories unchanged, back them up, fix path permissions or shape, and rerun `hakus doctor` before attempting migration."
            );
        }
    }
...
fn doctor_session_recovery_json(report: &DoctorSessionRecoveryReport) -> serde_json::Value {
    ...
    json!({
        ...
        "hakus_home_is_explicit": report.hakus_home_is_explicit,
...
fn doctor_setup_state(
    config: &Config,
    workspace: &Path,
) -> (hakus_config::SetupState, &'static str) {
    if let Ok(Some(state)) = hakus_config::SetupState::load() {
        return (state, "persisted");
    }

    (
        hakus_config::SetupState::derive_inherited(&doctor_inherited_setup_facts(
            config, workspace,
        )),
        "derived",
    )
...
fn doctor_inherited_setup_facts(
    config: &Config,
    workspace: &Path,
) -> hakus_config::InheritedConfigFacts {
    let user_constitution = hakus_config::UserConstitution::load().ok();
    let user_constitution_validity = user_constitution.as_ref().map_or(
        hakus_config::ConstitutionValidity::Unknown,
        hakus_config::UserConstitutionLoad::validity,
    );
    let has_user_constitution = user_constitution
        .as_ref()
        .is_some_and(|loaded| !matches!(loaded, hakus_config::UserConstitutionLoad::Missing));
    let has_expert_override = hakus_config::hakus_home()
        .ok()
...
fn print_doctor_setup_report(
    ...
    for step in hakus_config::SetupStep::ALL {
...
/// file was removed out-of-band. Stale `.tmp*` files in `$HAKUS_HOME`
...
fn doctor_setup_consistency(
    ...
    if source == "persisted"
        && matches!(
            ...
            hakus_config::ConstitutionSource::UserGlobal
        )
    {
        match hakus_config::UserConstitution::load() {
            Ok(hakus_config::UserConstitutionLoad::Missing) => {
                issues.push("setup_state_points_at_missing_user_constitution");
            }
            Ok(hakus_config::UserConstitutionLoad::Empty) => {
                issues.push("user_constitution_empty");
            }
            Ok(hakus_config::UserConstitutionLoad::Invalid(_)) => {
                issues.push("user_constitution_invalid");
            }
            Ok(hakus_config::UserConstitutionLoad::Unreadable(_)) | Err(_) => {
                issues.push("user_constitution_unreadable");
            }
            Ok(hakus_config::UserConstitutionLoad::Loaded(_)) => {}
        }
    }

    if doctor_home_has_stale_setup_temp_files() {
        issues.push("stale_setup_temp_files_in_hakus_home");
    }
...
fn doctor_home_has_stale_setup_temp_files() -> bool {
    let Ok(home) = hakus_config::hakus_home() else {
        return false;
    };
...
fn doctor_constitution_autonomy_preference() -> hakus_config::AutonomyPreference {
    hakus_config::UserConstitution::load()
        .ok()
        .and_then(|load| {
            load.constitution()
                .map(|constitution| constitution.autonomy_preference)
        })
        .unwrap_or(hakus_config::AutonomyPreference::Unspecified)
}
...
fn autonomy_preference_id(preference: hakus_config::AutonomyPreference) -> &'static str {
    match preference {
        hakus_config::AutonomyPreference::Unspecified => "unspecified",
        hakus_config::AutonomyPreference::Cautious => "cautious",
        hakus_config::AutonomyPreference::Balanced => "balanced",
        hakus_config::AutonomyPreference::Autonomous => "autonomous",
    }
}
...
/// Truth change only: the resolution itself is [`hakus_config`]'s.
fn doctor_runtime_telemetry(config: &Config) -> (bool, &'static str) {
    let (on, source) = hakus_config::resolved_telemetry_consent(config.telemetry);
    (on, source.as_str())
}
    ...
    json!({
        ...
        "auth": {
            ...
            "oauth_only": credential_help.acquisition
                == hakus_config::provider::CredentialAcquisition::OAuth,
        },
        "health": {
...
fn doctor_dsh_integration_report(
    ...
    let identity = dsh::hakus_route_identity(config, workspace);
    dsh::compute_status(
        &paths,
        detection,
        identity,
...
fn doctor_external_credential_consent_statuses(
    config: &Config,
) -> Vec<hakus_config::ExternalCredentialConsentStatus> {
    [
        ...
        .flat_map(|status| {
            let mut lines = vec![
                format!(
                    ...
                    status.owner,
                    hakus_config::quote_os_path(&status.path),
        ...
        doctor_external_credential_consent_statuses(config)
            ...
            .map(|status| {
                serde_json::json!({
                    ...
                    "owner": status.owner,
                    "path": hakus_config::quote_os_path(&status.path),
...
fn doctor_setup_report_json(config: &Config, workspace: &Path) -> serde_json::Value {
    ...
    let steps: Vec<_> = hakus_config::SetupStep::ALL
        .into_iter()
        .map(|step| {
            let entry = state.steps.get(&step);
            json!({
...
fn setup_step_id(step: hakus_config::SetupStep) -> &'static str {
    match step {
        hakus_config::SetupStep::Language => "language",
        hakus_config::SetupStep::ProviderModel => "provider_model",
        hakus_config::SetupStep::TrustSandbox => "trust_sandbox",
        hakus_config::SetupStep::ToolsMcp => "tools_mcp",
        hakus_config::SetupStep::Hotbar => "hotbar",
        hakus_config::SetupStep::RemoteRuntime => "remote_runtime",
        hakus_config::SetupStep::Persistence => "persistence",
        hakus_config::SetupStep::Constitution => "constitution",
        hakus_config::SetupStep::OperateFleet => "operate_fleet",
        hakus_config::SetupStep::Verification => "verification",
    }
}

fn setup_status_id(status: hakus_config::StepStatus) -> &'static str {
    match status {
        hakus_config::StepStatus::NotStarted => "not_started",
        hakus_config::StepStatus::Recommended => "recommended",
        hakus_config::StepStatus::Optional => "optional",
        hakus_config::StepStatus::Deferred => "deferred",
        hakus_config::StepStatus::InProgress => "in_progress",
        hakus_config::StepStatus::Verified => "verified",
        hakus_config::StepStatus::NeedsAction => "needs_action",
        hakus_config::StepStatus::Failed => "failed",
        hakus_config::StepStatus::Skipped => "skipped",
    }
}

fn constitution_choice_id(choice: hakus_config::ConstitutionChoice) -> &'static str {
    match choice {
        hakus_config::ConstitutionChoice::Unset => "unset",
        hakus_config::ConstitutionChoice::Bundled => "bundled",
        hakus_config::ConstitutionChoice::GuidedCustom => "guided_custom",
        hakus_config::ConstitutionChoice::ExpertOverride => "expert_override",
        hakus_config::ConstitutionChoice::Deferred => "deferred",
    }
}

fn constitution_source_id(source: hakus_config::ConstitutionSource) -> &'static str {
    match source {
        hakus_config::ConstitutionSource::Bundled => "bundled",
        hakus_config::ConstitutionSource::UserGlobal => "user_global",
        hakus_config::ConstitutionSource::ExpertOverride => "expert_override",
    }
}

fn constitution_validity_id(validity: hakus_config::ConstitutionValidity) -> &'static str {
    match validity {
        hakus_config::ConstitutionValidity::Unknown => "unknown",
        hakus_config::ConstitutionValidity::Valid => "valid",
        hakus_config::ConstitutionValidity::Invalid => "invalid",
        hakus_config::ConstitutionValidity::Empty => "empty",
        hakus_config::ConstitutionValidity::Unreadable => "unreadable",
    }
}

fn runtime_posture_source_id(source: hakus_config::RuntimePostureSource) -> &'static str {
    match source {
        hakus_config::RuntimePostureSource::Unset => "unset",
        hakus_config::RuntimePostureSource::Inherited => "inherited",
        hakus_config::RuntimePostureSource::Confirmed => "confirmed",
    }
}
...
fn run_doctor_json(
    ...
    let memory_enabled_env = std::env::var("HAKUS_MEMORY")
    ...
    let report = json!({
        ...
        "api_connectivity": {
            "checked": false,
            "status": "not_probed",
            "note": "JSON doctor is offline; use `hakus doctor --probe-api` or `--probe-local` for an explicit live check.",
        },
...
fn doctor_wire_protocol(provider: crate::config::ApiProvider) -> &'static str {
    let policy = provider
        ...
        .unwrap_or(hakus_config::provider::WirePolicy::Fixed(
            hakus_config::provider::WireFormat::ChatCompletions,
        ));
    match policy.fixed() {
        Some(hakus_config::provider::WireFormat::ChatCompletions) => "chat_completions",
        Some(hakus_config::provider::WireFormat::Responses) => "responses",
        Some(hakus_config::provider::WireFormat::AnthropicMessages) => "anthropic_messages",
        None => "model_aware",
    }
}
...
fn doctor_timeout_recovery_lines(config: &Config) -> Vec<String> {
    ...
    match config.api_provider() {
        ...
        {
            lines.push(
                "If this is a custom DeepSeek-compatible endpoint, set its HTTPS base URL in ~/.hakus/config.toml and rerun `hakus doctor`."
                    .to_string(),
            );
        }
        crate::config::ApiProvider::Deepseek | crate::config::ApiProvider::DeepseekCN => {
    ...
    lines.push(
        "Run `hakus doctor --json` and include `base_url`, `default_text_model`, and `api_connectivity` when filing an issue."
            .to_string(),
    );
    lines
}
...
fn list_sessions(limit: usize, search: Option<String>) -> Result<()> {
    ...
    if sessions.is_empty() {
        ...
        println!(
            ...
            "hakus".truecolor(human_r, human_g, human_b)
        );
        return Ok(());
    }
    ...
    println!(
        ...
        "hakus --continue".truecolor(action_r, action_g, action_b)
    );
...
fn init_project() -> Result<()> {
    ...
    match create_default_agents_md(&workspace) {
        Ok(path) => {
            ...
            println!("The instructions will be loaded automatically when you run hakus.");
        }
        Err(e) => {
            println!(
...
fn effective_config_profile(cli: &Cli) -> Option<String> {
    cli.profile
        .clone()
        .or_else(|| std::env::var("HAKUS_PROFILE").ok())
...
async fn run_xai_device_auth(config_path: Option<&Path>) -> Result<()> {
    ...
    println!(
        ...
        hakus_config::quote_os_path(&activation.auth_path),
        hakus_config::quote_os_path(&activation.config_path)
    );
    Ok(())
}
...
fn resolve_session_id(session_id: Option<String>, last: bool, workspace: &Path) -> Result<String> {
    if last {
        return latest_session_id_for_workspace(workspace)?.ok_or_else(|| {
            anyhow!(
                "No saved sessions found for workspace {}. Use `hakus sessions` to list all sessions, or `hakus resume <SESSION_ID>` to resume one explicitly.",
                workspace.display()
            )
        });
    }
...
fn run_review_receipt_check(diff: &str, args: &ReviewArgs) -> Result<()> {
    let (path, receipt) = if let Some(path) = args.receipt_path.as_ref() {
        ...
        crate::tools::review::latest_review_receipt_for_diff(diff)?.ok_or_else(|| {
            anyhow!(
                "No review receipt found for the current diff. Run `hakus review --write-receipt` first, or pass --receipt-path."
            )
        })?
    };
    let validation =
...
/// `hakus pr <N>` (#451) — fetch a GitHub PR via `gh`, format
    ...
    if !is_command_available("gh") {
        bail!(
            "`gh` CLI not found on PATH. Install GitHub CLI \
             ...
             so `hakus pr <N>` can fetch PR metadata and the diff."
        );
    }
...
async fn run_mcp_command(
    ...
    match command {
        McpCommand::Init { force } => {
            ...
            println!("Edit the file, then run `hakus mcp list` or `hakus mcp tools`.");
            Ok(())
        }
        McpCommand::List => {
        ...
        McpCommand::Add {
            ...
            {
                println!(
                    "OAuth is available for '{name}'. Run `hakus mcp login {name}` to authenticate."
                );
            }
            Ok(())
        }
        ...
        McpCommand::AddSelf { name, workspace } => {
            ...
            if cfg.servers.contains_key(&name) {
                bail!(
                    "MCP server '{name}' already exists in {}. Use `hakus mcp remove {name}` first, or choose a different --name.",
                    config_path.display()
                );
            }
            cfg.servers.insert(
            ...
            save_mcp_config(&config_path, &cfg)?;
            println!(
                "Registered Hakus as MCP server '{name}' in {}",
                config_path.display()
            );
            ...
            println!();
            println!("Tip: Use `hakus mcp validate` to test the connection.");
            println!("     Use `hakus serve --http` for the HTTP/SSE runtime API instead.");
            Ok(())
        }
    }
}
...
/// `~/.hakus/config.toml` or `--mouse-capture`.
...
/// matches, a one-line notice points at `hakus sessions`, and nothing is
        ...
        if let Some(newest) = mismatched.first() {
            eprintln!(
                "Note: an interrupted session from another workspace ({}) is \
                 available. Run `hakus sessions` to list saved sessions. Starting \
                 fresh in {}.",
...
/// attaching it to the new TUI instance. This keeps "open another hakus in
    ...
    if session_manager::workspace_scope_matches(&session_workspace, launch_workspace) {
        eprintln!(
            "Found an in-flight session snapshot ({age_str}). Starting a new \
             session. Run `hakus --continue` to resume it."
        );
    } else {
        eprintln!(
            "Note: an interrupted session from another workspace ({}) is \
...
fn merge_project_config_with_approval_baseline(
        ...
        if table.contains_key(*key) {
            eprintln!(
                "warning: project-scope config key `{key}` is ignored — \
                 set it in `~/.hakus/config.toml` instead. \
        ...
        && !v.is_empty()
    {
        ...
        if hakus_config::project_approval_policy_is_allowed(approval_baseline, v) {
    ...
    if let Some(v) = table.get("sandbox_mode").and_then(toml::Value::as_str)
        && !v.is_empty()
    {
        if hakus_config::project_sandbox_mode_is_allowed(config.sandbox_mode.as_deref(), v) {
...
fn merge_user_workspace_config(
    ...
    let allow_shell_from_env = std::env::var_os("HAKUS_ALLOW_SHELL").is_some()
...
async fn run_interactive_with_notice(
    ...
    // hygiene. On a large ~/.hakus they were the dominant startup cost
...
struct ExecStreamMeta {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    hakus_max_output_tokens: Option<u32>,
    /// Provenance of that ceiling: `documented`, `uncatalogued`, or
    /// `route-declared`.
    #[serde(skip_serializing_if = "Option::is_none")]
    hakus_max_output_tokens_source: Option<&'static str>,
    duration_ms: u64,
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// Process exit code `hakus exec` uses when a turn ends on a retryable
...
/// visibility, but the terminal `TurnCompleted` event carries the
...
fn exec_stream_value(event: &ExecStreamEvent) -> Result<serde_json::Value> {
    ...
    if let Some(object) = value.as_object_mut() {
        ...
        object.insert(
            "schema".to_string(),
            serde_json::json!("hakus.exec-stream"),
        );
    }
    Ok(value)
}
...
async fn run_workflow_tool_command_inner(
    ...
    merge_user_workspace_config(&mut config, cli.config.clone(), &workspace);
    if let Ok(env_url) =
        std::env::var("HAKUS_BASE_URL").or_else(|_| std::env::var("DEEPSEEK_BASE_URL"))
    {
        let trimmed = env_url.trim();
        if !trimmed.is_empty() {
            config.base_url = Some(trimmed.to_string());
    ...
    emit_exec_stream_event(&ExecStreamEvent::Metadata {
        meta: Box::new(ExecStreamMeta {
            ...
            hakus_max_output_tokens: None,
            hakus_max_output_tokens_source: None,
            duration_ms: u64::try_from(tool_started.elapsed().as_millis()).unwrap_or(u64::MAX),
            retry_count: None,
...
fn exec_stream_resume_hint(session_id: &str) -> String {
    if session_id.trim().is_empty() {
        ...
        "hakus exec --resume <redacted-session-id>".to_string()
    }
}
...
async fn run_exec_agent(
    ...
    let engine_config = EngineConfig {
        ...
        skills_scan_hakus_only: execution_config.skills_config().scan_hakus_only(),
        instructions: {
    ...
    loop {
        ...
        match event {
            ...
            Event::Error {
                ...
                // but the terminal TurnCompleted event carries the
            ...
            Event::TokenUsage {
                ...
                if output_format == ExecOutputFormat::StreamJson {
                    ...
                    emit_exec_stream_event(&ExecStreamEvent::TokenUsage {
                        turn: turn_usage_seq,
            ...
            Event::TurnCompleted {
                ...
                if output_format == ExecOutputFormat::StreamJson {
                    ...
                    let hakus_max_output_tokens =
                        crate::route_budget::effective_max_output_tokens_for_route(
                            effective_provider,
                            &latest_model,
                            active_route_limits,
                        );
                    let hakus_max_output_tokens_source =
                    ...
                    emit_exec_stream_event(&ExecStreamEvent::Metadata {
                        meta: Box::new(ExecStreamMeta {
                            ...
                            prompt_cache_write_tokens: usage.prompt_cache_write_tokens,
                            reasoning_tokens: usage.reasoning_tokens,
                            hakus_max_output_tokens: Some(hakus_max_output_tokens),
                            hakus_max_output_tokens_source: Some(
                                hakus_max_output_tokens_source,
                            ),
...
mod doctor_legacy_state_tests {
    ...
    fn roots(tmp: &TempDir) -> (PathBuf, PathBuf) {
        (tmp.path().join(".hakus"), tmp.path().join(".deepseek"))
    }
    ...
    fn doctor_reports_incomplete_session_migration_without_mutating_files() {
        ...
        assert_eq!(json["recovery_command"], "hakus sessions");
    ...
    #[test]
    fn explicit_hakus_home_skips_session_recovery_scan() {
        let tmp = TempDir::new().expect("tempdir");
        let (primary_root, legacy_root) = roots(&tmp);
        fs::create_dir_all(legacy_root.join("sessions")).expect("legacy sessions");
        ...
        assert!(report.hakus_home_is_explicit);
        assert_eq!(report.legacy_session_file_count, 0);
        assert_eq!(report.recoverable_file_count, 0);
        assert!(report.recoverable.is_empty());
        assert!(!report.needs_attention());
    ...
    #[test]
    fn doctor_state_roots_ignore_ambient_legacy_home_when_hakus_home_is_explicit() {
        let _env_lock = crate::test_support::lock_test_env();
        let tmp = TempDir::new().expect("tempdir");
        let explicit_home = tmp.path().join("isolated-hakus");
        ...
        let _hakus_home = EnvVarRestore::set("HAKUS_HOME", &explicit_home);
        ...
        assert!(
            report
                .iter()
                .all(|entry| entry.status == DoctorLegacyStateStatus::Absent),
            "doctor must not report ambient legacy state when HAKUS_HOME is explicit"
        );
        assert!(!report.iter().any(legacy_state_needs_attention));
        assert_eq!(
...
mod doctor_setup_state_tests {
    ...
    fn prepare_env(tmp: &TempDir) -> (crate::test_support::EnvVarGuard, PathBuf) {
        let hakus_home = tmp.path().join(".hakus");
        fs::create_dir_all(&hakus_home).expect("hakus home");
        (
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str()),
            hakus_home,
        )
    }
    ...
    fn doctor_setup_consistency_flags_missing_user_constitution() {
        ...
        let tmp = TempDir::new().expect("tempdir");
        let (_home_guard, _hakus_home) = prepare_env(&tmp);
        ...
        let state = hakus_config::SetupState {
            constitution_source: hakus_config::ConstitutionSource::UserGlobal,
            ..Default::default()
        };
    ...
    fn doctor_setup_consistency_flags_stale_temp_files() {
        let _guard = crate::test_support::lock_test_env();
        let tmp = TempDir::new().expect("tempdir");
        let (_home_guard, hakus_home) = prepare_env(&tmp);
        let _key = crate::test_support::EnvVarGuard::remove("DEEPSEEK_API_KEY");
        let _source = crate::test_support::EnvVarGuard::remove("DEEPSEEK_API_KEY_SOURCE");
        let workspace = tmp.path().join("workspace");
        fs::create_dir_all(&workspace).expect("workspace");
        fs::write(hakus_home.join(".tmpAbC123"), b"orphaned atomic write")
        ...
        let issues = report["consistency"]["issues"].to_string();
        assert!(
            issues.contains("stale_setup_temp_files_in_hakus_home"),
            "{issues}"
        );
    }
    ...
    fn doctor_setup_provider_model_json_covers_cn_codex_and_local_matrix() {
        ...
        let mut consent = hakus_config::ExternalCredentialConsentToml::read_only(
            hakus_config::ProviderKind::OpenaiCodex,
            hakus_config::ExternalCredentialSource::CodexCli,
        ...
        assert_eq!(
            ...
            "hakus auth external-revoke --provider openai-codex"
        );
        ...
        assert!(
            human.contains(&hakus_config::quote_os_path(&codex_auth_path)),
            "{human}"
        );
        ...
        consent.access = hakus_config::ExternalCredentialAccess::Managed;
    ...
    fn doctor_setup_report_json_uses_persisted_state() {
        ...
        let workspace = tmp.path().join("workspace");
        fs::create_dir_all(&workspace).expect("workspace");
        let mut state = hakus_config::SetupState::default();
        state.set_step(
            hakus_config::SetupStep::Language,
        ...
        state.set_step(
            hakus_config::SetupStep::ProviderModel,
            hakus_config::StepEntry::new(
                hakus_config::StepStatus::Verified,
                true,
                crate::tui::setup::CONSTITUTION_CHECKPOINT_VERSION,
        ...
        state.set_step(
            hakus_config::SetupStep::TrustSandbox,
            hakus_config::StepEntry::new(
                hakus_config::StepStatus::Verified,
                true,
                crate::tui::setup::CONSTITUTION_CHECKPOINT_VERSION,
        ...
        state.runtime_posture_source = hakus_config::RuntimePostureSource::Confirmed;
        state.save().expect("persist setup state");
        hakus_config::UserConstitution {
            autonomy_preference: hakus_config::AutonomyPreference::Balanced,
            ..Default::default()
        }
        .save()
    ...
    fn doctor_reports_resolved_telemetry_with_its_source() {
        ...
        let tmp = TempDir::new().expect("tempdir");
        let (_home_guard, _hakus_home) = prepare_env(&tmp);
        let _telemetry_env = crate::test_support::EnvVarGuard::remove("HAKUS_TELEMETRY");
        let _telemetry_alias_env = crate::test_support::EnvVarGuard::remove("DEEPSEEK_TELEMETRY");
        let _telemetry_floor =
            crate::test_support::EnvVarGuard::remove("HAKUS_TELEMETRY_FLOOR");
        let workspace = tmp.path().join("workspace");
        fs::create_dir_all(&workspace).expect("workspace");
    ...
    fn doctor_setup_report_json_fails_closed_without_operate_receipts() {
        ...
        state.set_step(
            hakus_config::SetupStep::OperateFleet,
            hakus_config::StepEntry::new(
                hakus_config::StepStatus::Verified,
                false,
                crate::tui::setup::CONSTITUTION_CHECKPOINT_VERSION,
...
mod doctor_endpoint_tests {
    ...
    fn timeout_recovery_keeps_default_deepseek_users_on_default_endpoint() {
        ...
        assert!(text.contains("hakus doctor --json"));
    }
...
mod terminal_mode_tests {
    ...
    fn plugin_registry_discovery_is_route_independent_and_read_only() {
        ...
        let hakus_home = temp.path().join("home");
        std::fs::create_dir_all(&workspace).unwrap();
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", &hakus_home);
        ...
        for route in [
            ...
            assert!(
                !hakus_home.join("plugins/state.json").exists(),
    ...
    fn doctor_operate_fleet_json_lists_multi_layer_profile_paths() {
        ...
        let project = workspace.join(".hakus").join("agents");
        ...
        let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", &home);
    ...
    fn prompt_flag_accepts_split_prompt_words_for_windows_cmd_shims() {
        let cli = parse_cli(&["hakus", "-p", "hello", "world"]);
    ...
    fn prompt_flag_starts_interactive_submit_input() {
        let cli = parse_cli(&["hakus", "-p", "read", "the", "project"]);
    ...
    fn companion_binary_reports_its_own_name() {
        assert_eq!(Cli::command().get_name(), "hakus-tui");
    }

    #[test]
    fn xai_device_auth_subcommand_parses() {
        let cli = parse_cli(&["hakus-tui", "auth", "xai-device"]);
        assert!(matches!(
            cli.command,
    ...
    fn exec_model_resolution_uses_provider_scoped_default() {
        let _env_lock = crate::test_support::lock_test_env();
        let _hakus_model = crate::test_support::EnvVarGuard::remove("HAKUS_MODEL");
    ...
    #[test]
    fn exec_model_resolution_prefers_hakus_model_env_override() {
        let _env_lock = crate::test_support::lock_test_env();
        let _hakus_model = crate::test_support::EnvVarGuard::set("HAKUS_MODEL", " auto ");
        let _deepseek_model =
    ...
    fn cli_route_execution_config_preserves_legacy_literal_custom_root_route() {
        ...
        let _source = crate::test_support::EnvVarGuard::remove("DEEPSEEK_API_KEY_SOURCE");
        let _cli_key = crate::test_support::EnvVarGuard::remove("HAKUS_CLI_API_KEY");
        let config = Config {
            provider: Some("custom".to_string()),
    ...
    fn exec_accepts_split_prompt_words_for_windows_cmd_shims() {
        let cli = parse_cli(&["hakus", "exec", "hello", "world"]);
    ...
    fn exec_keeps_model_flag_before_split_prompt_words() {
        let cli = parse_cli(&["hakus", "exec", "--model", "auto", "hello", "world"]);
        let Some(Commands::Exec(args)) = cli.command else {
            panic!("expected exec command");
        };
    ...
    fn exec_keeps_flags_before_split_prompt_words() {
        let cli = parse_cli(&["hakus", "exec", "--json", "hello", "world"]);
        let Some(Commands::Exec(args)) = cli.command else {
            panic!("expected exec command");
        };
    ...
    fn exec_model_reads_wait_for_foreign_test_env_overrides_to_restore() {
        ...
        let (reader, expected_after_restore) = {
            ...
            let temporary =
                crate::test_support::EnvVarGuard::set("HAKUS_MODEL", "temporary-model");
    ...
    fn exec_accepts_session_id_alias() {
        let cli = parse_cli(&["hakus", "exec", "--session-id", "abc123", "follow up"]);
        let Some(Commands::Exec(args)) = cli.command else {
            panic!("expected exec command");
        };
    ...
    fn exec_auto_does_not_authorize_sandbox_elevation() {
        let cli = parse_cli(&["hakus", "exec", "--auto", "run it"]);
        let Some(Commands::Exec(args)) = cli.command else {
            panic!("expected exec command");
        };
    ...
    fn exec_shell_only_tool_surface_env_sets_shell_allowlist() {
        let _env_lock = crate::test_support::lock_test_env();
        let _surface =
            crate::test_support::EnvVarGuard::set(HAKUS_TOOL_SURFACE_ENV, " shell-only ");
    ...
    fn exec_full_tool_surface_env_leaves_allowlist_unset() {
        let _env_lock = crate::test_support::lock_test_env();
        let _surface = crate::test_support::EnvVarGuard::set(HAKUS_TOOL_SURFACE_ENV, "full");
    ...
    fn exec_rejects_zero_max_turns() {
        let err = Cli::try_parse_from(["hakus", "exec", "--max-turns", "0", "hello"])
    ...
    fn exec_omits_the_headless_turn_cap_by_default() {
        let cli = parse_cli(&["hakus", "exec", "--auto", "benchmark this"]);
        let Some(Commands::Exec(args)) = cli.command else {
            panic!("expected exec command");
        };
    ...
    fn exec_accepts_continue_for_latest_workspace_session() {
        let cli = parse_cli(&["hakus", "exec", "--continue", "follow up"]);
        let Some(Commands::Exec(args)) = cli.command else {
            panic!("expected exec command");
        };
    ...
    fn sessions_footer_points_to_resume_subcommand() {
        let cli = parse_cli(&["hakus", "resume", "abc123"]);
        ...
        assert!(!last);
        assert_eq!(sessions_resume_command(), "hakus resume");
    ...
    fn plugin_registry_initialization_precedes_dotenv_for_all_launch_paths() {
        ...
        let cases: &[(&[&str], Expected)] = &[
            (&["hakus"], Expected::Plain),
            (&["hakus", "resume", "--last"], Expected::Resume),
            (&["hakus", "fork", "--last"], Expected::Fork),
            (&["hakus", "exec", "probe"], Expected::Exec),
            (&["hakus", "serve", "--mcp"], Expected::Serve),
        ];
    ...
    fn workspace_dotenv_loads_only_provider_credentials_and_preserves_shell_values() {
        ...
        let _home = crate::test_support::EnvVarGuard::remove("HAKUS_HOME");
        let _config = crate::test_support::EnvVarGuard::remove("HAKUS_CONFIG_PATH");
        ...
        std::fs::write(
            ...
            "DEEPSEEK_API_KEY=workspace-key\n\
             ...
             HAKUS_HOME=./attacker-home\n\
             HAKUS_CONFIG_PATH=./attacker.toml\n\
        ...
        assert!(std::env::var_os("HAKUS_HOME").is_none());
        assert!(std::env::var_os("HAKUS_CONFIG_PATH").is_none());
        ...
        assert_eq!(
            report.ignored,
            BTreeSet::from([
                "HAKUS_CONFIG_PATH".to_string(),
                "HAKUS_HOME".to_string(),
    ...
    fn workspace_dotenv_rejects_ambient_variable_substitution() {
        ...
        std::fs::write(
            &dotenv,
            "DEEPSEEK_API_KEY=${HAKUS_JS_SECRET_LEAK_TEST}\n",
        )
        .expect("write dotenv");
    ...
    fn workspace_dotenv_credential_allowlist_excludes_control_plane_names() {
        for provider in hakus_config::provider::providers_sorted_for_display() {
    ...
    fn exec_stream_turn_usage_event_serializes_reported_fields() {
        let event = ExecStreamEvent::TokenUsage {
            turn: 2,
            input_tokens: 1200,
            output_tokens: 180,
        ...
        assert_eq!(parsed["schema"], "hakus.exec-stream");
    ...
    fn exec_stream_pre_existing_event_type_tags_are_unchanged() {
        ...
        // ledger): the pre-token_usage event vocabulary keeps its exact tags.
        ...
        for (event, expected_type) in cases {
            ...
            assert_eq!(value["schema"], "hakus.exec-stream");
    ...
    fn workflow_receipt_stream_event_is_one_json_line() {
        ...
        assert_eq!(consumed["schema"], "hakus.exec-stream");
    ...
    fn exec_stream_metadata_redacts_resume_breadcrumbs() {
        ...
        let event = ExecStreamEvent::Metadata {
            meta: Box::new(ExecStreamMeta {
                ...
                hakus_max_output_tokens: Some(384_000),
                hakus_max_output_tokens_source: Some("documented"),
                duration_ms: 2500,
                retry_count: None,
        ...
        assert_eq!(
            ...
            "hakus exec --resume <redacted-session-id>"
        );
    ...
    fn removed_no_alt_screen_flag_is_rejected() {
        ...
        let error = Cli::try_parse_from(["hakus", "--no-alt-screen"])
    ...
    fn no_mouse_capture_flag_disables_mouse_capture() {
        let cli = parse_cli(&["hakus", "--no-mouse-capture"]);
        let config = Config::default();
    ...
    fn mouse_capture_flag_enables_mouse_capture() {
        let cli = parse_cli(&["hakus", "--mouse-capture"]);
        let config = Config::default();
...
mod project_config_tests {
    ...
    fn project_overlay_rejects_symlinked_primary_config() {
        ...
        let primary_dir = workspace.path().join(hakus_config::HAKUS_APP_DIR);
        let legacy_dir = workspace.path().join(hakus_config::LEGACY_APP_DIR);
    ...
    fn project_overlay_skips_when_workspace_is_home_directory() {
        let _guard = crate::test_support::lock_test_env();
        let tmp = tempdir().expect("tempdir");
        let project_dir = tmp.path().join(hakus_config::HAKUS_APP_DIR);
        fs::create_dir_all(&project_dir).expect("mkdir .hakus");
        fs::write(
...
"#,
    ...
    fn exec_no_project_config_skips_user_workspace_overlay() {
        // #4641: `hakus --no-project-config exec` must skip the
...
"#,
    ...
    fn project_overlay_skips_missing_config_file() {
        let tmp = tempdir().expect("tempdir");
        let mut config = Config {
            provider: Some("hakus".to_string()),
            ..Config::default()
        };
        merge_project_config(&mut config, tmp.path());
        // Untouched.
        assert_eq!(config.provider.as_deref(), Some("hakus"));
    }
...
mod doctor_mcp_tests {
    ...
    fn test_relative_stdio_path_arg_with_cwd_is_ok() {
        ...
        server.cwd = Some(PathBuf::from("/tmp/hakus-project"));
        match doctor_check_mcp_server(&server) {
            McpServerDoctorStatus::Ok(detail) => assert!(detail.contains("stdio")),
    ...
    mod mcp_auth_guidance_tests {
        ...
        fn mcp_auth_hint_is_actionable_for_connect_failures() {
            ...
            assert_eq!(
                hint,
                "MCP server 'nordic-mcp' requires OAuth authentication. Run `hakus mcp login nordic-mcp` to authenticate."
            );
        }
    }
...
mod doctor_live_probe_tests {
    ...
    fn ollama_cloud_probe_uses_hosted_opt_in_not_local_opt_in() {
        let cloud = hakus_config::provider::OLLAMA_CLOUD_BASE_URL;
        assert!(!doctor_should_probe_api(
...
mod setup_helper_tests {
    ...
    fn custom_provider_env_source_precedes_saved_secret_store() {
        ...
        let hakus_home = temp.path().join("hakus-home");
        std::fs::create_dir_all(&hakus_home).expect("create hakus home");
        let _home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str());
        let _backend = crate::test_support::EnvVarGuard::set("HAKUS_SECRET_BACKEND", "file");
        let _declared_env =
            crate::test_support::EnvVarGuard::set("QA_CUSTOM_API_KEY", "declared-env-key");
        let _deepseek_key = crate::test_support::EnvVarGuard::remove("DEEPSEEK_API_KEY");
        let _deepseek_source = crate::test_support::EnvVarGuard::remove("DEEPSEEK_API_KEY_SOURCE");
        hakus_secrets::Secrets::auto_detect()
    ...
    fn ollama_doctor_credential_source_is_route_aware() {
        ...
        let cloud = ollama_config(hakus_config::provider::OLLAMA_CLOUD_BASE_URL);
        assert_eq!(
            cloud.api_provider(),
            crate::config::ApiProvider::OllamaCloud
        );
    ...
    fn resolve_api_key_source_ignores_unresolved_provider_command_metadata() {
        ...
        let mut providers = crate::config::ProvidersConfig::default();
        providers.openai.auth = Some(hakus_config::ProviderAuthSourceToml {
            source: hakus_config::AuthSourceKind::Command,
    ...
    fn resolve_api_key_source_ignores_unresolved_provider_secret_metadata() {
        ...
        let mut providers = crate::config::ProvidersConfig::default();
        providers.openai.auth = Some(hakus_config::ProviderAuthSourceToml {
            source: hakus_config::AuthSourceKind::Secret,
            command: Vec::new(),
            timeout_ms: None,
            secret_id: Some("hakus/openai".to_string()),
        });
        let cfg = Config {
            provider: Some("openai".to_string()),
            providers: Some(providers),
