//! Shell abstraction layer for Hakus.
//!
//! Detects the user's shell at startup and provides a single entry point for
//! all command execution. Hakus never calls `Command::new("cmd")` (or
...
const TEMP_PS1_TAIL: &str = concat!(
    "$__hakusExit = if ($null -ne $LASTEXITCODE) { $LASTEXITCODE } else { 0 }\n",
    "Remove-Item -LiteralPath $MyInvocation.MyCommand.Path -Force ",
    "-ErrorAction SilentlyContinue\n",
    "if ($__hakusExit -ne 0) { exit $__hakusExit }\n",
);
...
/// Best-effort removal of leftover `hakus-shell-*.ps1` scripts (for
...
fn sweep_stale_temp_ps1(dir: &std::path::Path) {
    ...
    for entry in entries.flatten() {
        ...
        if !name.starts_with("hakus-shell-") || !name.ends_with(".ps1") {
            continue;
        }
        let stale = entry
            .metadata()
...
mod tests {
    ...
    fn powershell_multiline_uses_temp_file_invocation() {
        ...
        let exit_at = contents
            .find("if ($__hakusExit -ne 0) { exit $__hakusExit }")
    ...
    fn stale_temp_ps1_scripts_are_swept() {
        let dir = std::env::temp_dir();
        let stale = dir.join("hakus-shell-0-stale-test.ps1");
