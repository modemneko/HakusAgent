...
//!
//! `hakus fleet …` and the `/fleet …` slash command (and therefore its
...
pub fn fleet_ledger_path(workspace: &Path) -> PathBuf {
    workspace.join(".hakus").join("fleet.jsonl")
}
...
/// Exactly the text `hakus fleet status` has always printed.
#[must_use]
...
/// Artifact listing lines for `hakus fleet artifacts`.
#[must_use]
...
// ---------------------------------------------------------------------------
// Executor — the one code path behind `hakus fleet …` and `/fleet …`
// ---------------------------------------------------------------------------
    ...
    match operation {
        ...
        _ => ControlReceipt::unavailable(
            ...
            Availability::Unavailable {
                reason: hakus_lane::UnavailableReason::SurfaceNotSupported,
...
mod tests {
    use super::*;
    use hakus_lane::{ControlAuthority, LifecycleOutcome, PersistenceScope, UnavailableReason};
    use hakus_protocol::fleet::{FleetResolvedRoute, FleetTaskResult};
