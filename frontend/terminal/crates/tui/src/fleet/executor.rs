//! Fleet executor — runs a fleet worker as a real `hakus exec` subprocess.
//!
//! A fleet worker IS a headless `hakus exec` run. There is no separate
//! "fleet worker" execution engine: the sub-agent runtime, full tool surface,
//! and recursion depth all come from the one `hakus exec` runtime, so
...
//!   into the `hakus [route flags] exec --output-format stream-json …`
...
use anyhow::Result;
use hakus_config::FleetExecConfig;
use hakus_protocol::fleet::{FleetHostSpec, FleetTaskSpec, FleetWorkerEventPayload};
...
pub fn configured_hakus_binary() -> String {
    std::env::var("HAKUS_FLEET_HAKUS_BINARY")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .unwrap_or_else(|| "hakus".to_string())
}

/// Build the `hakus exec` argv that runs a fleet task headlessly.
///
...
fn build_worker_exec_command_from_prompt(
    ...
    // The canonical `hakus` dispatcher owns these route overrides as
    ...
    // dispatcher as well as when a host points directly at `hakus-tui`.
    ...
    FleetWorkerCommand::new(hakus_binary.to_string(), args)
}

/// Map one `hakus exec` stream-json line into a fleet ledger event.
///
...
pub fn map_exec_stream_line(line: &str) -> Option<FleetWorkerEventPayload> {
    ...
    match value.get("type").and_then(serde_json::Value::as_str)? {
        ...
        // heartbeat. `token_usage` covers thinking-heavy model calls that
        // produce no visible content between tool calls.
        "content" | "tool_result" | "token_usage" => Some(FleetWorkerEventPayload::Running),
...
/// Drives fleet workers as real `hakus exec` subprocesses on the local
/// host, incrementally draining each worker's stream-json output into fleet
/// ledger events.
///
/// The caller (the `hakus fleet run` loop / `FleetManager`) owns the
...
pub struct FleetExecutorAttempt {
    pub run_id: hakus_protocol::fleet::FleetRunId,
    pub task_id: String,
    pub attempt: u32,
}
...
mod tests {
    ...
    #[test]
    fn worker_command_is_a_headless_hakus_exec_run() {
        let exec = FleetExecConfig::default();
        let cmd = build_worker_exec_command("hakus", &task("read the file"), &exec, None);
        assert_eq!(cmd.program, "hakus");
    ...
    fn worker_command_threads_exec_hardening_flags() {
        ...
        let cmd = build_worker_exec_command("hakus", &task("audit"), &exec, Some("glm-5.1"));
        let exec_idx = cmd
            .args
            .iter()
    ...
    fn zero_max_turns_is_not_passed() {
        ...
        let cmd = build_worker_exec_command("hakus", &task("x"), &exec, None);
    ...
    fn default_max_turns_is_passed_as_bounded_flag() {
        ...
        let exec = FleetExecConfig::default();
        let joined = build_worker_exec_command("hakus", &task("x"), &exec, None)
            .args
            .join(" ");
        assert!(
    ...
    /// `hakus exec`), and prove the executor drains its events and terminal
    /// exit through the real host adapter — no hakus binary needed. This is
    ...
    /// passing ones — all without the hakus binary.
    #[cfg(unix)]
    #[test]
