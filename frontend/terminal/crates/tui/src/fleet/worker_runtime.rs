...
pub fn fleet_task_to_worker_spec_with_profiles(
    ...
    let max_spawn_depth = hakus_config::FleetExecConfig::default().max_spawn_depth;
...
fn route_protocol_label(protocol: hakus_config::route::RequestProtocol) -> &'static str {
    use hakus_config::route::RequestProtocol;
    match protocol {
...
fn loadout_intent_label(loadout: &hakus_config::FleetLoadout) -> Option<String> {
    if *loadout == hakus_config::FleetLoadout::Inherit {
        None
    } else {
...
fn fleet_task_prompt_with_profile(
    ...
    prompt.push_str("You have been summoned as a Hakus Fleet member (");
...
fn effective_fleet_loadout_with_source(
    worker_profile: Option<&FleetTaskWorkerProfile>,
    agent_profile: Option<&AgentProfile>,
) -> (hakus_config::FleetLoadout, Option<&'static str>) {
    if let Some(model_class) = worker_profile
        .and_then(|worker| worker.model_class.as_deref())
        .and_then(non_empty_trimmed)
    {
        return (
            hakus_config::FleetLoadout::from_name(model_class),
    ...
    {
        return (
            hakus_config::FleetLoadout::from_name(loadout),
            Some("task.loadout"),
        );
    }
    if let Some(loadout) = agent_profile
        .map(|profile| profile.profile.loadout.clone())
        .filter(|loadout| *loadout != hakus_config::FleetLoadout::Inherit)
    {
        return (loadout, Some("agent_profile.loadout"));
    }
    (hakus_config::FleetLoadout::Inherit, None)
}
...
/// explicit-only source as the headless `hakus exec` launch route (#4193),
...
/// worker's actual `hakus exec` subprocess should launch on (#4093 AC #4).
///
...
///   `hakus exec --provider <id>` can resolve `[providers.<id>]`.
...
fn fleet_worker_runtime_profile_for_loadout(
    ...
    spawn_depth: u32,
    max_spawn_depth: u32,
    loadout: &hakus_config::FleetLoadout,
    ...
    profile.model = if matches!(model_source, "task.model" | "agent_profile.model") {
        fleet_model_route_for_loadout(model, &hakus_config::FleetLoadout::Inherit)
    } else {
...
pub(crate) fn fleet_model_route_for_loadout(
    ...
    match loadout {
        hakus_config::FleetLoadout::Inherit => ModelRoute::Inherit,
        ...
        hakus_config::FleetLoadout::Fast => ModelRoute::Inherit,
        hakus_config::FleetLoadout::Custom(_) => ModelRoute::Auto,
    }
}
...
pub fn apply_exec_hardening(
    ...
    exec: &hakus_config::FleetExecConfig,
) -> AgentWorkerSpec {
...
pub(crate) fn network_posture_warning_for_task(
    ...
    let runtime = fleet_worker_runtime_profile_for_loadout(
        ...
        &model,
        0,
        hakus_config::FleetExecConfig::default().max_spawn_depth,
        &loadout,
        model_source,
    );
...
mod tests {
    use super::*;
    use hakus_protocol::fleet::{FleetHostSpec, FleetWorkspaceRequirements};
    ...
    fn agent_profile(
        ...
        AgentProfile {
            ...
            profile: hakus_config::FleetProfile {
                ...
                permissions: hakus_config::FleetProfilePermissions::default(),
                delegation: hakus_config::FleetDelegationHints::default(),
            },
    ...
    fn roster_member_agent_type_uses_role_then_slot() {
        let member = agent_profile(
            ...
            "synthesizer",
            None,
            hakus_config::FleetLoadout::Fast,
        );
        ...
        let mut slot_only = agent_profile(
            ...
            "summarizer",
            None,
            hakus_config::FleetLoadout::Inherit,
        );
        slot_only.profile.role.name = String::new();
        assert_eq!(
            slot_only.profile.slot,
            hakus_config::FleetSlot::Summarizer
        );
    ...
    fn fleet_task_prompt_includes_instructions_context_and_input_files() {
        ...
        assert!(prompt.contains("summoned as a Hakus Fleet member (general)"));
    ...
    fn fleet_worker_spec_resolves_agent_profile_role_prompt_and_loadout() {
        let profile = agent_profile(
            ...
            hakus_config::FleetLoadout::Custom("balanced".to_string()),
        );
        let task = fleet_task(
            "review",
            Some(worker_profile(
        ...
        assert!(
            spec.objective
                .contains("summoned as a Hakus Fleet member (reviewer)")
        );
    ...
    fn fleet_worker_launch_route_is_explicit_provider_only() {
        ...
        //     the exact provider id so `hakus exec --provider lm-studio`
    ...
    fn fleet_worker_spec_model_route_precedence_is_task_profile_role_then_session() {
        ...
        let mut profile =
            agent_profile("scout", "scout", None, hakus_config::FleetLoadout::Fast);
        profile.profile.model = Some("deepseek-v4-flash".to_string());
    ...
    fn fleet_worker_spec_defaults_to_shared_subagent_depth() {
        ...
        assert_eq!(spec.max_spawn_depth, hakus_config::DEFAULT_SPAWN_DEPTH);
        ...
        let hardened = apply_exec_hardening(spec, &hakus_config::FleetExecConfig::default());
    ...
    fn fleet_route_parity_uses_shared_router_candidates() {
        ...
        assert_eq!(
            fleet_model_route_for_loadout("auto", &hakus_config::FleetLoadout::Fast),
            ModelRoute::Inherit,
        );
        assert_eq!(
            fleet_model_route_for_loadout("auto", &hakus_config::FleetLoadout::Inherit),
            ModelRoute::Inherit,
        );
        assert_eq!(
            fleet_model_route_for_loadout(
                "auto",
                &hakus_config::FleetLoadout::Custom("strong".to_string())
            ),
            ModelRoute::Auto,
        );
    ...
    fn exec_hardening_caps_max_steps_to_max_turns() {
        ...
        let exec = hakus_config::FleetExecConfig {
            max_turns: 50,
            ..Default::default()
        };
