...
//! - its **save scope and source**: personal (`$HAKUS_HOME/fleets/`) or
//!   workspace (`.hakus/fleets/`), with the exact file path surfaced.
//!
//! There is exactly one store. The legacy per-role profile files
//! (`~/.hakus/agents/*.toml`, `.hakus/agents/*.toml`,
...
impl FleetFile {
    ...
    /// filesystems Hakus supports; collisions are detected at save.
    #[must_use]
...
fn personal_fleets_dir() -> Result<PathBuf, FleetStoreError> {
    hakus_config::hakus_home()
        ...
        .map_err(|e| FleetStoreError::Io {
            path: "$HAKUS_HOME/fleets".to_string(),
            message: e.to_string(),
        })
}

fn workspace_fleets_dir(workspace: &Path) -> PathBuf {
    workspace.join(".hakus").join(FLEET_DIR)
}
...
mod tests {
    ...
    /// A sealed HAKUS_HOME for personal-scope tests, created once per
    ...
    /// Point HAKUS_HOME at a sealed temp dir. Caller must hold
    /// `lock_test_env`.
    ...
    fn save_load_round_trips_in_workspace_scope() {
        ...
        assert!(
            path.ends_with(".hakus/fleets/deepseek-flash.toml"),
            "{path:?}"
        );
...
members = []"#;
    ...
    fn migration_preserves_pins_and_names_shadowing() {
        ...
        let agents_dir = ws.path().join(".hakus/agents");
...
"#,
        ...
        let receipt = migrate_legacy_roster(
            &hakus_config::FleetConfigToml::default(),
            ws.path(),
            true,
            FleetScope::Workspace,
        )
