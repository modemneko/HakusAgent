...
impl FleetAlertEvent {
    ...
    pub fn inspection_commands(&self) -> Vec<String> {
        let mut commands = vec!["hakus fleet status".to_string()];
        if let Some(worker_id) = &self.worker_id {
            commands.push(format!("hakus fleet inspect {worker_id}"));
        }
        commands
    }
}
...
fn pagerduty_body(event: &FleetAlertEvent, severity: &str, routing_key: String) -> Value {
    json!({
        ...
        "payload": {
            "summary": format!("Hakus fleet {}: {}", alert_class_label(event.class), short_reason(&event.reason)),
            "severity": severity,
            "source": "hakus",
...
mod tests {
    ...
    fn fleet_alert_dry_run_redacts_secrets() {
        ...
        assert!(payload.contains("hakus fleet inspect worker-1"));
    }
    ...
    fn fleet_alert_event_is_derived_from_ledgered_stale_worker_event() {
        ...
        assert_eq!(
            ...
            vec![
                "hakus fleet status".to_string(),
                "hakus fleet inspect worker-1".to_string()
            ]
        );
    }
