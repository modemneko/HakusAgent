...
pub(crate) fn fleet_search_roots(workspace: &std::path::Path) -> Vec<FleetSearchRoot> {
    ...
    if let Ok(home) = personal_fleet_root() {
        roots.push(FleetSearchRoot::new("hakus_home", home));
    }
    ...
    let credential =
        if crate::config::provider_route_is_keyless_self_hosted(identity.provider, &base_url) {
            ...
            hakus_telemetry::session_counters()
                .bump_error(hakus_telemetry::ErrorCounter::AuthPreflightFailed);
...
fn exact_member_profile(
    ...
    let profile = hakus_config::FleetProfile {
        slot: hakus_config::FleetSlot::Custom(member.role.clone()),
        role: hakus_config::FleetRole {
            name: posture_role.to_string(),
            description: Some(format!("exact fleet member `{}`", member.id)),
            instructions: None,
        },
        loadout: hakus_config::FleetLoadout::Inherit,
        ...
        permissions: hakus_config::FleetProfilePermissions {
        ...
        delegation: hakus_config::FleetDelegationHints {
    ...
    pub(crate) fn new(response: impl Into<String>) -> Arc<Self> {
        Arc::new(Self {
            ...
            identity: RouterIdentity {
                ...
                service_kind: hakus_workflow::REASONING_ROUTER_SERVICE_KIND.to_string(),
                legacy_inline: false,
                ...
                call: Some(
                    router_call_plan(
                        hakus_workflow::RouterCallReasoning::Low,
...
"#;
    ...
    fn a_deepseek_route_reports_low_as_low_and_medium_as_high() {
        ...
        let resolved = hakus_workflow::resolve_exact_member_reasoning(
            "implementer",
            &hakus_workflow::FrozenRoute {
        ...
        assert_eq!(
            resolved.effective(),
            hakus_workflow::EffectiveReasoning::Tier(ReasoningTier::Low)
        );
