...
pub struct FleetManager {
    ...
    exec_config: hakus_config::FleetExecConfig,
    ...
    fleet_config: hakus_config::FleetConfigToml,
...
impl FleetManager {
    pub fn open(workspace: impl AsRef<Path>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            exec_config: hakus_config::FleetExecConfig::default(),
            fleet_config: hakus_config::FleetConfigToml::default(),
    ...
    pub fn with_exec_config(mut self, exec_config: hakus_config::FleetExecConfig) -> Self {
    ...
    pub fn with_fleet_config(mut self, fleet_config: hakus_config::FleetConfigToml) -> Self {
    ...
    pub fn create_queued_run_with_descriptor(
        ...
        // dispatch.
        hakus_telemetry::session_counters().bump(hakus_telemetry::Counter::FleetDispatch);
    ...
    pub async fn run_to_completion(
        ...
        loop {
            ...
            let scheduling_error = self
                ...
                .err();
            self.drive_executor_tick(run_id, executor, hakus_binary, model)?;
            self.refresh_run_status(run_id)?;
    ...
    pub fn drive_executor_tick(
        ...
        report.started += self.start_leased_workers(run_id, executor, hakus_binary, model)?;
    ...
    fn write_log_artifact(
        ...
        let rel_path = PathBuf::from(".hakus")
            .join("fleet")
...
mod tests {
    ...
    fn restored_lease_without_launch_record_fails_durably_instead_of_poisoning_ticks() {
        ...
        manager
            .drive_executor_tick(&report.run_id, &mut executor, "unused-hakus", None)
    ...
    #[cfg(unix)]
    fn fake_hakus(dir: &TempDir, body: &str) -> PathBuf {
        use std::os::unix::fs::PermissionsExt;

        let path = dir.path().join("fake-hakus");
...
"#,
        ...
        let status = complete_with_fake_hakus(&reloaded, &report.run_id, 1, &fake);
...
"#,
        ...
        let status = complete_with_fake_hakus(&manager, &report.run_id, 1, &fake);
...
"#,
        ...
        let status = complete_with_fake_hakus(&manager, &report.run_id, 3, &fake);
...
"#,
        ...
        let smoke_task = |id: &str, role: &str, tools: Vec<&str>, marker: &str| {
            ...
            if role == "builder" {
                task.workspace = Some(FleetWorkspaceRequirements {
                    writable_paths: vec![PathBuf::from(".hakus/fleet")],
                    ..FleetWorkspaceRequirements::default()
                });
            }
