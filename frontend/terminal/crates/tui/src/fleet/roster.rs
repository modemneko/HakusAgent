...
//! - personal `$HAKUS_HOME/agents/*.toml` profile files,
//! - workspace `.hakus/agents/*.toml` profile files.
//!
...
//! - Project-scope profiles (`.hakus/agents/*.toml`) join the roster only
//!   when project-level config is trusted for the launch; `--no-project-config`
//!   opts the whole layer out, same as `.hakus/config.toml` (#485).
...
/// (`.hakus/agents/*.toml`) may join the dispatch roster (#5098). Set
...
impl FleetRoster {
    ...
    /// members come from `$HAKUS_HOME/agents/*.toml`, and workspace members
    /// come from `.hakus/agents/*.toml`. A load failure is logged and
    ...
    fn load_with_personal_dir_and_plugins(
        ...
        // whole layer out, same as `.hakus/config.toml`).
...
mod tests {
    ...
    fn personal_setup_target_round_trips_through_the_runtime_roster() {
        ...
        assert!(target.is_file(), "save must land under HAKUS_HOME");
    ...
    fn model_overrides_use_lowercased_ids_and_only_explicit_models() {
        ...
        let home = TempDir::new().unwrap();
        let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", home.path());
        // Isolate personal `$HAKUS_HOME/agents` so ambient developer
