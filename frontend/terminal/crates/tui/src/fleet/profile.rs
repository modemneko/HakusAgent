...
pub const WORKSPACE_AGENT_PROFILE_DIR: &str = ".hakus/agents";
...
impl FleetProfileScope {
    ...
    pub fn display_dir(self) -> &'static str {
        match self {
            ...
            Self::Personal => "$HAKUS_HOME/agents",
        }
    }
...
pub fn personal_agent_profile_dir() -> Result<PathBuf> {
    Ok(hakus_config::hakus_home()?.join(PERSONAL_AGENT_PROFILE_DIR))
}
...
/// extractor (which is private to hakus-config).
