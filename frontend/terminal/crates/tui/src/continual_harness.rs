...
fn state_path_for_read(workspace: &Path) -> Result<PathBuf> {
    let (_, dir) = hakus_config::resolve_project_state_dir(workspace, "harness")?;
...
fn state_path_for_write(workspace: &Path) -> Result<PathBuf> {
    ...
    Ok(hakus_config::ensure_project_state_dir(workspace, "harness")?.join("state.json"))
}
...
fn load_state(path: &Path) -> Result<HarnessState> {
    ...
    if state.schema_version > SCHEMA_VERSION {
        bail!(
            "continual harness state {} uses newer schema {}; this Hakus supports schema {}",
            path.display(),
