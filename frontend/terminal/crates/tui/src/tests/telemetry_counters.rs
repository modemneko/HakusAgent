use hakus_telemetry::counters::http_status_counter;
use hakus_telemetry::{ErrorCounter, session_counters};
...
fn a_custom_provider_is_recorded_as_the_literal_custom() {
    ...
    let closed: std::collections::BTreeSet<&str> = hakus_config::ProviderKind::ALL
        .iter()
...
fn the_turn_wall_histogram_buckets_by_wall_clock_not_per_turn_events() {
    ...
    let mut wall = hakus_telemetry::TurnWall::default();
    wall.observe_secs(0);
    wall.observe_secs(4);
    wall.observe_secs(5);
