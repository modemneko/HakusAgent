/* hakus-ocean: ambient ocean scene behind the DSH web UI.
 *
 ...
 * Off switch: body class `hakus-ocean-off` or
 * localStorage["hakus.ocean"] === "off". `palette` carries
 * { light: {base, accent, ink, dim}, dark: {...} } CSS hex colors taken from
 * the skin token table.
 */
function createOcean(palette) {
	var STORAGE_KEY = "hakus.ocean";
	var OFF_CLASS = "hakus-ocean-off";
	...
	function mount() {
		...
		canvas.setAttribute("data-hakus-ocean", "");
