//! The documented DSH plugin path: a Hakus bundle package installed into
//! a dedicated `hakus` DSH profile with `dsh plugin --profile hakus
//! add <absolute path>` (pnpm required).
//!
//! The bundle is an npm-shaped package under
//! `$HAKUS_HOME/integrations/dsh/bundle/` whose `cordis.patch.yml`
...
//! installed launcher, so `dsh --profile hakus` boots without `--patch`.
...
pub(crate) const BUNDLE_PACKAGE_NAME: &str = "hakus-dsh-bundle";
pub(crate) const BUNDLE_PATCH_FILE: &str = "cordis.patch.yml";
pub(crate) const BUNDLE_PROFILE: &str = "hakus";
...
pub(crate) const SKIN_INSERT_YAML: &str =
    "- insert: [{ id: hakus-skin, name: hakus-dsh-bundle }]\n";
...
pub(crate) struct DshBundleRecord {
    ...
    /// `$DSH_HOME/profiles/hakus` (DSH-owned).
    pub(crate) profile_dir: PathBuf,
    /// `$HAKUS_HOME/integrations/dsh/bundle` (Hakus-owned).
...
pub(crate) fn bundle_version(hakus_version: &str, patch_sha256: &str) -> String {
    format!(
        "{hakus_version}+dsh.{}",
...
/// Files the bundle package consists of (all Hakus-owned). `ocean` only
...
pub(crate) fn render_bundle_files(
    ...
    let version = bundle_version(hakus_version, &patch_sha);
    ...
    let mut package_json = serde_json::json!({
        ...
        "version": version,
        "private": true,
        "description": "DeepSeek Harness connected through Hakus: identity overlay bundle (generated; do not edit)",
        "license": "MIT",
        "dsh": dsh.clone(),
        "hakus": {
            "generated_by": "hakus integrations dsh install-bundle",
    ...
    if skin {
        ...
        package_json["hakus"]["skin_sha256"] = serde_json::json!(skin::skin_tokens_sha256());
        package_json["hakus"]["skin_source"] = serde_json::json!(SKIN_SOURCE);
        package_json["hakus"]["brand_sha256"] = serde_json::json!(brand::brand_sha256());
        if ocean {
            package_json["hakus"]["ocean_scene_sha256"] =
                serde_json::json!(scene::scene_sha256());
        }
    }
    let readme = format!(
        "# {BUNDLE_PACKAGE_NAME}\n\nDeepSeek Harness connected through Hakus.\n\nGenerated bundle: `{BUNDLE_PATCH_FILE}` carries the exact Hakus provider/model/endpoint identity (no credentials). Installed into the dedicated DSH profile `{BUNDLE_PROFILE}` with `dsh plugin --profile {BUNDLE_PROFILE} add <this directory>`. Regenerated by `hakus integrations dsh update`; removed by `hakus integrations dsh remove-bundle`. Do not edit by hand.\n"
    );
    let notice = "This bundle is generated by Hakus and configures the official DeepSeek Harness (dsh).\n\nDeepSeek Harness — MIT License, Copyright (c) 2026 DeepSeek. The DeepSeek Harness copyright and permission notice apply to the harness packages this bundle references; this bundle redistributes none of them.\n\nThis generated bundle is provided under the MIT License.\n".to_string();
    let mut files = vec![
        (
            "package.json",
            format!(
...
pub(crate) fn write_bundle(
    ...
    for (name, text) in render_bundle_files(hakus_version, overlay_text, skin, ocean) {
...
pub(crate) fn remove_bundle_files(bundle_dir: &Path) -> Result<Vec<PathBuf>> {
    ...
    let _ = std::fs::remove_dir(bundle_dir.join("lib"));
    // Only remove the directory when Hakus left nothing else in it.
...
pub(crate) fn client_half_stale(bundle_dir: &Path, skin: bool, ocean: bool) -> Option<String> {
    ...
    if skin {
        ...
        match std::fs::read_to_string(&client) {
            ...
            Ok(_) => Some(
                "bundle lib/client.js was modified outside Hakus; run `update`".to_string(),
            ),
...
/// Hakus bundle, in that order so Hakus's rows patch last.
