...
use super::identity::{
    HakusRouteIdentity, DshAdapter, DshPermissionMode, WireProtocol, dsh_reasoning_effort,
...
fn permission_never_broadens_without_explicit_confirmation() {
    ...
    // Hakus at workspace-write can never be lifted to full access.
...
fn deepseek_route_maps_to_native_adapter_with_exact_identity() {
    ...
    assert!(overlay.contains("DeepSeek Harness connected through Hakus"));
    assert!(
...
fn ollama_keyless_route_writes_no_credential_reference() {
    ...
    assert_eq!(
        ...
        DshAdapter::PiAiOpenAiCompatible {
            route_id: "hakus-ollama".to_string()
        }
    );
    let overlay = render_overlay(&mapped).unwrap();
    assert!(overlay.contains("provider: 'hakus-ollama'"));
...
fn responses_dialect_route_is_carried_not_approximated() {
    ...
    assert_eq!(
        mapped.adapter,
        DshAdapter::PiAiOpenAiCompatible {
            route_id: "hakus-deepseek".to_string()
        }
    );
    assert_eq!(mapped.dsh_provider(), Some("hakus-deepseek"));
    let overlay = render_overlay(&mapped).unwrap();
    assert!(overlay.contains("provider: 'hakus-deepseek'"));
    ...
    assert_eq!(plan.mapped.dsh_provider(), Some("hakus-deepseek"));
}
...
fn status_surfaces_route_carryability_before_plan() {
    ...
    assert!(
        line.contains("deepseek/deepseek-v4-flash is carryable via hakus-deepseek"),
        "{line}"
    );
...
fn lab_paths() -> (tempfile::TempDir, DshPaths) {
    let dir = tempfile::tempdir().unwrap();
    let paths = DshPaths::under(&dir.path().join("hakus-home"));
    (dir, paths)
}
...
fn brand_lockup_css_rules_are_not_accidentally_nested() {
    ...
    // `#hakus-brand-mark { ... }` block. CSS nesting resolves a bare
    // nested selector against its parent, so `#hakus-brand-mark svg`
    // nested under `#hakus-brand-mark` means
    // "#hakus-brand-mark #hakus-brand-mark svg" — which matches
    ...
    // declaration.
    let marker = "#hakus-brand-mark svg{display:block;width:24px;height:24px;}";
    let at = css_region
        .find(marker)
...
#[test]
fn launch_strips_only_hakus_injected_credentials() {
    let none = launch_env_strip_list(None, &["ZAI_API_KEY".to_string()]);
    assert_eq!(none, ["HAKUS_CLI_API_KEY", "DEEPSEEK_API_KEY_SOURCE"]);
    ...
    fn run(&self, _binary: &std::path::Path, args: &[&str]) -> std::io::Result<(bool, String)> {
        ...
        match args {
            ...
            ["plugin", "--profile", "hakus", "add", spec] => {
                ...
                let json = serde_json::json!({"name": "dsh-profile-hakus", "private": true, "dsh": {"profile": {"bundles": bundles}}});
            ...
            ["plugin", "--profile", "hakus", "remove", name] => {
...
fn bundle_files_are_npm_shaped_and_carry_the_overlay_rows() {
    ...
    assert_eq!(pkg["name"], "hakus-dsh-bundle");
...
fn install_update_remove_bundle_lifecycle_uses_documented_plugin_commands() {
    ...
    let profile_dir = detection.dsh_home.join("profiles").join("hakus");
    ...
    assert_eq!(record.profile, "hakus");
...
fn failed_plugin_add_leaves_no_bundle_record_or_files() {
    ...
    detection.dsh_home = dir.path().join("dsh-home");
    let runner = PluginRunner {
        profile_dir: detection.dsh_home.join("profiles/hakus"),
        calls: Default::default(),
        fail_add: true,
    };
    let id = identity(
...
fn ocean_scene_fragment_mounts_a_canvas_and_honours_the_guards() {
    ...
    assert!(js.contains("data-hakus-ocean"));
    // motion guards
...
fn brand_fragment_is_explicit_responsive_and_slot_safe() {
    ...
    assert!(js.contains("function HakusBrand()"));
    assert!(js.contains("hakus-brand-lockup"));
    assert!(js.contains("WHALE BROTHERS"));
    assert!(js.contains("HAKUS"));
    ...
    assert!(!js.contains("window.__hakusBrand"));
...
fn bundle_manifest_records_the_ocean_decision_and_scene_sha() {
    ...
    let pkg: serde_json::Value = serde_json::from_str(by_name["package.json"]).unwrap();
    assert_eq!(pkg["hakus"]["ocean"], true);
    assert_eq!(
        pkg["hakus"]["brand_sha256"],
        super::brand::brand_sha256()
    );
    assert_eq!(
        pkg["hakus"]["ocean_scene_sha256"],
        scene::scene_sha256()
    );
    assert_eq!(
        by_name[bundle::BUNDLE_CLIENT_FILE],
    ...
    let pkg: serde_json::Value = serde_json::from_str(by_name["package.json"]).unwrap();
    assert_eq!(pkg["hakus"]["ocean"], false);
    assert!(pkg["hakus"].get("ocean_scene_sha256").is_none());
    assert_eq!(
        by_name[bundle::BUNDLE_CLIENT_FILE],
...
fn update_with_ocean_off_rewrites_the_client_half_and_receipt() {
    ...
    detection.dsh_home = dir.path().join("dsh-home");
    let runner = PluginRunner {
        profile_dir: detection.dsh_home.join("profiles").join("hakus"),
        calls: Default::default(),
        fail_add: false,
    };
    let id = identity(
