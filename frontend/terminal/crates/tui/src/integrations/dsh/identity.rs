//! Exact Hakus route identity → DeepSeek Harness overlay mapping.
//!
...
/// Non-secret facts about the route Hakus is currently configured to use.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub(crate) struct HakusRouteIdentity {
    ...
    /// Wire protocol Hakus speaks to this endpoint.
    ...
    /// Hakus reasoning tier as configured (`off|low|medium|high|xhigh|max|ultra`).
    pub(crate) reasoning_effort: Option<String>,
    /// Hakus sandbox mode (`read-only|workspace-write|danger-full-access|external-sandbox`).
    pub(crate) sandbox_mode: Option<String>,
    /// Hakus approval policy (`suggest|auto|never`).
...
/// DSH permission mode mirrored from the Hakus posture.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
pub(crate) struct MappedIdentity {
    pub(crate) source: HakusRouteIdentity,
    pub(crate) adapter: DshAdapter,
    /// DSH `reasoningEffort` (`off|high|max`) for the native adapter; `None`
    /// when Hakus has no explicit tier or the adapter cannot express it.
...
/// Map a Hakus reasoning tier onto DSH's `off | high | max`.
...
/// Mirror the Hakus posture. Full access is only granted when Hakus
...
pub(crate) fn permission_mode_for(
    ...
    if hakus_full {
        if allow_full_access {
            return (
                ...
                Some(
                    "DSH danger-full-access mirrors Hakus full access; DSH will not ask before file effects."
                        .to_string(),
                ),
            );
        }
        return (
            ...
            Some(
                "Hakus runs with full access, but the DSH overlay stays at workspace-write; pass --allow-full-access to mirror it."
                    .to_string(),
            ),
        );
    }
    let note = match approval.as_deref() {
        Some("never" | "deny" | "denied") => Some(
            "Hakus approval policy is `never`; DSH keeps its own ask-before-effects policy at workspace-write."
                .to_string(),
        ),
        _ => None,
    };
...
/// Hakus wire protocol — identity-preserving, never an approximation.
///
...
pub(crate) fn map_identity(
    ...
    if is_deepseek && identity.protocol == WireProtocol::ChatCompletions {
        if identity.reasoning_effort.is_some() && dsh_effort.is_none() {
            disclosures.push(format!(
                "Hakus reasoning tier `{}` has no DSH equivalent; DSH keeps its default (high).",
                identity.reasoning_effort.as_deref().unwrap_or("")
            ));
        }
        disclosures.push(
            "DSH resolves DEEPSEEK_API_KEY from its own environment or $DSH_HOME/.credentials.yaml; Hakus does not hand over a key."
                .to_string(),
        );
        return MappedIdentity {
            source: identity.clone(),
    ...
    // Every wire dialect Hakus can speak, `dsh-llm-pi-ai` declares a
    ...
    if identity.keyless_local {
        ...
        disclosures.push(format!(
            "DSH resolves {env} from its own environment or $DSH_HOME/.credentials.yaml; Hakus does not hand over a key."
        ));
    } else {
        disclosures.push(
...
pub(crate) fn render_overlay(mapped: &MappedIdentity) -> Option<String> {
    ...
    out.push_str("# DeepSeek Harness connected through Hakus.\n");
    out.push_str("# Generated by `hakus integrations dsh connect`; do not edit by hand.\n");
    out.push_str("# Identity only: no API key, token, or credential document is written here.\n");
    out.push_str(&format!(
        "# hakus.provider={} hakus.model={} hakus.workspace={}\n",
    ...
    match &mapped.adapter {
        ...
        DshAdapter::PiAiOpenAiCompatible { route_id } => {
            ...
            out.push_str(&format!(
                "        displayName: {}\n",
                yaml_str(&format!("{} (via Hakus)", src.provider_label))
            ));
