//! Ambient ocean scene for the DSH bundle: whales and Hakus glyph fish
...
pub(crate) const OCEAN_STORAGE_KEY: &str = "hakus.ocean";
/// Body class that turns the scene off in the browser.
pub(crate) const OCEAN_OFF_CLASS: &str = "hakus-ocean-off";
/// `window` handle the scene exposes (`start`, `stop`, `setIntensity`, `setScheme`).
pub(crate) const OCEAN_WINDOW_HANDLE: &str = "__hakusOcean";
...
/// keeps text surfaces calm while letting the Hakus scene read clearly.
