//! DeepSeek Harness (`dsh`) connected through Hakus.
//!
//! A thin, reversible adapter around DSH's *documented* seams:
//!
//! - detection reads `dsh --version` / `--help`, `$DSH_HOME` inventory;
//! - connection writes only under `$HAKUS_HOME/integrations/dsh/`: a
//!   `--patch` overlay pinning the exact Hakus route identity and an
//!   append-only receipt; the Hakus palette rides the bundle profile
...
//! - removal deletes only Hakus-owned files.
//!
//! Hakus never copies, prints, or embeds API keys, OAuth documents, or
...
pub(crate) use identity::{
    HakusRouteIdentity, DshAdapter, MappedIdentity, WireProtocol, map_identity, render_overlay,
    sha256_hex,
};
...
pub(crate) const OVERLAY_FILE: &str = "hakus.patch.yml";
pub(crate) const RECEIPT_FILE: &str = "receipt.json";
pub(crate) const SKIN_FILE: &str = "hakus-dsh-skin.css";
pub(crate) const SKIN_PREVIEW_FILE: &str = "hakus-dsh-skin-preview.html";
pub(crate) const RELATIONSHIP_LABEL: &str = "DeepSeek Harness connected through Hakus";
pub(crate) const CLI_COMMAND: &str = "hakus integrations dsh";

/// Hakus-owned files for this integration.
...
pub(crate) struct DshPaths {
    ...
    /// Hakus-owned bundle package directory (documented DSH plugin path).
    pub(crate) bundle_dir: PathBuf,
}

impl DshPaths {
    pub(crate) fn under(hakus_home: &Path) -> Self {
        let root = hakus_home.join(INTEGRATION_DIR);
        Self {
    ...
    pub(crate) fn from_process() -> Result<Self> {
        Ok(Self::under(&hakus_config::hakus_home()?))
    }
}
...
pub(crate) enum DshIntegrationState {
    ...
    /// Installed and usable, but no Hakus overlay exists.
    Detected { version: String },
    /// Overlay present and matches the current Hakus route.
    Connected { version: String },
    /// Overlay present but the current Hakus route (or its file) drifted.
...
pub(crate) struct DshStatusReport {
    ...
    /// The identity Hakus would write *now* (may be `None` when the route
...
fn client_or_patch_stale(
    ...
    if disk_patch_sha256 != expected_sha.as_deref() {
        return Some(
            "bundle cordis.patch.yml was modified outside Hakus; run `update`".to_string(),
        );
    }
    None
}
...
pub(crate) fn compute_status(
    ...
    current_identity: Result<HakusRouteIdentity, String>,
            ...
            DshCompatibility::Verified | DshCompatibility::NewerUnverified { .. } => {
                ...
                match record.as_ref() {
                    ...
                    Some(record) => {
                        ...
                        {
                            Some(
                                "overlay file was modified outside Hakus; run `update`"
                                    .to_string(),
                            )
                            ...
                            match current_identity.as_ref() {
                                Some(now) if !now.mappable() => Some(
                                    "current Hakus route cannot be carried by DSH; overlay is stale"
                                        .to_string(),
                                ),
                                Some(now) => {
                                    ...
                                    match expected {
                                        ...
                                        Some(_) => Some(format!(
                                            "Hakus route is now {}/{}; overlay pins {}/{}; run `update`",
...
pub(crate) fn plan(
    ...
    let overlay_text = render_overlay(&mapped).ok_or_else(|| match &mapped.adapter {
        DshAdapter::Unsupported { reason } => anyhow::anyhow!(
            "current Hakus route {}/{} cannot be carried by DSH: {reason}",
    ...
    if skin {
        disclosures.push(
            "Skin: Hakus palette is applied through the bundle profile via overrideTokens (on by default for install-bundle). The --patch overlay path is unchanged; launch --profile web|headless stays overlay-only."
                .to_string(),
        );
        if ocean {
            disclosures.push(format!(
...
pub(crate) fn apply_plan(
    ...
    let bundle_record = match doc.current.as_ref().and_then(|r| r.bundle.clone()) {
        Some(mut b) if event == DshReceiptEvent::Update => {
            ...
            b.package_version = bundle::bundle_version(&hakus_version(), &sha);
...
/// Delete only Hakus-owned files; keep the receipt history with a
...
pub(crate) fn remove(paths: &DshPaths) -> Result<Vec<PathBuf>> {
    ...
    if doc.current.as_ref().is_some_and(|r| r.bundle.is_some()) {
        anyhow::bail!(
            "the Hakus bundle is still installed in DSH profile `{}`; run `{CLI_COMMAND} remove-bundle` first",
            bundle::BUNDLE_PROFILE
        );
    }
    ...
    doc.push(DshReceiptEntry {
        ...
        note: Some(format!(
            "removed {} Hakus-owned file(s); $DSH_HOME untouched",
            removed.len()
        )),
    });
    doc.save(&paths.receipt)?;
...
pub(crate) struct LaunchSpec {
    ...
    /// Variables Hakus itself injected into this process (a `--api-key`
    /// or keyring-bridged credential) that must not leak into the child.
    pub(crate) strip_env: Vec<String>,
    pub(crate) cwd: PathBuf,
}

/// Names to strip from the child environment when Hakus's own dispatcher
/// materialized a credential into this process. A key the user exported in
/// their shell is theirs and is left alone; a key Hakus bridged from
/// `--api-key` or the keyring is Hakus's and is not handed over.
pub(crate) fn launch_env_strip_list(
    api_key_source: Option<&str>,
    provider_env_vars: &[String],
) -> Vec<String> {
    let mut out = vec![
        "HAKUS_CLI_API_KEY".to_string(),
...
/// Install the Hakus bundle into the dedicated `hakus` DSH profile via
/// the documented `dsh plugin --profile hakus add <path>` (pnpm).
pub(crate) fn install_bundle(
    ...
    let bundle_record = DshBundleRecord {
        ...
        package_version: bundle::bundle_version(&hakus_version(), &patch_sha),
...
/// `dsh plugin --profile hakus remove hakus-dsh-bundle`, then delete
/// only the Hakus-owned bundle files. The DSH profile directory (and the
...
/// Derive the non-secret route identity from a loaded Hakus config.
pub(crate) fn hakus_route_identity(
    config: &crate::config::Config,
    workspace: &Path,
) -> Result<HakusRouteIdentity, String> {
    ...
    let protocol = match candidate.protocol() {
        hakus_config::provider::WireFormat::ChatCompletions => WireProtocol::ChatCompletions,
        hakus_config::provider::WireFormat::Responses => WireProtocol::Responses,
        hakus_config::provider::WireFormat::AnthropicMessages => {
