//! Hakus palette for the DeepSeek Harness web surface.
//!
...
//! Brothers / Hakus identity lockup; it leaves DSH-owned branding intact.
...
pub(crate) const SKIN_SOURCE: &str = "hakus-dsh-bundle";
...
pub(crate) fn skin_tokens() -> BTreeMap<String, (String, String)> {
    ...
    // Bounded mapping of DSH `--dsw-alias-*` variables onto Hakus tokens.
...
///
/// The Whale Brothers / Hakus lockup is spliced in for every skin and
...
/// (`localStorage["hakus.ocean"] = "off"` or body class
/// `hakus-ocean-off`) skips both the canvas and the veil.
...
\t\t\t\t{{ name: \"shell.overlay\", id: \"hakus-brand-lockup\", order: 100, label: \"Hakus\" }},\n\
\t\t\t\t() => React.createElement(HakusBrand),\n\
\t\t\t));\n\
\t\t\tif (oceanOn) {{\n\
\t\t\t\tctx.effect(() => {{\n\
