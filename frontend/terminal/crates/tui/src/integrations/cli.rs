//! `hakus integrations …` command surface (dispatched from the outer
//! `hakus` binary as a passthrough).
...
fn print_status(report: &DshStatusReport) {
    ...
    println!("  Hakus-owned files: {}", report.paths_root.display());
    println!(
        "  overlay: {}{}",
    ...
    if let Some(record) = report.record.as_ref() {
        ...
        if record.skin_enabled {
            println!(
                "  skin: on (Hakus palette via overrideTokens in the bundle profile; `{CLI_COMMAND} update --skin false` turns it off)"
            );
    ...
    match (&report.current_identity, &report.current_identity_error) {
        (Some(now), _) if now.mappable() => println!(
            "  current Hakus route: {}/{} · {} · would map via {}",
        ...
        (None, Some(error)) => println!("  current Hakus route: unresolved ({error})"),
        (None, None) => {}
    }
    match &report.state {
        ...
        DshIntegrationState::Connected { .. } | DshIntegrationState::StaleVersion { .. } => {
            println!(
                "  next: `{CLI_COMMAND} launch [--profile web|headless|hakus] [dsh app args]`"
            )
        }
        _ => {}
    }
...
fn run_dsh(config: &Config, workspace: &Path, command: DshIntegrationCommand) -> Result<()> {
    match command {
        ...
        DshIntegrationCommand::Plan {
            ...
            let identity = dsh::hakus_route_identity(config, workspace)
                .map_err(|e| anyhow::anyhow!("cannot resolve the current Hakus route: {e}"))?;
            let plan = dsh::plan(
                &paths,
                &report.detection,
                &identity,
        ...
        DshIntegrationCommand::Connect {
            ...
            print_plan(&plan, "connect");
            confirm(yes, "Write these Hakus-owned files and connect DSH?")?;
            let record =
        ...
        DshIntegrationCommand::Update {
            ...
            confirm(yes, "Rewrite the Hakus overlay for DSH?")?;
            let record =
        ...
        DshIntegrationCommand::InstallBundle { app, yes } => {
            ...
            println!(
                "  will write (Hakus-owned): {}/{{package.json,cordis.patch.yml,README.md,NOTICE.md}}",
            ...
            confirm(
                yes,
                "Install the Hakus bundle into DSH profile `hakus`?",
            )?;
        ...
        DshIntegrationCommand::RemoveBundle { yes } => {
            ...
            confirm(
                yes,
                "Remove the Hakus bundle from DSH profile `hakus`?",
            )?;
            let removed = dsh::remove_bundle(&paths, &report.detection, &dsh::ProcessRunner)?;
            println!(
                "removed bundle; deleted {} Hakus-owned file(s)",
                removed.len()
            );
            Ok(())
        }
        DshIntegrationCommand::Remove { yes } => {
            let paths = DshPaths::from_process()?;
            println!(
                "remove will delete only Hakus-owned files under {}:",
                paths.root.display()
            );
