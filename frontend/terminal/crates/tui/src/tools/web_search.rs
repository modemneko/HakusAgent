...
mod tests {
    ...
    async fn firecrawl_keyless_request_is_headerless_and_keyed_request_is_explicit() {
        ...
        let (entries, mode) = WebSearchTool
            .run_firecrawl_search_at(&endpoint, "hakus", 5, 5_000, None)
            .await
            .expect("keyless Firecrawl search");
        WebSearchTool
            .run_firecrawl_search_at(&endpoint, "hakus", 5, 5_000, Some("fc-secret"))
            .await
    ...
    fn searxng_url_uses_search_path_and_json_format() {
        ...
        let (search_url, _) =
            searxng_search_url(Some("https://search.example/searxng/search"), "hakus")
