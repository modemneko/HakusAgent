...
const REVIEW_CLIENT_UNAVAILABLE: &str = "Review tool requires an active Hakus model client";
...
pub fn write_review_receipt(
    ...
    let path = if let Some(path) = path_override {
        ...
        let dir = hakus_config::ensure_state_dir("review-receipts")?;
        let digest = receipt
            .diff_fingerprint
            .strip_prefix("sha256:")
...
pub fn latest_review_receipt_for_diff(
    ...
    let dir = hakus_config::resolve_state_dir("review-receipts")?;
    if !dir.is_dir() {
        return Ok(None);
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn missing_review_client_uses_hakus_provider_neutral_language() {
        ...
        assert_eq!(
            error,
            "Failed to locate tool: Review tool requires an active Hakus model client"
        );
    ...
    async fn staged_diff_with_base_compares_merge_base_to_index() {
        ...
        fixture_git(repo.path(), &["config", "user.name", "Hakus Test"]);
        fixture_git(
            repo.path(),
            &["config", "user.email", "hakus-test@example.invalid"],
        );
    ...
    fn pre_push_diff_review_receipt_includes_fingerprint_and_risk() {
        ...
        let receipt = build_review_receipt(
            ...
            vec![ReviewReceiptCheck {
                name: "cargo test -p hakus-tui".to_string(),
