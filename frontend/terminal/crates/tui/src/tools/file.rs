...
fn config_backup_path_for_credential_guard(config_path: &Path) -> PathBuf {
    let mut file_name = config_path
        ...
        .unwrap_or_else(|| std::ffi::OsString::from(hakus_config::CONFIG_FILE_NAME));
    file_name.push(".bak");
    config_path
        .parent()
...
pub(crate) fn is_hakus_credential_path(path: &Path) -> bool {
    let candidate = canonical_path_for_credential_guard(path);

    if let Ok(active_config) = hakus_config::resolve_config_path(None)
...
impl ReadFileTool {
    ...
    /// Hakus hash/snapshot protocol into its small-contract-shaped model contract.
    pub(super) async fn execute_contract_read(
        ...
        if is_hakus_credential_path(&file_path) {
            return Err(ToolError::permission_denied(
                "read cannot expose Hakus configuration or credential-store files; use `hakus config list` or `hakus auth status` for safe inspection",
            ));
        }
...
impl ToolSpec for ReadFileTool {
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        if is_hakus_credential_path(&file_path) {
            return Err(ToolError::permission_denied(
                "File `read` cannot expose CodeWhale configuration or credential-store files; use `hakus config list` or `hakus auth status` for safe inspection",
            ));
        }
