...
const READONLY_ENV_MARKER: &str = "HAKUS_INTERNAL_READONLY_ARGV";
...
impl ShellCompletionEvidence {
    ...
    pub(crate) fn artifact_bytes(&self) -> Vec<u8> {
        ...
        serde_json::json!({
            "schema": "hakus.shell_completion.evidence.v1",
...
impl ShellManager {
    ...
    /// Transfer every still-running `persist:true` process out of Hakus's
        ...
        if persist {
            ...
            if !persistent_services_enabled_for(context) {
                return Err(ToolError::not_available(
                    "persistent background services (persist:true) are only available on Unix in the real headless `hakus exec` host under an explicit danger-full-access / full shell authority. They are rejected in interactive sessions, desktop/app-server hosts, Fleet/sub-agents, restricted or external sandboxes, and TTY/interactive/stdin modes.",
                ));
            }
        }
        ...
        match result {
            Ok(result) => {
                ...
                let mut output = if interactive {
                    ...
                    format!(
                        "Persistent service staged: {task_id_str}. Probe readiness with a separate command. Hakus will transfer ownership only if this exec finishes successfully."
                    )
