...
/// it via `[subagents] api_timeout_secs` in `~/.hakus/config.toml`. The
...
/// gated behind `HAKUS_SUBAGENT_PERF_TRACE=1`. The atomic increments are
...
fn subagent_perf_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        std::env::var("HAKUS_SUBAGENT_PERF_TRACE")
...
/// `hakus_config::FleetRole`, which is the open config-side role
...
pub struct SubAgentResult {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub runtime_permissions: Option<hakus_protocol::fleet::FleetEffectivePermissions>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
struct SpawnRequest {
    ...
    /// headless `hakus exec` launch path).
    ...
    cwd: Option<PathBuf>,
    /// Optional first-class git worktree isolation. When set, Hakus
...
///
/// Sourced from [`hakus_config::DEFAULT_SPAWN_DEPTH`] so standalone
...
pub const DEFAULT_MAX_SPAWN_DEPTH: u32 = hakus_config::DEFAULT_SPAWN_DEPTH;
...
/// absolute [`hakus_config::MAX_SPAWN_DEPTH_CEILING`].
///
...
/// `<hakus:subagent.done>` sentinel that the model expects in the
...
pub struct SubAgentRuntime {
    ...
    /// the interactive-TUI twin of the headless `hakus exec --provider`
...
/// Hakus process" (owner report, 2026-08-04).
...
impl CoordinationProcessLock {
    fn acquire(state_root: &Path) -> Result<Self> {
        ...
        let lock_dir = requested_root.join(".hakus").join("state");
        ...
        match ready_rx.recv_timeout(Duration::from_secs(5)) {
            ...
            Ok(Err(error)) => {
                ...
                if holder_pid == Some(std::process::id()) {
                    ...
                    Err(anyhow!(
                        "another Hakus process{} owns delegated coordination for {}: {error}",
                        holder_pid
...
impl SubAgentManager {
    ...
    fn cleanup_inner(&mut self, active_session_id: Option<&str>, max_age: Duration) -> usize {
        ...
        // coordination flock (#2.6). Two Hakus sessions in one workspace is
...
/// `state_root/.hakus/state`. Distinct state roots intentionally do not
    ...
    fn input_schema(&self) -> Value {
        ...
        json!({
            ...
            "properties": {
                ...
                "profile": {
                    "type": "string",
                    "description": "Optional Fleet roster member to run this child as (e.g. reviewer, scout, builder, verifier, synthesizer, manager, or a custom member from project .hakus/agents/, personal $HAKUS_HOME/agents/, or [fleet.profiles] config). The member supplies role posture, model routing, thinking tier, instruction overlay, and delegation bounds. Named profiles bind 1:1 to their configured route — the child's model and thinking come from the profile or inherit the parent; there is no per-call model override on this surface. See /fleet. For fast exploration use the scout role."
                },
                "worktree": {
                    "type": "boolean",
            ...
            && let Some(memo_map) = inspect_memo
        {
            ...
            if unchanged {
                ...
                let payload = json!({
                    ...
                    "hint": "No change since your last check. Checking again in a loop is the anti-pattern; one blocking wait is not. Make one agent(action=\"wait\") call — until=\"all\" to join every running child in a single block — or continue independent work, or end your turn. Results arrive automatically as <hakus:subagent.done> sentinels.",
                });
...
/// automatically as `<hakus:subagent.done>` sentinels, so ending the
...
/// still delivered as `<hakus:subagent.done>` sentinels by the runtime;
...
async fn wait_result_payload(
    ...
    let note = if timed_out {
        "Wait timed out with children still running. Do not poll — wait again (until=\"all\" blocks for the whole batch), continue independent work, or end your turn; results arrive automatically as <hakus:subagent.done> sentinels."
    } else if settled_entries.is_empty() {
        "No sub-agents are running anymore."
    } else {
        "Full results arrive as <hakus:subagent.done> sentinels — read those before synthesizing; do not re-peek settled children unless you need the full projection."
    };
    let payload = json!({
        "action": "wait",
...
fn mint_child_route_receipt(
    ...
    let receipt = ChildRouteReceipt {
        ...
        runtime_build_sha: option_env!("HAKUS_BUILD_COMMIT")
...
pub(crate) async fn spawn_workflow_task(
    request: hakus_workflow_js::TaskRequest,
    manager: SharedSubAgentManager,
    mut runtime: SubAgentRuntime,
...
fn subagent_skill_catalog(context: &ToolContext) -> String {
    let mode =
        crate::skills::SkillDiscoveryMode::from_hakus_only(context.skills_scan_hakus_only);
...
/// Build a `<hakus:subagent.done>` JSON sentinel for a successful child.
...
fn subagent_done_sentinel(agent_id: &str, res: &SubAgentResult, truncated: bool) -> String {
    ...
    format!("<hakus:subagent.done>{payload}</hakus:subagent.done>")
}
...
fn child_completion_runtime_message(completions: &[SubAgentCompletion]) -> Message {
    let mut text = String::from(
        "<hakus:runtime_event kind=\"child_subagent_completion\" visibility=\"internal\">\n\
...
then re-plan dependent work before claiming completion.\n",
    ...
    text.push_str("</hakus:runtime_event>");
...
fn optional_input_str<'a>(input: &'a Value, keys: &[&str]) -> Result<Option<&'a str>, ToolError> {
    for key in keys {
        ...
        let text = value
            .as_str()
            .ok_or_else(|| hakus_tools::type_mismatch(key, value, "a string"))?;
...
fn parse_optional_u64(input: &Value, keys: &[&str]) -> Result<Option<u64>, ToolError> {
    ...
    value
        .as_u64()
        .map(Some)
        .ok_or_else(|| hakus_tools::type_mismatch(key, value, "a non-negative integer"))
}
...
fn parse_tool_name_list(input: &Value, key: &str) -> Result<Option<Vec<String>>, ToolError> {
    ...
    let array = value
        .as_array()
        .ok_or_else(|| hakus_tools::type_mismatch(key, value, "an array of tool names"))?;
    let mut tools: Vec<String> = Vec::new();
    for item in array {
        let tool = item
            .as_str()
            .ok_or_else(|| hakus_tools::type_mismatch(&format!("{key}[]"), item, "a string"))?;
...
fn parse_spawn_request(input: &Value) -> Result<SpawnRequest, ToolError> {
    ...
    let max_depth = parse_optional_u64(input, &["max_depth", "maxDepth", "max_spawn_depth"])?
        .map(|depth| {
            let ceiling = hakus_config::MAX_SPAWN_DEPTH_CEILING;
            u32::try_from(depth)
                .map_err(|_| {
...
fn apply_spawn_profile(
    ...
    let is_general_slot = matches!(member.profile.slot, hakus_config::FleetSlot::General);
    ...
    // headless `hakus exec` argv. Without this, `agent { profile: "x" }`
...
fn resolve_spawn_model_selection(
    ...
    if let Some(member) = member {
        ...
        if member.profile.loadout == hakus_config::FleetLoadout::Fast {
            return Ok(SpawnModelSelection {
...
fn parse_optional_bool(input: &Value, names: &[&str]) -> Result<Option<bool>, ToolError> {
    ...
    value
        .as_bool()
        .map(Some)
        .ok_or_else(|| hakus_tools::type_mismatch(name, value, "a boolean"))
}
...
/// snapshot, and only then the legacy hardcoded table for Hakus-only /
