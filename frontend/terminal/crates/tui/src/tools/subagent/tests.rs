...
/// A second Hakus session in the same workspace is ordinary usage. Losing
...
fn fleet_roster_with(id: &str, profile: hakus_config::FleetProfile) -> FleetRoster {
    let tmp = tempdir().expect("tempdir");
    let config = hakus_config::FleetConfigToml {
...
/// not be shadowed by the operator's personal `~/.hakus/agents/*.toml`.
fn isolated_fleet_roster_with(
    id: &str,
    mut profile: hakus_config::FleetProfile,
) -> FleetRoster {
...
fn custom_fleet_profile(role: &str) -> hakus_config::FleetProfile {
    hakus_config::FleetProfile {
        slot: hakus_config::FleetSlot::from_name(role),
        role: hakus_config::FleetRole {
            name: role.to_string(),
            description: None,
            instructions: None,
        },
...
fn spawn_model_selection_has_stable_four_tier_precedence_and_source() {
    ...
    strong_profile.loadout = hakus_config::FleetLoadout::Custom("strong".to_string());
    ...
    fast_profile.loadout = hakus_config::FleetLoadout::Fast;
...
fn spawn_route_sources_refresh_reads_current_disk() {
    ...
    let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", home.path());
    let workspace = tempfile::tempdir().expect("workspace tempdir");
    let agents = workspace.path().join(".hakus").join("agents");
...
fn test_child_max_spawn_depth_profile_hint_only_narrows() {
    ...
    assert_eq!(
        child_max_spawn_depth_for_spawn(
            2,
            0,
            Some(hakus_config::MAX_SPAWN_DEPTH_CEILING),
            None
        ),
        2
    );
...
/// only the headless `hakus exec` argv. Direct and workflow spawns share
...
fn forked_subagent_messages_preserve_parent_prefix_then_append_task() {
    ...
    assert!(message_text(&messages[1]).contains("<hakus:fork_state>"));
    assert_eq!(messages[2].role, "system");
    assert!(message_text(&messages[2]).contains("<hakus:subagent_context>"));
...
fn plugin_agent_profile_survives_restart_and_spawn_rechecks_disable() {
    ...
    let config = hakus_config::FleetConfigToml::default();
...
async fn small_surface_read_only_child_discovers_web_deferred() {
    ...
    let same_batch = registry
        .execute_from_surface(
            ...
            &request_active,
            "Web",
            json!({"action": "search", "query": "hakus"}),
        )
        .await
...
fn persist_state_rejects_symlinked_state_directory() {
    ...
    let hakus_dir = workspace.join(".hakus");
    let state_dir = hakus_dir.join("state");
    std::fs::create_dir_all(&hakus_dir).expect("mkdir hakus");
...
fn fresh_forked_and_nested_subagents_share_authority_bound_skill_catalogs() {
    ...
    let tmp = tempdir().expect("tempdir");
    let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", tmp.path().join("home"));
    let workspace = tmp.path().join("workspace");
    let native_skill = workspace.join(".agents/skills/native-review");
    let plugin_root = workspace.join(".hakus/plugins/demo");
...
fn subagent_done_sentinel_format_is_well_formed() {
    ...
    assert!(sentinel.starts_with("<hakus:subagent.done>"));
    assert!(sentinel.ends_with("</hakus:subagent.done>"));

    // The inner JSON parses and carries the expected fields.
    let inner = sentinel
        .trim_start_matches("<hakus:subagent.done>")
        .trim_end_matches("</hakus:subagent.done>");
...
async fn child_write_tool_fails_closed_outside_registered_scope() {
    let _env_lock = crate::test_support::lock_test_env();
    let home = tempdir().expect("isolated HAKUS_HOME");
    let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", home.path());
    let tmp = tempdir().expect("tempdir");
...
pub(crate) fn stub_runtime() -> SubAgentRuntime {
    ...
    let workspace = std::env::temp_dir().join("hakus-test-stub");
...
fn subagent_budget_exhaustion_completion_carries_budget_exhausted_sentinel() {
    ...
    let inner = completion
        .payload
        .split("<hakus:subagent.done>")
        .nth(1)
        .and_then(|chunk| chunk.split("</hakus:subagent.done>").next())
...
fn subagent_completion_payload_carries_existing_sentinel_format() {
    ...
    // `<hakus:subagent.done>` sentinel on line 2. This test pins the
    ...
    assert!(
        !first.starts_with("<hakus:subagent.done>"),
        "summary should not be the sentinel itself"
    );
    assert!(
        second.starts_with("<hakus:subagent.done>"),
        "second line is the sentinel"
    );
    assert!(second.ends_with("</hakus:subagent.done>"));
    assert!(
...
fn write_json_atomic_survives_concurrent_writers() {
    ...
    let path = Arc::new(base.join(".hakus").join("subagents").join("state.json"));
    let mut handles = Vec::new();
    for i in 0..16 {
...
fn coordination_process_lock_rejects_second_process() {
    const ROLE_ENV: &str = "HAKUS_TEST_COORDINATION_LOCK_ROLE";
    const WORKSPACE_ENV: &str = "HAKUS_TEST_COORDINATION_LOCK_WORKSPACE";
    const TEST_NAME: &str =
...
fn an_exact_member_with_tools_false_gets_no_model_tools_at_all() {
    ...
    let authority = crate::fleet::exact::ChildAuthority::clamp(
        hakus_workflow::PermissionCeiling::ROUTER,
        hakus_workflow::PermissionCeiling::preset("full").expect("preset"),
    );
...
async fn an_exact_member_without_a_network_tool_really_loses_the_network_surface() {
    let tmp = tempdir().expect("tempdir");
    let authority = crate::fleet::exact::ChildAuthority::clamp(
        hakus_workflow::PermissionCeiling::preset("read_write").expect("preset"),
        hakus_workflow::PermissionCeiling::preset("full").expect("preset"),
    );
...
async fn a_read_only_inspection_member_gets_only_bounded_web_search() {
    ...
    let authority = crate::fleet::exact::ChildAuthority::clamp_for_role(
        "scout",
        hakus_workflow::PermissionCeiling::preset("read_only").expect("preset"),
        hakus_workflow::PermissionCeiling::preset("full").expect("preset"),
    );
...
fn a_shell_capable_read_only_member_keeps_test_selection_arguments() {
    for (name, input) in [
        (
            "Run",
            json!({"action": "tests", "args": "-p hakus-tui"}),
        ),
        (
            "Run",
...
async fn a_parent_read_only_session_narrows_a_full_exact_member_in_the_child_registry() {
    let tmp = tempdir().expect("tempdir");
    let session = hakus_workflow::PermissionCeiling {
        write: false,
        network_tool: false,
        shell: hakus_workflow::ShellCeiling::ReadOnly,
        delegation_depth: 0,
        tools: true,
    };
    let authority = crate::fleet::exact::ChildAuthority::clamp(
...
fn the_session_ceiling_reflects_the_live_parent_posture() {
    ...
    assert_ne!(
        ...
        hakus_workflow::ShellCeiling::Full
    );
}
...
fn read_only_with_shell_registry() -> (tempfile::TempDir, SubAgentToolRegistry) {
    ...
    let authority = crate::fleet::exact::ChildAuthority::clamp_for_role(
        "verifier",
        hakus_workflow::PermissionCeiling::preset("verifier").expect("preset"),
        hakus_workflow::PermissionCeiling::preset("full").expect("preset"),
    );
    assert!(
...
fn a_verifier_runs_bounded_test_selections_and_nothing_else_at_dispatch() {
    ...
    for (name, input) in [
        ...
        (
            "Run",
            json!({"action": "tests", "args": "-p hakus-tui exact_fleet"}),
        ),
...
fn the_authority_fingerprint_distinguishes_every_envelope_it_names() {
    let session = hakus_workflow::PermissionCeiling::preset("full").expect("preset");
    let fingerprint = |preset: &str, role: &str| {
        crate::fleet::exact::ChildAuthority::clamp_for_role(
            role,
            hakus_workflow::PermissionCeiling::preset(preset).expect("preset"),
            session,
        )
        .fingerprint()
    };
...
fn the_launched_authority_is_the_one_the_spawn_boundary_accepts() {
    let authority = crate::fleet::exact::ChildAuthority::clamp_for_role(
        "auditor",
        hakus_workflow::PermissionCeiling::preset("analyst").expect("preset"),
        hakus_workflow::PermissionCeiling::preset("full").expect("preset"),
    );
...
/// same-process handover (not "another Hakus process"), and must
