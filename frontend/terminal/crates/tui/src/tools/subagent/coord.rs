...
/// `<hakus:subagent.done>` sentinels that start a fresh turn (#4097).
...
fn wait_all_payload(
    ...
    let note = if timed_out {
        "Timed out with children still running. Do not poll — wait again (until=all), or end your turn; results arrive as <hakus:subagent.done> sentinels."
    } else if settled.is_empty() {
        "No sub-agents were running; nothing to join."
    } else {
        "Every watched child has settled. Full results arrive as <hakus:subagent.done> sentinels — synthesize from those."
    };
    let payload = json!({
        "action": "wait",
        "until": "all",
