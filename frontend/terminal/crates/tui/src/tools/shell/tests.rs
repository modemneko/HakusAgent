...
#[cfg(unix)]
const SHELL_DESCENDANT_HELPER_ENV: &str = "HAKUS_SHELL_DESCENDANT_HELPER";
#[cfg(unix)]
const SHELL_DESCENDANT_PID_FILE_ENV: &str = "HAKUS_SHELL_DESCENDANT_PID_FILE";
...
fn exec_shell_parallel_flags_are_input_aware() {
    ...
    assert!(tool.starts_detached_for(&json!({
        "command": "cargo test -p hakus-tui --bins",
        "tty": true
    })));
    assert!(!tool.starts_detached_for(&json!({
        "command": "cargo check --workspace"
...
#[cfg(unix)]
const SHELL_SIGTERM_HELPER_ENV: &str = "HAKUS_SHELL_SIGTERM_HELPER";
#[cfg(unix)]
const SHELL_ESCAPE_HELPER_ENV: &str = "HAKUS_SHELL_ESCAPE_HELPER";
#[cfg(unix)]
const SHELL_ESCAPED_GRANDCHILD_ENV: &str = "HAKUS_SHELL_ESCAPED_GRANDCHILD";
...
async fn persistent_service_requires_explicit_headless_exec_authority() {
    ...
    assert!(error.to_string().contains("real headless `hakus exec`"));
}
...
async fn committed_persistent_service_survives_manager_drop_and_reports_identity() {
    ...
    assert!(
        marker.exists(),
        "released service must survive Hakus manager teardown"
    );
}
