...
fn gap_fingerprint(gaps: &[String]) -> Option<String> {
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"hakus-goal-gaps-v1\0");
...
impl GoalSnapshot {
    ...
    #[must_use]
    pub fn from_thread_goal(goal: &hakus_protocol::ThreadGoal) -> Self {
...
pub fn thread_goal_status_projection(
    status: hakus_protocol::ThreadGoalStatus,
) -> (GoalStatus, Option<GoalPauseReason>) {
    match status {
        hakus_protocol::ThreadGoalStatus::Active => (GoalStatus::Active, None),
        hakus_protocol::ThreadGoalStatus::Paused => {
            (GoalStatus::Paused, Some(GoalPauseReason::User))
        }
        hakus_protocol::ThreadGoalStatus::Complete => (GoalStatus::Complete, None),
        hakus_protocol::ThreadGoalStatus::Blocked => (GoalStatus::Blocked, None),
        hakus_protocol::ThreadGoalStatus::UsageLimited => {
            (GoalStatus::Paused, Some(GoalPauseReason::UsageLimit))
        }
        hakus_protocol::ThreadGoalStatus::BudgetLimited => {
...
mod tests {
        ...
        let completed = update
            .execute(
                json!({
                    ...
                    "verification": {
                        "status": "passed",
                        "check": "cargo test -p hakus-tui goal_loop",
    ...
    fn protocol_thread_goal_converts_to_runtime_snapshot() {
        let snapshot = GoalSnapshot::from_thread_goal(&hakus_protocol::ThreadGoal {
