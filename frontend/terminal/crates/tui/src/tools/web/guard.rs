...
mod tests {
    ...
    fn run_ambient_proxy_probe(kind: AmbientProxyKind, guarded: bool) -> (bool, bool) {
        ...
        command
            .env("HAKUS_PROXY_PROBE_CHILD", "1")
            .env(
                "HAKUS_PROXY_PROBE_GUARDED",
                if guarded { "1" } else { "0" },
            )
            .env("HAKUS_PROXY_PROBE_TARGET_PORT", target_port.to_string());
        match kind {
            AmbientProxyKind::Http => {
                let proxy = format!("http://127.0.0.1:{proxy_port}");
                command
                    .env("HAKUS_PROXY_PROBE_SCHEME", "http")
            ...
            AmbientProxyKind::HttpsConnect => {
                let proxy = format!("http://127.0.0.1:{proxy_port}");
                command
                    .env("HAKUS_PROXY_PROBE_SCHEME", "https")
    ...
    async fn guarded_transport_proxy_probe_child() {
        if std::env::var_os("HAKUS_PROXY_PROBE_CHILD").is_none() {
            return;
        }
        let guarded = std::env::var("HAKUS_PROXY_PROBE_GUARDED").as_deref() == Ok("1");
        let scheme = std::env::var("HAKUS_PROXY_PROBE_SCHEME").expect("probe scheme");
        let target_port = std::env::var("HAKUS_PROXY_PROBE_TARGET_PORT")
    ...
    async fn unresolved_hostname_is_rejected_before_request() {
        let url =
            reqwest::Url::parse("https://hakuscli-unresolvable-fetch-target.invalid/resource")
                .unwrap();
        let err = validate_fetch_target(&url, &ctx(), "fetch_url")
            .await
    ...
    async fn web_run_tool_label_is_used_in_dns_error() {
        let url =
            reqwest::Url::parse("https://hakuscli-unresolvable-web-run-target.invalid/resource")
                .unwrap();
