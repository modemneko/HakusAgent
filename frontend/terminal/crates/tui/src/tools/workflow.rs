...
//!
//! The JS VM stays in `hakus-workflow-js`; this module supplies the TUI
...
/// stream persists per-event in `.hakus/workflow-runs.jsonl` (#2974).
...
struct WorkflowTaskStartedEvent {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    fleet_receipt: Option<hakus_workflow::FleetTaskReceipt>,
}
...
impl WorkflowRunRecord {
    ...
    /// available in `.hakus/workflow-runs.jsonl`.
...
impl ToolSpec for WorkflowTool {
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        hakus_telemetry::session_counters().bump(hakus_telemetry::Counter::WorkflowRun);
        match action {
...
fn bind_exact_fleet_task_request(
    ...
    session: hakus_workflow::PermissionCeiling,
    request: &mut TaskRequest,
...
fn orphaned_fleet_receipt_line(
    receipt: &hakus_workflow::FleetTaskReceipt,
    error: &str,
) -> String {
    format!(
...
async fn route_admitted_exact_task(
    ...
    binding: &crate::fleet::exact::ExactMemberBinding,
    request: &mut TaskRequest,
) -> Result<hakus_workflow::FleetTaskReceipt, DriverError> {
    let fleet = operation.snapshot().fleet().qualified();
...
/// Persist a durable per-run report under `.hakus/reports/<run_id>.md`
...
fn write_run_report_artifact(workspace: &Path, record: &WorkflowRunRecord) {
    ...
    let dir = workspace.join(".hakus").join("reports");
...
-> &'static Mutex<HashMap<PathBuf, hakus_config::WorkflowConfigToml>> {
    static STORE: OnceLock<Mutex<HashMap<PathBuf, hakus_config::WorkflowConfigToml>>> =
        OnceLock::new();
...
pub(crate) fn set_session_workflow_config(
    workspace: &Path,
    config: hakus_config::WorkflowConfigToml,
) {
...
pub(crate) fn session_workflow_config(
    workspace: &Path,
) -> Option<hakus_config::WorkflowConfigToml> {
    session_workflow_config_store()
        .lock()
        .unwrap_or_else(|poison| poison.into_inner())
...
fn workflow_config_for(runtime: &SubAgentRuntime) -> hakus_config::WorkflowConfigToml {
...
///
/// Full detail remains available in `.hakus/workflow-runs.jsonl`; every
...
fn read_workflow_source_path(
    ...
    if !context.trust_mode {
        ...
        // alongside the workspace: `~/.hakus/workflows/*.workflow.js`
        ...
        let home_store = crate::config::effective_home_dir()
            .map(|home| home.join(".hakus").join("workflows"))
        ...
        if !canonical.starts_with(&workspace) && !inside_home_store {
            return Err(ToolError::permission_denied(format!(
                "workflow source_path must stay inside the workspace or ~/.hakus/workflows: {}",
                canonical.display()
            )));
        }
    }
...
fn leaf_task_options_expression(
    ...
    let write_roots = if spec.mode == TaskMode::ReadWrite {
        spec.file_scope
            .iter()
            .map(|scope| hakus_workflow::normalize_file_scope_root(scope))
            .collect::<Vec<_>>()
    } else {
        Vec::new()
    };
...
fn validate_leaf_runtime_contract(spec: &LeafSpec) -> Result<(), ToolError> {
    ...
    for scope in &spec.file_scope {
        let normalized = hakus_workflow::normalize_file_scope_root(scope);
...
fn is_write_or_shell_tool(tool: &str) -> bool {
    ...
    // that drifted from `elevation.rs`'s — see `hakus_workflow::is_write_tool`.
    hakus_workflow::is_write_tool(tool) || hakus_workflow::is_shell_tool(tool)
}
...
mod journal {
    ...
    pub(super) const HAKUS_DIR: &str = ".hakus";
    ...
    impl WorkflowRunJournal {
        fn open(workspace: &Path) -> Self {
            let dir = workspace.join(HAKUS_DIR);
            if let Err(err) = std::fs::create_dir_all(&dir) {
                warn!(
...
/// existing run journal. Never creates `.hakus/` or the ledger.
...
mod tests {
    ...
    use hakus_workflow::{IsolationMode, leaf_is_write_capable};
    use std::sync::atomic::{AtomicUsize, Ordering};

    #[test]
    fn settled_runs_leave_a_report_artifact_under_hakus_reports() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let mut record = WorkflowRunRecord::new(
    ...
    fn running_runs_write_no_report_artifact() {
        ...
        write_run_report_artifact(tmp.path(), &record);
        assert!(
            !tmp.path().join(".hakus").join("reports").exists(),
    ...
    fn source_path_accepts_the_home_workflow_store_and_rejects_elsewhere() {
        ...
        let store = home.join(".hakus").join("workflows");
        ...
        assert!(
            err.to_string()
                .contains("workspace or ~/.hakus/workflows"),
            "{err}"
        );
    }
...
"#;
    ...
    fn exact_session() -> hakus_workflow::PermissionCeiling {
    ...
    fn exact_workflow_with(
        ...
        let document = hakus_workflow::FleetDocument::parse(text).expect("exact fleet parses");
        crate::fleet::exact::ExactFleetWorkflow::for_tests(
            &document,
            hakus_workflow::QualifiedFleetId {
    ...
    fn workflow_tool_honors_the_session_workflow_config() {
        ...
        let config = crate::config::Config {
            workflow: Some(hakus_config::WorkflowConfigToml {
    ...
    async fn a_role_keyed_gate_still_fires_when_the_member_id_differs() {
        ...
        let gates = vec![GateSpec {
            ...
            on_fail: hakus_workflow::GateOnFail::Block,
...
"#;
    ...
    async fn one_reasoning_router_profile_serves_two_fleets() {
        ...
        let second = {
            ...
            let document =
                hakus_workflow::FleetDocument::parse(&text).expect("second fleet parses");
            crate::fleet::exact::ExactFleetWorkflow::for_tests(
                &document,
                hakus_workflow::QualifiedFleetId {
    ...
    async fn editing_the_fleet_file_after_start_does_not_move_a_running_route() {
        ...
        let roots = vec![hakus_workflow::FleetSearchRoot::new(
            "workspace",
            tmp.path(),
        )];
        let (document, id) =
            hakus_workflow::FleetDocument::load_by_name("glm-pair", &roots).expect("load");
        ...
        let (reloaded, reloaded_id) =
            hakus_workflow::FleetDocument::load_by_name("glm-pair", &roots).expect("reload");
...
"#;
    ...
    async fn role_only_leaf_omits_type_and_resolves_through_named_fleet() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let _home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", tmp.path());
...
"#,
    ...
    async fn workflow_status_survives_tool_rebuild_via_journal() {
        ...
        let journal_path = tmp.path().join(".hakus/workflow-runs.jsonl");
        assert!(
            journal_path.exists(),
    ...
    async fn stopship_acceptance_fixture_emits_role_gate_and_terminal_receipts() {
        ...
        let compiled =
            hakus_workflow::compile_javascript_workflow("stopship.workflow.js", &source)
                .expect("compile stopship acceptance fixture");
        let hakus_workflow::WorkflowNode::Sequence(sequence) = &compiled.nodes[0] else {
            panic!("stopship fixture should be one ordered role chain");
        };
        for (index, node) in sequence.children.iter().enumerate() {
            let hakus_workflow::WorkflowNode::Leaf(leaf) = node else {
