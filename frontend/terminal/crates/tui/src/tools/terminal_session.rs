...
fn durable_path(name: &str, workspace: &Path) -> Result<PathBuf, String> {
    ...
    let digest = hasher
        ...
        .collect::<String>();
    #[cfg(test)]
    let state_dir = workspace.join(".hakus-test-terminal-sessions");
    #[cfg(not(test))]
    let state_dir = hakus_config::ensure_state_dir("terminal-sessions")
...
fn start_command(session: &mut TerminalSession, command: &str) -> Result<(), String> {
    ...
    let marker = format!("__HAKUS_TERM_{}__", Uuid::new_v4().simple());
