...
//! `~/.hakus/sessions/<session>/artifacts/`. The historical
//! `~/.hakus/tool_outputs/<sanitised-id>.txt` directory remains only for
...
/// startup to keep `~/.hakus/tool_outputs/` from growing without
...
/// Resolve `~/.hakus/tool_outputs/`. Returns `None` if the home
...
/// `~/.hakus/sessions/` succeeds in a trusted/auto session and is refused
...
/// content to a sibling file under `~/.hakus/tool_outputs/`,
...
/// user's real `~/.hakus/` directory. This uses explicit test hooks instead
    ...
    F: FnOnce() -> R,
{
    ...
    // artifacts.rs tests.
    let prior_spillover =
        set_test_spillover_root(Some(home.join(".hakus").join(SPILLOVER_DIR_NAME)));
    let prior_artifacts = crate::artifacts::set_test_artifact_sessions_root(Some(
        home.join(".hakus").join("sessions"),
    ));
...
mod tests {
    ...
    fn with_test_home_overrides_storage_roots_without_home_resolution() {
        ...
        with_test_home(tmp.path(), || {
            assert_eq!(
                ...
                Some(tmp.path().join(".hakus").join("tool_outputs").as_path())
            );
            assert_eq!(
    ...
    fn write_spillover_creates_directory_and_writes_file() {
        ...
        with_test_home(tmp.path(), || {
            ...
            // Directory landed under `<HOME>/.hakus/tool_outputs/`.
            ...
            let components: Vec<&str> = path
                ...
                .collect();
            assert!(
                components.contains(&".hakus") && components.contains(&"tool_outputs"),
                "spillover path missing expected `.hakus/tool_outputs/...` segments: {path:?}"
            );
        });
    }
    ...
    fn apply_spillover_with_artifact_writes_session_file_and_plain_preview() {
        ...
        with_test_home(tmp.path(), || {
            ...
            assert!(
                !tmp.path()
                    .join(".hakus/tool_outputs/call-big.txt")
                    .exists(),
    ...
    fn adaptive_evidence_passes_through_when_preview_budget_covers_output() {
        ...
        with_test_home(tmp.path(), || {
            ...
            assert!(
                !tmp.path()
                    .join(".hakus/sessions/session-covered/artifacts/art_call-covered.txt")
                    .exists()
            );
        });
    }
