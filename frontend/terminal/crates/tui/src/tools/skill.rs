...
impl ToolSpec for LoadSkillTool {
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        // can't find.
        let discovery_mode =
            SkillDiscoveryMode::from_hakus_only(context.skills_scan_hakus_only);
        ...
        let Some(skill) = registry.get(name) else {
            ...
            let hint = if available.is_empty() {
                let dirs: Vec<String> = context
                    ...
                    .collect();
                if dirs.is_empty() {
                    if context.skills_scan_hakus_only {
...
fn ensure_native_skill_file_present(skill: &Skill) -> Result<(), ToolError> {
    ...
    let message = format!(
        "Skill `{}` is registered at {} but that file no longer exists on disk, \
         ...
         came from (`skills_dir` in config.toml, `$HAKUS_HOME`, or the OS home) — \
...
mod tests {
    ...
    fn plugin_skill_load_fails_closed_when_reviewed_bundle_drifts() {
        ...
        let bundle = tmp.path().join(".hakus/plugins/demo");
    ...
    async fn execute_lists_available_skills_for_empty_star_and_list_names() {
        ...
        let _cw_home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", tmp.path().join("cw-home"));
    ...
    #[tokio::test]
    async fn execute_respects_hakus_only_skill_discovery() {
        let tmp = tempdir().unwrap();
        let workspace = tmp.path().to_path_buf();
        write_skill(
        ...
        let hakus_dir = workspace.join(".hakus").join("skills");
        write_skill(
            &hakus_dir,
            "hakus-only",
            "CodeWhale skill",
        ...
        let context = ToolContext::new(workspace).with_skills_config(hakus_dir, true);
        let tool = LoadSkillTool;

        let result = tool
            .execute(json!({"name": "hakus-only"}), &context)
            .await
        ...
        assert!(
            msg.contains("claude-only") && msg.contains("hakus-only"),
    ...
    async fn execute_loads_configured_external_skill_without_workspace_trust() {
        ...
        let home = tmp.path().join("home");
        let global_skills = home.join(".hakus/skills");
        ...
        // separately proves that ~/.hakus/skills enters the default catalog.
