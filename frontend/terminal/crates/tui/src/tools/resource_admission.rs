...
//!
//! Fleet and Workflow workers execute in separate Hakus processes, so an
...
//! `HAKUS_HOME`. The default of two permits is deliberately conservative
...
fn configured_heavy_command_limit() -> usize {
    std::env::var("HAKUS_HEAVY_COMMAND_LIMIT")
        .ok()
...
fn admission_root() -> PathBuf {
    ...
    if let Some(home) = hakus_paths::user_home() {
        return home.join(".hakus").join("resource-admission");
    }
    std::env::temp_dir().join("hakus-resource-admission")
}
...
mod tests {
    ...
    #[test]
    fn whitespace_hakus_home_uses_shared_user_home_for_admission_state() {
        ...
        let _hakus_home = crate::test_support::EnvVarGuard::set("HAKUS_HOME", " \t ");

        assert_eq!(
            admission_root(),
            home.join(".hakus").join("resource-admission")
        );
    }
    ...
    fn infers_only_expensive_rust_compilation_commands() {
        for command in [
            "cargo test -p hakus-tui shell::tests",
    ...
    async fn real_cargo_command_is_admitted_and_released() {
        if std::env::var_os("HAKUS_RESOURCE_ADMISSION_RUST_ACCEPTANCE").is_none() {
            tracing::info!(
                "skipping opt-in real-rust admission acceptance; \
                 set HAKUS_RESOURCE_ADMISSION_RUST_ACCEPTANCE=1 to run"
            );
            return;
        }
        let temp = tempfile::tempdir().expect("tempdir");
