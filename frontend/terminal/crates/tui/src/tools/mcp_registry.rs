...
// === Cached index types ===
//
// The cache file (`~/.hakus/mcp-index.json`) is the on-disk source of
    ...
    fn cache_path(&self) -> Result<PathBuf, ToolError> {
        match &self.cache_path_override {
            ...
            None => dirs::home_dir()
                ...
                .map(|h| h.join(".hakus").join("mcp-index.json")),
        }
    }
}
...
mod tests {
    ...
    ///   cargo test -p hakus-tui --bin hakus-tui --locked \
    ...
    async fn execute_writes_cache_file_and_returns_summary() {
        ...
        let cache_path = tmp_path.join(".hakus").join("mcp-index.json");
    ...
    async fn fresh_cache_serves_catalog_without_network() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let cache_dir = tmp.path().join(".hakus");
