...
use hakus_config::route::CapabilityState;
...
pub(crate) async fn execute_read_media(
    ...
    if crate::tools::file::is_hakus_credential_path(&file_path) {
        return Err(ToolError::permission_denied(
            "read_media cannot read Hakus configuration or credential-store files; use `hakus config list` or `hakus auth status` for safe inspection",
        ));
    }
...
mod tests {
    ...
    async fn read_media_credential_path_is_denied() {
        ...
        let _config_path = crate::test_support::EnvVarGuard::remove("HAKUS_CONFIG_PATH");
        ...
        assert!(
            err.to_string()
                .contains("cannot read Hakus configuration or credential-store"),
            "{}",
            err
        );
    }
    ...
    async fn read_media_credential_symlink_escape_is_denied() {
        ...
        let _hakus_home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", home_tmp.path());
        let _config_path = crate::test_support::EnvVarGuard::remove("HAKUS_CONFIG_PATH");
        let _legacy_config_path = crate::test_support::EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
