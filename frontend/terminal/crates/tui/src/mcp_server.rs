//! MCP server implementation for exposing Hakus tools over stdio.
        ...
        for entry in &self.exposed_tools {
            ...
            match entry.internal.as_str() {
                "deepseek" => {
                    tools.push(json!({
                        "name": "deepseek",
                        "description": "Send a prompt to Hakus and get a response. Creates a new conversation thread.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "prompt": {
                                    "type": "string",
                                    "description": "The user prompt to send to Hakus"
                                },
                                "model": {
                                    "type": "string",
                ...
                "deepseek-reply" => {
                    tools.push(json!({
                        "name": "deepseek-reply",
                        "description": "Continue an existing conversation thread with Hakus. Requires a thread_id from a previous deepseek call.",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "thread_id": {
...
fn model_client_init_error(error: impl std::fmt::Display) -> RpcError {
    RpcError {
        code: -32000,
        message: format!("Failed to create Hakus model client: {error}"),
    }
}
...
mod tests {
    ...
    #[test]
    fn model_failures_use_hakus_provider_neutral_language() {
        let init = model_client_init_error("missing credential");
        assert_eq!(init.code, -32000);
        assert_eq!(
            init.message,
            "Failed to create Hakus model client: missing credential"
        );
