...
/// tests to the developer's real `~/.hakus` state, which is exactly the
...
mod tests {
    ...
    #[test]
    fn ambient_hakus_home_is_not_a_test_seal() {
        let _lock = lock_test_env();
        let _ambient = EnvVarGuard::set("HAKUS_HOME", "/tmp/ambient-hakus-home");
        unregister_guarded_env_key("HAKUS_HOME");

        let sealed = guarded_environment_provides_state_paths();

        register_guarded_env_key("HAKUS_HOME");
    ...
    fn removing_home_variables_does_not_seal_a_missing_path() {
        ...
        assert_eq!(
            ...
            unsealed_test_state_root().join(hakus_config::CONFIG_FILE_NAME)
        );
    }
    ...
    fn lock_without_sealed_paths_does_not_use_developer_config() {
        ...
        assert!(
            ...
            "holding lock_test_env without an EnvVarGuard must not read ~/.hakus ({})",
            path.display()
        );
        assert_eq!(
            path,
    ...
    fn unguarded_state_writes_use_isolated_test_root() {
        const PROBE_ENV: &str = "HAKUS_TEST_STATE_ISOLATION_PROBE";
        const RECEIPT_ENV: &str = "HAKUS_TEST_STATE_ISOLATION_RECEIPT";
        ...
        if std::env::var_os(PROBE_ENV).is_some() {
            ...
            let direct_config_path =
                crate::config::save_workspace_trust(Path::new("/tmp/hakus-test-workspace"))
        ...
        let user_state = sentinel.path().join(".hakus");
    ...
    fn config_path_read_waits_for_foreign_env_redirect_to_restore() {
        ...
        let redirected = std::env::temp_dir().join(format!(
            "hakus-config-path-read-barrier-{}",
            std::process::id()
        ));
