...
pub struct PreparedToolImage {
    pub block: Option<hakus_tools::ToolResultContentBlock>,
    pub note: String,
}

/// Prepare one provider-neutral tool-result image using Hakus's existing
...
pub fn prepare_tool_image_bytes(bytes: &[u8], mime_type: &str) -> PreparedToolImage {
    ...
    PreparedToolImage {
        block: Some(hakus_tools::ToolResultContentBlock::Image {
...
pub(crate) fn bound_rich_tool_result(
    ...
    for block in rich.content_blocks.drain(..) {
        let hakus_tools::ToolResultContentBlock::Image { mime_type, data } = &block;
