...
//! per-process file under `~/.hakus/logs/tui-YYYY-MM-DD-PID.log`, and (on
...
//!      `~/.hakus/logs/tui-YYYY-MM-DD-PID.log` so `tracing::warn!` /
...
mod tests {
    ...
    #[test]
    fn log_directory_honors_hakus_home_as_hard_override() {
        let _lock = crate::test_support::lock_test_env();
        let tmp = tempfile::TempDir::new().unwrap();
        // SAFETY: serialised by lock_test_env.
        unsafe {
