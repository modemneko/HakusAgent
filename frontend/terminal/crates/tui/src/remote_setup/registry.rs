//! Table-driven registries for `hakus remote-setup`.
//!
...
//!   `hakus_config::provider` registry (see [`super::bundle::ProviderInfo`]).

/// Where a cloud target stores the runtime/provider secrets.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SecretStore {
    /// Secrets live in `/etc/hakus/*.env` files on the host.
    EnvFile,
...
impl SecretStore {
    ...
    pub fn label(self) -> &'static str {
        match self {
            SecretStore::EnvFile => "EnvFile (/etc/hakus/*.env)",
...
pub const TELEGRAM: BridgeSpec = BridgeSpec {
    ...
    service_unit: "hakus-telegram-bridge.service",
    ...
    install_dir: "/opt/hakus/telegram-bridge",
};
...
pub const FEISHU: BridgeSpec = BridgeSpec {
    ...
    service_unit: "hakus-feishu-bridge.service",
...
/// `/etc/hakus/*.env`, native+systemd install driven by a cloud-init
...
fn lighthouse_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    let restart_bridge = format!("hakus-{}-bridge", inputs.bridge_slug);
    vec![
        ProvisionStep::new(
...
fn azure_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    let provider_secret = format!("hakus-{}-key", inputs.provider_slug);
    vec![
        ProvisionStep::new(
...
fn digitalocean_plan(inputs: &DeployInputs) -> Vec<ProvisionStep> {
    ...
    vec![
        ...
        ProvisionStep::new(
            "On the Droplet: write /etc/hakus/*.env, install both systemd units, enable --now",
            "bash",
            &["scripts/tencent-lighthouse/install-services.sh"],
        ),
    ]
