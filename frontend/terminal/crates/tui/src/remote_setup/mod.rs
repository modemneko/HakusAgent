//! `hakus remote-setup` — guided generation of a remote-agent deploy bundle.
//!
...
//! [`registry::BRIDGES`], and the existing `hakus_config::provider` registry
...
/// Flags for `hakus remote-setup` (clap), per the RFC command surface.
...
pub struct RemoteSetupArgs {
    ...
    /// Bundle output directory (default `./hakus-deploy/<cloud>-<bridge>`).
...
pub fn run_remote_setup(args: RemoteSetupArgs) -> Result<()> {
    ...
    let out_dir = args.out.clone().unwrap_or_else(|| {
        PathBuf::from("hakus-deploy").join(format!("{}-{}", cloud.slug, bridge.slug))
    });
...
fn print_header() {
    ...
    println!("{}", "Hakus Remote Setup".truecolor(r, g, b).bold());
    println!("{}", "======================".truecolor(r, g, b));
    println!("Generate a deploy bundle for a remote Hakus agent (cloud + chat bridge).");
}
...
fn resolve_provider(args: &RemoteSetupArgs) -> Result<ProviderInfo> {
    if let Some(slug) = &args.provider {
        return ProviderInfo::from_slug(slug).ok_or_else(|| {
            anyhow::anyhow!(
                ...
                hakus_config::ProviderKind::names_hint()
            )
        });
    }
    if args.non_interactive {
    ...
    let providers: Vec<ProviderInfo> = hakus_config::ProviderKind::all()
        .iter()
