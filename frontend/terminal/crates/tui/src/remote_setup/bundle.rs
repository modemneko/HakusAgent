...
//!
//! Env files lead with `HAKUS_*` keys; `DEEPSEEK_*` are documented as legacy
...
fn render_runtime_env(i: &BundleInputs) -> String {
    ...
    out.push_str("# HAKUS_* keys are canonical. DEEPSEEK_* are read as legacy aliases.\n\n");
...
# Legacy /etc/deepseek is loaded first for old installs; /etc/hakus wins.\n\
EnvironmentFile=-/etc/deepseek/runtime.env\n\
EnvironmentFile=-/etc/hakus/runtime.env\n\
ExecStart=/bin/sh -lc 'exec /home/hakus/.cargo/bin/hakus serve --http --host 127.0.0.1 --port \"${{HAKUS_RUNTIME_PORT:-${{DEEPSEEK_RUNTIME_PORT:-{port}}}}}\" --workers \"${{HAKUS_RUNTIME_WORKERS:-${{DEEPSEEK_RUNTIME_WORKERS:-{workers}}}}}\" --auth-token \"${{HAKUS_RUNTIME_TOKEN:-${{DEEPSEEK_RUNTIME_TOKEN}}}}\"'\n\
Restart=on-failure\n\
RestartSec=5\n\
NoNewPrivileges=true\n\
PrivateTmp=true\n\
ProtectSystem=full\n\
ReadWritePaths=/home/hakus/.hakus /home/hakus/.deepseek {workspace}\n\n\
[Install]\n\
