//! Bundle rendering for `hakus remote-setup`.
//!
...
//!
//! Env files lead with `HAKUS_*` keys. Provider-specific variables such as
//! `DEEPSEEK_API_KEY` are emitted only when they are the selected provider's
//! canonical variables. The provider lives entirely in `runtime.env` (the
//! bridge is pure transport and never needs to know which provider is behind
//! the runtime).
...
/// `hakus_config::provider` registry (the single source of truth).
#[derive(Debug, Clone)]
...
impl ProviderInfo {
    ...
    pub fn from_slug(slug: &str) -> Option<Self> {
        let kind = hakus_config::ProviderKind::parse(slug)?;
        let p = hakus_config::provider::provider_for_kind(kind);
        let key_var = p.env_vars().first().copied().unwrap_or("HAKUS_API_KEY");
        Some(Self {
            slug: p.id().to_string(),
...
impl BundleInputs {
    ...
    pub fn deploy_inputs(&self) -> DeployInputs {
        DeployInputs {
            ...
            instance_name: "hakus-remote".to_string(),
            image: "ghcr.io/hmbown/hakus:latest".to_string(),
        }
    }
}
...
pub fn render_bundle(inputs: &BundleInputs) -> Vec<RenderedFile> {
    vec![
        ...
        RenderedFile {
            relative_path: "hakus-runtime.service".to_string(),
...
fn render_runtime_env(i: &BundleInputs) -> String {
    ...
    out.push_str("# Hakus runtime config — generated by `hakus remote-setup`.\n");
    out.push_str("# Provider configuration lives here; the bridge is pure transport.\n");
    out.push_str("# HAKUS_* keys are canonical runtime controls.\n\n");

    out.push_str(&format!("HAKUS_PROVIDER={}\n", i.provider.slug));
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        "HAKUS_MODEL={}   # provider default is {}\n",
    ...
    out.push_str(&format!("HAKUS_RUNTIME_TOKEN={}\n", i.runtime_token));
    out.push_str(&format!("HAKUS_RUNTIME_PORT={}\n", i.port));
    out.push_str(&format!("HAKUS_RUNTIME_WORKERS={}\n", i.workers));
...
fn render_bridge_env(i: &BundleInputs) -> String {
    let mut out = String::new();
    out.push_str(&format!(
        "# Hakus {} bridge config — generated by `hakus remote-setup`.\n",
        i.bridge.display
    ));
    out.push_str("# Transport only: forwards chat <-> the local runtime. No provider keys here.\n");
    out.push_str("# HAKUS_* keys are canonical for the bridge transport.\n\n");
    ...
    out.push_str(&format!("HAKUS_RUNTIME_URL={DEFAULT_RUNTIME_URL}\n"));
    out.push_str("# Must match HAKUS_RUNTIME_TOKEN in runtime.env.\n");
    out.push_str(&format!("HAKUS_RUNTIME_TOKEN={}\n", i.runtime_token));
    out.push_str(&format!("HAKUS_WORKSPACE={}\n", i.workspace));
    out.push_str(&format!("HAKUS_MODEL={}\n", i.model));
    out.push_str("HAKUS_MODE=agent\n");
    out.push_str("HAKUS_ALLOW_SHELL=true\n");
    out.push_str("HAKUS_TRUST_MODE=false\n");
    out.push_str("HAKUS_AUTO_APPROVE=false\n\n");
    ...
    out.push_str("\n# --- bridge tuning ---\n");
    out.push_str(&format!(
        "{}_THREAD_MAP_PATH=/var/lib/hakus-{}-bridge/thread-map.json\n",
    ...
    out.push_str("HAKUS_TURN_TIMEOUT_MS=900000\n");
    out
}
...
EnvironmentFile=-/etc/hakus/runtime.env\n\
ExecStart=/bin/sh -lc 'exec /home/hakus/.cargo/bin/hakus serve --http --host 127.0.0.1 --port \"${{HAKUS_RUNTIME_PORT:-{port}}}\" --workers \"${{HAKUS_RUNTIME_WORKERS:-{workers}}}\" --auth-token \"${{HAKUS_RUNTIME_TOKEN}}\"'\n\
Restart=on-failure\n\
RestartSec=5\n\
NoNewPrivileges=true\n\
PrivateTmp=true\n\
ProtectSystem=full\n\
ReadWritePaths=/home/hakus/.hakus {workspace}\n\n\
[Install]\n\
...
fn render_bridge_unit(i: &BundleInputs) -> String {
    format!(
        "[Unit]\n\
Description=Hakus {display} Phone Bridge\n\
Wants=network-online.target hakus-runtime.service\n\
After=network-online.target hakus-runtime.service\n\n\
[Service]\n\
Type=simple\n\
User=hakus\n\
Group=hakus\n\
WorkingDirectory={install_dir}\n\
EnvironmentFile=-/etc/hakus/{slug}-bridge.env\n\
...
PrivateTmp=true\n\
ProtectSystem=full\n\
ReadWritePaths=/var/lib/hakus-{slug}-bridge\n\n\
[Install]\n\
WantedBy=multi-user.target\n",
...
fn render_runbook(i: &BundleInputs) -> String {
    ...
    out.push_str(&format!(
        "# Hakus remote-setup runbook — {} + {}\n\n",
        i.cloud.display, i.bridge.display
    ));
    out.push_str("Generated by `hakus remote-setup` (generate-only). Nothing was run on\n");
    ...
    out.push_str("| `hakus-runtime.service` | systemd unit for the runtime API. |\n");
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        "- A random `HAKUS_RUNTIME_TOKEN` was generated (`{}`). It is identical in\n",
        i.runtime_token
    ));
    ...
    out.push_str("```sh\nsudo install -d -m 750 /etc/hakus\n");
    out.push_str(&format!(
        "sudo install -m 600 runtime.env /etc/hakus/runtime.env\n\
sudo install -m 600 {slug}.env /etc/hakus/{slug}-bridge.env\n\
sudo install -m 644 hakus-runtime.service /etc/systemd/system/hakus-runtime.service\n\
sudo install -m 644 {unit} /etc/systemd/system/{unit}\n\
sudo systemctl daemon-reload\n\
sudo systemctl enable --now hakus-runtime {unit}\n```\n\n",
        slug = i.bridge.slug,
    ...
    out.push_str("```sh\nsudo systemctl status hakus-runtime --no-pager\n");
    out.push_str(&format!(
...
mod tests {
    ...
    fn bundle_renders_expected_file_set() {
        ...
        assert!(names.contains(&"hakus-runtime.service"));
        assert!(names.contains(&"hakus-feishu-bridge.service"));
    ...
    fn env_files_lead_with_hakus_keys() {
        ...
        let runtime = &files
            ...
            .unwrap()
            .contents;
        assert!(runtime.contains("HAKUS_PROVIDER=deepseek"));
        assert!(runtime.contains("HAKUS_RUNTIME_TOKEN="));
        assert!(runtime.contains("HAKUS_RUNTIME_PORT="));
        // Provider key var present; DEEPSEEK_API_KEY is DeepSeek's canonical
        // provider variable, not a storage-path compatibility alias.
        assert!(runtime.contains("DEEPSEEK_API_KEY="));
        assert!(!runtime.to_lowercase().contains("legacy alias"));
        assert!(!runtime.contains("DEEPSEEK_RUNTIME_"));
        ...
        let bridge = &files
            ...
            .unwrap()
            .contents;
        assert!(bridge.contains("HAKUS_RUNTIME_URL="));
    ...
    fn every_cloud_bridge_provider_triple_renders() {
        // Cover the matrix per the RFC §Tests; assert HAKUS_* + matching token
            ...
            for bridge in &[FEISHU, TELEGRAM] {
                for provider_slug in &["deepseek", "openai", "moonshot"] {
                    ...
                    let runtime = &files
                        ...
                        .unwrap()
                        .contents;
                    assert!(runtime.contains(&format!("HAKUS_PROVIDER={provider_slug}")));
                    let token_line = format!("HAKUS_RUNTIME_TOKEN={}", inputs.runtime_token);
    ...
    #[test]
    fn systemd_units_reference_hakus_paths() {
        let inputs = sample_inputs(&LIGHTHOUSE, &FEISHU, "deepseek");
        let files = render_bundle(&inputs);
        let unit = &files
            .iter()
            .find(|f| f.relative_path == "hakus-runtime.service")
            .unwrap()
            .contents;
        assert!(unit.contains("/etc/hakus/runtime.env"));
        assert!(unit.contains("HAKUS_RUNTIME_TOKEN"));
        assert!(!unit.contains("/etc/deepseek"));
        assert!(!unit.contains(".deepseek"));

        let bridge_unit = &files
            .iter()
            .find(|f| f.relative_path == "hakus-feishu-bridge.service")
            .unwrap()
            .contents;
        assert!(bridge_unit.contains("/etc/hakus/feishu-bridge.env"));
        assert!(!bridge_unit.contains("/etc/deepseek"));
    }
}
