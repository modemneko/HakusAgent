//! Persistent background task manager for Hakus agent work.
//!
...
pub fn default_tasks_dir() -> PathBuf {
    ...
    hakus_paths::user_home()
        ...
        .unwrap_or_else(|| PathBuf::from(".hakus").join("tasks"))
}
...
mod tests {
                ...
                &serde_json::json!({
                    "task_updates": {
                        "gate": {
                            "id": "gate_test",
                            "gate": "test",
                            "command": "cargo test -p hakus-tui --lib",
                            "cwd": ".",
                            "exit_code": 0,
                            "status": "passed",
    ...
    #[test]
    fn default_tasks_dir_prefers_existing_hakus_tasks() {
        let temp_home = tempfile::tempdir().unwrap();
        let home = temp_home.path();
    ...
    #[test]
    fn default_tasks_dir_uses_hakus_tasks_for_new_installs() {
        let temp_home = tempfile::tempdir().unwrap();
        let home = temp_home.path();
    ...
    #[test]
    fn task_and_runtime_roots_honor_explicit_hakus_home() {
        ...
        let _tasks_override = EnvVarGuard::remove("HAKUS_TASKS_DIR");
        let _legacy_tasks_override = EnvVarGuard::remove("DEEPSEEK_TASKS_DIR");
        let _runtime_override = EnvVarGuard::remove("HAKUS_RUNTIME_DIR");
    ...
    #[cfg(unix)]
    #[test]
    fn non_unicode_hakus_home_is_preserved_by_task_and_runtime_roots() {
