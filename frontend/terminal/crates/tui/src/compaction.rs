...
//!
//! HakusAgent 对齐：对应 Python `hakus.context.ContextManager` 的上下文
//! 压缩职责。Python 用离散的 `CompressionLevel`（NONE/TRUNCATE/SUMMARIZE/
//! CIRCUIT_BREAK）分档；本实现保留上游更精细的 token-threshold 自动触发 +
//! 手动 `/compact <focus>` 机制（优点保留），档位概念由阈值与开关承载。
...
fn tool_args_preview(input: &serde_json::Value) -> String {
    let redacted = hakus_config::persistence::redact_json_secrets(input);
...
/// User-pinned facts from `/anchor` (`.hakus/anchors.md`). These are the
...
mod tests {
    ...
    fn tool_args_preview_redacts_sensitive_first_without_dropping_siblings() {
        ...
        assert_eq!(preview["api_key"], hakus_config::persistence::REDACTED);
    ...
    fn tool_args_preview_redacts_nested_sensitive_values_recursively() {
        ...
        assert_eq!(
            preview["meta"]["token"],
            hakus_config::persistence::REDACTED
        );
    ...
    fn tool_args_preview_redacts_complete_multi_word_secret_value() {
        ...
        assert_eq!(preview["password"], hakus_config::persistence::REDACTED);
