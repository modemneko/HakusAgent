...
pub(crate) fn plan_if_due(
    ...
    let store = match hakus_config::ConfigStore::load(config_path) {
        Ok(store) => store,
        Err(error) => {
...
fn plan_for_store_and_state(
    store: hakus_config::ConfigStore,
    ...
    let gate = NoticeGate {
        ...
        floor_in_force: hakus_config::telemetry_floor_in_force(),
    };
    if gate.may_ask() {
...
pub(crate) fn apply_decision(
    ...
    // The modal can remain open while another Hakus process records an
...
fn saved_preference_enabled_at(config: &crate::config::Config, setup_state_path: &Path) -> bool {
    ...
    match hakus_telemetry::load_setup_state_for_decision_at(setup_state_path) {
...
pub(crate) fn apply_persistent_preference(
    ...
    let telemetry_root = hakus_config::hakus_home()
        .ok()
        .map(|home| home.join(hakus_telemetry::TELEMETRY_DIR));
...
fn apply_persistent_preference_at(
    ...
    let wipe_result = match telemetry_root.as_deref().filter(|root| root.is_dir()) {
        Some(root) => hakus_telemetry::buffer::wipe(root),
        None => Ok(()),
    };
...
fn write_config_preference(config_path: Option<PathBuf>, enabled: bool) -> Result<()> {
    let mut store = hakus_config::ConfigStore::load(config_path)?;
    store
        .config
...
mod tests {
    ...
    fn corrupt_setup_state_is_never_replaced_with_telemetry_defaults() {
        ...
        let store = hakus_config::ConfigStore::load(Some(config_path)).expect("load config");
    ...
    fn settings_off_persists_both_registers_and_stops_the_armed_buffer() {
        ...
        std::fs::write(
            hakus_telemetry::buffer::buffer_path(&telemetry_root),
            "queued-event\n",
        )
        .expect("seed buffer");
        ...
        assert!(hakus_telemetry::buffer::tombstone_present(
            &telemetry_root
        ));
        assert_eq!(
            std::fs::read_to_string(hakus_telemetry::buffer::buffer_path(&telemetry_root))
    ...
    fn settings_on_is_saved_for_next_launch_without_clearing_the_tombstone() {
        ...
        assert!(
            hakus_telemetry::buffer::tombstone_present(&telemetry_root),
            "the already-running process stays off; the next launch clears this after a fresh permission check"
        );

        let store = hakus_config::ConfigStore::load(Some(config_path))
        ...
        assert!(
            hakus_telemetry::decide_in_home(
                Some(dir.path()),
                &resolved,
                &reloaded_state,
