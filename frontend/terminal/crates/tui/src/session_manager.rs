...
/// `hakus web` has an empty registry and is therefore never blocked, which
...
/// here as proof that no other Hakus process still owns the directory.
#[must_use]
...
fn live_session_conflict(session_id: &str) -> std::io::Error {
    std::io::Error::new(
        ...
        format!(
            "session '{session_id}' is open in an interactive Hakus session; \
...
/// stamped with the instance that created them, so a later Hakus
...
impl SessionManager {
    ...
    /// instance (an earlier or sibling Hakus process)?
    ///
    ...
    /// swallows — were never collected by anything. A real `~/.hakus`
    ...
    ///   That check is process-local; a second Hakus sharing `$HOME`
...
/// OSC 0 terminal title, `hakus sessions` stdout, and the picker, so the
...
mod tests {
    ...
    /// A real `~/.hakus/sessions` held 780 directories whose session had
        ...
        let messages = vec![
            Message {
                ...
                content: vec![ContentBlock::ToolUse {
                    ...
                    input: serde_json::json!({"command": "cargo test -p hakus-tui"}),
                    caller: None,
                    thought_signature: None,
                }],
            },
    ...
    fn default_manager_copies_legacy_sessions_when_primary_already_exists() {
        ...
        let _hakus_home = crate::test_support::EnvVarGuard::remove("HAKUS_HOME");

        let primary_sessions = home.join(".hakus").join("sessions");
    ...
    #[test]
    fn explicit_hakus_home_disables_legacy_session_copy() {
        let _lock = crate::test_support::lock_test_env();
        let tmp = tempdir().expect("tempdir");
        let home = tmp.path().join("home");
        let explicit_home = tmp.path().join("explicit-hakus");
        let _home = crate::test_support::EnvVarGuard::set("HOME", &home);
        let _hakus_home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", &explicit_home);
    ...
    #[cfg(unix)]
    #[test]
    fn non_unicode_hakus_home_is_still_an_explicit_session_boundary() {
