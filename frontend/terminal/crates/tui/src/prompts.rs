...
pub struct PromptSessionContext<'a> {
    ...
    /// `{model_id}`. Defaults to `"hakus"` when the caller doesn't supply one.
    pub model_id: &'a str,
    ...
    /// Restrict skill discovery to Hakus-owned roots plus explicit
    /// `skills_dir` configuration.
    pub skills_scan_hakus_only: bool,
...
pub(crate) fn load_user_constitution_block() -> Option<String> {
    ...
    let path = match hakus_config::UserConstitution::path() {
        Ok(path) => path,
        Err(err) => {
            tracing::warn!(
                target: "prompts",
    ...
    match hakus_config::UserConstitution::load_from(&path) {
        hakus_config::UserConstitutionLoad::Loaded(constitution) => {
            constitution.render_block(None)
        }
        hakus_config::UserConstitutionLoad::Missing
        | hakus_config::UserConstitutionLoad::Empty => None,
        hakus_config::UserConstitutionLoad::Invalid(err) => {
            tracing::warn!(
                target: "prompts",
        ...
        hakus_config::UserConstitutionLoad::Unreadable(err) => {
            tracing::warn!(
                target: "prompts",
...
fn user_constitution_disabled_by_setup_state() -> bool {
    match hakus_config::SetupState::load() {
        Ok(Some(state)) => matches!(
            ...
            hakus_config::ConstitutionChoice::Bundled
                ...
                | hakus_config::ConstitutionChoice::ExpertOverride
        ),
        Ok(None) => false,
        Err(err) => {
            tracing::warn!(
...
/// Embedder hook for replacing Hakus's byte-stable base/personality prompt
/// segment.
...
// optional file in the Hakus config directory. This lets users repurpose
...
// also set an explicit opt-in flag (`HAKUS_ALLOW_BASE_PROMPT_OVERRIDE`).
...
pub const BASE_PROMPT_OVERRIDE_OPT_IN_ENV: &str = "HAKUS_ALLOW_BASE_PROMPT_OVERRIDE";
...
/// Resolve the Hakus config directory and load any prompt overrides found
...
impl BasePromptOrigin {
    ...
    pub(crate) fn label(self) -> &'static str {
        match self {
            Self::Bundled => "bundled in this hakus-tui build (BASE_PROMPT, compiled in)",
...
你正在 hakus 中运行。无论任务上下文（代码、错误日志、文件名）\
...
hakus を実行しています。タスクコンテキスト（コード、エラーログ、\
...
Você está rodando dentro do hakus. Escreva tanto \
...
Bạn đang chạy trong hakus. Cho dù ngữ cảnh tác vụ (mã nguồn, nhật ký lỗi, tên tệp) \
...
Hakus's constitution governs your behavior. Ground truth underlies the
...
pub(crate) fn system_prompt_for_mode_with_context_skills_session_and_approval_for_host(
    ...
    // conventions show up in the catalogue. Users can opt into a Hakus-only
    // scan with `[skills] scan_hakus_only = true`. When an explicit
    ...
    let skill_discovery_mode = crate::skills::SkillDiscoveryMode::from_hakus_only(
        session_context.skills_scan_hakus_only,
    );
    ...
    // hard caps — unified with `hakus_core::fragments`. This covers
    ...
    if let Some(fragment) =
        hakus_core::fragments::load_additional_project_instruction_fragment(workspace)
    {
...
mod tests {
    ...
    const HANDOFF_BLOCK_MARKER: &str = "left a relay artifact at `.hakus/handoff.md`";
    ...
    fn static_prompt_composer_receives_context_and_replaces_layers() {
        ...
        let composer: Box<StaticPromptComposer> = Box::new(|ctx| {
            ...
            // The 0.9.0 core is model-agnostic ("You are Hakus") and
            // folds tone in — no per-model id line, no separate personality
            // section in default_layers.
            assert!(ctx.default_layers.contains("You are Hakus"));
            assert!(
                ctx.default_layers
    ...
    fn yolo_mode_uses_the_shared_completion_contract() {
        // `hakus exec --auto` runs AppMode::Yolo; the verify-then-stop
    ...
    fn compose_prompt_for_v4_model_stays_model_fact_free() {
        ...
        assert!(prompt.contains("You are Hakus"));
    ...
    fn compose_prompt_is_model_agnostic_in_preamble() {
        ...
        let kimi =
            compose_prompt_with_approval_model_and_shell(Personality::Calm, "moonshotai/kimi-k2.6");
        assert!(
            flash.contains("You are Hakus"),
            "0.9.0 preamble must open with the model-agnostic Hakus stance"
        );
        assert!(
    ...
    fn authority_recap_appears_in_full_prompt() {
        ...
        assert!(
            text.contains("Hakus's constitution governs your behavior"),
    ...
    fn constitution_has_no_separate_personality_tier() {
        ...
        let prompt = compose_prompt_with_approval_model_and_shell(Personality::Calm, "hakus");
        assert!(
        ...
        // Verify the preamble still carries the Hakus identity.
        assert!(prompt.contains("You are Hakus"));
    ...
    fn render_environment_block_keeps_actionable_host_facts_without_version() {
        ...
        assert!(!block.contains("- hakus_version:"));
    ...
    fn environment_block_is_inserted_into_system_prompt() {
        ...
        assert!(!prompt.contains("- hakus_version:"));
    ...
    fn user_global_constitution_block_is_injected_separately() {
        ...
        let hakus_home = tmp.path().join("hakus-home");
        std::fs::create_dir_all(&hakus_home).expect("hakus home");
        let _hakus_home =
            crate::test_support::EnvVarGuard::set("HAKUS_HOME", hakus_home.as_os_str());

        let constitution = hakus_config::UserConstitution {
            about: Some("Maintains Hakus release lanes.".to_string()),
            ...
            ..hakus_config::UserConstitution::default()
        };
        constitution
            .save_to(
                &hakus_home
                    .join(hakus_config::user_constitution::USER_CONSTITUTION_FILE_NAME),
            )
        ...
        assert!(prompt.contains("Maintains Hakus release lanes."));
        assert!(prompt.contains("Prefer live verification before claims."));
        assert!(
            !prompt.contains(&hakus_home.display().to_string()),
    ...
    fn bundled_choice_disables_user_global_constitution_block() {
        ...
        state
            .save_to(&hakus_home.join(hakus_config::setup_state::SETUP_STATE_FILE_NAME))
        ...
        assert!(!prompt.contains("<hakus_user_constitution"));
    ...
    fn invalid_user_global_constitution_is_skipped() {
        ...
        std::fs::create_dir_all(&hakus_home).expect("hakus home");
        std::fs::write(
            hakus_home.join(hakus_config::user_constitution::USER_CONSTITUTION_FILE_NAME),
            "{ not valid json",
        )
    ...
    fn constitution_md_carries_required_structure() {
        let md = BASE_PROMPT;
        assert!(md.contains("## Hakus"), "missing title");
    ...
    fn compose_prompt_deterministic_order() {
        let prompt = compose_prompt_with_approval_model_and_shell(Personality::Calm, "hakus");
        let base_pos = prompt.find("## Hakus").unwrap();
    ...
    fn personality_is_folded_into_constitution() {
        ...
        let calm = compose_prompt_with_approval_model_and_shell(Personality::Calm, "hakus");
        assert!(!calm.contains("## Personality:"));
        assert!(calm.contains("Take the work seriously. Don't take"));
        assert!(calm.contains("You are Hakus"));
    }
    ...
    fn compose_prompt_is_byte_stable_across_calls() {
        ...
        let a = compose_prompt_with_approval_model_and_shell(Personality::Calm, "hakus");
        let b = compose_prompt_with_approval_model_and_shell(Personality::Calm, "hakus");
