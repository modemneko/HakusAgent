...
//! `$DSH_HOME/.credentials.yaml`. Hakus may read `DEEPSEEK_API_KEY` from
//! that exact file only after `hakus auth external-consent`. The file is
...
pub(crate) fn deepseek_api_key_from_grant(
    ...
    if grant.source() != hakus_config::ExternalCredentialSource::DshCli {
        bail!(
