...
const PRODUCTION_CONTROL_PLANE: &str = "https://api.hakus.net/";
...
const APP_ORIGIN_HOST: &str = "app.hakus.net";
...
pub struct RemoteStart {
    ...
    /// always provide a private directory under the Hakus home.
...
/// shows no link. Links are validated against the Hakus app origin before
/// they are ever displayed or opened; the terminal never invents one.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct RemoteLinks {
    /// `https://app.hakus.net/session?run=<runId>` for the live run.
    pub run_url: Option<String>,
    /// `https://app.hakus.net/settings?section=workspaces` for this computer.
...
impl RemoteControlController {
    ...
    pub fn observe_engine_event(&mut self, event: &EngineEvent) {
        ...
        match event {
            ...
            EngineEvent::ToolCallFinished { id, result, .. } => {
            ...
            EngineEvent::TurnCompleted { usage, status, .. } => {
...
pub fn target_ref(workspace: &Path) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"hakus-remote-target:v2\0");
...
fn bounded_remote_error_message(error: &str) -> String {
    ...
    let redacted = hakus_config::persistence::redact_secrets(&without_nul);
...
async fn enroll_device(
    ...
    let interval = value
        ...
        .ok_or_else(|| {
            "Hakus returned an invalid device authorization interval.".to_string()
        })?;
    let expires_in = value
        .get("expiresIn")
        .and_then(Value::as_u64)
        .filter(|value| (60..=1800).contains(value))
        .ok_or_else(|| "Hakus returned an invalid device authorization expiry.".to_string())?;
    ...
    loop {
        ...
        let response = client
            ...
            .send()
            .await
            .map_err(|_| "Remote-control authorization could not reach Hakus.".to_string())?;
...
fn enrollment_from_exchange(
    ...
    if value.get("status").and_then(Value::as_str) != Some("approved") {
        return Err("Hakus returned an invalid runner credential.".to_string());
    }
    let record = value
        .get("enrollment")
        .filter(|value| value.is_object())
        .ok_or_else(|| "Hakus returned an invalid runner credential.".to_string())?;
    ...
    let target_grant_ref = record
        ...
        .ok_or_else(|| "Hakus returned no grant for this session.".to_string())?
        .to_string();
    Ok(LiveEnrollment {
        persisted: PersistedEnrollment {
            schema_version: 1,
...
async fn refresh_enrollment(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|_| "Remote-control credential refresh could not reach Hakus.".to_string())?;
    if matches!(
        response.status(),
    ...
    let record = value
        .get("enrollment")
        .filter(|value| value.is_object())
        .ok_or_else(|| "Hakus returned an invalid refreshed credential.".to_string())?;
    if record.get("id").and_then(Value::as_str) != Some(persisted.runner_enrollment_id.as_str())
        ...
        || !exact_capabilities(record.get("capabilities"))
    {
        return Err("Hakus returned a mismatched refreshed credential.".to_string());
    }
    Ok(LiveEnrollment {
        persisted,
        access_token: access_token(&value)?,
...
fn parse_runner_connection(
    ...
    let response = value
        ...
        .ok_or_else(|| "Hakus returned an invalid runner attachment response.".to_string())?;
    let runner = response
        .get("runner")
        .and_then(Value::as_object)
        .ok_or_else(|| "Hakus returned an invalid runner lease.".to_string())?;
    ...
    if !runner_binding_matches {
        return Err("Hakus returned a runner lease for a different session.".to_string());
    }
    ...
    let attachment = response
        ...
        .ok_or_else(|| "Hakus returned an invalid session attachment.".to_string())?;
    let run_id = attachment
        .get("runId")
        .and_then(Value::as_str)
        .filter(|value| valid_opaque_ref(value))
        .map(ToString::to_string)
        .ok_or_else(|| "Hakus returned an invalid attached run.".to_string())?;
    let workspace_id = attachment
        .get("workspaceId")
        .and_then(Value::as_str)
        .filter(|value| valid_opaque_ref(value))
        .map(ToString::to_string)
        .ok_or_else(|| "Hakus returned an invalid attached workspace.".to_string())?;
    let runtime_cursor = attachment
        .get("runtimeCursor")
        .and_then(Value::as_u64)
        .filter(|value| *value <= JS_MAX_SAFE_INTEGER)
        .ok_or_else(|| "Hakus returned an invalid runtime event cursor.".to_string())?;
    let snapshot_present = attachment
        .get("snapshotPresent")
        .and_then(Value::as_bool)
        .ok_or_else(|| "Hakus returned an invalid snapshot receipt.".to_string())?;
...
/// Parse a web link and accept it only on the Hakus app origin (or, in
...
async fn list_runs(
    ...
    let runs = value
        ...
        .ok_or_else(|| "Hakus returned an invalid runner run list.".to_string())?;
    runs.iter()
        .map(|run| {
            run.get("id")
                .and_then(Value::as_str)
                .filter(|value| valid_opaque_ref(value))
                .map(ToString::to_string)
                .ok_or_else(|| "Hakus returned an invalid runner run.".to_string())
        })
        .collect()
}
...
async fn list_commands(
    ...
    let commands = value
        ...
        .ok_or_else(|| "Hakus returned an invalid command list.".to_string())?;
    commands
        .iter()
        .map(|item| {
            let seq = item
                ...
                .ok_or_else(|| "Hakus returned an invalid command sequence.".to_string())?;
            let command = item
                .get("command")
                .filter(|value| value.is_object())
                .cloned()
                .ok_or_else(|| "Hakus returned an invalid typed command.".to_string())?;
            Ok(ListedCommand {
                seq,
                command,
                ack_status: item
...
fn parse_remote_command(value: &Value, expected_run_id: &str) -> Result<RemoteCommand, String> {
    ...
    match value.get("type").and_then(Value::as_str) {
        ...
        _ => Err("Hakus sent an unsupported remote command.".to_string()),
    }
}
...
async fn public_request(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|_| "Remote control could not reach Hakus.".to_string())?;
    if !response.status().is_success() {
        return Err(format!(
            "Hakus rejected remote-control enrollment ({}).",
            response.status()
        ));
    }
    read_bounded_json(response).await
...
async fn read_bounded_json(response: reqwest::Response) -> Result<Value, String> {
    ...
    {
        return Err("Hakus returned an oversized remote-control response.".to_string());
    }
    let bytes = response
        .bytes()
        .await
        .map_err(|_| "Hakus returned an unreadable response.".to_string())?;
    if bytes.len() > MAX_RESPONSE_BYTES {
        return Err("Hakus returned an oversized remote-control response.".to_string());
    }
    serde_json::from_slice(&bytes)
        .map_err(|_| "Hakus returned an invalid remote-control response.".to_string())
}
...
fn load_persisted_enrollment() -> Result<Option<PersistedEnrollment>, String> {
    let Some(raw) = hakus_secrets::Secrets::auto_detect()
...
fn delete_persisted_enrollment() {
    if let Err(error) = hakus_secrets::Secrets::auto_detect().delete(ENROLLMENT_SECRET_SLOT) {
...
fn access_token(value: &Value) -> Result<String, String> {
    let token = value
        ...
        .ok_or_else(|| "Hakus returned an invalid runner access token.".to_string())?
        .to_string();
    if jwt_expiry(&token).is_none_or(|expiry| expiry <= epoch_seconds()) {
        return Err("Hakus returned an expired runner access token.".to_string());
    }
    Ok(token)
}
...
fn validate_authorization_url(value: &str, user_code: &str) -> Result<(), String> {
    let url = Url::parse(value)
        .map_err(|_| "Hakus returned an invalid authorization URL.".to_string())?;
    let pairs = url.query_pairs().collect::<Vec<_>>();
    if url.scheme() != "https"
        || url.host_str() != Some("app.hakus.net")
        ...
        || pairs[0].1 != user_code
    {
        return Err("Hakus returned an invalid authorization URL.".to_string());
    }
    Ok(())
}
...
fn string_field(value: &Value, field: &str) -> Result<String, String> {
    value
        ...
        .map(ToString::to_string)
        .ok_or_else(|| format!("Hakus returned an invalid {field}."))
}
...
mod tests {
    ...
    fn observed_git_repo_is_owner_name_not_a_path() {
        ...
        assert_eq!(
            normalize_observed_git_repo("/Volumes/VIXinSSD/CW/hakus"),
            None
        );
    }
    ...
    fn connect_body_can_carry_an_observed_repo_without_a_path() {
        let enrollment = fixture_enrollment("https://api.hakus.net/");
    ...
    fn connection_links_are_parsed_from_the_runner_lease() {
        ...
        value["runner"]["runUrl"] = json!("https://app.hakus.net/session?run=run_fixture");
        value["runner"]["computerUrl"] =
            json!("https://app.hakus.net/settings?section=workspaces");
        ...
        assert_eq!(
            ...
            Some("https://app.hakus.net/session?run=run_fixture")
        );
        assert_eq!(
            connection.links.computer_url.as_deref(),
            Some("https://app.hakus.net/settings?section=workspaces")
        );
    }
    ...
    fn connection_links_off_origin_or_for_another_run_are_dropped_not_fatal() {
        ...
        let start = fixture_start();
        for spoofed in [
            "http://app.hakus.net/session?run=run_fixture",
            "https://app.hakus.net.evil.example/session?run=run_fixture",
            "https://evil.example/session?run=run_fixture",
            "https://user:pw@app.hakus.net/session?run=run_fixture",
            "https://app.hakus.net:8443/session?run=run_fixture",
            "https://app.hakus.net/session?run=run_fixture#token=abc",
            "https://app.hakus.net/session?run=run_other",
            "https://app.hakus.net/session?run=run_fixture&next=https://evil.example",
            "https://app.hakus.net/logout?run=run_fixture",
            "",
            "not a url",
        ] {
            let mut value = fixture_connection_response();
    ...
    fn banner_leads_with_the_session_link_when_present() {
        ...
        assert_eq!(
            with_link,
            "REMOTE CONTROL · https://app.hakus.net/session?run=run_fixture · /rc stop returns input here"
        );
        ...
        let notice =
            remote_control_link_notice("https://app.hakus.net/session?run=run_fixture");
        assert!(notice.starts_with(
            "Remote control is live at https://app.hakus.net/session?run=run_fixture"
        ));
    ...
    fn controller_exposes_links_only_while_connected() {
        ...
        event_tx
            .send(RemoteEvent::Connected {
                ...
                links: RemoteLinks {
                    run_url: Some("https://app.hakus.net/session?run=run_fixture".to_string()),
                    computer_url: Some(
                        "https://app.hakus.net/settings?section=workspaces".to_string(),
                    ),
                },
            })
            .unwrap();
        ...
        assert!(
            controller
                .status_line()
                .contains("open https://app.hakus.net/session?run=run_fixture")
        );
    ...
    fn authorization_url_is_exact_and_cannot_redirect_or_add_parameters() {
        assert!(
            validate_authorization_url(
                "https://app.hakus.net/runner/authorize?user_code=ABCD-EFGH-JKLM",
                "ABCD-EFGH-JKLM",
            )
            .is_ok()
        );
        for spoofed in [
            "http://app.hakus.net/runner/authorize?user_code=ABCD-EFGH-JKLM",
            "https://app.hakus.net.evil.example/runner/authorize?user_code=ABCD-EFGH-JKLM",
            "https://app.hakus.net/runner/authorize?user_code=ABCD-EFGH-JKLM&next=https://evil.example",
            "https://app.hakus.net/runner/authorize?user_code=WRONG-CODE",
        ] {
