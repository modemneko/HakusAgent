...
pub struct PluginManifest {
    /// Missing means the legacy, pre-versioned Hakus manifest. Legacy
...
pub struct PluginInventory {
    ...
    /// MCP servers that launch a child process under the Hakus user's
...
/// enablement, and staging: it names whether Hakus can activate the
...
pub enum PluginCompatibility {
    ...
    Partial,
    /// The bundle only declares surfaces Hakus cannot activate yet.
    Unsupported,
}
...
/// as the legacy Hakus format. Both parse into the same
...
/// detect mid-validation drift); Hakus-specific data arrives through
/// `extensions["net.hakus"]` and unknown namespaces are ignored.
fn parse_manifest(
...
/// The historical Hakus `plugin.toml` name rule: lowercase ASCII letters,
...
fn hash_bundle(
    ...
    hasher.update(b"hakus-plugin-content-v2\0");
...
fn hash_path(
    ...
    if metadata.is_dir() {
        ...
        file_hasher.update(b"hakus-plugin-file-bytes-v1\0");
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn bundle_hash_frames_adversarial_binary_records() {
        let left = tempfile::tempdir().unwrap();
        let right = tempfile::tempdir().unwrap();
        ...
        let mut adversarial = b"alpha\0unix-executable\0\0F\0hakus-os-path-v1\0".to_vec();
        ...
        adversarial.extend_from_slice(&(b"b.bin".len() as u64).to_le_bytes());
        adversarial.extend_from_slice(b"b.bin");
        adversarial.extend_from_slice(b"omega");
        fs::write(right.path().join("a.bin"), adversarial).unwrap();

        let left = PluginManifest::validate_from_path(&left.path().join("plugin.toml")).unwrap();
        let right = PluginManifest::validate_from_path(&right.path().join("plugin.toml")).unwrap();
        assert_ne!(left.content_hash, right.content_hash);
        assert_eq!(left.capability_hash, right.capability_hash);
    }
