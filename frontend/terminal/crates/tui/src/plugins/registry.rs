...
impl PluginRegistry {
    ...
    pub fn enable(&mut self, selector: &str) -> Result<(), String> {
        ...
        if plugin.staged_root.is_none() {
            return Err(format!(
                "Plugin bundle `{}` has no verified Hakus runtime snapshot; review and trust it again before enablement",
                plugin.name()
            ));
        }
...
/// sibling Hakus-owned stores (e.g. the marketplace catalog store) share
...
fn runtime_stage_path(state_path: &Path, id: &PluginId, content_hash: &str) -> PathBuf {
    ...
    hasher.update(b"hakus-plugin-stage-v2\0");
...
fn stage_bundle(state_path: &Path, plugin: &LoadedPlugin) -> Result<PathBuf, String> {
    ...
    if destination.exists() {
        if !staged_bundle_matches(&destination, &plugin.content_hash, &plugin.capability_hash) {
            return Err(
                "Existing Hakus plugin runtime snapshot failed content validation; remove the exact .runtime entry and review again"
                    .to_string(),
            );
        }
    ...
    let staged = (|| {
        ...
        if !staged_bundle_matches(&temporary, &plugin.content_hash, &plugin.capability_hash) {
            return Err(
                "Plugin bundle changed while Hakus was staging it; no runtime authority was granted"
                    .to_string(),
            );
        }
...
/// Hakus-owned immutable runtime copy, then require the named adapter to
...
pub fn verify_plugin_component_authority(
    ...
    if !PluginActivationPolicy::current().is_supported(capability) {
        return Err(format!(
            "Plugin bundle `{}` capability `{}` is inactive in this Hakus build",
...
/// Recheck a persisted plugin receipt, the mutable reviewed source, and the
/// Hakus-owned immutable runtime copy. This function performs no writes.
pub fn verify_plugin_authority(authority: &PluginAuthority) -> Result<(), String> {
    ...
    for (label, manifest_path) in [
        ...
        ("Hakus runtime snapshot", &authority.staged_manifest),
    ] {
        let current =
...
mod windows_acl_tests {
    ...
    fn owner_only_acl_rehardens_without_write_owner_access() {
        ...
        // Simulate an already-private Hakus object whose owner is still
