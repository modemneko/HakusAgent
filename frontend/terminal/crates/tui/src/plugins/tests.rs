...
fn aba_source_skill_body_is_replaced_by_the_staged_snapshot_before_activation() {
    ...
    digest.update(b"hakus-plugin-file-bytes-v1\0");
    ...
    assert!(
        ...
        "active Skill paths must point into the Hakus-owned staged tree"
    );
}
    ...
    let plugin = write_json_bundle(
        ...
        r#"{
            ...
            "extensions": {
                ...
                "net.hakus": {"capabilities": {"network_hosts": ["example.com"]}}
            }
        }"#,
    );
    fs::create_dir_all(plugin.join("skills/demo")).unwrap();
        ...
        r#"{
            ...
            "mcpServers": {
                ...
                "remote": {
                    ...
                    "extensions": {
                        "net.hakus": {"env_headers": {"Authorization": "THIRD_PARTY_REMOTE_TOKEN"}},
...
fn export_slugifies_legacy_names_and_collision_is_an_error() {
    ...
    assert_eq!(
        plugin_json["extensions"]["net.hakus"]["display_name"],
        "a--b"
    );
}
...
fn export_moves_custom_skills_layout_to_the_standard_tree() {
    ...
    assert!(
        ...
        "a standard-layout bundle emits no Hakus extension at all: {plugin_json}"
    );
