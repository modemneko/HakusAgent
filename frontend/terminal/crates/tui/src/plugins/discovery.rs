    ...
    hakus_config::hakus_home()
        ...
        .unwrap_or_else(|error| {
            ...
            tracing::warn!(
                target: "plugins",
                %error,
                "Hakus home could not be resolved; user plugin discovery is disabled"
            );
            std::env::temp_dir()
                .join(format!(
                    ".hakus-home-unavailable-{}",
...
pub fn default_workspace_plugins_dir(workspace: &Path) -> PathBuf {
    workspace.join(".hakus").join("plugins")
}
...
fn load_plugin(
    ...
    if validated.canonical_root.parent() != Some(canonical_discovery_root) {
        return Err(format!(
            "plugin bundle resolved outside its Hakus-owned discovery root: {}",
    ...
    // snapshots parsed from the Hakus-owned staged tree in `apply_state`.
...
fn hash_skill_bytes(bytes: &[u8]) -> String {
    ...
    hasher.update(b"hakus-plugin-file-bytes-v1\0");
    hasher.update(bytes);
    hasher
        .finalize()
        .iter()
...
fn plugin_id(scope: PluginScope, name: &str, canonical_root: &Path) -> PluginId {
    ...
    hasher.update(b"hakus-plugin-id-v2\0");
