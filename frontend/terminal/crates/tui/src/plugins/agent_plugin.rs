...
//! SKILL.md` tree that already matches Hakus's skill layout.
//!
//! This module owns the mapping between that format and Hakus's internal
//! [`PluginManifest`]:
//!
...
//!   here. Hakus-specific fields round-trip through
//!   `extensions["net.hakus"]`.
...
/// native Agent Plugins format; `plugin.toml` is the legacy Hakus format
/// and stays readable.
pub const PLUGIN_JSON_NAME: &str = "plugin.json";
pub const PLUGIN_TOML_NAME: &str = "plugin.toml";
/// Kimi Code's native plugin manifest. Hakus consumes the compatible
...
/// Hakus's reverse-domain extension namespace.
pub const HAKUS_NAMESPACE: &str = "net.hakus";
...
///
/// The input shape is deliberately closed. Kimi capabilities Hakus does
...
/// Hakus's own `extensions["net.hakus"]` payload: every manifest
...
/// *other* vendors' namespaces, not to a silently dropped Hakus field.
#[derive(Debug, Clone, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct HakusPluginExtension {
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// omitted and is then inferred from `command` vs `url`. Hakus-only
/// server options ride in `extensions["net.hakus"]`.
#[derive(Debug, Clone, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
...
/// Hakus-only MCP server options, carried per-server under
/// `extensions["net.hakus"]` in `mcp.json` so the standard fields round-
/// trip losslessly with Hakus's richer review model (timeouts, tool
/// filters, env-backed credentials, enablement).
#[derive(Debug, Clone, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct HakusMcpExtension {
    #[serde(default, skip_serializing_if = "BTreeMap::is_empty")]
...
fn standard_server_to_config(
    ...
    let extension = hakus_mcp_extension(&server.extensions)?;
...
fn hakus_mcp_extension(
    extensions: &BTreeMap<String, serde_json::Value>,
) -> Result<HakusMcpExtension, String> {
    match extensions.get(HAKUS_NAMESPACE) {
        Some(value) => serde_json::from_value(value.clone()).map_err(|error| {
            format!("mcp.json server extensions[\"{HAKUS_NAMESPACE}\"] is invalid: {error}")
        }),
        None => Ok(HakusMcpExtension::default()),
    }
}
...
/// Hakus extension gets the default skills spec exactly when that
/// directory exists.
pub fn standard_to_manifest(
    ...
    let extension = match standard.extensions.get(HAKUS_NAMESPACE) {
        Some(value) => {
            serde_json::from_value::<HakusPluginExtension>(value.clone()).map_err(|error| {
                format!("plugin.json extensions[\"{HAKUS_NAMESPACE}\"] is invalid: {error}")
            })?
        }
        None => HakusPluginExtension::default(),
    };
...
/// in `author.name`, so a Hakus-origin author round-trips verbatim.
...
pub fn manifest_to_standard(
    ...
    let extension = HakusPluginExtension {
    ...
    let plugin_json = StandardPluginManifest {
        ...
        extensions: if extension.is_empty() {
            ...
            let value = serde_json::to_value(&extension)
                .map_err(|error| format!("failed to encode Hakus extension: {error}"))?;
            BTreeMap::from([(HAKUS_NAMESPACE.to_string(), value)])
        },
    };
...
fn config_to_standard_server(
    ...
    let extension = HakusMcpExtension {
        env_headers: config
            .env_headers
            .iter()
    ...
    let extensions = if extension.is_empty() {
        ...
        let value = serde_json::to_value(&extension)
            .map_err(|error| format!("failed to encode Hakus MCP extension: {error}"))?;
        BTreeMap::from([(HAKUS_NAMESPACE.to_string(), value)])
    };
...
// ─────────────────────────────────────────────────────────────────────────────
//
// Every document Hakus writes is re-validated against the standard's shape
...
mod tests {
    ...
    /// A full-featured legacy manifest: every Hakus-specific field the
...
"#;
    ...
    fn emitted_documents_carry_only_standard_root_keys() {
        ...
        let emission = manifest_to_standard(&internal, &BTreeSet::new()).unwrap();
        let plugin_value = serde_json::to_value(&emission.plugin_json).unwrap();
        // mcp_servers never leak into the closed plugin.json root; Hakus
        // data is namespaced.
        ...
        assert_eq!(
            mcp_value["mcpServers"]["remote"]["extensions"][HAKUS_NAMESPACE]["connect_timeout"],
            serde_json::json!(30)
        );
    }
    ...
    fn slugified_export_preserves_display_name_and_collision_is_an_error() {
        ...
        let extension = emission.plugin_json.extensions[HAKUS_NAMESPACE].clone();
    ...
    fn unknown_extensions_namespaces_are_ignored_not_rejected() {
        ...
        let bad_extension = r#"{
            "$schema": "https://agent-plugins.org/schemas/plugin.json",
            "name": "acme.tools",
            "extensions": {"net.hakus": {"surprise": true}}
        }"#;
    ...
    fn mcp_json_transports_and_reserved_env_names() {
        ...
        let extended = r#"{"mcpServers": {"r": {"url": "https://example.com/mcp",
            "extensions": {"net.hakus": {"connect_timeout": 30, "required": true,
