...
//!
//! Every parser (`kimi`, `claude`, `codex`, `hakus`) funnels foreign
...
//!   enters Hakus disabled and untrusted and goes through the existing
//!   content/capability hash review.
//! - Foreign `policy` blocks (Codex `INSTALLED_BY_DEFAULT`) never trigger
//!   auto-install; Hakus only installs on an explicit operator action.
...
pub enum MarketplaceFormat {
    ...
    #[default]
    Auto,
    /// Hakus native catalog: `plugins[]` with `name` + install-spec
    /// `source` strings.
    Hakus,
...
pub enum MarketplaceSourceSpec {
    ...
    GitUrl { url: String },
    /// npm package declaration. Hakus does not execute npm; the plan
    /// stays unsupported.
    Npm { package: String },
    /// A source form the catalog format defines but Hakus refuses to
...
/// Whether and how Hakus's existing installer could fetch a source.
...
pub enum MarketplaceInstallPlan {
    ...
    /// Cannot be installed by Hakus today, and why.
