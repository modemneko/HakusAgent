...
fn parse_claude_entry(
    ...
    if obj.get("strict") == Some(&Value::Bool(false)) {
        entry_diags.push(MarketplaceDiagnostic::warning(
            "NON_STRICT_ENTRY",
            format!("Claude plugin `{name}` sets strict=false; Hakus always reviews the installed manifest"),
            Some(name.clone()),
            Some(index),
        ));
    }
...
/// These are catalog-side declarations; Hakus counts them for
    ...
    match discriminator {
        "github" => {
            ...
            if git_ref.is_some() || sha.is_some() {
                diags.push(MarketplaceDiagnostic::warning(
                    "UNAPPLIED_PIN",
                    "Hakus's GitHub installer resolves the default branch; ref/sha pins are recorded but not yet applied".to_string(),
                    None,
                    None,
                ));
            }
        ...
        "npm" => {
            ...
            (
                ...
                MarketplaceInstallPlan::Unsupported {
                    reason: "Hakus does not execute npm; install the plugin from a GitHub repo, tarball, or local path".to_string(),
                    raw: value.to_string(),
                },
                diags,
            )
        ...
        "command" => (
            MarketplaceSourceSpec::Refused {
                reason: "Claude `command` sources execute arbitrary shell commands and are never run by Hakus".to_string(),
            },
            MarketplaceInstallPlan::Unsupported {
