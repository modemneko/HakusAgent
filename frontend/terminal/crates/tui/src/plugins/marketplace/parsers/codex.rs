...
//! `category`.
//!
//! `policy` is display-only in Hakus: `INSTALLED_BY_DEFAULT` never
...
fn parse_codex_entry(
    ...
    // only. Hakus never auto-installs and never auto-authenticates.
    ...
    match installation {
        ...
        Some("INSTALLED_BY_DEFAULT") => {
            entry_diags.push(MarketplaceDiagnostic::warning(
                "NO_AUTO_INSTALL",
                "Codex policy `INSTALLED_BY_DEFAULT` is ignored: Hakus installs only on an explicit operator action".to_string(),
                Some(name.clone()),
                Some(index),
            ));
        }
