...
use anyhow::Result;
use hakus_execpolicy::{AskForApproval, ExecPolicyContext};
...
pub struct EngineConfig {
    ...
    pub active_route_limits: Option<hakus_config::route::RouteLimits>,
    ...
    /// `workspace/.hakus/state`. Embedders may set a session-scoped root
    ...
    /// `[subagents] max_depth = N` in `~/.hakus/config.toml`.
    ...
    pub exec_policy_engine: hakus_execpolicy::ExecPolicyEngine,
...
impl Default for EngineConfig {
    fn default() -> Self {
        Self {
            ...
            exec_policy_engine: hakus_execpolicy::ExecPolicyEngine::new(Vec::new(), Vec::new()),
...
/// HakusAgent 对齐门面：Python 后端 `hakus.agent.AgentCore` 的 Rust 对应。
///
/// Python AgentCore 聚合 LLM 客户端 + 工具注册表 + 上下文管理，入口是
/// `run_turn()` 流式事件；Rust `Engine` 聚合 model client + ToolRegistry +
/// context/compaction，入口同样是 [`Engine::run_turn`]。新代码优先用
/// `AgentCore` 这个名字。
pub type AgentCore = Engine;
...
pub struct Engine {
    ...
    active_route_limits: Option<hakus_config::route::RouteLimits>,
    active_route_capabilities: hakus_config::route::RouteCapabilities,
    ...
    /// turn-loop's empty-tool_uses branch to surface `<hakus:subagent.done>`
...
impl Engine {
    ...
    fn env_only_api_key_recovery_hint(api_config: &Config) -> Option<String> {
        ...
        Some(format!(
            "The rejected key came from {env_var}; no saved config key is present.\n\
             Run `hakus auth status` to inspect credential sources, then \
             `hakus auth set --provider {provider}` to save a valid key in ~/.hakus/config.toml, \
    ...
    pub fn new(mut config: EngineConfig, api_config: &Config) -> (Self, EngineHandle) {
        ...
        let system_prompt =
            prompts::system_prompt_for_mode_with_context_skills_session_and_approval_for_host(
                ...
                prompts::PromptSessionContext {
                    ...
                    skills_scan_hakus_only: config.skills_scan_hakus_only,
        ...
        let approval_receipt_store = Ok(ApprovalReceiptStore::new(
            std::env::temp_dir().join(format!("hakus-approval-tests-{}", uuid::Uuid::new_v4())),
        ));
        let engine = Engine {
            config,
            ...
            api_provider_id,
            active_route_limits,
            active_route_capabilities: hakus_config::route::RouteCapabilities::default(),
            rx_op,
    ...
    fn bounded_redacted_goal_failure_detail(&self, detail: &str) -> Option<String> {
        ...
        let detail = self.deepseek_client.as_ref().map_or_else(
            || hakus_config::persistence::redact_secrets(detail),
                ...
                EngineRunInput::Operation(op) => match *op {
                    ...
                    Op::SetModel {
                        ...
                        self.active_route_capabilities =
                            hakus_config::route::RouteCapabilities::default();
                    ...
                    Op::SetSubagentRuntimeConfig {
                        ...
                        self.config.max_spawn_depth =
                            max_spawn_depth.min(hakus_config::MAX_SPAWN_DEPTH_CEILING);
                    ...
                    Op::ReloadMcp { config_path, tx } => {
                        let result = self.reload_mcp_pool(config_path).await.map_err(|error| {
                            hakus_config::persistence::redact_secrets(&format!("{error:#}"))
                        });
    ...
    async fn handle_send_message(
        ...
        // receipt — an ambient `Config` read at TurnStarted or TurnCompleted
        ...
        // actually installed above — the same client `Event::TurnCompleted`
        ...
        // failed TurnCompleted instead of unwinding through `engine.run()` and
                ...
                (
                    ...
                    Some(format!(
                        "The engine hit an internal error and stopped this turn: {detail}. \
                         ...
                         A crash report was saved to ~/.hakus/crashes/."
                    )),
                )
            }
        };
        ...
        // event can no longer arrive after `TurnCompleted` and be mistaken for
        ...
        // Post-turn snapshot. Fire-and-forget: TurnCompleted is already
        ...
        // Fire-and-forget: TurnCompleted is already emitted. The advisor
    ...
    /// Interrupted, immediately before their `TurnCompleted` event.
    ...
    fn build_tool_context_for_turn(
        ...
        .with_skills_config(
            ...
            self.config.skills_scan_hakus_only,
        )
        ...
        let base =
            prompts::system_prompt_for_mode_with_context_skills_session_and_approval_for_host(
                ...
                prompts::PromptSessionContext {
                    ...
                    skills_scan_hakus_only: self.config.skills_scan_hakus_only,
    ...
    /// history boundary. This is the Hakus equivalent of Codex clearing
...
pub(crate) fn exec_shell_ask_rule_decision_for_policy(
    exec_policy_engine: &hakus_execpolicy::ExecPolicyEngine,
    tool_name: &str,
    tool_input: &Value,
    workspace: &Path,
    approval_mode: crate::tui::approval::ApprovalMode,
...
fn tool_ask_rule_decision_for_context(
    ...
    if !decision.allow {
    ...
    } else if decision.matched_action == Some(hakus_execpolicy::PermissionAction::Allow) {
        ...
        hakus_telemetry::session_counters()
            .bump(hakus_telemetry::Counter::ApprovalAutoAllowed);
        Some(ToolAskRuleDecision::Allow)
    } else {
        None
    }
...
pub(crate) struct NextTurnPromptContext {
    ...
    pub(crate) route_limits: Option<hakus_config::route::RouteLimits>,
...
struct TurnToolBuild {
    ...
    /// emits `TurnCompleted`: that is what makes detached-child usage accounting
...
pub(crate) struct TurnRouteContext {
    pub(crate) provider: ApiProvider,
    pub(crate) model: String,
    pub(crate) capabilities: hakus_config::route::RouteCapabilities,
    pub(crate) limits: Option<hakus_config::route::RouteLimits>,
