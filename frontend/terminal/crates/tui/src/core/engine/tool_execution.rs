...
pub(crate) fn emit_tool_audit(event: serde_json::Value) {
    let Some(path) = std::env::var_os("HAKUS_TOOL_AUDIT_LOG")
...
impl Engine {
    ...
    pub(super) async fn execute_tool_with_lock(
        ...
        telemetry.bump(hakus_telemetry::Counter::ToolCalls);
        if tool_name == "memory_search" {
            telemetry.bump(hakus_telemetry::Counter::MemorySearch);
        }
        match &outcome {
            Ok(result) => {
                tracing::debug!(
            ...
            Err(err) => {
                ...
                match err {
                    ToolError::PermissionDenied { .. } => {
                        telemetry.bump_error(hakus_telemetry::ErrorCounter::ToolDeniedByPolicy)
                    }
                    ToolError::Timeout { .. } => {
                        telemetry.bump_error(hakus_telemetry::ErrorCounter::ToolTimeout);
                    }
                    _ => {}
                }
                tracing::warn!(
