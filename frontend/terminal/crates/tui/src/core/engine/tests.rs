...
async fn terminal_barrier_joins_foreground_child_before_flushing_mailbox() {
    ...
    assert!(
        ...
        "mailbox flushing, and therefore TurnCompleted, waits for the owned child"
    );
    assert!(
...
async fn queued_manual_compaction_cancellation_is_idempotent_and_skips_route_activation() {
    ...
    assert!(matches!(
        ...
        [
            ...
            Event::TurnCompleted { status: TurnOutcomeStatus::Interrupted, .. }
...
async fn tool_response_crossing_goal_budget_issues_second_provider_request() {
    ...
    while !saw_blocked_goal {
        ...
        match event {
            Event::ToolCallFinished { name, result, .. } if name == "get_goal" => {
...
async fn queued_goal_clear_refreshes_prompt_and_cancels_stale_continuation() {
    ...
    // synthetic continuation that TurnCompleted schedules.
    handle
        .send(Op::SetGoalStatus {
...
async fn started_nonretryable_continuation_failure_blocks_goal_with_bounded_reason() {
    ...
    assert!(!blocker.contains(leaked_secret), "{blocker}");
    assert!(
        blocker.contains(hakus_config::persistence::REDACTED),
        "{blocker}"
    );
    assert!(
...
async fn denied_synthetic_tool_is_blocked_by_the_same_turn_policy_at_execution() {
    ...
    let denied = std::iter::from_fn(|| events.try_recv().ok()).find_map(|event| match event {
        Event::ToolCallFinished { name, result, .. } if name == TOOL_SEARCH_NAME => Some(result),
        _ => None,
    });
    let error = denied
...
/// that only serves `read_file`, collecting every `ToolCallFinished` event as
...
async fn run_budgeted_read_turn(
    ...
    let completions = std::iter::from_fn(|| events.try_recv().ok())
        .filter_map(|event| match event {
            Event::ToolCallFinished { id, result, .. } => Some((id, result)),
            _ => None,
        })
        .collect::<Vec<_>>();
...
fn structured_state_block_carries_stable_state_without_work() {
    let state = StructuredState {
        ...
        workspace: PathBuf::from("/workspace/hakus"),
        cwd: Some(PathBuf::from("/workspace/hakus")),
...
fn env_only_auth_error_gets_recovery_hint() {
    ...
    assert!(message.contains("hakus auth status"));
    assert!(message.contains("hakus auth set --provider deepseek"));
}
...
fn plugin_tools_dir_honors_missing_custom_directory_without_fallback() {
    let missing = PathBuf::from("definitely-missing-hakus-plugin-dir");
...
fn ask_rule_engine(command: &str) -> hakus_execpolicy::ExecPolicyEngine {
    hakus_execpolicy::ExecPolicyEngine::with_rulesets(vec![
        hakus_execpolicy::Ruleset::user(vec![], vec![])
            .with_ask_rules(vec![hakus_execpolicy::ToolAskRule::exec_shell(command)]),
    ])
}

fn file_ask_rule_engine(tool: &str, path: &str) -> hakus_execpolicy::ExecPolicyEngine {
    hakus_execpolicy::ExecPolicyEngine::with_rulesets(vec![
        hakus_execpolicy::Ruleset::user(vec![], vec![]).with_ask_rules(vec![
            hakus_execpolicy::ToolAskRule::file_path(tool, path),
        ]),
    ])
}
...
async fn injected_model_drives_real_engine_navigation_trajectory() {
        ...
        .expect("timed out waiting for deterministic navigation")
    {
        match event {
            ...
            Event::ToolCallFinished { name, result, .. } if name == "File" => {
...
async fn injected_model_sandbox_escalation_applies_only_after_exact_call_approval() {
    ...
    loop {
        ...
        match event {
            ...
            Event::ToolCallFinished { id, result, .. } if id == "call-escalated-bash" => {
        ...
        *op_auto_approve = auto_approve;
        ...
        loop {
            ...
            match event {
                ...
                Event::ToolCallFinished { id, result, .. }
...
async fn max_steps_exhaustion_fails_as_budget_never_completed() {
    ...
    let (status, error) = loop {
        let event = tokio::time::timeout(model_turn_event_timeout(), rx.recv())
            ...
            .expect("engine event");
        if let Event::TurnCompleted { status, error, .. } = event {
            break (status, error);
        }
    };
    drop(rx);
...
async fn injected_model_complete_tool_block_at_max_output_tokens_executes() {
    ...
    {
        match event {
            Event::TokenUsage { .. } => saw_turn_usage = true,
...
async fn collect_guardian_journey_with_receipts(
    ...
    let terminal_usage = loop {
        let event = tokio::time::timeout(model_turn_event_timeout(), rx.recv())
            ...
            .expect("engine event stream closed");
        match event {
            Event::ToolCallFinished { id, result, .. } if id == call_id => {
                assert!(
                    completion.replace(result).is_none(),
                    "duplicate tool result"
                );
            }
            Event::TokenUsage { usage, .. } => usage_events.push(usage),
...
fn exec_shell_allow_rule_decision_allows_only_exact_command_in_scoped_repo() {
    let rule = hakus_execpolicy::ToolAskRule::exec_shell("cargo test")
        .into_exact_workspace_allow("/repo");
    let config = EngineConfig {
        exec_policy_engine: hakus_execpolicy::ExecPolicyEngine::with_rulesets(vec![
            hakus_execpolicy::Ruleset::user(vec![], vec![]).with_ask_rules(vec![rule]),
        ]),
        ..EngineConfig::default()
    };
...
fn apply_patch_allow_requires_every_touched_path_to_match() {
    let rules = ["src/a.rs", "src/b.rs"]
        .into_iter()
        .map(|path| {
            hakus_execpolicy::ToolAskRule::file_path("apply_patch", path)
                .into_exact_workspace_allow("/repo")
        })
        .collect();
    let config = EngineConfig {
        exec_policy_engine: hakus_execpolicy::ExecPolicyEngine::with_rulesets(vec![
            hakus_execpolicy::Ruleset::user(vec![], vec![]).with_ask_rules(rules),
        ]),
        ..EngineConfig::default()
    };
    ...
    let (engine, handle) = Engine::new(EngineConfig::default(), &Config::default());
    {
        ...
        goal.mark_complete(
            ...
            crate::tools::goal::GoalCompletionVerification {
                ...
                check: "cargo test -p hakus-tui runtime_goal_updates_emit_ui_snapshot"
                    .to_string(),
...
async fn measure_production_mode_tool_catalogs() -> serde_json::Value {
    ...
    let _hakus_home = EnvVarGuard::set("HAKUS_HOME", home.join(".hakus"));
    ...
    let _ocr = EnvVarGuard::set("HAKUS_LOCAL_OCR_UNAVAILABLE", "1");
    ...
    for (mode_name, mode) in [
        ...
        let route = TurnRouteContext {
            ...
            capabilities: hakus_config::route::RouteCapabilities::default(),
            limits: None,
...
fn print_mode_runtime_contract_metrics() {
    ...
    let _home = EnvVarGuard::set("HOME", &home);
    let _userprofile = EnvVarGuard::set("USERPROFILE", &home);
    let hakus_home = home.join(".hakus");
...
-> (serde_json::Value, Vec<RepresentativePromptStage>, String) {
    ...
    let workspace = tmp.path().join("workspace");
    let home = tmp.path().join("home");
    let skills_dir = workspace.join(".hakus").join("skills");
    fs::create_dir_all(&workspace).expect("create isolated workspace");
    fs::create_dir_all(&home).expect("create isolated home");
...
async fn run_shell_command_op_executes_without_approval_modal() {
    ...
    let _audit = EnvVarGuard::set("HAKUS_TOOL_AUDIT_LOG", &audit_path);
    let (mut engine, handle) = Engine::new(EngineConfig::default(), &Config::default());
    ...
    while let Some(event) = rx.recv().await {
        match event {
            ...
            Event::ToolCallFinished { id, name, result } => {
                saw_complete = true;
                assert!(id.starts_with(USER_SHELL_TOOL_ID_PREFIX));
                assert_eq!(name, "Bash");
...
async fn operate_model_shell_uses_normal_approval_and_workspace_sandbox() {
        ...
        .expect("timed out waiting for Operate tool event")
    {
        match event {
            ...
            Event::ToolCallFinished { name, result, .. } if name == "Bash" => {
...
async fn assert_full_access_model_tool_batch_is_blocked(
        ...
        .expect("timed out waiting for Full Access boundary event")
    {
        match event {
            ...
            Event::ToolCallFinished { name, result, .. }
...
async fn assert_full_access_model_tool_batch_runs(
        ...
        .expect("timed out waiting for Full Access auto-approval event")
    {
        match event {
            ...
            Event::ToolCallFinished { name, result, .. } if expected.contains(name.as_str()) => {
...
async fn full_access_permission_allow_cannot_bypass_repo_law() {
    let _lock = lock_test_env();
    let workspace = tempdir().expect("tempdir");
    let law_dir = workspace.path().join(".hakus");
    ...
    let allow_rule = hakus_execpolicy::ToolAskRule::file_path("write_file", "CHANGELOG.md")
    ...
    let engine_config = EngineConfig {
        ...
        subagents_enabled: false,
        exec_policy_engine: hakus_execpolicy::ExecPolicyEngine::with_rulesets(vec![
            hakus_execpolicy::Ruleset::user(vec![], vec![]).with_ask_rules(vec![allow_rule]),
        ]),
        ..EngineConfig::default()
    };
    let tool_input =
...
async fn auto_review_auto_resolves_hallucinated_question_without_prompting() {
        ...
        .expect("timed out waiting for Auto-Review question event")
    {
        match event {
            ...
            Event::ToolCallFinished { name, result, .. } if name == REQUEST_USER_INPUT_NAME => {
...
async fn full_access_permission_allow_cannot_bypass_background_catastrophic_floor() {
    ...
    let allow_rule = hakus_execpolicy::ToolAskRule::exec_shell(command.clone())
        .into_exact_workspace_allow(workspace.path().to_string_lossy().into_owned());
    let tool_input = json!({
        "action": "run",
        "command": command,
...
async fn yolo_mode_executes_publish_like_shell_without_prompt() {
        ...
        .expect("timed out waiting for engine event")
    {
        match event {
            ...
            Event::ToolCallFinished { name, .. } if name == "Bash" => {
...
fn turn_tool_context_uses_planned_authority_and_route_not_installed_session() {
    ...
    let route = TurnRouteContext {
        ...
        capabilities: hakus_config::route::RouteCapabilities::default(),
        limits: Some(hakus_config::route::RouteLimits {
...
//
// Hakus has one To-do list. The model learns what is on it the way it
...
async fn a_non_empty_todo_adds_nothing_to_the_request_messages() {
    ...
    for message in &request {
        ...
        assert!(!text.contains("hakus:work"), "{text}");
    }
}
...
async fn provider_request_bodies_never_carry_the_todo_list() {
    ...
    for request in &requests {
        ...
        assert!(!body.contains("hakus:work"), "{body}");
    }
...
async fn sync_session_projects_persisted_subagent_handoff_for_headless_restore() {
    ...
    let payload = concat!(
        ...
        "<hakus:subagent.done>{\"agent_id\":\"agent_headless\",",
...
fn route_context_budget_prefers_resolved_route_limits() {
    let _lock = lock_test_env();
    let limits = hakus_config::route::RouteLimits {
...
fn reasoning_max_does_not_add_a_second_deepseek_v4_output_reservation() {
    let _lock = lock_test_env();
    let _hakus = EnvVarGuard::remove("HAKUS_MAX_OUTPUT_TOKENS");
    ...
    let request = hakus_core::request::prepare_primary_turn_request(
        hakus_core::request::PrimaryTurnRequest {
            model: "DeepSeek-V4-Flash".to_string(),
            messages: Vec::new(),
            max_tokens: cap,
            system: None,
...
fn task_gate_run_results_are_structured_before_context_insertion() {
    let output = ToolResult::success(
        json!({
            "gate": {
                "id": "gate_abcd1234",
                "gate": "clippy",
                "command": "cargo clippy -p hakus-tui --all-targets --all-features --locked -- -D warnings",
                "cwd": "/repo",
                "exit_code": 1,
                "status": "failed",
                ...
                "log_path": "/repo/.hakus/runtime/gate.log",
    ...
    assert!(context.contains("cargo clippy -p hakus-tui"));
    assert!(context.contains("summary: warning promoted to error"));
    assert!(context.contains("log_path: /repo/.hakus/runtime/gate.log"));
}
...
fn turn_metadata_is_byte_identical_across_identical_consecutive_turns() {
    ...
    let prompt_context = NextTurnPromptContext::for_planned_turn(
        ...
        Some(hakus_config::route::RouteLimits {
...
fn turn_metadata_names_the_effective_sandbox_posture() {
    ...
    assert!(
        external_meta.contains("isolation unverified by Hakus"),
        "{external_meta}"
    );
    assert!(
...
fn filter_tool_call_delta_strips_deepseek_xml_marker() {
    let mut in_block = false;
    let visible = filter_tool_call_delta(
        "before <hakus:tool_call name=\"x\">payload</hakus:tool_call> after",
        &mut in_block,
    );
    assert!(!in_block);
...
// `hakus exec` exited 1. In a headless host no operator watches the
...
/// event through the terminal TurnCompleted.
async fn run_headless_turn_with_flaky_network(
    ...
    loop {
        ...
        let terminal = matches!(event, Event::TurnCompleted { .. });
        events.push(event);
        if terminal {
            break;
        }
...
async fn headless_turn_retries_mid_stream_network_drop_and_recovers() {
    ...
    let (status, error) = events
        .iter()
        .find_map(|event| match event {
            Event::TurnCompleted { status, error, .. } => Some((status, error)),
            _ => None,
        })
        .expect("terminal TurnCompleted");
    assert_eq!(
        *status,
        TurnOutcomeStatus::Completed,
...
async fn cancellation_after_terminal_usage_still_charges_the_turn() {
    ...
    *model.token.lock().expect("token cell") = Some(engine.shared_cancel_token.clone());
    ...
    let status = events
        .iter()
        .find_map(|event| match event {
            Event::TurnCompleted { status, .. } => Some(*status),
            _ => None,
        })
        .expect("terminal TurnCompleted");
...
/// flaky client and collect every event through the terminal TurnCompleted.
...
async fn background_completion_after_a_turn_is_delivered_once_on_the_next_turn() {
    ...
    assert_eq!(evidence["schema"], "hakus.shell_completion.evidence.v1");
