...
/// explains why Hakus may retry before any output and why it must surface
...
pub(super) fn stream_read_error_user_message(message: &str, any_content_received: bool) -> String {
    ...
    let retry_note = if any_content_received {
        "Some output had already streamed, so Hakus is surfacing the warning instead of replaying the request and risking duplicated output."
    } else {
        "No output had streamed yet, so Hakus will retry automatically while retry budget remains."
    };
    format!(
...
pub(crate) const TOOL_CALL_MARKER_PAIRS: [(&str, &str); 28] = [
    ("[TOOL_CALL]", "[/TOOL_CALL]"),
    ("<hakus:tool_call", "</hakus:tool_call>"),
    ("<tool_call", "</tool_call>"),
    ("<invoke ", "</invoke>"),
