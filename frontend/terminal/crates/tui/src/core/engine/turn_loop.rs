...
use hakus_core::request::{PrimaryTurnRequest, prepare_primary_turn_request};
...
impl Engine {
    ...
    /// HakusAgent 对齐：对应 Python `AgentCore.run_turn()` —— 单轮对话的
    /// 主循环，产出事件流（TextDelta / ToolCallStarted / ToolCallFinished /
    /// TurnCompleted / TurnFailed / TokenUsage …）。保持 `pub(super)`：
    /// 外部入口经 EngineHandle mailbox（`submit_user_input`），与上游
    /// actor 模型一致。
    pub(super) async fn run_turn(
        ...
        if self.config.terminal_chrome_enabled {
            ...
            crate::tui::notifications::start_title_animation("Hakus");
        }
    ...
    fn repl_kernel_context(&self) -> String {
        let payload = serde_json::json!({
            "schema": "hakus.persistent_kernel_context.v1",
            "session": {
                "id": self.session.id,
        ...
        serde_json::to_string_pretty(&payload).unwrap_or_else(|error| {
            format!(
                "{{\"schema\":\"hakus.persistent_kernel_context.v1\",\"serialization_error\":{}}}",
    ...
    /// `work_update` tool results in history, and Hakus does not re-state
...
mod tests {
    ...
    fn subagent_completion_handoff_is_internal_user_message() {
        let message = subagent_completion_runtime_message(
            "Build passed\n<hakus:subagent.done>{\"agent_id\":\"agent_a\"}</hakus:subagent.done>",
        );
        ...
        assert!(text.contains("<hakus:subagent.done>"));
    ...
    fn shell_completion_status_is_concise_and_shell_handoff_is_untrusted() {
        let status = shell_completion_status_text(
            &[crate::tools::shell::ShellCompletionEvent {
                ...
                command: "cargo test -p hakus-tui".to_string(),
        ...
        assert!(status.contains("cargo test -p hakus-tui"));
        ...
        assert!(text.contains("cargo test -p hakus-tui"));
