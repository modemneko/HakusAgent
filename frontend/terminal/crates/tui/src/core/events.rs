...
/// `TurnCompleted` without relying on mutable global selection state. Non-model
...
pub struct TurnRoute {
    ...
    /// starts — by `TurnCompleted` a provider switch, an auto-router hop, or a
...
pub enum Event {
    ...
    /// provider reported usage for it. Unlike `TurnCompleted`, which fires
    ...
    /// Fired fire-and-forget after `TurnCompleted` when the advisor is enabled
