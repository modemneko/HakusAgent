...
fn write_atomic_with_permissions(
    ...
    // such as `/save <path>`; only sweep Hakus-owned state/config dirs.
    if permission_policy == AtomicWritePermissions::Private && is_hakus_owned_state_dir(parent)
    {
...
/// sitting in a real `~/.hakus/` from a single day three weeks earlier.
...
/// - **Older than an hour** — so a concurrent write by another Hakus
...
/// 2. Writes a crash dump to `~/.hakus/crashes/<timestamp>-<name>.log`.
///
...
/// best-effort crash dump to `~/.hakus/crashes/`, so diagnostics land in
...
pub fn record_caught_panic(name: &'static str, message: &str) {
    ...
    hakus_telemetry::record_blocking(hakus_telemetry::Event::Panic {
        site: hakus_telemetry::reduce_panic_site(
            location.file(),
            location.line(),
            location.column(),
        ),
...
/// Write a panic dump file to `~/.hakus/crashes/`.
///
...
/// panics, a crash dump is written to `~/.hakus/crashes/` and the panic
...
mod atomic_write_tests {
    ...
    /// Seal HOME / HAKUS_HOME to `tmp` so sweep policy is deterministic
    /// and never inspects the developer's real `~/.hakus`.
    fn seal_product_home(
        ...
        let guards = vec![
            ...
            EnvVarGuard::set("HAKUS_HOME", &product),
    ...
    fn a_private_atomic_write_in_a_product_dir_collects_strays() {
        ...
        assert!(
            super::is_hakus_owned_state_dir(&product),
            "sealed HAKUS_HOME must count as a product dir"
        );
    ...
    fn a_private_atomic_write_to_a_user_chosen_dest_does_not_sweep() {
        ...
        assert!(
            !super::is_hakus_owned_state_dir(&user_dir),
    ...
    #[test]
    fn atomic_write_product_dir_detection_matches_hakus_home_not_siblings() {
        let _lock = crate::test_support::lock_test_env();
        let tmp = tempfile::TempDir::new().expect("tempdir");
        ...
        assert!(super::is_hakus_owned_state_dir(&product));
        assert!(super::is_hakus_owned_state_dir(
            &product.join("sessions")
        ));
        assert!(!super::is_hakus_owned_state_dir(&sibling));
        assert!(!super::is_hakus_owned_state_dir(tmp.path()));
        drop(explicit_guards);

        use crate::test_support::EnvVarGuard;
        let _home = EnvVarGuard::set("HOME", tmp.path());
        let _userprofile = EnvVarGuard::set("USERPROFILE", tmp.path());
        let _no_explicit = EnvVarGuard::remove("HAKUS_HOME");
        let _no_config = EnvVarGuard::remove("HAKUS_CONFIG_PATH");
        let _no_legacy_config = EnvVarGuard::remove("DEEPSEEK_CONFIG_PATH");
        let _no_legacy_home = EnvVarGuard::remove("DEEPSEEK_HOME");

        assert!(super::is_hakus_owned_state_dir(
            &tmp.path().join(".hakus").join("sessions")
        ));
        assert!(super::is_hakus_owned_state_dir(
