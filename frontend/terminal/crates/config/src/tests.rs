...
fn permissions_toml_deserializes_action_deny_and_allow() {
    ...
    assert_eq!(
        ...
        hakus_execpolicy::PermissionAction::Deny
    );
    assert_eq!(
        permissions.rules[1].action,
        hakus_execpolicy::PermissionAction::Allow
    );
    assert_eq!(
        permissions.rules[2].action,
        hakus_execpolicy::PermissionAction::Ask
    ); // default
}
...
fn provider_secret_auth_source_deserializes() {
    ...
    assert_eq!(auth.secret_id.as_deref(), Some("hakus/openai"));
...
fn config_store_loads_sibling_permissions_toml() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "hakus-permissions-schema-{}-{unique}",
        std::process::id()
    ));
...
fn config_store_exec_policy_engine_uses_sibling_permissions() {
    ...
    let unique = SystemTime::now()
        ...
        .as_nanos();
    let dir = std::env::temp_dir().join(format!(
        "hakus-permissions-engine-{}-{unique}",
        std::process::id()
    ));
    fs::create_dir_all(&dir).expect("mkdir");
    let config_path = dir.join(CONFIG_FILE_NAME);
    ...
    let decision = store
        ...
        .check(hakus_execpolicy::ExecPolicyContext {
            ...
            path: None,
            ask_for_approval: hakus_execpolicy::AskForApproval::UnlessTrusted,
...
fn config_store_appends_exact_workspace_allow_rules() {
    ...
    let exact = store
        ...
        .check(hakus_execpolicy::ExecPolicyContext {
            ...
            tool: Some("exec_shell"),
            path: None,
            ask_for_approval: hakus_execpolicy::AskForApproval::OnRequest,
            sandbox_mode: Some("workspace-write"),
        })
        .expect("check exact grant");
    assert_eq!(
        exact.matched_action,
        Some(hakus_execpolicy::PermissionAction::Allow)
    );
...
fn config_store_rejects_broad_or_unscoped_allow_rules() {
    ...
    unscoped.action = hakus_execpolicy::PermissionAction::Allow;
    assert!(
        store
    ...
    prefix.action = hakus_execpolicy::PermissionAction::Allow;
...
impl EnvGuard {
    fn without_deepseek_runtime_overrides() -> Self {
        let guard = Self {
            ...
            hakus_provider: env::var_os("HAKUS_PROVIDER"),
            hakus_model: env::var_os("HAKUS_MODEL"),
            hakus_base_url: env::var_os("HAKUS_BASE_URL"),
...
impl hakus_secrets::KeyringStore for RecordingSecretsStore {
    fn get(&self, key: &str) -> Result<Option<String>, hakus_secrets::SecretsError> {
        self.gets.lock().unwrap().push(key.to_string());
        Ok(self.values.get(key).cloned().or_else(|| self.value.clone()))
    }

    fn set(&self, key: &str, _value: &str) -> Result<(), hakus_secrets::SecretsError> {
        self.sets.lock().unwrap().push(key.to_string());
        Ok(())
    }

    fn delete(&self, key: &str) -> Result<(), hakus_secrets::SecretsError> {
...
///      `hakus config set providers.moonshot.*` CLI invokes.
///   3. Switch the active provider through `HAKUS_PROVIDER` —
...
fn moonshot_kimi_code_smoke_config_set_then_resolve() -> Result<()> {
    ...
    // Same key paths a user would run via `hakus config set`.
...
fn project_merge_denies_credentials_endpoints_and_provider_selection() {
    ...
    // A repo-local `.hakus/config.toml` cannot aim telemetry at a host of
...
fn app_homes_prefer_home_env_before_platform_home_fallback() {
    let _lock = env_lock();
    let home =
        std::env::temp_dir().join(format!("hakus-config-home-env-{}", std::process::id()));
    let userprofile = std::env::temp_dir().join(format!(
        "hakus-config-userprofile-{}",
        std::process::id()
    ));
    let _env = StateEnvRestore {
        home: env::var_os("HOME"),
        userprofile: env::var_os("USERPROFILE"),
        hakus_home: env::var_os("HAKUS_HOME"),
    };
    ...
    // Safety: test-only environment mutation is serialized by env_lock().
    unsafe {
        env::set_var("HAKUS_HOME", &explicit);
    }
    assert_eq!(hakus_home().expect("explicit home"), explicit);
}

#[test]
fn relative_hakus_home_is_a_hard_error() {
    let _lock = env_lock();
    let _env = StateEnvRestore {
        home: env::var_os("HOME"),
        userprofile: env::var_os("USERPROFILE"),
    ...
    // Safety: test-only environment mutation is serialized by env_lock().
    unsafe {
        env::set_var("HAKUS_HOME", ".hakus");
    }

    let error = hakus_home().expect_err("relative global home must fail closed");
...
fn migrate_config_is_disabled_and_does_not_read_legacy_path() {
    ...
    let home = std::env::temp_dir().join(format!(
        "hakus-config-migration-{}-{unique}",
        std::process::id()
    ));
    let legacy_dir = home.join(LEGACY_APP_DIR);
    let primary_dir = home.join(HAKUS_APP_DIR);
...
#[test]
fn explicit_hakus_home_bypasses_legacy_config_fallback_and_migration() {
    let _lock = env_lock();
    let unique = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .expect("clock")
        .as_nanos();
    let home = std::env::temp_dir().join(format!(
        "hakus-config-explicit-isolation-{}-{unique}",
        std::process::id()
    ));
    ...
    let explicit_home = home.join("isolated-hakus");
    let _env = StateEnvRestore {
        home: env::var_os("HOME"),
        userprofile: env::var_os("USERPROFILE"),
        hakus_home: env::var_os("HAKUS_HOME"),
    ...
    unsafe {
        env::set_var("HOME", &home);
        env::set_var("USERPROFILE", &home);
        env::set_var("HAKUS_HOME", &explicit_home);
    }

    assert_eq!(
        default_config_path().expect("default config path"),
        explicit_home.join(CONFIG_FILE_NAME),
        "explicit HAKUS_HOME must not read ambient legacy config"
    );
    assert!(
        migrate_config_if_needed()
            .expect("migration check")
            .is_none(),
        "explicit HAKUS_HOME must not migrate ambient legacy config"
    );
    assert!(
        !explicit_home.join(CONFIG_FILE_NAME).exists(),
        "legacy config must not be copied into explicit HAKUS_HOME"
    );
...
impl Drop for StateEnvRestore {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            match self.hakus_home.take() {
                Some(value) => env::set_var("HAKUS_HOME", value),
                None => env::remove_var("HAKUS_HOME"),
            }
        }
    }
}
...
impl StateDirEnv {
    ...
    fn primary(&self, sub: &str) -> PathBuf {
        self.home.join(HAKUS_APP_DIR).join(sub)
    }
}
...
fn state_migration_notice_explains_migration_is_disabled() {
    let migration = StateMigration {
        ...
        primary_path: PathBuf::from("/home/alice/.hakus/sessions"),
...
fn copied_state_migration_notice_is_also_disabled() {
    let migration = StateMigration {
        ...
        primary_path: PathBuf::from("/home/alice/.hakus/catalog"),
...
#[test]
fn explicit_hakus_home_bypasses_legacy_state_fallback_and_migration() {
    let _lock = env_lock();
    let unique = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .expect("clock")
        .as_nanos();
    let state_env = StateDirEnv::install(unique);
    let explicit_home = state_env.home.join("isolated-hakus");
    // Safety: test-only environment mutation is serialized by env_lock().
    unsafe {
        env::set_var("HAKUS_HOME", &explicit_home);
    }
    ...
    assert_eq!(
        resolve_state_dir("catalog").expect("resolve"),
        primary,
        "explicit HAKUS_HOME must not read ambient legacy state"
    );
    ...
    assert!(
        ...
        "explicit HAKUS_HOME must not migrate ambient legacy state"
    );
    assert!(
        !primary.join("legacy.json").exists(),
        "legacy contents must not be copied into an explicit HAKUS_HOME"
    );
    let _ = fs::remove_dir_all(&state_env.home);
}
...
fn project_state_resolvers_reject_path_traversal_subdirs() {
    ...
    assert_eq!(
        created,
        canonical_workspace.join(HAKUS_APP_DIR).join("a/b")
    );
}
...
fn resolve_config_path_rejects_relative_env_path_before_cwd_resolution() {
    ...
    impl Drop for ConfigPathEnvGuard {
        fn drop(&mut self) {
            // Safety: test-only environment mutation is serialized by env_lock().
            unsafe {
                match self.hakus.as_ref() {
                    Some(value) => env::set_var("HAKUS_CONFIG_PATH", value),
                    None => env::remove_var("HAKUS_CONFIG_PATH"),
                }
    ...
    let _guard = ConfigPathEnvGuard {
        hakus: env::var_os("HAKUS_CONFIG_PATH"),
        deepseek: env::var_os("DEEPSEEK_CONFIG_PATH"),
    };

    // Safety: test-only environment mutation is serialized by env_lock().
    unsafe {
        env::set_var("HAKUS_CONFIG_PATH", "../config.toml");
...
fn load_project_config_rejects_symlinked_primary_config() {
    ...
    let primary_dir = workspace.path().join(HAKUS_APP_DIR);
...
fn load_project_config_keeps_unknown_provider_names_strict() {
    let workspace = tempfile::tempdir().expect("workspace tempdir");
    let config_dir = workspace.path().join(HAKUS_APP_DIR);
...
fn config_store_save_creates_one_time_backup_before_changed_write() {
    ...
    let unique = SystemTime::now()
        ...
        .as_nanos();
    let dir = std::env::temp_dir().join(format!(
        "hakus-config-backup-save-{}-{unique}",
        std::process::id()
    ));
    fs::create_dir_all(&dir).expect("mkdir");
    let path = dir.join(CONFIG_FILE_NAME);
...
fn config_store_save_preserves_comments_with_other_keys() {
    ...
    // then changes model via `hakus config set model`.
    let dir = tempfile::tempdir().expect("tempdir");
    let config_path = dir.path().join(CONFIG_FILE_NAME);
    fs::write(
        &config_path,
...
fn fireworks_and_together_base_url_and_auth_metadata() {
    ...
    assert_eq!(
        hakus_secrets::env_for("fireworks").as_deref(),
        Some("fw-test-key")
    );
    assert_eq!(
        hakus_secrets::env_for("together").as_deref(),
        Some("tg-test-key")
    );
    unsafe {
...
"#,
    ...
    assert_eq!(
        hakus_secrets::env_for("opencode-go").as_deref(),
        Some("go-env-key")
    );
    let resolved = config.resolve_runtime_options(&CliRuntimeOverrides::default());
    assert_eq!(resolved.base_url, DEFAULT_OPENCODE_GO_BASE_URL);
...
"#,
    ...
    assert_eq!(
        hakus_secrets::env_for("tokenhub").as_deref(),
...
fn first_party_provider_env_model_overrides_pass_through() {
    ...
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    unsafe {
        env::set_var("HAKUS_PROVIDER", "minimax");
...
fn legacy_dual_wire_env_provider_preserves_named_table_credentials() {
    let _lock = env_lock();
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    // HAKUS_PROVIDER must resolve legacy spellings to the same kind TOML
    ...
    for (slug, kind, table, base_url) in [
        ...
        unsafe {
            env::set_var("HAKUS_PROVIDER", slug);
        }
...
fn sakana_env_overrides_resolve_fugu_route() {
    ...
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    unsafe {
        env::set_var("HAKUS_PROVIDER", "sakana");
...
fn hakus_provider_env_switches_active_provider() {
    ...
    assert_eq!(
        ...
        ProviderSource::Env("HAKUS_PROVIDER")
    );
    assert_eq!(resolved.base_url, DEFAULT_KIMI_CODE_BASE_URL);
    assert_eq!(resolved.model, DEFAULT_KIMI_CODE_MODEL);
    assert_eq!(resolved.api_key.as_deref(), Some("kimi-code-key"));
...
/// When both `HAKUS_PROVIDER` and the legacy `DEEPSEEK_PROVIDER`
/// are set, the public alias wins — a user adopting `HAKUS_*` in a
/// fresh shell config is not tripped up by a stale legacy export still
/// living in their dotfiles.
#[test]
fn hakus_provider_env_wins_over_deepseek_provider_env() {
    let _lock = env_lock();
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    // Safety: test-only env mutation guarded by env_lock().
    unsafe {
...
/// `HAKUS_MODEL` is the user-facing env alias for picking a model
/// against the active provider. It must be honored by the runtime
/// resolver in place of `DEEPSEEK_MODEL`.
#[test]
fn hakus_model_env_alias_overrides_default_for_active_provider() {
    let _lock = env_lock();
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    // Safety: test-only env mutation guarded by env_lock().
    unsafe {
        env::set_var("HAKUS_PROVIDER", "moonshot");
        env::set_var("HAKUS_MODEL", "custom-kimi-test-model");
    }
    let config = ConfigToml::default();
...
#[test]
fn blank_hakus_model_env_alias_does_not_override_default_for_active_provider() {
    let _lock = env_lock();
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    // Safety: test-only env mutation guarded by env_lock().
    unsafe {
        env::set_var("HAKUS_PROVIDER", "moonshot");
        env::set_var("HAKUS_MODEL", "   ");
    }
    let config = ConfigToml::default();
...
fn config_file_resolves_above_env_and_keyring() {
    use hakus_secrets::KeyringStore;
    let _lock = env_lock();
    let _env = EnvGuard::without_deepseek_runtime_overrides();
    // Safety: env mutation guarded by env_lock().
    unsafe { std::env::set_var("DEEPSEEK_API_KEY", "env-key") };

    let store = std::sync::Arc::new(hakus_secrets::InMemoryKeyringStore::new());
...
struct TelemetryEnvGuard {
    ...
    hakus_endpoint: Option<OsString>,
...
impl TelemetryEnvGuard {
    fn take() -> Self {
        let lock = env_lock();
        let guard = Self {
            hakus: env::var_os("HAKUS_TELEMETRY"),
            ...
            hakus_endpoint: env::var_os("HAKUS_TELEMETRY_ENDPOINT"),
        ...
        unsafe {
            env::remove_var("HAKUS_TELEMETRY");
            env::remove_var("DEEPSEEK_TELEMETRY");
            env::remove_var(TELEMETRY_FLOOR_ENV);
            env::remove_var("HAKUS_TELEMETRY_ENDPOINT");
    ...
    fn set(&self, value: &str) {
        // Safety: test-only environment mutation guarded by the module mutex.
        unsafe {
            env::set_var("HAKUS_TELEMETRY", value);
        }
    }
    ...
    fn set_endpoint(&self, value: &str) {
        // Safety: test-only environment mutation guarded by the module mutex.
        unsafe {
            env::set_var("HAKUS_TELEMETRY_ENDPOINT", value);
        }
    }
...
impl Drop for TelemetryEnvGuard {
    fn drop(&mut self) {
        // Safety: test-only environment mutation guarded by the module mutex.
        unsafe {
            match self.hakus.take() {
                Some(value) => env::set_var("HAKUS_TELEMETRY", value),
                None => env::remove_var("HAKUS_TELEMETRY"),
            }
            ...
            match self.hakus_endpoint.take() {
                Some(value) => env::set_var("HAKUS_TELEMETRY_ENDPOINT", value),
                None => env::remove_var("HAKUS_TELEMETRY_ENDPOINT"),
            }
...
fn env_telemetry_off_is_a_floor_over_cli_on() {
    ...
    // `HAKUS_TELEMETRY=0`. Off is a floor, not one more precedence rung.
...
fn persisted_telemetry_off_is_a_floor_over_cli_on() {
    ...
    // `HAKUS_TELEMETRY=true`, which also outranked the child's own copy of
...
fn a_run_scoped_off_is_a_kill_switch_and_not_a_revocation() {
    // Regression: an explicit `HAKUS_TELEMETRY=0` marked the run as an
...
fn telemetry_consent_names_its_source() {
    ...
    // Nobody said anything: off, by default (HakusAgent fork).
...
fn config_display_for_telemetry_reports_resolved_consent_with_source() {
    ...
    assert_eq!(
        ...
        // HakusAgent fork default: telemetry off unless explicitly enabled.
        Some("off (default)")
    );
...
fn the_dispatcher_states_the_floor_rather_than_letting_the_child_infer_it() {
    ...
    // shipped default: both arrive as `HAKUS_TELEMETRY=false`. So the
...
fn telemetry_explicit_off_distinguishes_an_answer_from_the_default() {
    ...
    // Nobody said anything: default off (HakusAgent fork), no explicit opt-out.
    let resolved = ConfigToml::default().resolve_runtime_options(&CliRuntimeOverrides::default());
    assert!(!resolved.telemetry);
    assert!(!resolved.telemetry_explicit_off);
...
fn an_unconfigured_endpoint_resolves_to_the_shipped_default() {
    ...
    assert_eq!(
        ...
        "https://telemetry.hakus.net/v1/telemetry"
    );
    let resolved = ConfigToml::default().resolve_runtime_options(&CliRuntimeOverrides::default());
    assert_eq!(
    ...
    // HakusAgent fork: telemetry defaults OFF (upstream endpoint does not
    // exist here); explicit opt-in is required to turn it on.
    assert!(!resolved.telemetry);
    assert!(!resolved.telemetry_explicit_off);
}
...
///
/// `None` at the resolved layer is what `hakus-telemetry`'s client reads as
...
fn an_empty_endpoint_keeps_the_dry_run_sink_reachable() {
    ...
    // `HAKUS_TELEMETRY_ENDPOINT= hakus …` would silently keep sending.
    let config = ConfigToml {
        telemetry_endpoint: Some("https://collector.internal/v1/batch".to_string()),
        ..ConfigToml::default()
    };
