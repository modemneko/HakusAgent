...
mod hakus_agent_bridge;
pub mod modes;
mod harness;
pub use hakus_agent_bridge::api_key_for_provider as hakus_agent_yaml_api_key;
...
pub use hakus_execpolicy::ToolAskRule;
use hakus_execpolicy::{ExecPolicyEngine, PermissionAction, Ruleset};
use hakus_secrets::SecretSource;
pub use hakus_secrets::Secrets;
...
pub fn is_upstream_auth_header(name: &str) -> bool {
    ...
    // generated by Hakus itself. `auth_mode = "none"` is an endpoint
...
pub struct ProviderConfigToml {
    ...
    /// User-curated model list for this provider. These ids surface in the
    /// `/model` picker alongside catalog rows (unknown ids render as custom
    /// rows), letting one provider carry several switchable models without
    /// catalog support. `model` stays the single active choice.
    ...
    /// Hakus-owned xAI OAuth generation selected by config. The value is a
    /// validated basename under `$HAKUS_HOME/credentials`, never an
    /// arbitrary path.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub oauth_credential_generation: Option<String>,
    /// Preserve provider fields introduced by newer Hakus versions and by
...
pub struct ConfigToml {
    ...
    /// and `hakus-tui` can share a single config file.
    pub api_key: Option<String>,
    ...
    /// Native tool catalog controls shared with `hakus-tui`.
    #[serde(default)]
...
/// HakusAgent 对齐：Python `hakus.permission.PermissionMode`。
///
/// - `Ask`（默认）：每个危险工具调用都询问用户
/// - `Bypass`：全部放行（不提示）
/// - `DangerAuto`：`Bypass` 的显式别名（Python 侧 AUTO 改名遗留）
///
/// 与上游 execpolicy 的 `AskForApproval` 通过
/// [`PermissionMode::from_ask_for_approval`] / [`PermissionMode::to_ask_for_approval`]
/// 互转：`OnRequest`/`UnlessTrusted`/`OnFailure` → `Ask`（程度不同但都
/// 属"会问"），`Never` → `Bypass`。
...
pub enum PermissionMode {
    ...
    Ask,
    Bypass,
    #[serde(alias = "auto", alias = "danger_auto")]
    DangerAuto,
}

impl PermissionMode {
    /// 从上游 approval 策略收窄到 HakusAgent 三态。
    #[must_use]
    pub fn from_ask_for_approval(mode: &hakus_execpolicy::AskForApproval) -> Self {
        match mode {
            hakus_execpolicy::AskForApproval::Never => Self::Bypass,
            _ => Self::Ask,
        }
    }

    /// 投影回上游 approval 策略（`DangerAuto`/`Bypass` → `Never`）。
    #[must_use]
    pub fn to_ask_for_approval(self) -> hakus_execpolicy::AskForApproval {
        match self {
            Self::Ask => hakus_execpolicy::AskForApproval::OnRequest,
            Self::Bypass | Self::DangerAuto => hakus_execpolicy::AskForApproval::Never,
        }
    }
}

impl ConfigToml {
    /// HakusAgent 对齐：`hakus.providers.resolve_provider` 的 Rust 对应。
    ///
    /// 优先级（与 Python 侧一致）：
    /// 1. `explicit` — 调用方显式指定（CLI `--provider` / 请求参数）
    /// 2. `HAKUS_PROVIDER` 环境变量
    /// 3. 配置文件里的 `provider` 字段
    /// 4. `default` 兜底
    ///
    /// 返回小写 provider id，两端（TUI / 桌面 sidecar / CLI）共用同一条链。
    #[must_use]
    pub fn resolve_provider(&self, explicit: Option<&str>, default: &str) -> String {
        if let Some(explicit) = explicit.map(str::trim).filter(|value| !value.is_empty()) {
            return explicit.to_ascii_lowercase();
        }
        if let Ok(env) = std::env::var("HAKUS_PROVIDER")
            && !env.trim().is_empty()
        {
            return env.trim().to_ascii_lowercase();
        }
        let configured = self.provider.as_str();
...
impl ConfigToml {
    ...
    pub fn resolve_runtime_options_with_secrets(
        ...
        let (provider, provider_source) = if let Some(provider) = cli.provider {
            ...
            (
                provider,
                ProviderSource::Env(env.provider_source.unwrap_or("HAKUS_PROVIDER")),
            )
        } else {
        ...
        // Root `default_text_model` is the key `hakus model set` writes and
        ...
        // and therefore `hakus model resolve`, reported a provider default.
        //
        ...
        // `HAKUS_TELEMETRY=0` are run-scoped kill switches: they must stop
...
/// `HAKUS_TELEMETRY=0`, `telemetry = false`, and a recorded decline all
/// stop the session long before an endpoint is read.
///
/// An explicit value — `HAKUS_TELEMETRY_ENDPOINT` or `telemetry_endpoint` in
...
/// `$HAKUS_HOME/telemetry/dryrun.jsonl`, and no HTTP client is constructed.
pub const DEFAULT_TELEMETRY_ENDPOINT: &str = "https://telemetry.hakus.net/v1/telemetry";

/// The dispatcher's statement to the TUI child about *why* telemetry is off.
///
/// Private to the `hakus` → `hakus-tui` hop, in the same spirit as
/// `DEEPSEEK_API_KEY_SOURCE`. Set to `1`/`0` on every delegated run.
pub const TELEMETRY_FLOOR_ENV: &str = "HAKUS_TELEMETRY_FLOOR";
...
/// default, and the dispatcher forwards a resolved `HAKUS_TELEMETRY=false`
...
///
/// With no statement (a directly launched `hakus-tui`) the raw environment
...
pub fn telemetry_floor_in_force() -> bool {
    ...
    let Ok(raw) =
        std::env::var("HAKUS_TELEMETRY").or_else(|_| std::env::var("DEEPSEEK_TELEMETRY"))
    else {
        return false;
    };
...
pub enum TelemetrySource {
    ...
    Cli,
    /// `HAKUS_TELEMETRY`/`DEEPSEEK_TELEMETRY`, including the dispatcher's
...
fn read_telemetry_env() -> (Option<bool>, bool) {
    let Some(raw) = std::env::var("HAKUS_TELEMETRY")
    ...
    match parse_bool(&raw) {
        ...
        Err(_) => {
            tracing::warn!(
                "Invalid HAKUS_TELEMETRY/DEEPSEEK_TELEMETRY value '{raw}'; expected one of \
...
fn telemetry_consent_from_env(
    ...
    // HakusAgent fork: telemetry defaults OFF. Upstream shipped batches to
    // its own endpoint by default; in this fork that endpoint does not
    // exist, and default-on collection is not something we want. Opt in
    // via config or env if ever needed.
...
/// A provider owner that Hakus can identify with high confidence when an
...
pub fn provider_base_url_is_official(provider: ProviderKind, base_url: &str) -> bool {
    ...
    match provider {
        ...
        // Custom routes have no Hakus-owned official endpoint. The
...
fn env_api_key_for_provider(provider: ProviderKind) -> Option<String> {
    let from_env = if provider == ProviderKind::Huggingface {
        ...
        hakus_secrets::env_for(provider.as_str())
    };
    // HakusAgent repo bridge: when neither config.toml nor the process env
    // had a key, fall back to the repository's root `config.yaml` api_keys
    // map (located by walking up from the CWD / binary directory).
    from_env.or_else(|| hakus_agent_bridge::api_key_for_provider(provider.as_str()))
}
...
pub struct ResolvedRuntimeOptions {
    ...
    /// buffered events of a user who merely set `HAKUS_TELEMETRY=0` for one
    ...
    pub fn append_allow_rules(&mut self, rules: &[ToolAskRule]) -> Result<usize> {
        for rule in rules {
            ...
            let Some(workspace) = rule
                .workspace
                .as_deref()
                .and_then(hakus_execpolicy::normalize_workspace_scope)
            else {
            ...
            if let Some(path) = rule.path.as_deref()
                && hakus_execpolicy::normalize_workspace_relative_path(path, &workspace)
...
/// backend (e.g. an [`hakus_secrets::InMemoryKeyringStore`]) before
/// any resolver runs.
...
pub fn ensure_project_state_dir(workspace: &Path, subdir: &str) -> Result<PathBuf> {
    ensure_safe_state_subdir(subdir)?;
    let workspace = normalize_project_workspace(workspace)?;
    let dir = workspace.join(HAKUS_APP_DIR).join(subdir);
    std::fs::create_dir_all(&dir)
        .with_context(|| format!("failed to create {}/", dir.display()))?;
    Ok(dir)
}
...
pub fn resolve_config_path(explicit: Option<PathBuf>) -> Result<PathBuf> {
    ...
    if let Some(path) = hakus_paths::config_path_override().map_err(anyhow::Error::new)? {
        return normalize_config_file_path(path);
    }
    default_config_path()
}
...
/// happens to be named `.hakus`; other custom locations (for example
/// `HAKUS_CONFIG_PATH=~/team.toml` or an isolated test directory) stay
...
pub fn config_path_is_workspace_scoped(path: &Path) -> bool {
    config_path_is_workspace_scoped_with_context(
        ...
        hakus_paths::hakus_home_override()
            ...
            .flatten()
            .as_deref(),
        hakus_paths::user_home().as_deref(),
...
fn config_path_is_workspace_scoped_with_context(
    path: &Path,
    explicit_hakus_home: Option<&Path>,
    user_home: Option<&Path>,
    current_dir: Option<&Path>,
) -> bool {
    if let Some(home) = explicit_hakus_home
...
mod credential_scope_tests {
    ...
    fn checkout_config_outside_cwd_is_workspace_scoped_via_git_marker() {
        ...
        std::fs::create_dir_all(repo.join(".hakus")).expect("app dir");
        assert!(config_path_is_workspace_scoped_with_context(
            &repo.join(".hakus/config.toml"),
            None,
            Some(Path::new("/home/user")),
            Some(Path::new("/somewhere/else")),
        ));
    ...
    fn user_global_and_custom_locations_are_not_workspace_scoped() {
        ...
        let temp = tempfile::tempdir().expect("tempdir");
        assert!(!config_path_is_workspace_scoped_with_context(
            &temp.path().join(".hakus/config.toml"),
            None,
            Some(home),
            elsewhere,
        ));
    ...
    #[test]
    fn explicit_hakus_home_config_is_user_global_even_when_dir_is_app_named() {
        let temp = tempfile::tempdir().expect("tempdir");
        let repo = temp.path().join("repo");
        let explicit = repo.join(".hakus");
        // Even with cwd inside the repo, the explicit HAKUS_HOME config is
        ...
        assert!(config_path_is_workspace_scoped_with_context(
            &repo.join("other/.hakus/config.toml"),
            Some(&explicit),
            Some(Path::new("/home/user")),
...
fn permission_removal_token(path: &Path, raw: &str, index: usize) -> String {
    ...
    hasher.update(b"hakus-permission-removal-v1\0");
...
struct EnvRuntimeOverrides {
    ...
    /// `HAKUS_TELEMETRY`/`DEEPSEEK_TELEMETRY` was set to something
    ...
    /// `HAKUS_TELEMETRY_ENDPOINT`/`DEEPSEEK_TELEMETRY_ENDPOINT`. Overrides
...
impl EnvRuntimeOverrides {
    fn load() -> Self {
        ...
        Self {
            ...
            arcee_model: std::env::var("ARCEE_MODEL")
                .ok()
                .filter(|v| !v.trim().is_empty()),
            verbosity: std::env::var("HAKUS_VERBOSITY")
                .or_else(|_| std::env::var("DEEPSEEK_VERBOSITY"))
                .ok(),
            output_mode: std::env::var("HAKUS_OUTPUT_MODE")
                .or_else(|_| std::env::var("DEEPSEEK_OUTPUT_MODE"))
                .ok(),
            auth_mode: std::env::var("HAKUS_AUTH_MODE")
                .or_else(|_| std::env::var("DEEPSEEK_AUTH_MODE"))
                .ok(),
            log_level: std::env::var("HAKUS_LOG_LEVEL")
            ...
            // `HAKUS_TELEMETRY_ENDPOINT=` select the shipped endpoint —
            // the opposite of what anyone typing it means. Resolution reads an
            // empty override as "contact nobody, write the dry-run file".
            telemetry_endpoint: std::env::var("HAKUS_TELEMETRY_ENDPOINT")
                .or_else(|_| std::env::var("DEEPSEEK_TELEMETRY_ENDPOINT"))
                .ok(),
            approval_policy: std::env::var("HAKUS_APPROVAL_POLICY")
                .or_else(|_| std::env::var("DEEPSEEK_APPROVAL_POLICY"))
                .ok(),
            sandbox_mode: std::env::var("HAKUS_SANDBOX_MODE")
                .or_else(|_| std::env::var("DEEPSEEK_SANDBOX_MODE"))
                .ok(),
            yolo: std::env::var("HAKUS_YOLO")
                .or_else(|_| std::env::var("DEEPSEEK_YOLO"))
                .ok()
                .and_then(|v| match parse_bool(&v) {
                    Ok(b) => Some(b),
                    Err(_) => {
                        tracing::warn!("Invalid HAKUS_YOLO/DEEPSEEK_YOLO value '{v}', expected true/false");
                        None
                    }
                }),
            http_headers: std::env::var("HAKUS_HTTP_HEADERS")
                .or_else(|_| std::env::var("DEEPSEEK_HTTP_HEADERS"))
                .ok()
                .and_then(|value| match parse_http_headers(&value) {
                    Ok(h) => Some(h),
                    Err(_) => {
                        tracing::warn!("Invalid HAKUS_HTTP_HEADERS/DEEPSEEK_HTTP_HEADERS value, expected format: header1=val1,header2=val2");
                        None
                    }
                })
                .filter(|headers| !headers.is_empty()),
            deepseek_base_url: std::env::var("HAKUS_BASE_URL")
    ...
    fn load_provider() -> (Option<ProviderKind>, Option<&'static str>) {
        if let Ok(value) = std::env::var("HAKUS_PROVIDER") {
            let parsed = ProviderKind::parse_config_identity(&value);
            return (parsed, parsed.map(|_| "HAKUS_PROVIDER"));
        }
    ...
    fn base_url_for(&self, provider: ProviderKind) -> Option<String> {
        ...
        match provider {
            ...
            // No dedicated HAKUS_CUSTOM_BASE_URL env override: a custom
