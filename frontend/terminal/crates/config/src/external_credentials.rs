...
/// The side-effect envelope Hakus may use for an external credential.
...
fn agy_profile_base() -> PathBuf {
    match std::env::var_os("APPDATA") {
        Some(value) if !value.is_empty() => PathBuf::from(value),
        _ => hakus_paths::user_home().unwrap_or_else(|| PathBuf::from(".")),
    }
    .join("Antigravity")
}
...
pub fn external_credential_consent_status(
    ...
    let semantics = match access {
        ExternalCredentialAccess::Disabled => {
            "disabled; no external-credential probing, reading, refresh, discovery, identity-provider or network acquisition, writes, or rewrites; normal requests to the explicitly selected provider may use Hakus-owned credentials"
        }
    ...
    ExternalCredentialConsentStatus {
        ...
        revoke_command: format!(
            "hakus auth external-revoke --provider {}",
            provider.as_str()
        ),
    }
}
