...
pub enum CredentialAcquisition {
    ...
    LocalOptional,
    /// An OAuth-only route; Hakus does not collect an API key for it.
    OAuth,
...
pub const fn credential_help(kind: ProviderKind) -> CredentialHelp {
    ...
    match kind {
        ...
        ProviderKind::OpenaiCodex => CredentialHelp {
            ...
            guidance: "Run `codex login`, then explicitly grant Hakus read-only access to that exact Codex credential file; or use a process-scoped token environment variable.",
        },
        ...
        ProviderKind::Xai => CredentialHelp {
            ...
            docs_url: None,
            guidance: "Use an xAI Console API key or Hakus's native device login. Reading an existing Grok CLI file requires explicit provider-scoped read-only consent.",
        },
        ...
        ProviderKind::Antigravity => CredentialHelp {
            ...
            guidance: "Sign in with the official agy CLI (1.1.13). Hakus can read that login's token read-only from the exact pinned state.vscdb after `hakus auth external-consent`; it never writes or refreshes it. An ANTIGRAVITY_API_KEY or AGY_ADC_AUTH in the process wins over the file.",
        },
        ProviderKind::Google => CredentialHelp {
            ...
            guidance: "Create a Google AI Studio API key. Hakus uses the official Gemini OpenAI-compatible endpoint and never reads Google OAuth files.",
        },
...
pub fn credential_help_for_route(kind: ProviderKind, base_url: &str) -> CredentialHelp {
    ...
    if is_exact_kimi_code_route(kind, base_url) {
        return CredentialHelp {
            ...
            docs_url: None,
            guidance: "Create a Kimi Code membership-plan API key in the Kimi Code console. This route uses api.kimi.com/coding/v1; Hakus does not import Kimi CLI credentials.",
        };
    }
