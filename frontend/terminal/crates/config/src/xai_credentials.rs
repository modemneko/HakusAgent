//! Naming and cleanup policy for Hakus-owned xAI OAuth generations.
//!
...
/// Stable handle to Hakus's private xAI OAuth directory.
///
/// The lexical `$HAKUS_HOME/credentials` boundary is retained verbatim.
...
pub fn validate_xai_oauth_generation(value: &str) -> Result<&str> {
    if !is_valid_xai_oauth_generation(value) {
        bail!(
            "invalid Hakus-owned xAI OAuth generation; expected xai-auth-<32 lowercase hex>.json"
        );
    }
    Ok(value)
}
...
pub fn xai_oauth_credentials_dir() -> Result<PathBuf> {
    lexical_absolute_path(&crate::hakus_home()?.join("credentials"))
}
...
/// `credentials` symlink would erase the lexical Hakus-owned boundary and
...
fn lexical_absolute_path(path: &Path) -> Result<PathBuf> {
    let absolute = if path.is_absolute() {
        ...
        std::env::current_dir()
            .context("resolving the Hakus credentials directory")?
            .join(path)
    };
    if absolute
        .components()
    ...
    {
        bail!(
            "Hakus credentials directory must be lexically normalized: {}",
...
/// Serialize every Hakus-owned xAI OAuth lifecycle mutation across threads
...
/// from concurrent Hakus readers. A failed authority mutation restores the
...
impl XaiOAuthCredentialStore {
    ...
    pub fn read_to_string(&self, name: &str) -> Result<Option<String>> {
        ...
        if metadata.len() > XAI_OAUTH_FILE_LIMIT {
            bail!(
                "Hakus-owned xAI OAuth file {} exceeds the {} byte limit",
        ...
        (&mut file)
            ...
            .with_context(|| {
                format!(
                    "reading Hakus-owned xAI OAuth file {}",
                    crate::quote_os_path(&self.directory.join(name))
                )
            })?;
        ...
        String::from_utf8(bytes).map(Some).map_err(|_| {
            anyhow::anyhow!(
                "Hakus-owned xAI OAuth file {} is not valid UTF-8",
                crate::quote_os_path(&self.directory.join(name))
            )
        })
    }
    ...
    /// stale Hakus reader from using it while authority changes.
    pub fn stage_revocation(&self) -> Result<XaiOAuthRevocation> {
        #[cfg(windows)]
        {
            // Every Hakus reader/writer takes the lifecycle lock, so a
...
fn owned_auth_names_in_store(store: &XaiOAuthCredentialStore) -> Result<Vec<String>> {
    ...
    if duplicated < 0 {
        return Err(std::io::Error::last_os_error())
            .context("duplicating Hakus credentials directory handle");
    }
    ...
    if stream.is_null() {
        ...
        return Err(error).context("enumerating Hakus credentials directory");
    }
    ...
    if unsafe { libc::closedir(stream) } != 0 {
        return Err(std::io::Error::last_os_error())
            .context("closing Hakus credentials directory enumeration");
    }
    names.sort();
    Ok(names)
}
...
fn owned_auth_names_in_store(store: &XaiOAuthCredentialStore) -> Result<Vec<String>> {
    ...
    let entries = fs::read_dir(&store.directory).with_context(|| {
        format!(
            "failed to inspect Hakus credentials directory {}",
            crate::quote_os_path(&store.directory)
        )
    })?;
...
fn validate_owned_auth_name(name: &str) -> Result<()> {
    anyhow::ensure!(
        ...
        "invalid Hakus-owned xAI OAuth basename"
    );
    Ok(())
}
...
fn open_owned_credentials_directory(directory: &Path) -> Result<XaiOAuthCredentialStore> {
    ...
    for component in directory.components() {
        let Component::Normal(name) = component else {
            ...
            bail!(
                "Hakus credentials directory has an unsupported component: {}",
            ...
            if created != 0 {
                ...
                if error.kind() != std::io::ErrorKind::AlreadyExists {
                    return Err(error).with_context(|| {
                        format!(
                            "creating a component of Hakus credentials directory {}",
                            crate::quote_os_path(directory)
                        )
                    });
                }
        ...
        if fd < 0 {
            return Err(std::io::Error::last_os_error()).with_context(|| {
                format!(
                    "opening Hakus credentials directory without following links: {}",
                    crate::quote_os_path(directory)
                )
            });
        }
    ...
    let metadata = current.metadata().with_context(|| {
        format!(
            "inspecting Hakus credentials directory {}",
            crate::quote_os_path(directory)
        )
    })?;
    anyhow::ensure!(
        metadata.is_dir(),
        "Hakus credentials path must be a directory"
    );
    anyhow::ensure!(
        metadata.uid() == unsafe { libc::geteuid() },
        "Hakus credentials directory must be owned by the current user"
    );
    current
        .set_permissions(fs::Permissions::from_mode(0o700))
        .with_context(|| {
            format!(
                "securing Hakus credentials directory {}",
                crate::quote_os_path(directory)
            )
        })?;
...
impl XaiOAuthCredentialStore {
    fn open_at(&self, name: &str, flags: i32, mode: libc::mode_t) -> Result<Option<File>> {
        ...
        if fd < 0 {
            ...
            return Err(error).with_context(|| {
                format!(
                    "opening Hakus-owned xAI OAuth path {}",
    ...
    fn write_owned_file(&self, name: &str, bytes: &[u8], allow_replace: bool) -> Result<()> {
        ...
        let result = (|| -> Result<()> {
            ...
            self.directory_handle
                .sync_all()
                .context("syncing Hakus credentials directory")?;
            Ok(())
        })();
        drop(temp);
        if result.is_err() {
    ...
    fn remove_raw(&self, name: &str) -> Result<bool> {
        ...
        if unsafe { libc::unlinkat(self.directory_handle.as_raw_fd(), name.as_ptr(), 0) } != 0 {
            ...
            return Err(error).context("removing Hakus-owned xAI OAuth file");
        }
        Ok(true)
    }
...
fn validate_owned_file_handle(file: &File, path: &Path) -> Result<fs::Metadata> {
    ...
    let metadata = file.metadata().with_context(|| {
        format!(
            "inspecting Hakus-owned xAI OAuth file {}",
...
fn open_owned_credentials_directory(directory: &Path) -> Result<XaiOAuthCredentialStore> {
        ...
        match component {
            ...
            Component::Normal(name) => {
                ...
                let handle = options.open(&current).with_context(|| {
                    format!(
                        "opening Hakus credentials directory component {}",
    ...
    anyhow::ensure!(
        !handles.is_empty(),
        "Hakus credentials directory cannot be a volume root"
    );
    ...
    let final_directory = secure_options.open(directory).with_context(|| {
        format!(
            "opening Hakus credentials directory for owner-only security: {}",
            crate::quote_os_path(directory)
        )
    })?;
    validate_windows_handle_path(&final_directory, directory, true)?;
    secure_windows_owner_only_handle(&final_directory, true)
        .context("securing Hakus credentials directory for the current user")?;
    verify_windows_owner_only_handle(&final_directory)
        .context("verifying Hakus credentials directory ownership")?;
        ...
        for _ in 0..8 {
            ...
            let file = match options.open(&path) {
                ...
                Err(error) => {
                    return Err(error).with_context(|| {
                        format!(
                            "creating Hakus-owned xAI OAuth lifecycle lock {}",
                            crate::quote_os_path(&path)
                        )
                    });
                }
...
fn validate_owned_file_handle(file: &File, path: &Path) -> Result<fs::Metadata> {
    ...
    verify_windows_owner_only_handle(file)
        .context("Hakus-owned xAI OAuth file is not current-user-only")?;
    Ok(metadata)
}
...
fn validate_windows_handle_path(
    ...
    let metadata = file.metadata().with_context(|| {
        format!(
            "inspecting Hakus-owned path {}",
            crate::quote_os_path(expected)
        )
    })?;
    anyhow::ensure!(
        metadata.file_attributes() & FILE_ATTRIBUTE_REPARSE_POINT == 0,
        "Hakus-owned xAI OAuth path must not be a reparse point"
    );
    anyhow::ensure!(
        if expect_directory {
            metadata.is_dir()
        ...
        "Hakus-owned xAI OAuth path has the wrong filesystem type"
    );
    ...
    if needed == 0 {
        return Err(std::io::Error::last_os_error())
            .context("resolving Hakus-owned xAI OAuth handle path");
    }
    ...
    anyhow::ensure!(
        ...
        "Hakus-owned xAI OAuth path was redirected while opening"
    );
    Ok(metadata)
}
...
fn secure_windows_owner_only_handle(file: &File, inherit_to_children: bool) -> Result<()> {
    ...
    if result != ERROR_SUCCESS {
        return Err(std::io::Error::from_raw_os_error(result as i32))
            .context("building a current-user-only DACL for Hakus-owned xAI OAuth storage");
    }
    ...
    if result != ERROR_SUCCESS {
        return Err(std::io::Error::from_raw_os_error(result as i32))
            .context("applying a current-user-only DACL to Hakus-owned xAI OAuth storage");
    }
    Ok(())
}
...
fn verify_windows_owner_only_handle(file: &File) -> Result<()> {
    ...
    if result != ERROR_SUCCESS {
        return Err(std::io::Error::from_raw_os_error(result as i32))
            .context("reading Hakus-owned xAI OAuth security descriptor");
    }
    let _descriptor = WindowsLocalAllocation(descriptor.cast());
    anyhow::ensure!(
        !owner.is_null() && unsafe { EqualSid(owner, user.sid()) } != 0,
        "Hakus-owned xAI OAuth storage owner is not the current user"
    );
    anyhow::ensure!(
        !dacl.is_null(),
        "Hakus-owned xAI OAuth storage must have an owner-only DACL"
    );
    let mut count = 0;
    ...
    if result != ERROR_SUCCESS {
        return Err(std::io::Error::from_raw_os_error(result as i32))
            .context("reading Hakus-owned xAI OAuth DACL entries");
    }
    let _entries = WindowsLocalAllocation(entries.cast());
    anyhow::ensure!(
        count == 1 && !entries.is_null(),
        "Hakus-owned xAI OAuth DACL must grant only one user"
    );
    ...
    anyhow::ensure!(
        ...
        "Hakus-owned xAI OAuth DACL is not current-user-only"
    );
    Ok(())
}
...
/// Explicit logout policy: remove the legacy Hakus-owned file and every
