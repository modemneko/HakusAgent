...
//! `$HAKUS_HOME`, written atomically through [`crate::persistence`] so it is
...
/// Filename of the setup-state sidecar under `$HAKUS_HOME`.
...
pub enum ConstitutionChoice {
    ...
    /// (`$HAKUS_HOME/prompts/constitution.md` + opt-in env).
    ExpertOverride,
...
pub enum ConstitutionSource {
    ...
    #[default]
    Bundled,
    /// A structured `constitution.json` under `$HAKUS_HOME`.
    UserGlobal,
...
impl SetupState {
    ...
    /// Path to the setup-state sidecar under `$HAKUS_HOME`.
    pub fn path() -> Result<PathBuf> {
        Ok(crate::hakus_home()?.join(SETUP_STATE_FILE_NAME))
    }
