...
pub const CHECKSUM_MANIFEST_ASSET: &str = "hakus-artifacts-sha256.txt";
...
pub const CNB_REPO_URL: &str = "https://cnb.cool/hakus.net/hakus";

/// Environment variable that overrides the base URL for release asset downloads.
pub const RELEASE_BASE_URL_ENV: &str = "HAKUS_RELEASE_BASE_URL";
...
pub const CNB_MIRROR_ENV: &str = "HAKUS_USE_CNB_MIRROR";
...
pub const UPDATE_USER_AGENT: &str = "hakus-updater";

const CNB_RELEASE_ASSET_BASE: &str = "https://cnb.cool/hakus.net/hakus/-/releases/download";
...
/// CNB mirror if `HAKUS_USE_CNB_MIRROR=1`. Returns `None` when no override
/// is configured.
...
/// True when `HAKUS_USE_CNB_MIRROR` is the override actually in effect —
        ...
        "GitHub release downloads may be blocked or slow on this network.\n\
         For mainland China, use one of these fallback paths:\n\
           1. Source build from the CNB mirror, installing the shipped binary:\n\
              cargo install --git {CNB_REPO_URL} --tag vX.Y.Z hakus-cli --locked --force\n\
           2. Use a binary asset mirror:\n\
              {RELEASE_BASE_URL_ENV}=https://<mirror>/<release-assets>/ {UPDATE_VERSION_ENV}=X.Y.Z hakus update\n\
         The mirror directory must contain {CHECKSUM_MANIFEST_ASSET} and the hakus platform binary."
    )
}
...
mod tests {
    ...
    fn release_base_url_from_env_uses_cnb_mirror_last() {
        ...
        assert_eq!(
            ...
            Some("https://cnb.cool/hakus.net/hakus/-/releases/download/v1.2.3".to_string())
        );
    ...
    fn cnb_mirror_override_is_active_only_without_an_explicit_base_url() {
        ...
        for disabled in ["", "0", "true", "yes", " 1 "] {
            ...
            assert!(
                !cnb_mirror_override_active(),
                "only HAKUS_USE_CNB_MIRROR=1 may force CNB, got {disabled:?}"
            );
        }
    }
    ...
    fn update_network_fallback_hint_mentions_required_mirror_inputs() {
        ...
        assert!(
            hint.contains("hakus platform binary"),
            "hint must describe the sole implementation asset"
        );
        assert!(
            !hint.contains("hakus-tui"),
