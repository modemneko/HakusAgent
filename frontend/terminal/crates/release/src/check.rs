...
/// directory (`~/.hakus/update-check.json` by default).
...
const OPT_OUT_ENV: &[&str] = &["HAKUS_NO_UPDATE_CHECK", "NO_UPDATE_NOTIFIER"];
...
#[must_use]
pub fn cache_path_in(hakus_home: &Path) -> PathBuf {
    hakus_home.join(UPDATE_CHECK_CACHE_FILE)
}
...
mod tests {
    ...
    fn suppression_reason_names_the_responsible_variable() {
        assert_eq!(
            SuppressionReason::OptOut("HAKUS_NO_UPDATE_CHECK").variable(),
            "HAKUS_NO_UPDATE_CHECK"
        );
        assert_eq!(
