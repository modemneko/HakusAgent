...
/// today (`hakus_workflow_js::WORKFLOW_MAX_CONCURRENT`, from which the tui
/// driver sizes its per-run admission semaphore). This crate cannot import
/// that constant directly because `hakus-workflow-js` depends on
/// `hakus-workflow`.
///
...
impl ResolvedSearchConcurrency {
    ...
    /// `hakus_config::WorkflowConfigToml::max_concurrent`) and a Fleet
    /// profile's `delegation.max_concurrency` hint
    /// (`hakus_config::FleetDelegationHints::max_concurrency`).
    ///
