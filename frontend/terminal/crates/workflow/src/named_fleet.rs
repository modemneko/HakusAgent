...
/// and `hakus_home/glm-pair` are different Fleets, and the loader refuses
/// to guess between them for exact definitions.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FleetSearchRoot {
    /// Non-secret origin label, e.g. `workspace` or `hakus_home`.
    pub origin: String,
...
impl FleetDocument {
    ...
    /// once any candidate is an exact fleet: a personal `~/.hakus` Fleet
