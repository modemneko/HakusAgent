...
fn a_router_defined_in_two_origins_is_ambiguous_until_qualified() {
    ...
    let roots = vec![
        FleetSearchRoot::new("hakus_home", &home),
    ...
    assert_eq!(
        ReasoningRouterProfile::load_by_name("hakus_home/luna-low", &roots)
            .expect("qualified")
            .0
            .model,
        "gpt-5.6-luna-mini"
...
fn the_routing_summary_is_transmitted_once_and_disclosed_without_content() {
    ...
    let input = hakus_workflow::RouterCallInput {
        fleet: "workspace/glm-pair".to_string(),
        member_id: "implementer".to_string(),
        frozen: hakus_workflow::FrozenRoute {
...
/// A personal `~/.hakus` Fleet must not silently shadow — or be shadowed
...
fn an_exact_fleet_defined_in_two_origins_is_ambiguous_until_qualified() {
    ...
    let (home_document, home_id) =
        FleetDocument::load_by_name("hakus_home/glm-pair", &roots).expect("qualified load");
    assert_eq!(home_id.qualified(), "hakus_home/glm-pair");
    assert_eq!(
        home_document
            .exact()
            .expect("exact")
...
fn legacy_fleets_in_two_origins_keep_first_hit_wins() {
    ...
    assert!(document.is_legacy());
    assert_eq!(id.origin, "hakus_home");
    assert_eq!(
...
fn a_legacy_inline_router_still_works_and_normalizes() {
    ...
    let exact = document.exact().expect("exact");
    let captured = hakus_workflow::captured_legacy_inline_router(exact).expect("inline router");
