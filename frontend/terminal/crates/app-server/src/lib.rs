...
use hakus_config::{CliRuntimeOverrides, ConfigStore};
use hakus_core::Runtime;
use hakus_hooks::{HookDispatcher, JsonlHookSink, StdoutHookSink, UnixSocketHookSink};
...
///
/// The headless runtime emits [`hakus_protocol::EventFrame::UserInputRequest`]
...
struct AppState {
    ...
    config: Arc<RwLock<hakus_config::ConfigToml>>,
    ...
    let approval_mode = {
        ...
        cfg.approval_policy
            ...
            .and_then(|p| match p.trim().to_ascii_lowercase().as_str() {
                "auto" | "yolo" => Some(hakus_execpolicy::AskForApproval::UnlessTrusted),
                "never" | "deny" => Some(hakus_execpolicy::AskForApproval::Never),
                _ => None,
            })
            .unwrap_or(hakus_execpolicy::AskForApproval::OnRequest)
    };
...
fn resolve_auth_token(options: &AppServerOptions) -> Result<Option<String>> {
    ...
    if !has_explicit_token && !options.listen.ip().is_loopback() {
        bail!(
            "refusing non-loopback app-server bind without explicit auth token; pass --auth-token or set HAKUS_APP_SERVER_TOKEN"
        );
    }
...
fn app_server_auth_status_lines(has_explicit_token: bool) -> Vec<&'static str> {
    ...
    vec![
        ...
        "  Pass --auth-token or set HAKUS_APP_SERVER_TOKEN when another client needs to connect.",
    ]
}
...
async fn interrupt_stdio_turn(
    ...
    let mut request = hakus_release::platform_http_client_builder()
...
impl RuntimeBridge {
    async fn start(config_path: Option<&Path>) -> Result<Self> {
        ...
        let mut bridge = Self {
            ...
            client: hakus_release::platform_http_client_builder()
                .build()
    ...
    fn runtime_command(config_path: Option<&Path>, port: u16, auth_token: &str) -> Result<Command> {
        ...
        // The TUI/runtime server already accepts HAKUS_RUNTIME_TOKEN /
        ...
        command
            ...
            .arg("--port")
            .arg(port.to_string())
            .env("HAKUS_RUNTIME_TOKEN", auth_token)
    ...
    match req {
        AppRequest::Capabilities => AppResponse {
            ...
            data: json!({
                ...
                "events": ["response_start", "text_delta", "response_end", "tool_call_start", "tool_call_result", "mcp_startup_update", "mcp_startup_complete"],
        ...
        AppRequest::ThreadLoadedList => {
            ...
            let response = runtime
                .handle_thread(hakus_protocol::ThreadRequest::List(
                    hakus_protocol::ThreadListParams {
...
async fn apply_config_update(
    state: &AppState,
    snapshot: hakus_config::ConfigToml,
    exec_policy: Option<hakus_execpolicy::ExecPolicyEngine>,
    persist: bool,
) {
...
async fn persist_config(state: &AppState, config: hakus_config::ConfigToml) -> Result<()> {
...
mod tests {
    ...
    async fn stdio_runtime_bridge_streams_response_delta_events() {
        ...
        assert_eq!(
            event_types,
            vec!["response_start", "text_delta", "response_end"]
        );
    ...
    async fn stdio_runtime_bridge_applies_thread_start_hints() {
        async fn create_thread(Json(body): Json<Value>) -> Json<Value> {
            ...
            assert_eq!(body["workspace"], "/tmp/hakus-stdio");
            Json(json!({
                "id": "thr_runtime",
        ...
        let runtime_id = bridge
            .ensure_runtime_thread(
                ...
                Some(RuntimeThreadHint {
                    ...
                    workspace: Some(PathBuf::from("/tmp/hakus-stdio")),
                }),
            )
            .await
    ...
    fn runtime_bridge_command_keeps_auth_token_out_of_argv() {
        ...
        assert!(
            envs.iter()
                .any(|(k, v)| k == "HAKUS_RUNTIME_TOKEN" && v == token),
            "token must be carried via HAKUS_RUNTIME_TOKEN: {envs:?}"
        );
        assert!(
            envs.iter()
    ...
    fn generated_auth_status_does_not_render_token() {
        ...
        assert!(rendered.contains("HAKUS_APP_SERVER_TOKEN"));
    }
