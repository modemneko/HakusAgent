...
fn system_backend_diagnostic_is_literal_unknown_and_not_probed() {
    ...
    let _home = EnvVarGuard::set("HAKUS_HOME", tmp.path());
...
fn file_backend_diagnostic_reads_metadata_not_secret_contents() {
    ...
    clear_known_envs();
    let tmp = tempfile::tempdir().unwrap();
    let hakus_home = std::fs::canonicalize(tmp.path())
        .unwrap()
        .join("isolated-home");
    let path = hakus_home.join("secrets").join("secrets.json");
...
fn absent_file_backend_diagnostic_creates_no_state() {
    ...
    clear_known_envs();
    let tmp = tempfile::tempdir().unwrap();
    let hakus_home = tmp.path().join("never-created-home");
    let path = hakus_home.join("secrets").join("secrets.json");
    let _home = EnvVarGuard::set("HAKUS_HOME", &hakus_home);
    let _backend = EnvVarGuard::set(SECRET_BACKEND_ENV, "file");
...
fn file_backend_diagnostic_rejects_a_symlinked_parent() {
    ...
    let _home = EnvVarGuard::set("HAKUS_HOME", &linked_home);
    let _backend = EnvVarGuard::set(SECRET_BACKEND_ENV, "file");
