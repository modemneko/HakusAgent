//! Shared secure-storage contract for Hakus account sessions.
//!
...
pub const DEFAULT_ACCOUNT_API_BASE: &str = "https://api.hakus.net";
/// Environment variable that selects the account API origin.
pub const ACCOUNT_API_BASE_ENV: &str = "HAKUS_CLOUD_API_BASE";
/// Explicit opt-in for storing account sessions in the private local file.
pub const ACCOUNT_ALLOW_FILE_SESSION_STORE_ENV: &str = "HAKUS_CLOUD_ALLOW_FILE_SESSION_STORE";
/// OS credential-manager service shared by CLI, TUI, and Runtime API.
pub const ACCOUNT_KEYRING_SERVICE: &str = "hakus-cloud";
...
pub struct AccountSession {
    /// Durable session identifier shared across Hakus surfaces.
    pub id: String,
...
pub struct AccountUser {
    /// Stable Hakus account identifier.
    #[serde(default)]
    pub id: String,
...
/// Versioned secure-storage envelope shared by every local Hakus surface.
///
...
pub enum AccountSessionError {
    ...
    #[error("the local Hakus account session is unreadable")]
    UnreadableRecord(#[source] serde_json::Error),
    /// Stored or newly returned authentication credentials are malformed.
    #[error("the Hakus account session contains invalid credentials")]
    InvalidCredentials,
    /// No approved secure session backend is available.
    #[error(
        "Hakus account sessions require an OS credential manager; set {ACCOUNT_ALLOW_FILE_SESSION_STORE_ENV}=1 only to explicitly opt into the private local file"
    )]
...
pub fn account_auth_slot(profile: &str, api_base: &str) -> String {
    ...
    format!("hakus-cloud-auth-v1-{encoded}")
}
...
mod tests {
    ...
    fn runtime_receipt_is_token_free_and_preserves_only_explicit_scopes() {
        ...
        let store = AccountSessionStore::new(secrets, Some("work"), "https://api.hakus.net");
        store
            .save(auth(
                "acct-1",
                "session-1",
    ...
    fn profiles_and_origins_preserve_same_account_and_cross_account_isolation() {
        let (secrets, _) = test_store();
        let default = AccountSessionStore::new(secrets.clone(), None, "https://api.hakus.net");
        let work =
            AccountSessionStore::new(secrets.clone(), Some("work"), "https://api.hakus.net");
