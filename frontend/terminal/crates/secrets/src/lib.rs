...
use hakus_paths::hakus_home_is_explicit;
...
pub const SECRET_BACKEND_ENV: &str = "HAKUS_SECRET_BACKEND";
/// Legacy alias for [`SECRET_BACKEND_ENV`].
pub const LEGACY_SECRET_BACKEND_ENV: &str = "DEEPSEEK_SECRET_BACKEND";
const FILE_BACKEND_LABEL: &str = "file-based (~/.hakus/secrets/)";
...
/// a JSON file under `~/.hakus/secrets/` ([`FileKeyringStore`]), or an
...
pub trait KeyringStore: Send + Sync {
    ...
    /// storage backend is active. Examples: `"file-based (~/.hakus/secrets/)"`,
...
/// at `<home>/.hakus/secrets/secrets.json` with Unix file mode `0600`
...
fn default_hakus_secrets_path() -> Result<PathBuf, SecretsError> {
    Ok(hakus_paths::hakus_home()
        .map_err(|error| {
...
impl Secrets {
    ...
    pub fn file_backed_read_only() -> Self {
        ...
        // `.hakus-secrets.json` beside the working directory must not
            ...
            Err(err) => {
                tracing::error!(
                    "could not resolve the file-backed secret path ({err}); credentials will not be read. \
                     Fix: set HAKUS_HOME to an absolute path or make HOME/USERPROFILE resolvable"
                );
                Self::read_only_empty_store()
            }
        }
...
mod tests {
    ...
    /// `cargo test -p hakus-secrets -- --ignored` on a desktop machine:
    ...
    fn probe_performs_a_real_backend_read() {
        let store = DefaultKeyringStore::new("hakus-probe-live-check");
        store
            .probe()
    ...
    #[test]
    fn read_only_auto_detect_respects_explicit_hakus_home_isolation() {
        let _lock = env_lock();
        clear_known_envs();
        let tmp = tempfile::tempdir().unwrap();
        let hakus_home = tmp.path().join("isolated-hakus-home");
        let _home = EnvVarGuard::set("HOME", tmp.path());
        let _userprofile = EnvVarGuard::set("USERPROFILE", tmp.path());
        ...
        let legacy = tmp
            ...
            .join("secrets")
            .join("secrets.json");
        let primary = hakus_home.join("secrets").join("secrets.json");
        FileKeyringStore::new(&legacy)
        ...
        assert_eq!(
            ...
            None,
            "an explicit HAKUS_HOME must not read ambient legacy secrets"
        );
        assert!(
            !primary.exists(),
    ...
    fn file_backed_read_only_never_reads_a_cwd_relative_store() {
        ...
        // used to fall back to a planted `.hakus-secrets.json` in the cwd.
        let _hakus_home = EnvVarGuard::set("HAKUS_HOME", "relative-hakus-home");
        let planted = tmp.path().join(".hakus-secrets.json");
        std::fs::write(
            &planted,
    ...
    #[test]
    fn file_default_path_uses_hakus_home() {
        let _lock = env_lock();
        clear_known_envs();
        let tmp = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("HOME", tmp.path());
    ...
    #[test]
    fn file_default_path_honors_hakus_home() {
        let _lock = env_lock();
        clear_known_envs();
        let tmp = tempfile::tempdir().unwrap();
        let custom = tmp.path().join("custom-hakus");
        let _home = EnvVarGuard::set("HOME", tmp.path());
        let _userprofile = EnvVarGuard::set("USERPROFILE", tmp.path());
        let _hakus_home = EnvVarGuard::set("HAKUS_HOME", &custom);
    ...
    #[test]
    fn default_path_with_explicit_hakus_home_does_not_migrate_ambient_legacy() {
        // FR003-C001: explicit HAKUS_HOME must not silently import ambient
        ...
        let path = FileKeyringStore::default_path().unwrap();
        assert_eq!(path, hakus_home.join("secrets").join("secrets.json"));
        assert!(
            !path.exists(),
            "explicit HAKUS_HOME must not create/migrate a primary store from ambient legacy"
        );
        ...
        assert_eq!(
            secrets.get("deepseek").unwrap(),
            None,
            "explicit HAKUS_HOME must not surface ambient legacy credentials"
        );
    }
    ...
    fn file_backed_default_refuses_relative_secret_path() {
        ...
        let secrets =
            Secrets::file_backed_from_default_path(Ok(PathBuf::from(".hakus-secrets.json")));
        assert!(matches!(
