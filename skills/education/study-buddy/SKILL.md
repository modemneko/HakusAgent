...
└── skills/
    ├── education/study-buddy/SKILL.md      ← 当前文件（学习项目主线）
    ├── research/qingyan-research/SKILL.md  ← 深度网研，把搜索内容生成 HTML 研究报告（步骤2 调用）
    ├── documents/pdf/SKILL.md               ← HTML → PDF 转换，把研究报告转成 PDF 给 create_project（步骤2 调用）
    ├── research/web-search/                 ← 搜索工具（铁律一调用）
